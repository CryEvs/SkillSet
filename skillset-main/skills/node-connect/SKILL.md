---
name: node-connect
description: Diagnose SkillSet node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify SkillSet is advertising that route, then fix pairing/auth.
...
- whether `skillset devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `skillset qr --json`. It uses the same setup-code payload Android scans.

```bash
skillset config get gateway.mode
skillset config get gateway.bind
skillset config get gateway.tailscale.mode
skillset config get gateway.remote.url
skillset config get gateway.auth.mode
skillset config get gateway.auth.allowTailscale
skillset config get plugins.entries.device-pair.config.publicUrl
skillset qr --json
skillset devices list
skillset nodes status
```

If this SkillSet instance is pointed at a remote gateway, also run:

```bash
skillset qr --remote --json
```
...
`skillset qr --json` success means:
...
If `skillset qr --json` says `Gateway is only bound to loopback`:
...
- `skillset devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run skillset qr again, rescan, then approve the pending device pairing.`
