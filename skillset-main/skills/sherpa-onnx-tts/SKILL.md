...
1. Download the runtime for your OS (extracts into `$SKILLSET_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.skillset/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$SKILLSET_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.skillset/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active SkillSet config file (`$SKILLSET_CONFIG_PATH`, default `~/.skillset/skillset.json`):
