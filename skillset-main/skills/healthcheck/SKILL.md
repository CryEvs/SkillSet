---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for SkillSet deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, SkillSet cron scheduling for periodic checks, or version status checks on a machine running SkillSet (laptop, workstation, Pi, VPS).
---

# SkillSet Host Hardening

## Overview

Assess and harden the host running SkillSet, then align it to a user-defined risk tolerance without breaking access. Use SkillSet security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim SkillSet changes the host firewall, SSH, or OS updates; it does not.
...
5. SkillSet gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if SkillSet can access sensitive data.
...
### 2) Run SkillSet security audits (read-only)

As part of the default read-only checks, run `skillset security audit --deep`. Only offer alternatives if the user requests them:

1. `skillset security audit` (faster, non-probing)
2. `skillset security audit --json` (structured output)

Offer to apply SkillSet safe defaults (numbered):

1. `skillset security audit --fix`

Be explicit that `--fix` only tightens SkillSet defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check SkillSet version/update status (read-only)

As part of the default read-only checks, run `skillset update status`.
...
- Credential hygiene notes (location of SkillSet creds, prefer disk encryption)
...
- SkillSet security audit (re-run)
...
After SkillSet install or first hardening pass, run at least one baseline audit and version check:

- `skillset security audit`
- `skillset security audit --deep`
- `skillset update status`

Ongoing monitoring is recommended. Use the SkillSet cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `skillset cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `skillset update status`
...
Before creating, `skillset cron list` and match on exact `name`. If found, `skillset cron edit <id> ...`.
If not found, `skillset cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `skillset update status` (preferred for source checkouts and channels)
2. `npm view skillset version` (published npm version)

## SkillSet command accuracy

Use only supported commands and flags:

- `skillset security audit [--deep] [--fix] [--json]`
- `skillset status` / `skillset status --deep`
- `skillset health --json`
- `skillset update status`
- `skillset cron add|list|runs|run`

Do not invent CLI flags or imply SkillSet enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by SkillSet compaction:
