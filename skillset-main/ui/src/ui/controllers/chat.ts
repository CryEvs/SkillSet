...
const SYNTHETIC_TRANSCRIPT_REPAIR_RESULT =
  "[skillset] missing tool result in session history; inserted synthetic error result for transcript repair.";
...
function resolveAttachmentType(mimeType: string): "image" | "audio" {
  return mimeType.startsWith("audio/") ? "audio" : "image";
}

function isAudioAttachmentMimeType(mimeType: string): boolean {
  ...
  return hasAttachments
    ? attachments
        .map((att) => {
          ...
          return {
            type: resolveAttachmentType(parsed.mimeType),
...
export async function sendChatMessage(
  ...
  if (hasAttachments) {
    for (const att of attachments) {
      if (isAudioAttachmentMimeType(att.mimeType)) {
        contentBlocks.push({
          type: "attachment",
          attachment: {
            url: att.dataUrl,
...
export function handleChatEvent(state: ChatState, payload?: ChatEventPayload) {
  ...
  // See https://github.com/skillset/skillset/issues/1909
