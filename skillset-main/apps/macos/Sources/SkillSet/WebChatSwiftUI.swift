...
private let webChatSwiftLogger = Logger(subsystem: "ai.skillset", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "skillset.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: SkillSetChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> SkillSetChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(SkillSetChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            SkillSetChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return SkillSetChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [SkillSetChatAttachmentPayload]) async throws -> SkillSetChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> SkillSetChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                SkillSetGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    SkillSetAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: SkillSetProtocol.ModelChoice) -> SkillSetChatModelChoice {
        SkillSetChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<SkillSetChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any SkillSetChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = SkillSetChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: SkillSetChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<SkillSetChatView>) -> NSViewController
    {
