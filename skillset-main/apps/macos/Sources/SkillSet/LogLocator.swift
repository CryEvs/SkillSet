...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["SKILLSET_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/skillset")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("skillset-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("skillset-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/skillset/ (rolling or stdout), or nil if none exist.
