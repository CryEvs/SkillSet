...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.skillset", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["SKILLSET_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = SkillSetConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.skillsetExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "skillset CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["SKILLSET_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "skillset@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "skillset@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "skillset@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "skillset@\(target)"])
            }

        statusHandler("Installing skillset@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed skillset@\(target)")
        } else {
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("skillset ") {
            normalized = String(normalized.dropFirst("skillset ".count))
        }
