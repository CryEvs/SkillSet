...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: SkillSetProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: SkillSetProtocol.AnyCodable] = [
            "presence": .init([SkillSetProtocol.AnyCodable(entry)]),
        ]
        let payload = SkillSetProtocol.AnyCodable(payloadMap)
