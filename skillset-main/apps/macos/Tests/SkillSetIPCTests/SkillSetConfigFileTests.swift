...
@Suite(.serialized)
struct SkillSetConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("skillset-config-\(UUID().uuidString)")
            .appendingPathComponent("skillset.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["SKILLSET_CONFIG_PATH": override]) {
            #expect(SkillSetConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["SKILLSET_CONFIG_PATH": override]) {
            ...
            #expect(SkillSetConfigFile.remoteGatewayPort() == 19999)
            #expect(SkillSetConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(SkillSetConfigFile.remoteGatewayPort(matchingHost: "gateway") == 19999)
            #expect(SkillSetConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
        }
    }
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["SKILLSET_CONFIG_PATH": override]) {
            ...
            SkillSetConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = SkillSetConfigFile.loadDict()
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["SKILLSET_CONFIG_PATH": override]) {
            ...
            SkillSetConfigFile.clearRemoteGatewayUrl()
            let root = SkillSetConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        let dir = FileManager().temporaryDirectory
            .appendingPathComponent("skillset-state-\(UUID().uuidString)", isDirectory: true)
            .path

        await TestIsolation.withEnvValues([
            "SKILLSET_CONFIG_PATH": nil,
            "SKILLSET_STATE_DIR": dir,
        ]) {
            #expect(SkillSetConfigFile.stateDirURL().path == dir)
            #expect(SkillSetConfigFile.url().path == "\(dir)/skillset.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("skillset-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("skillset.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "SKILLSET_STATE_DIR": stateDir.path,
            "SKILLSET_CONFIG_PATH": configPath.path,
        ]) {
            SkillSetConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-skillset-config-file")
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let loaded = SkillSetConfigFile.loadDict()
