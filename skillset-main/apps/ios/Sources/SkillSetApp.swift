...
@MainActor
enum SkillSetAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class SkillSetAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.skillset.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.skillset.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.skillset.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? SkillSetAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "skillset.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "skillset.watch.promptId"
    static let sessionKeyKey = "skillset.watch.sessionKey"
    static let actionPrimaryIDKey = "skillset.watch.action.primary.id"
    static let actionPrimaryLabelKey = "skillset.watch.action.primary.label"
    static let actionSecondaryIDKey = "skillset.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "skillset.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "skillset.watch.action.primary"
    static let actionSecondaryIdentifier = "skillset.watch.action.secondary"
    static let actionIdentifierPrefix = "skillset.watch.action."
    static let actionIDKeyPrefix = "skillset.watch.action.id."
    static let actionLabelKeyPrefix = "skillset.watch.action.label."
    static let categoryPrefix = "skillset.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: SkillSetWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> SkillSetWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SkillSetWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "SkillSet" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [SkillSetWatchAction]) -> [UNNotificationAction] {
...
struct SkillSetApp: App {
    ...
    @UIApplicationDelegateAdaptor(SkillSetAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        SkillSetAppModelRegistry.appModel = appModel
...
extension SkillSetApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("SkillSet: installing uncaught exception handler")
