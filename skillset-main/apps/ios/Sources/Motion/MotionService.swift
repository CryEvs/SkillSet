...
final class MotionService: MotionServicing {
    func activities(params: SkillSetMotionActivityParams) async throws -> SkillSetMotionActivityPayload {
        ...
        let mapped: [SkillSetMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return SkillSetMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: SkillSetPedometerParams) async throws -> SkillSetPedometerPayload {
        ...
        let payload: SkillSetPedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = SkillSetPedometerPayload(
