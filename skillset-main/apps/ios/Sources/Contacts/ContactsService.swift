...
final class ContactsService: ContactsServicing {
    ...
    func search(params: SkillSetContactsSearchParams) async throws -> SkillSetContactsSearchPayload {
        ...
        return SkillSetContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: SkillSetContactsAddParams) async throws -> SkillSetContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return SkillSetContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return SkillSetContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> SkillSetContactPayload {
        SkillSetContactPayload(
