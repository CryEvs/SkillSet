...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__SKILLSET_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<SkillSetTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.skillset", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = SkillSetTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> SkillSetTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return SkillSetTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> SkillSetTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> SkillSetTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return SkillSetTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> SkillSetTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = SkillSetTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: SkillSetTalkPTTStopPayload) {
