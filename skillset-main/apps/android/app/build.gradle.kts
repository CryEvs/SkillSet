...
val androidStoreFile = providers.gradleProperty("SKILLSET_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("SKILLSET_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("SKILLSET_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("SKILLSET_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
    error(
        "Missing Android release signing properties. Set SKILLSET_ANDROID_STORE_FILE, " +
            "SKILLSET_ANDROID_STORE_PASSWORD, SKILLSET_ANDROID_KEY_ALIAS, and " +
            "SKILLSET_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
    )
}
...
android {
    ...
    sourceSets {
        getByName("main") {
            assets.directories.add("../../shared/SkillSetKit/Sources/SkillSetKit/Resources")
        }
    }

    defaultConfig {
        applicationId = "ai.skillset.app"
        minSdk = 31
        targetSdk = 36
    ...
    productFlavors {
        create("play") {
            dimension = "store"
            buildConfigField("boolean", "SKILLSET_ENABLE_SMS", "false")
            buildConfigField("boolean", "SKILLSET_ENABLE_CALL_LOG", "false")
        }
        create("thirdParty") {
            dimension = "store"
            buildConfigField("boolean", "SKILLSET_ENABLE_SMS", "true")
            buildConfigField("boolean", "SKILLSET_ENABLE_CALL_LOG", "true")
        }
    }
    ...
    onVariants { variant ->
        variant.outputs
            ...
            .forEach { output ->
                ...
                val outputFileName =
                    if (flavorName == null) {
                        "skillset-$versionName-$buildType.apk"
                    } else {
                        "skillset-$versionName-$flavorName-$buildType.apk"
                    }
