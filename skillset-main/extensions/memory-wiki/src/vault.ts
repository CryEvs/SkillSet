...
import {
  ...
  withTrailingNewline,
} from "skillset/plugin-sdk/memory-host-markdown";
...
function buildIndexMarkdown(): string {
  return withTrailingNewline(
    replaceManagedMarkdownBlock({
      original: "# Wiki Index\n",
      heading: "## Generated",
      startMarker: "<!-- skillset:wiki:index:start -->",
      endMarker: "<!-- skillset:wiki:index:end -->",
...
- Use \`.skillset-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
This vault is maintained by the SkillSet memory-wiki plugin.
...
- \`.skillset-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.

## Notes
<!-- skillset:human:start -->
<!-- skillset:human:end -->
`);
}
...
export async function initializeMemoryWikiVault(
  ...
  await writeFileIfMissing(
    path.join(rootDir, ".skillset-wiki", "state.json"),
    withTrailingNewline(
      JSON.stringify(
        {
          version: 1,
  ...
  await writeFileIfMissing(path.join(rootDir, ".skillset-wiki", "log.jsonl"), "", createdFiles);
