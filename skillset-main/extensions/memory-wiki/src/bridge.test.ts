...
} from "skillset/plugin-sdk/memory-host-events";
...
describe("syncMemoryWikiBridgeSources", () => {
  ...
  it("imports public memory artifacts and stays idempotent across reruns", async () => {
    ...
    const appConfig: SkillSetConfig = {
      agents: {
    ...
    const logLines = (await fs.readFile(path.join(vaultDir, ".skillset-wiki", "log.jsonl"), "utf8"))
      .trim()
      .split("\n");
