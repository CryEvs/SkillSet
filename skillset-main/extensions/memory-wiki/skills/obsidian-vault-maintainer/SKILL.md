...
- Start from `skillset wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `skillset wiki obsidian status` before shelling out, then prefer the dedicated helpers like `skillset wiki obsidian search`, `skillset wiki obsidian open`, `skillset wiki obsidian command`, and `skillset wiki obsidian daily`.
