# @skillset/memory-wiki

Persistent wiki compiler and Obsidian-friendly knowledge vault for **SkillSet**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.skillset-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
```bash
skillset wiki status
skillset wiki doctor
skillset wiki init
skillset wiki ingest ./notes/alpha.md
skillset wiki compile
skillset wiki lint
skillset wiki search "alpha"
skillset wiki get entity.alpha --from 1 --lines 80

skillset wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

skillset wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

skillset wiki bridge import
skillset wiki unsafe-local import

skillset wiki obsidian status
skillset wiki obsidian search "alpha"
skillset wiki obsidian open syntheses/alpha-summary.md
skillset wiki obsidian command workspace:quick-switcher
skillset wiki obsidian daily
```
...
- The compiled agent digests in `.skillset-wiki/cache/agent-digest.json` and `.skillset-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
