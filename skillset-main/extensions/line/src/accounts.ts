...
import { resolveAccountEntry } from "skillset/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: SkillSetConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: SkillSetConfig): string {
