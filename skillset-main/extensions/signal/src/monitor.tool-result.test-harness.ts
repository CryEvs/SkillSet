import type { MockFn } from "skillset/plugin-sdk/testing";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/skillset-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("skillset/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("skillset/plugin-sdk/conversation-runtime")>(
    "skillset/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("skillset/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("skillset/plugin-sdk/security-runtime")>(
    "skillset/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("skillset/plugin-sdk/reply-runtime"),
      import("skillset/plugin-sdk/infra-runtime"),
    ]);
    resetInboundDedupe();
    config = {
