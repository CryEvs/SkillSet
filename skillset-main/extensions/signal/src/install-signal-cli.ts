...
import { runPluginCommandWithTimeout } from "skillset/plugin-sdk/run-command";
import type { RuntimeEnv } from "skillset/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "skillset/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredSkillSetTmpDir(), "skillset-signal-"));
