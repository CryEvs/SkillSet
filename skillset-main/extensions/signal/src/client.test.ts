...
vi.mock("skillset/plugin-sdk/fetch-runtime", () => ({
  resolveFetch: (...args: unknown[]) => resolveFetchMock(...args),
}));

vi.mock("skillset/plugin-sdk/core", async () => {
  const actual = await vi.importActual<typeof import("skillset/plugin-sdk/core")>(
    "skillset/plugin-sdk/core",
  );
  return {
    ...actual,
