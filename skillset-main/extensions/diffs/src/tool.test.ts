...
describe("diffs tool", () => {
  ...
  beforeEach(async () => {
    ({ store, cleanup: cleanupRootDir } = await createDiffStoreHarness("skillset-diffs-tool-"));
  });
  ...
  it("uses configured viewerBaseUrl when tool input omits baseUrl", async () => {
    const tool = createDiffsTool({
      api: createApi({
        viewerBaseUrl: "https://example.com/skillset/",
      }),
      store,
      defaults: DEFAULT_DIFFS_TOOL_DEFAULTS,
      viewerBaseUrl: "https://example.com/skillset",
    ...
    expect(readTextContent(result, 0)).toContain(
      "https://example.com/skillset/plugins/diffs/view/",
    );
    expect((result?.details as Record<string, unknown>).viewerUrl).toEqual(
      expect.stringContaining("https://example.com/skillset/plugins/diffs/view/"),
    );
  });
...
function createApi(pluginConfig?: Record<string, unknown>): SkillSetPluginApi {
...
function createToolWithScreenshotter(
  ...
  defaults = DEFAULT_DIFFS_TOOL_DEFAULTS,
  context: SkillSetPluginToolContext = {
    agentId: "main",
    sessionId: "session-123",
    messageChannel: "discord",
    agentAccountId: "default",
