...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    ...
    ).toBe("https://example.com/skillset");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/skillset?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/skillset#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/skillset/" })).toBe(true);
  });
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/skillset",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/skillset/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("skillsetDiffsReady");
