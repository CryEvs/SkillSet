...
Read-only diff viewer plugin for **SkillSet** agents.
...
Set plugin-wide defaults in `~/.skillset/skillset.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/skillset",
        },
      },
    },
  },
...
Before:
SkillSet supports plugins.

After:
SkillSet supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/skillset-diffs`).
