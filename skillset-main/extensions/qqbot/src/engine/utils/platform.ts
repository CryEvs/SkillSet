 ...
 * `skillset/plugin-sdk`. The temp-directory fallback is delegated
...
/** Return a path under `~/.skillset/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".skillset", "qqbot", ...subPaths);
}

/** Return a path under `~/.skillset/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.skillset/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under SkillSet's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".skillset", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.skillset/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.skillset/media`, SkillSet's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `skillset/src/media/local-roots.ts`). Using it as a
 ...
 */
export function getSkillSetMediaDir(): string {
  return path.join(getHomeDir(), ".skillset", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".skillset", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and SkillSet's shared `~/.skillset/media`
  ...
  const allowedRoots = [getSkillSetMediaDir(), getQQBotMediaPath()];
