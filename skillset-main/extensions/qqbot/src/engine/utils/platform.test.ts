...
describe("qqbot local media path remapping", () => {
  ...
  function createSkillSetTestRoot() {
    const actualHome = getHomeDir();
    const skillsetDir = path.join(actualHome, ".skillset");
    fs.mkdirSync(skillsetDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(skillsetDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createSkillSetTestRoot();
  ...
  it("allows structured payload files inside sibling SkillSet media subdirectories", () => {
    ...
    // trust the shared `~/.skillset/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".skillset", "media", "outbound");
