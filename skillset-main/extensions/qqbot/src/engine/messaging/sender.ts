...
let _skillsetVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${_pluginVersion} (Node/${process.versions.node}; ${os.platform()}; SkillSet/${_skillsetVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; skillsetVersion?: string }): void {
  if (options.pluginVersion) {
    _pluginVersion = options.pluginVersion;
  }
  if (options.skillsetVersion) {
    _skillsetVersion = options.skillsetVersion;
  }
}

/** Update the SkillSet framework version in the User-Agent (called after runtime injection). */
export function setSkillSetVersion(version: string): void {
  if (version) {
    _skillsetVersion = version;
  }
}
