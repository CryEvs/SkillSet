...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<SkillSetConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<SkillSetConfig["channels"]>["telegram"]>>,
): SkillSetConfig {
  return {
    channels: {
      telegram: {
