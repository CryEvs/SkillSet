...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): SkillSetConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as SkillSetConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): SkillSetConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as SkillSetConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: SkillSetConfig, accountId = "media") {
    await handleTelegramAction(
