...
import type { ChannelGroupPolicy } from "skillset/plugin-sdk/config-runtime";
import type {
  ReplyToMode,
...
import { getRuntimeConfigSnapshot } from "skillset/plugin-sdk/runtime-config-snapshot";
...
type TelegramChunkMode = ReturnType<
  typeof import("skillset/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("skillset/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): SkillSetConfig => telegramDeps.loadConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: SkillSetConfig): TelegramAccountConfig => {
    try {
