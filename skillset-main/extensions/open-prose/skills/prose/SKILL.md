...
metadata: { "skillset": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == SkillSet `sessions_spawn`
- **File I/O** == SkillSet `read`/`write`
- **Remote fetch** == SkillSet `web_fetch` (or `exec` with curl when POST is required)
