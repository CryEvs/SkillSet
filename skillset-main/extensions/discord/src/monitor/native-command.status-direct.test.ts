...
vi.mock("skillset/plugin-sdk/reply-dispatch-runtime", async () => {
  const actual = await vi.importActual<typeof import("skillset/plugin-sdk/reply-dispatch-runtime")>(
...
vi.mock("skillset/plugin-sdk/command-status-runtime", () => ({
...
function createConfig(params?: { requireMention?: boolean }): SkillSetConfig {
  return {
    commands: {
...
describe("discord native /status", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setDispatchReplyWithDispatcher(
      runtimeModuleMocks.dispatchReplyWithDispatcher as typeof import("skillset/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithDispatcher,
    );
