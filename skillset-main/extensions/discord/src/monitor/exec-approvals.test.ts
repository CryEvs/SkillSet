...
vi.mock("skillset/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("skillset/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<SkillSetConfig["channels"]>["discord"]>["execApprovals"],
): SkillSetConfig {
  return {
    channels: {
      discord: {
