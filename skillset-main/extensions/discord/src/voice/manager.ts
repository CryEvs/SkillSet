...
import type { DiscordAccountConfig, TtsConfig } from "skillset/plugin-sdk/config-runtime";
...
import { parseTtsDirectives } from "skillset/plugin-sdk/speech";
...
function resolveVoiceTtsConfig(params: { cfg: SkillSetConfig; override?: TtsConfig }): {
  cfg: SkillSetConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredSkillSetTmpDir(), "discord-voice-"));
