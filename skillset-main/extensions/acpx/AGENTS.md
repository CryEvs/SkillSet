...
The bundled ACPX extension is a thin SkillSet wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `skillset/acpx`, not in this extension.
...
Use this flow when SkillSet needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `skillset/acpx` repo first.
2. In SkillSet, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild SkillSet and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for SkillSet ACP validation.
...
If a change feels like shared ACP runtime behavior instead of SkillSet-specific glue, move it to `skillset/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
