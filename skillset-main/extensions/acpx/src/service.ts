...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: SkillSetPluginServiceContext): Promise<void> {
      if (process.env.SKILLSET_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (SKILLSET_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (process.env.SKILLSET_SKIP_ACPX_RUNTIME_PROBE === "1") {
        return;
      }
    ...
    async stop(_ctx: SkillSetPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
