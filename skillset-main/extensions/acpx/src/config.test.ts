...
describe("embedded acpx plugin config", () => {
  it("resolves workspace stateDir and cwd by default", () => {
    const workspaceDir = "/tmp/skillset-acpx";
  ...
  it("keeps explicit timeoutSeconds config", () => {
    const resolved = resolveAcpxPluginConfig({
      ...
      workspaceDir: "/tmp/skillset-acpx",
    });
  ...
  it("injects the built-in plugin-tools MCP server only when explicitly enabled", () => {
    ...
    const server = resolved.mcpServers["skillset-plugin-tools"];
  ...
  it("injects the built-in SkillSet tools MCP server only when explicitly enabled", () => {
    const resolved = resolveAcpxPluginConfig({
      rawConfig: {
    ...
    const server = resolved.mcpServers["skillset-tools"];
    expect(server).toBeDefined();
    expect(server.command).toBe(process.execPath);
    expect(Array.isArray(server.args)).toBe(true);
    expect(server.args?.length).toBeGreaterThan(0);
  ...
  it("keeps the runtime json schema in sync with the manifest config schema", () => {
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(pluginRoot, "skillset.plugin.json"), "utf8"),
