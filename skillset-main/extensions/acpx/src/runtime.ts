...
const SKILLSET_BRIDGE_EXECUTABLE = "skillset";
const SKILLSET_BRIDGE_SUBCOMMAND = "acp";
...
function isSkillSetBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === SKILLSET_BRIDGE_EXECUTABLE) {
    return parts[1] === SKILLSET_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^skillset(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === SKILLSET_BRIDGE_SUBCOMMAND;
}
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isSkillSetBridgeCommand(command);
}
