...
vi.mock("skillset/plugin-sdk/ssrf-runtime", async () => {
  const original = (await vi.importActual("skillset/plugin-sdk/ssrf-runtime")) as Record<
    string,
    unknown
  >;
...
function getDescribedActions(cfg: SkillSetConfig, accountId?: string): string[] {
