import { createScopedDmSecurityResolver } from "skillset/plugin-sdk/channel-config-helpers";
...
function resolveZalouserOutboundChunkMode(cfg: SkillSetConfig, accountId?: string) {
  return getZalouserRuntime().channel.text.resolveChunkMode(cfg, "zalouser", accountId);
}

function resolveZalouserOutboundTextChunkLimit(cfg: SkillSetConfig, accountId?: string) {
...
export const zalouserPairingTextAdapter = {
  ...
  notify: async ({ cfg, id, message }: { cfg: SkillSetConfig; id: string; message: string }) => {
    const { sendMessageZalouser } = await loadZalouserChannelRuntime();
