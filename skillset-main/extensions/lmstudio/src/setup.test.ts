import { CUSTOM_LOCAL_AUTH_MARKER } from "skillset/plugin-sdk/provider-auth";
import type { SkillSetConfig } from "skillset/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "skillset/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "skillset/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "skillset/plugin-sdk/provider-setup";
import type { WizardPrompter } from "skillset/plugin-sdk/setup";
...
vi.mock("skillset/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("skillset/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("skillset/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("skillset/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
function buildConfig(): SkillSetConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
function buildDiscoveryContext(params?: {
  ...
  return {
    config: params?.config ?? ({} as SkillSetConfig),
    env: params?.env ?? {},
