...
vi.mock("skillset/plugin-sdk/agent-harness-runtime", async (importOriginal) => ({
  ...(await importOriginal<typeof import("skillset/plugin-sdk/agent-harness-runtime")>()),
...
function buildCurrentCodexApprovalElicitation() {
  return {
    ...
    _meta: {
      ...
      tool_params_display: [
        { name: "repo", display_name: "Repository", value: "skillset/skillset" },
      ],
    },
    requestedSchema: {
      type: "object",
...
describe("Codex app-server elicitation bridge", () => {
  ...
  it("accepts current Codex MCP approval elicitations with an empty form schema", async () => {
    ...
    expect(approvalRequest.description).toContain("Repository: skillset/skillset");
  });
