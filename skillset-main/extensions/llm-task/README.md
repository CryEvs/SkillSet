...
`skillset.invoke --each`) without adding new SkillSet code per workflow.
...
This extension depends on SkillSet internal modules (the embedded agent runner).
It is intended to ship as a **bundled** SkillSet extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.skillset/extensions` as a standalone plugin directory.
