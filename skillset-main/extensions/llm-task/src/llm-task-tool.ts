...
export function createLlmTaskTool(api: SkillSetPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via skillset.invoke.",
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredSkillSetTmpDir(), "skillset-llm-task-"),
        );
