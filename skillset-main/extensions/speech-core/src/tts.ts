...
import { normalizeChannelId, type ChannelId } from "skillset/plugin-sdk/channel-targets";
import type {
  SkillSetConfig,
  TtsAutoMode,
  TtsConfig,
...
import { redactSensitiveText } from "skillset/plugin-sdk/logging-core";
import {
  resolveSendableOutboundReplyParts,
  type ReplyPayload,
} from "skillset/plugin-sdk/reply-payload";
import { isVerbose, logVerbose } from "skillset/plugin-sdk/runtime-env";
import { resolvePreferredSkillSetTmpDir } from "skillset/plugin-sdk/sandbox";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.SKILLSET_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: SkillSetConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: SkillSetConfig): TtsProvider {
...
export function resolveTtsConfig(cfg: SkillSetConfig): ResolvedTtsConfig {
...
function resolveEffectiveTtsAutoState(params: { cfg: SkillSetConfig; sessionAuto?: string }): {
...
export function buildTtsSystemPromptHint(cfg: SkillSetConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: SkillSetConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredSkillSetTmpDir();
