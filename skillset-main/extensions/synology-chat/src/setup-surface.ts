...
function getChannelConfig(cfg: SkillSetConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: SkillSetConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: SkillSetConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: SkillSetConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL SkillSet uses to send replies back to Chat.",
      ],
