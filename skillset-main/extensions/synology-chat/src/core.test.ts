...
const synologyChatSetupPlugin = {
  ...
  config: {
    ...
    resolveAllowFrom: ({ cfg, accountId }: { cfg: SkillSetConfig; accountId?: string }) =>
...
describe("synology-chat core", () => {
  beforeEach(() => {
    ...
    delete process.env.SKILLSET_BOT_NAME;
  });
...
describe("synology-chat account resolution", () => {
  ...
  it("returns full defaults for empty config", () => {
    ...
    expect(account.botName).toBe("SkillSet");
  });
  ...
  it("uses env var fallbacks", () => {
    ...
    process.env.SKILLSET_BOT_NAME = "TestBot";
