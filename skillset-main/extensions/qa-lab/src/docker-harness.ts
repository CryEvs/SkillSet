...
const QA_LAB_UI_OVERLAY_DIR = "/opt/skillset-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        SKILLSET_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${params.bindUiDist ? `    volumes:\n      - ${qaLabUiMount}:${QA_LAB_UI_OVERLAY_DIR}:ro\n` : ""}    healthcheck:
    ...
    command:
      ...
      - "http://skillset-qa-gateway:18789/"
      - --control-ui-token
      - "${params.gatewayToken}"
...
${imageBlock}    pull_policy: never
    ...
    environment:
      SKILLSET_CONFIG_PATH: /tmp/skillset/skillset.json
      ...
      SKILLSET_PROFILE: ""
    volumes:
      - ./state:/opt/skillset-scaffold:ro
      - ${repoMount}:/opt/skillset-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/skillset/workspace /tmp/skillset/state && cp /opt/skillset-scaffold/skillset.json /tmp/skillset/skillset.json && cp -R /opt/skillset-scaffold/seed-workspace/. /tmp/skillset/workspace/ && ln -snf /opt/skillset-repo /tmp/skillset/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t skillset:qa-local-prebaked --build-arg SKILLSET_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "skillset:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "skillset.json"),
  ];
