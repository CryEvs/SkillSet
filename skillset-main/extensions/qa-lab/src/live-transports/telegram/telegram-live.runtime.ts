...
const TELEGRAM_QA_ENV_KEYS = [
  "SKILLSET_QA_TELEGRAM_GROUP_ID",
  "SKILLSET_QA_TELEGRAM_DRIVER_BOT_TOKEN",
  "SKILLSET_QA_TELEGRAM_SUT_BOT_TOKEN",
] as const;
const TELEGRAM_QA_CAPTURE_CONTENT_ENV = "SKILLSET_QA_TELEGRAM_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "SKILLSET_QA_REDACT_PUBLIC_METADATA";
const QA_SUITE_PROGRESS_ENV = "SKILLSET_QA_SUITE_PROGRESS";
...
export function resolveTelegramQaRuntimeEnv(
  ...
  const groupId = resolveEnvValue(env, "SKILLSET_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("SKILLSET_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "SKILLSET_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "SKILLSET_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
export async function runTelegramQaLive(params: {
  ...
  const publishedCleanupIssues = redactPublicMetadata
    ? cleanupIssues.map(() => "details redacted (SKILLSET_QA_REDACT_PUBLIC_METADATA=1)")
    : cleanupIssues;
