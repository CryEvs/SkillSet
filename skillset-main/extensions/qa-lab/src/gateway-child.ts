...
import { formatErrorMessage } from "skillset/plugin-sdk/error-runtime";
import type { ModelProviderConfig } from "skillset/plugin-sdk/provider-model-shared";
import { fetchWithSsrFGuard } from "skillset/plugin-sdk/ssrf-runtime";
import { resolvePreferredSkillSetTmpDir } from "skillset/plugin-sdk/temp-path";
import {
...
const QA_GATEWAY_CHILD_BLOCKED_SECRET_ENV_VARS = Object.freeze([
  "SKILLSET_QA_CONVEX_SECRET_CI",
  "SKILLSET_QA_CONVEX_SECRET_MAINTAINER",
]);
...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    SKILLSET_HOME: params.homeDir,
    SKILLSET_CONFIG_PATH: params.configPath,
    SKILLSET_STATE_DIR: params.stateDir,
    SKILLSET_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    SKILLSET_NO_RESPAWN: "1",
    SKILLSET_TEST_FAST: "1",
    SKILLSET_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    SKILLSET_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { SKILLSET_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { SKILLSET_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: SkillSetConfig) => SkillSetConfig;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredSkillSetTmpDir(), "skillset-qa-suite-"),
  );
  ...
  const configPath = path.join(tempRoot, "skillset.json");
  ...
  const keepTemp = process.env.SKILLSET_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
  let child: ReturnType<typeof spawn> | null = null;
  let cfg!: SkillSetConfig;
