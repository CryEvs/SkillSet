...
export async function loadQaRunnerModelOptions(params: { repoRoot: string; signal?: AbortSignal }) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredSkillSetTmpDir(), "skillset-qa-model-catalog-"),
  );
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const child = spawn(nodeExecPath, ["dist/index.js", "models", "list", "--all", "--json"], {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          SKILLSET_HOME: homeDir,
          SKILLSET_CONFIG_PATH: configPath,
          SKILLSET_STATE_DIR: stateDir,
          SKILLSET_OAUTH_DIR: path.join(stateDir, "credentials"),
          SKILLSET_CODEX_DISCOVERY_LIVE: "0",
        },
