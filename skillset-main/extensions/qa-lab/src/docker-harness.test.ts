...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "skillset.json"),
    ...
    expect(compose).toContain('      - "http://skillset-qa-gateway:18789/"');
    expect(compose).toContain("      - --send-kickoff-on-start");
    expect(compose).toContain("      - --ui-dist-dir");
    expect(compose).toContain('      - "/opt/skillset-qa-lab-ui"');
    expect(compose).toContain(":/opt/skillset-repo:ro");
    expect(compose).toContain("./state:/opt/skillset-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/skillset-scaffold/seed-workspace/. /tmp/skillset/workspace/",
    );
    expect(compose).toContain("SKILLSET_CONFIG_PATH: /tmp/skillset/skillset.json");
    expect(compose).toContain("SKILLSET_STATE_DIR: /tmp/skillset/state");
    expect(compose).toContain('SKILLSET_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("SKILLSET_GATEWAY_TOKEN=qa-token");
    ...
    const config = await readFile(path.join(outputDir, "state", "skillset.json"), "utf8");
    ...
    expect(config).toContain('"/tmp/skillset/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/skillset",
        imageName: "skillset:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("skillset:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t skillset:qa-local-prebaked --build-arg SKILLSET_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/skillset",
      ),
    ]);
  });
});
