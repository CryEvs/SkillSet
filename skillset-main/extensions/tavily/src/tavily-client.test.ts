...
vi.mock("skillset/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: skillset", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "skillset" });
  });

  it("runTavilyExtract sends X-Client-Source: skillset", async () => {
