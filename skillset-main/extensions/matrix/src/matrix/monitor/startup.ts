...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleSkillSetDevices.length > 0) {
      params.logger.warn(
        `matrix: stale SkillSet devices detected for ${params.auth.userId}: ${deviceHealth.staleSkillSetDevices.map((device) => device.deviceId).join(", ")}. Run 'skillset matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
