...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'skillset matrix verify status --verbose --account ${accountId}' and 'skillset matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another SkillSet runtime is using the same account. " +
    `Check 'skillset matrix verify status --verbose --account ${accountId}' and 'skillset matrix devices list --account ${accountId}'.`
  );
}
