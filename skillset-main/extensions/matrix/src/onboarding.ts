...
import type { WizardPrompter } from "skillset/plugin-sdk/matrix-runtime-shared";
...
import { isPrivateNetworkOptInEnabled } from "skillset/plugin-sdk/ssrf-policy";
import {
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  await params.prompter.note(
    [
      ...
      "SkillSet agents will not join invited rooms or fresh DM-style invites unless you set autoJoin.",
  ...
  const deviceName =
    normalizeStringifiedOptionalString(
      await params.prompter.text({
        ...
        initialValue: existing.deviceName ?? "SkillSet Gateway",
      }),
    ) ?? "";
