...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<SkillSetConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<SkillSetConfig["channels"]>["matrix"]>>,
): SkillSetConfig {
  return {
    channels: {
      matrix: {
