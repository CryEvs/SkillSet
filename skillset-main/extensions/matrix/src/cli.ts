...
import type { ChannelSetupInput } from "skillset/plugin-sdk/setup";
...
import { isSkillSetManagedMatrixDevice } from "./matrix/device-health.js";
...
function formatMatrixCliCommand(command: string, accountId?: string): string {
  ...
  return `skillset matrix ${command}${suffix}`;
}
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleSkillSetDeviceIds: addedDevices
        .filter((device) => !device.current && isSkillSetManagedMatrixDevice(device.displayName))
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.skillset.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleSkillSetDeviceIds.length > 0) {
              console.log(
                `Matrix device hygiene warning: stale SkillSet devices detected (${result.deviceHealth.staleSkillSetDeviceIds.join(", ")}). Run 'skillset matrix devices prune-stale --account ${result.accountId}'.`,
              );
            }
            ...
            const bindHint = `skillset agents bind --agent <id> --bind matrix:${result.accountId}`;
  ...
  devices
    .command("prune-stale")
    .description("Delete stale SkillSet-managed devices for this account")
    .option("--account <id>", "Account ID (for multi-account setups)")
    .option("--verbose", "Show detailed diagnostics")
    .option("--json", "Output as JSON")
    .action(async (options: { account?: string; verbose?: boolean; json?: boolean }) => {
      ...
      await runMatrixCliCommand({
        ...
        onText: (result, verbose) => {
          printAccountLabel(accountId);
          console.log(
            `Deleted stale SkillSet devices: ${result.deletedDeviceIds.length ? result.deletedDeviceIds.join(", ") : "none"}`,
          );
