...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("SKILLSET_TEST_MEMORY_UNSAFE_REINDEX", "1");
    clearRegistry();
  ...
  async function getFtsSessionManager(params: {
    ...
    vi.stubEnv("SKILLSET_STATE_DIR", path.join(workspaceDir, params.stateDirName));
    const cfg = createCfg({
