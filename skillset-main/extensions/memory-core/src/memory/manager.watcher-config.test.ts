...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): SkillSetConfig {
    const defaults: NonNullable<NonNullable<SkillSetConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: SkillSetConfig) {
