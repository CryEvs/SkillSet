...
vi.mock("./cli.host.runtime.js", async () => {
  const [runtimeCli, runtimeCore, runtimeFiles] = await Promise.all([
    import("skillset/plugin-sdk/memory-core-host-runtime-cli"),
    import("skillset/plugin-sdk/memory-core-host-runtime-core"),
    import("skillset/plugin-sdk/memory-core-host-runtime-files"),
  ]);
  return {
...
let defaultRuntime: typeof import("skillset/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("skillset/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("skillset/plugin-sdk/memory-core-host-runtime-cli").setVerbose;
let fixtureRoot = "";
...
beforeAll(async () => {
  ...
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("skillset/plugin-sdk/memory-core-host-runtime-cli"));
...
describe("memory cli", () => {
  ...
  it("reports dreaming blocked when another explicit heartbeat agent excludes main", async () => {
    ...
    mockManager({
      probeVectorAvailability: vi.fn(async () => true),
      status: () => makeMemoryStatus({ workspaceDir: "/tmp/skillset" }),
      close,
    });
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining(
        'Dreaming status: blocked - dreaming is enabled but will not run because heartbeat is disabled for "main". See https://docs.skillset.ai/concepts/dreaming#troubleshooting',
      ),
    );
    expect(close).toHaveBeenCalled();
  });
  ...
  it("reports dreaming blocked for the configured default agent when it is not main", async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining(
        'Dreaming status: blocked - dreaming is enabled but will not run because heartbeat is disabled for "ops". See https://docs.skillset.ai/concepts/dreaming#troubleshooting',
      ),
    );
    expect(close).toHaveBeenCalled();
  });
  ...
  it("shows the fix hint only before --fix has been run", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await runMemoryCli(["status"]);
      expect(log).toHaveBeenCalledWith(
        expect.stringContaining("Fix: skillset memory status --fix --agent main"),
      );
  ...
  it("writes grounded rem backfill entries into DREAMS.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(dreams).toContain("skillset:dreaming:backfill-entry");
  ...
  it("prefers persistence-relevant evidence over narrated operational logs in grounded what happened", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        historyPath,
        [
          "## SkillSet / runtime / workflow preferences and corrections",
    ...
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "*January 1, 2025*",
          "",
          "<!-- skillset:dreaming:backfill-entry day=2025-01-01 source=memory/2025-01-01.md -->",
          "",
          "What Happened",
          "1. Remove this entry.",
          "",
          "<!-- skillset:dreaming:diary:end -->",
          "",
        ].join("\n"),
        "utf-8",
      );
  ...
  it("applies top promote candidates into MEMORY.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(memoryText).toContain("skillset-memory-promotion:");
