import { resolveDefaultAgentId } from "skillset/plugin-sdk/config-runtime";
import {
  isHeartbeatEnabledForAgent,
  peekSystemEventEntries,
  resolveHeartbeatIntervalMs,
} from "skillset/plugin-sdk/infra-runtime";
import type { SkillSetConfig, SkillSetPluginApi } from "skillset/plugin-sdk/memory-core";
import {
...
} from "skillset/plugin-sdk/memory-core-host-status";
...
const DREAMING_SYSTEM_EVENT_TEXT = "__skillset_memory_core_short_term_promotion_dream__";
...
export function resolveDreamingBlockedReason(cfg: SkillSetConfig): string | null {
  ...
  return `dreaming is enabled but will not run because heartbeat is disabled for "${defaultAgentId}". See https://docs.skillset.ai/concepts/dreaming#troubleshooting`;
}
...
export function registerShortTermPromotionDreaming(api: SkillSetPluginApi): void {
  ...
  const resolveCurrentConfig = (): SkillSetConfig =>
