...
import { resolveMemoryDreamingConfig } from "skillset/plugin-sdk/memory-core-host-status";
...
function resolveMemoryCorePluginConfig(cfg: SkillSetConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: SkillSetConfig, enabled: boolean): SkillSetConfig {
...
function formatStatus(cfg: SkillSetConfig): string {
...
export function registerDreamingCommand(api: SkillSetPluginApi): void {
  api.registerCommand({
    name: "dreaming",
