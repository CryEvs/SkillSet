...
import { resolveMemoryRemDreamingConfig } from "skillset/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "skillset/plugin-sdk/routing";
...
function resolveMemoryPluginConfig(cfg: SkillSetConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredSkillSetTmpDir(), "skillset-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: SkillSetConfig): string {
...
function resolveAgent(cfg: SkillSetConfig, agent?: string) {
...
function resolveAgentIds(cfg: SkillSetConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: skillset memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredSkillSetTmpDir(), "skillset-rem-backfill-"),
      );
      try {
