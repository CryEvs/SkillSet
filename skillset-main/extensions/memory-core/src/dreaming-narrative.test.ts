...
import * as configRuntimeModule from "skillset/plugin-sdk/config-runtime";
import {
  RequestScopedSubagentRuntimeError,
  SUBAGENT_RUNTIME_REQUEST_SCOPE_ERROR_CODE,
} from "skillset/plugin-sdk/error-runtime";
import * as memoryCoreHostRuntimeCoreModule from "skillset/plugin-sdk/memory-core-host-runtime-core";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("skillset.memoryCore.dreamingNarrative.fileLocks");
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("skillset:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("skillset-dreaming-backfill-");
    ...
    expect(content.match(/skillset:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("skillset-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- skillset:dreaming:diary:start -->");
    expect(content).toContain("<!-- skillset:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- skillset:dreaming:diary:start -->");
    const end = content.indexOf("<!-- skillset:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- skillset:dreaming:light:start -->\n- Candidate: test\n<!-- skillset:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("skillset-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("skillset-dreaming-narrative-");
    const stateDir = await createTempWorkspace("skillset-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("skillset-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("skillset-dreaming-narrative-");
