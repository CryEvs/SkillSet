...
import { canResolveEnvSecretRefInReadOnlyPath } from "skillset/plugin-sdk/extension-shared";
import { resolveSecretInputString, normalizeSecretInput } from "skillset/plugin-sdk/secret-input";
...
type WebFetchConfig = NonNullable<SkillSetConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: SkillSetConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: SkillSetConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: SkillSetConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: SkillSetConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: SkillSetConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: SkillSetConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: SkillSetConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: SkillSetConfig, override?: number): number {
