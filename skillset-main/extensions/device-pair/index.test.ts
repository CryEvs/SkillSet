...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredSkillSetTmpDir: vi.fn(() => path.join(os.tmpdir(), "skillset-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredSkillSetTmpDir: pluginApiMocks.resolvePreferredSkillSetTmpDir,
...
function createApi(params?: {
  runtime?: SkillSetPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as SkillSetPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: SkillSetPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredSkillSetTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredSkillSetTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the SkillSet iOS app:");
    ...
    expect(text).not.toContain("![SkillSet pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the SkillSet iOS app:");
