import { NON_ENV_SECRETREF_MARKER } from "skillset/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "skillset/plugin-sdk/runtime-env";
import { withEnv, withEnvAsync } from "skillset/plugin-sdk/testing";
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      await expect(maybeTool.execute({ query: "SkillSet" })).resolves.toMatchObject({
