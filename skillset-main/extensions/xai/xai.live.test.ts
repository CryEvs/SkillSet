...
import { loadConfig, type SkillSetConfig } from "skillset/plugin-sdk/config-runtime";
...
const LIVE_IMAGE_MODEL = process.env.SKILLSET_LIVE_XAI_IMAGE_MODEL?.trim() || "grok-imagine-image";
const liveEnabled = XAI_API_KEY.trim().length > 0 && process.env.SKILLSET_LIVE_TEST === "1";
...
describeLive("xai plugin live", () => {
  it("synthesizes TTS through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "SkillSet xAI text to speech integration test OK.",
      cfg,
      providerConfig: {
        apiKey: XAI_API_KEY,
        baseUrl: "https://api.x.ai/v1",
    ...
    const telephony = await speechProvider.synthesizeTelephony?.({
      text: "SkillSet xAI telephony check OK.",
      cfg,
      providerConfig: {
        apiKey: XAI_API_KEY,
        baseUrl: "https://api.x.ai/v1",
  ...
  it("transcribes audio through the registered media provider", async () => {
    ...
    const speechProvider = requireRegisteredProvider(speechProviders, "xai");
    const cfg = createLiveConfig();
    const phrase = "SkillSet xAI speech to text integration test OK.";
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const speechProvider = requireRegisteredProvider(speechProviders, "xai");
    const cfg = createLiveConfig();
    const phrase = "SkillSet xAI realtime transcription integration test OK.";
