...
import type { ChannelSetupWizard } from "skillset/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "skillset/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<SkillSetConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: SkillSetConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: SkillSetConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: SkillSetConfig, accountId: string): string {
  if (
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so SkillSet can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for SkillSet" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("skillset channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
