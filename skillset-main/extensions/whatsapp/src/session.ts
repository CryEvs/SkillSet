...
import { VERSION } from "skillset/plugin-sdk/cli-runtime";
import { resolveAmbientNodeProxyAgent } from "skillset/plugin-sdk/extension-shared";
import { danger, success } from "skillset/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "skillset/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "skillset/plugin-sdk/text-runtime";
import {
  readCredsJsonRaw,
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("skillset channels login")}`,
              ),
            );
          }
        }
