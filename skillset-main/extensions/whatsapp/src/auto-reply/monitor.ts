import { resolveAccountEntry } from "skillset/plugin-sdk/account-core";
import { resolveInboundDebounceMs } from "skillset/plugin-sdk/channel-inbound";
import { formatCliCommand } from "skillset/plugin-sdk/cli-runtime";
import { hasControlCommand } from "skillset/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "skillset/plugin-sdk/infra-runtime";
import { enqueueSystemEvent } from "skillset/plugin-sdk/infra-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "skillset/plugin-sdk/reply-history";
import { resolveAgentRoute } from "skillset/plugin-sdk/routing";
import { logVerbose } from "skillset/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "skillset/plugin-sdk/runtime-env";
import { getChildLogger } from "skillset/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "skillset/plugin-sdk/runtime-env";
    ...
    while (true) {
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("skillset channels login --channel web")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("skillset channels login --channel web")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
