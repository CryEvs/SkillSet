...
describe("jidToE164", () => {
  it("maps @lid using reverse mapping file", async () => {
    await withTempDir("skillset-state-", async (stateDir) => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("skillset-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("skillset-lid-a-", async (first) => {
      await withTempDir("skillset-lid-b-", (second) => {
