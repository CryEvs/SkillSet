...
import { optimizeImageToPng } from "skillset/plugin-sdk/media-runtime";
import { resolveStateDir } from "skillset/plugin-sdk/state-paths";
import { resolvePreferredSkillSetTmpDir } from "skillset/plugin-sdk/temp-path";
import { captureEnv } from "skillset/plugin-sdk/testing";
import { mockPinnedHostnameResolution } from "skillset/plugin-sdk/testing";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredSkillSetTmpDir(), "skillset-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub SKILLSET_STATE_DIR.
    // Also keep it outside the SkillSet temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["SKILLSET_STATE_DIR"]);
    process.env.SKILLSET_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "skillset-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredSkillSetTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default SkillSet state workspace and sandbox roots", async () => {
  ...
  it("rejects default SkillSet state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
