...
vi.mock("skillset/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("skillset/plugin-sdk/media-runtime")>(
    "skillset/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `skillset-inbound-media-${crypto.randomUUID()}`);
