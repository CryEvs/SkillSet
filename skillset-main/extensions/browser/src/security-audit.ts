import type { SkillSetPluginSecurityAuditContext } from "skillset/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "skillset/plugin-sdk/secret-input";
import { formatCliCommand } from "skillset/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "skillset/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: SkillSetPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("skillset security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.SKILLSET_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.SKILLSET_GATEWAY_PASSWORD) ||
