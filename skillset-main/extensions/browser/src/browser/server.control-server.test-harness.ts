...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "skillset" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.SKILLSET_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "skillset",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/skillset" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    skillset: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchSkillSetChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveSkillSetUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopSkillSetChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "skillset";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.SKILLSET_GATEWAY_PORT;
  process.env.SKILLSET_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.SKILLSET_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.SKILLSET_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.SKILLSET_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.SKILLSET_GATEWAY_PASSWORD;
  } else {
    process.env.SKILLSET_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
