...
const { stopSkillSetChromeMock } = vi.hoisted(() => ({
  stopSkillSetChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopSkillSetChrome: stopSkillSetChromeMock,
}));
...
beforeEach(() => {
  ...
  stopSkillSetChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["skillset", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      skillset: vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap.skillset).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopSkillSetChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("skillset browser stop failed: Error: oops");
  });
});
