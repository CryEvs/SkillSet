  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          skillset: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.SKILLSET_GATEWAY_PORT;
    process.env.SKILLSET_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.SKILLSET_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.SKILLSET_GATEWAY_PASSWORD;
