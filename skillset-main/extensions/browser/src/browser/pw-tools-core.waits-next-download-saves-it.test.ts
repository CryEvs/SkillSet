...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredSkillSetTmpDir: vi.fn(() => "/tmp/skillset"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-skillset-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-skillset-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredSkillSetTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredSkillSetTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredSkillSetTmpDir.mockReturnValue("/tmp/skillset");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".skillset-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredSkillSetTmpDir.mockReturnValue("/tmp/skillset-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "skillset-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "skillset-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredSkillSetTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "skillset-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "skillset-preferred", "downloads")}${path.sep}`),
    );
  });
