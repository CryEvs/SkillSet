...
function localSkillSetProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "skillset",
...
function createLocalSkillSetResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localSkillSetProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveSkillSetUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-reset-"));
    const profileDir = path.join(tempRoot, "skillset");
    ...
    const ops = createLocalSkillSetResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveSkillSetUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-reset-no-own-"));
    const profileDir = path.join(tempRoot, "skillset");
    fs.mkdirSync(profileDir, { recursive: true });
