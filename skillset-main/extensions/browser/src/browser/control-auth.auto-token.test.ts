...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => SkillSetConfig>(),
  writeConfigFile: vi.fn<(cfg: SkillSetConfig) => Promise<void>>(async (_cfg) => {}),
  resolveGatewayAuth: vi.fn(
    ({
      authConfig,
    }: {
      authConfig?: NonNullable<NonNullable<SkillSetConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: SkillSetConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): SkillSetConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: SkillSetConfig) {
