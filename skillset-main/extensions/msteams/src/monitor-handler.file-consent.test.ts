...
import type { SkillSetConfig, PluginRuntime, RuntimeEnv } from "../runtime-api.js";
import {
  ...
  return {
    ...
    state: {
      resolveStateDir: (env?: NodeJS.ProcessEnv) => {
        ...
        return stateDir ?? path.join(os.homedir(), ".skillset");
      },
    },
...
describe("msteams file consent invoke FS fallback", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "skillset-msteams-invoke-"));
    process.env.SKILLSET_STATE_DIR = tmpDir;
