...
import { resolvePluginConfigObject, type SkillSetConfig } from "skillset/plugin-sdk/config-runtime";
...
} from "skillset/plugin-sdk/provider-stream-shared";
import {
 ...
 * IDs that may not contain the model name). When SkillSet's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if SkillSet identifies this as an Anthropic model via the ARN heuristic.
 ...
 * the SkillSet config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: SkillSetPluginApi): void {
  ...
  function resolveCurrentPluginConfig(
    config: SkillSetConfig | undefined,
