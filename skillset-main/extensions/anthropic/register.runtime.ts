import { formatCliCommand, parseDurationMs } from "skillset/plugin-sdk/cli-runtime";
import type {
  SkillSetPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type SkillSetConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  upsertAuthProfile,
  validateAnthropicSetupToken,
} from "skillset/plugin-sdk/provider-auth";
import {
  cloneFirstTemplateModel,
  type ProviderPlugin,
} from "skillset/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "skillset/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("skillset models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("skillset models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("skillset doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: SkillSetPluginApi): void {
