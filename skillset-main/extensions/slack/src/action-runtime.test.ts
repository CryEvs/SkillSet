...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): SkillSetConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as SkillSetConfig;
  ...
  async function resolveReadToken(cfg: SkillSetConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: SkillSetConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
