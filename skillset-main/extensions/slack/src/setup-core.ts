import { hasConfiguredSecretInput } from "skillset/plugin-sdk/secret-input";
import {
  ...
  type SkillSetConfig,
} from "skillset/plugin-sdk/setup-runtime";
import { formatDocsLink } from "skillset/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: SkillSetConfig, accountId: string): SkillSetConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: SkillSetConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: SkillSetConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: SkillSetConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: SkillSetConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: SkillSetConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: SkillSetConfig; accountId: string }) =>
    ...
    disable: (cfg: SkillSetConfig) => setSetupChannelEnabled(cfg, channel, false),
