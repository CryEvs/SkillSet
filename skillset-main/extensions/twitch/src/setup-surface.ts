...
import { normalizeOptionalAccountId } from "skillset/plugin-sdk/account-id";
import { getChatChannelMeta, type ChannelPlugin } from "skillset/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: SkillSetConfig, requestedAccountId?: string): string {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: SKILLSET_TWITCH_ACCESS_TOKEN",
...
export async function configureWithEnvToken(
  ...
  accountId: string = resolveSetupAccountId(cfg),
): Promise<{ cfg: SkillSetConfig } | null> {
  const resolvedAccountId = accountId.trim()
    ? normalizeRequestedSetupAccountId(accountId)
    : resolveSetupAccountId(cfg);
  ...
  const useEnv = await prompter.confirm({
    message: "Twitch env var SKILLSET_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.SKILLSET_TWITCH_ACCESS_TOKEN?.trim();
