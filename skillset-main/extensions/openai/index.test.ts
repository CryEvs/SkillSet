...
import * as providerAuth from "skillset/plugin-sdk/provider-auth-runtime";
import * as providerHttp from "skillset/plugin-sdk/provider-http";
import type { ProviderPlugin } from "skillset/plugin-sdk/provider-model-shared";
...
type OpenAIRefreshDelegateGlobal = typeof globalThis & {
  __SKILLSET_TEST_REFRESH_OPENAI_CODEX_TOKEN__?: (...args: unknown[]) => unknown;
};
...
vi.mock("@mariozechner/pi-ai/oauth", () => ({
  ...
  refreshOpenAICodexToken: vi.fn((...args: unknown[]) =>
    openAIRefreshDelegateGlobal().__SKILLSET_TEST_REFRESH_OPENAI_CODEX_TOKEN__?.(...args),
  ),
}));
...
describe("openai plugin", () => {
  ...
  it("bootstraps the env proxy dispatcher before refreshing codex oauth credentials", async () => {
    ...
    openAIRefreshDelegateGlobal().__SKILLSET_TEST_REFRESH_OPENAI_CODEX_TOKEN__ =
    ...
    try {
      ...
      delete openAIRefreshDelegateGlobal().__SKILLSET_TEST_REFRESH_OPENAI_CODEX_TOKEN__;
    }
  });
