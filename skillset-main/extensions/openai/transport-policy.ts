...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-skillset-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-skillset-turn-id": turnId,
      "x-skillset-turn-attempt": attempt,
    },
    metadata: {
      skillset_session_id: sessionHeaders["x-skillset-session-id"] ?? "",
      skillset_turn_id: turnId,
      skillset_turn_attempt: attempt,
      skillset_transport: ctx.transport,
    },
  };
}
