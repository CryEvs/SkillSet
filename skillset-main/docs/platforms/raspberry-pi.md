---
summary: "SkillSet on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up SkillSet on a Raspberry Pi
  - Running SkillSet on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# SkillSet on Raspberry Pi

## Goal

Run a persistent, always-on SkillSet Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u skillset-gateway.service -f
```

## 9) Access the SkillSet Dashboard
...
```bash
ssh user@gateway-host 'skillset dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/skillset-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/skillset-compile-cache
mkdir -p /var/tmp/skillset-compile-cache
export SKILLSET_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `SKILLSET_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running SkillSet, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit skillset-gateway.service
```

```ini
[Service]
Environment=SKILLSET_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/skillset-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep SkillSet state/cache on SSD-backed storage to avoid SD-card
...
Most SkillSet features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled skillset-gateway.service

# Enable if not
systemctl --user enable skillset-gateway.service
...
```bash
# Check logs
journalctl --user -u skillset-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/skillset  # if using hackable install
npm run build
systemctl --user restart skillset-gateway.service
```
