---
summary: "SkillSet on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up SkillSet on DigitalOcean
  - Looking for cheap VPS hosting for SkillSet
title: "DigitalOcean (Platform)"
---

# SkillSet on DigitalOcean

## Goal

Run a persistent SkillSet Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
skillset config set gateway.bind tailnet
skillset gateway restart
```
...
```bash
skillset channels login whatsapp
# Scan QR code
```
...
- `~/.skillset/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u skillset-gateway.service --no-pager -n 50
```
