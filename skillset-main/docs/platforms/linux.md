...
2. `npm i -g skillset@latest`
3. `skillset onboard --install-daemon`
...
SkillSet installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `skillset gateway install` and
`skillset onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/skillset-gateway[-<profile>].service`:

```
[Unit]
Description=SkillSet Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. SkillSet therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, SkillSet starts the child through a short
...
- SkillSet-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `SKILLSET_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
