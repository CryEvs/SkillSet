...
service via the external `skillset` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.skillset.gateway`
  (or `ai.skillset.<profile>` when using `--profile`/`SKILLSET_PROFILE`; legacy `com.skillset.*` is supported).
