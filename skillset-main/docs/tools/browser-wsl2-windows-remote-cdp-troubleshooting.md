...
read_when:
  - Running SkillSet Gateway in WSL2 while Chrome lives on Windows
...
- SkillSet Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/skillset/skillset/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- SkillSet and Chrome are on the same machine
...
- SkillSet points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, SkillSet is not the problem yet.
...
Fix that before touching SkillSet config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point SkillSet at the address that is reachable from WSL2:
...
- use HTTP(S) when you want SkillSet to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting SkillSet to succeed
...
```bash
skillset browser open https://example.com --browser-profile remote
skillset browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `skillset browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `skillset browser stop --browser-profile remote`
...
3. SkillSet config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug SkillSet config or Control UI auth
