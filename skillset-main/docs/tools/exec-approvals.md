...
`ask: "always"` in `~/.skillset/exec-approvals.json` keeps prompting even if
session or config defaults request `ask: "on-miss"`.
Use `skillset approvals get`, `skillset approvals get --gateway`, or
`skillset approvals get --node <id|name|ip>` to inspect the requested policy,
host policy sources, and the effective result.
For the local machine, `skillset exec-policy show` exposes the same merged view and
`skillset exec-policy set|preset` can synchronize the local requested policy with the
local host approvals file in one step. When a local scope requests `host=node`,
`skillset exec-policy show` reports that scope as node-managed at runtime instead of
...
- **gateway host** → `skillset` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, SkillSet also tries to bind
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.skillset/exec-approvals.sock",
...
- requested exec policy in SkillSet config (`tools.exec.*`)
- host-local approvals policy in `~/.skillset/exec-approvals.json`
...
- In YOLO mode, SkillSet does not add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
```bash
skillset config set tools.exec.host gateway
skillset config set tools.exec.security full
skillset config set tools.exec.ask off
...
```bash
skillset approvals set --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `skillset exec-policy` does not synchronize node approvals
- `skillset exec-policy set --host node` is rejected
- node exec approvals are fetched from the node at runtime, so node-targeted updates must use `skillset approvals --node ...`
...
When `tools.exec.strictInlineEval=true`, SkillSet treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.skillset/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `skillset approvals allowlist ...`).
- `skillset security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `skillset doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, SkillSet still rejects the `env` builtin in safe-bin
...
`~/.skillset/exec-approvals.json` directly.

CLI: `skillset approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
After an approved async exec finishes, SkillSet sends a followup `agent` turn to the same session.
...
topics, SkillSet preserves the topic for the approval prompt and the post-approval follow-up.
...
When an async exec approval is denied, SkillSet prevents the agent from reusing
