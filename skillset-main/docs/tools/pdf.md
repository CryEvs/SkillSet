...
The tool is only registered when SkillSet can resolve a PDF-capable model config for the agent:
...
- The fallback chain is auth-aware. A configured `provider/model` only counts if
  SkillSet can actually authenticate that provider for the agent.
...
- If text extraction succeeds but image extraction would require vision on a
  text-only model, SkillSet drops the rendered images and continues with the
  extracted text.
