...
read_when:
  - You want shorter `exec` or `bash` tool results in SkillSet
...
```bash
skillset config set plugins.entries.tokenjuice.enabled true
```

Equivalent:

```bash
skillset plugins enable tokenjuice
```

SkillSet already ships the plugin. There is no separate `plugins install`
or `tokenjuice install skillset` step.
...
```bash
skillset config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
skillset plugins disable tokenjuice
```
