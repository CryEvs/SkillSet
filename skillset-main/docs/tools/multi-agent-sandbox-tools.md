...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `skillset sandbox explain`.
...
Legacy `agent.*` configs are migrated by `skillset doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=skillset-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${SKILLSET_STATE_DIR:-$HOME/.skillset}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
