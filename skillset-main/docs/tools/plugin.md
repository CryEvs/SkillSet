---
summary: "Install, configure, and manage SkillSet plugins"
read_when:
...
Plugins extend SkillSet with new capabilities: channels, model providers,
...
search, and more. Some plugins are **core** (shipped with SkillSet), others
...
<Steps>
  ...
  <Step title="Install a plugin">
    ```bash
    # From npm
    skillset plugins install @skillset/voice-call

    # From a local directory or archive
    skillset plugins install ./my-plugin
    skillset plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install clawhub:@skillset/voice-call
...
`skillset doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`skillset.install.allowInvalidConfigRecovery`.

Packaged SkillSet installs do not eagerly install every bundled plugin's
runtime dependency tree. When a bundled SkillSet-owned plugin is active from
...
`skillset plugins install`.

## Plugin types

SkillSet recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `skillset.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to SkillSet features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `skillset plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
| Plugin          | Package                | Docs                                 |
| --------------- | ---------------------- | ------------------------------------ |
| Matrix          | `@skillset/matrix`     | [Matrix](/channels/matrix)           |
| Microsoft Teams | `@skillset/msteams`    | [Microsoft Teams](/channels/msteams) |
| Nostr           | `@skillset/nostr`      | [Nostr](/channels/nostr)             |
| Voice Call      | `@skillset/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@skillset/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@skillset/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with SkillSet)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` — bundled browser plugin for the browser tool, `skillset browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
watch + in-process restart enabled (the default `skillset gateway` path), that
...
SkillSet scans for plugins in this order (first match wins):
...
<Steps>
  ...
  <Step title="Workspace plugins">
    `\<workspace\>/.skillset/<plugin-root>/*.ts` and `\<workspace\>/.skillset/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global plugins">
    `~/.skillset/<plugin-root>/*.ts` and `~/.skillset/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with SkillSet. Many are enabled by default (model providers, speech).
...
```bash
skillset plugins list                       # compact inventory
skillset plugins list --enabled            # only loaded plugins
skillset plugins list --verbose            # per-plugin detail lines
skillset plugins list --json               # machine-readable inventory
skillset plugins inspect <id>              # deep detail
skillset plugins inspect <id> --json       # machine-readable
skillset plugins inspect --all             # fleet-wide table
skillset plugins info <id>                 # inspect alias
skillset plugins doctor                    # diagnostics

skillset plugins install <package>         # install (ClawHub first, then npm)
skillset plugins install clawhub:<pkg>     # install from ClawHub only
skillset plugins install <spec> --force    # overwrite existing install
skillset plugins install <path>            # install from local path
skillset plugins install -l <path>         # link (no copy) for dev
skillset plugins install <plugin> --marketplace <source>
skillset plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
skillset plugins install <spec> --pin      # record exact resolved npm spec
skillset plugins install <spec> --dangerously-force-unsafe-install
skillset plugins update <id-or-npm-spec> # update one plugin
skillset plugins update <id-or-npm-spec> --dangerously-force-unsafe-install
skillset plugins update --all            # update all
skillset plugins uninstall <id>          # remove config/install records
...
Bundled plugins ship with SkillSet. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `skillset plugins enable <id>`.

`--force` overwrites an existing installed plugin or hook pack in place. Use
`skillset plugins update <id-or-npm-spec>` for routine upgrades of tracked npm
plugins. It is not supported with `--link`, which reuses the source path instead
of copying over a managed install target.

`skillset plugins update <id-or-npm-spec>` applies to tracked installs. Passing
...
the resolved version and recorded artifact identity, SkillSet skips the update
...
override instead, while `skillset skills install` remains the separate ClawHub
...
`skillset plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`skillset plugins` CLI reference](/cli/plugins) for full details.
...
SkillSet loads the entry object and calls `register(api)` during plugin
