---
summary: "Export redacted trajectory bundles for debugging an SkillSet agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an SkillSet session
...
Trajectory capture is SkillSet's per-session flight recorder. It records a
...
SkillSet writes the bundle under the workspace:

```text
.skillset/trajectory-exports/skillset-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.skillset/trajectory-exports/`. Absolute
...
Trajectory capture is on by default for SkillSet agent runs.
...
```json
{
  "traceSchema": "skillset-trajectory",
  "schemaVersion": 1
}
```
...
| `metadata.json`       | SkillSet version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
SkillSet also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `SKILLSET_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export SKILLSET_TRAJECTORY_DIR=/var/lib/skillset/trajectories
```

When this variable is set, SkillSet writes one JSONL file per session id in that
directory.

## Disable capture

Set `SKILLSET_TRAJECTORY=0` before starting SkillSet:

```bash
export SKILLSET_TRAJECTORY=0
```
...
SkillSet redacts sensitive values before writing export files:
...
- confirm SkillSet was started without `SKILLSET_TRAJECTORY=0`
- check whether `SKILLSET_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.skillset/trajectory-exports/`
