...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/skillset`)
...
If the page fails to load, start the Gateway first: `skillset gateway`.
...
created. Re-run `skillset devices list` before approval.
...
reconnect. SkillSet keeps the old approval active, blocks the broader reconnect,
and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `skillset devices revoke --device <id> --role <role>`. See
...
`/__skillset/control-ui-config.json`. That endpoint is gated by the same
...
- Config: view/edit `~/.skillset/skillset.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop skillset`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. SkillSet
...
```bash
skillset gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
SkillSet **blocks** Control UI connections without device identity.
...
```bash
SKILLSET_CONTROL_UI_BASE_PATH=/skillset/ pnpm ui:build
```
