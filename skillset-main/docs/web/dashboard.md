...
- Re-open anytime: `skillset dashboard` (copies link, opens browser if possible, shows SSH hint if headless).
...
- **Shared-secret token source**: `gateway.auth.token` (or
  `SKILLSET_GATEWAY_TOKEN`); `skillset dashboard` can pass it via URL fragment
...
- If `gateway.auth.token` is SecretRef-managed, `skillset dashboard`
...
- If `gateway.auth.token` is configured as a SecretRef and is unresolved in your
  current shell, `skillset dashboard` still prints a non-tokenized URL plus
  actionable auth setup guidance.
- **Shared-secret password**: use the configured `gateway.auth.password` (or
  `SKILLSET_GATEWAY_PASSWORD`). The dashboard does not persist passwords across
  reloads.
...
- Ensure the gateway is reachable (local: `skillset status`; remote: SSH tunnel `ssh -N -L 18789:127.0.0.1:18789 user@host` then open `http://127.0.0.1:18789/`).
...
- Retrieve or supply the shared secret from the gateway host:
  - Token: `skillset config get gateway.auth.token`
  ...
  - SecretRef-managed token: resolve the external secret provider or export
    `SKILLSET_GATEWAY_TOKEN` in this shell, then rerun `skillset dashboard`
  - No shared secret configured: `skillset doctor --generate-gateway-token`
