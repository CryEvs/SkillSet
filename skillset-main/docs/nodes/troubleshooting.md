...
```bash
skillset nodes describe --node <idOrNameOrIp>
skillset nodes canvas snapshot --node <idOrNameOrIp>
skillset logs --follow
```
...
skillset nodes status
skillset approvals get --node <idOrNameOrIp>
skillset approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`skillset approvals get --node ...`), not in the gateway pairing record.
