...
read_when:
  - You want to use Vercel AI Gateway with SkillSet
...
<Tip>
SkillSet auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    skillset onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    skillset models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
SkillSet accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. SkillSet resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the SkillSet Gateway runs as a daemon (launchd/systemd), make sure
    ...
    `~/.skillset/.env` or via `env.shellEnv` to ensure the gateway process can
    read it.
    </Warning>
