---
summary: "Use Z.AI (GLM models) with SkillSet"
read_when:
  - You want Z.AI / GLM models in SkillSet
...
for authentication. Create your API key in the Z.AI console. SkillSet uses the `zai` provider
with a Z.AI API key.
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. SkillSet detects the matching Z.AI endpoint from the key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        skillset onboard --auth-choice zai-api-key
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        skillset models list --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        skillset onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        skillset onboard --auth-choice zai-coding-cn

        # General API
        skillset onboard --auth-choice zai-global

        # General API CN (China region)
        skillset onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
SkillSet currently seeds the bundled `zai` provider with:
