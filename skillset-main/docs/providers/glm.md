---
summary: "GLM model family overview + how to use it in SkillSet"
read_when:
  - You want GLM models in SkillSet
...
GLM is a **model family** (not a company) available through the Z.AI platform. In SkillSet, GLM
...
<Steps>
  ...
  <Step title="Set GLM as the default model">
    ```bash
    skillset config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
...
<Tip>
`zai-api-key` lets SkillSet detect the matching Z.AI endpoint from the key and
...
SkillSet currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, SkillSet inspects the key format
