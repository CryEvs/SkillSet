---
summary: "Use Venice AI privacy-focused models in SkillSet"
read_when:
  - You want privacy-focused inference in SkillSet
...
<Steps>
  ...
  <Step title="Configure SkillSet">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        skillset onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    skillset agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, SkillSet shows all available Venice models. Pick based on your needs:
...
```bash
skillset models set venice/kimi-k2-5
skillset models set venice/claude-opus-4-6
```

List all available models:

```bash
skillset models list | grep venice
```

You can also run `skillset configure`, select **Model/auth**, and choose **Venice AI**.
...
SkillSet automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
skillset agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
skillset agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
skillset agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
skillset agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
skillset agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `skillset models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
