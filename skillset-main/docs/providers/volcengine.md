...
read_when:
  - You want to use Volcano Engine or Doubao models with SkillSet
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    skillset onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    skillset models list --provider volcengine
    skillset models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `skillset onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, SkillSet falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.skillset/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running SkillSet as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about SkillSet setup.
  </Card>
</CardGroup>
