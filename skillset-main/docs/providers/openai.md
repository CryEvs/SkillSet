---
summary: "Use OpenAI via API keys or Codex subscription in SkillSet"
read_when:
  - You want to use OpenAI models in SkillSet
...
OpenAI provides developer APIs for GPT models. SkillSet supports two auth routes:
...
OpenAI explicitly supports subscription OAuth usage in external tools and workflows like SkillSet.

## SkillSet feature coverage

| OpenAI capability         | SkillSet surface                          | Status                                                 |
| ------------------------- | ----------------------------------------- | ------------------------------------------------------ |
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        skillset onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        skillset onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        skillset models list --provider openai
        ```
      </Step>
    </Steps>
    ...
    <Warning>
    SkillSet does **not** expose `openai/gpt-5.3-codex-spark` on the direct API path. Live OpenAI API requests reject that model. Spark is Codex-only.
    </Warning>
    ...
    **Best for:** using your ChatGPT/Codex subscription instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        skillset onboard --auth-choice openai-codex
        ```

        Or run OAuth directly:

        ```bash
        skillset models auth login --provider openai-codex
        ```
        ...
        ```bash
        skillset models auth login --provider openai-codex --device-code
        ```
      </Step>
      <Step title="Set the default model">
        ```bash
        skillset config set agents.defaults.model.primary openai-codex/gpt-5.4
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        skillset models list --provider openai-codex
        ```
      </Step>
    </Steps>
    ...
    <Note>
    Onboarding no longer imports OAuth material from `~/.codex`. Sign in with browser OAuth (default) or the device-code flow above — SkillSet manages the resulting credentials in its own agent auth store.
    </Note>

    ### Context window cap

    SkillSet treats model metadata and the runtime context cap as separate values.
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for SkillSet on macOS" size=3840x2160 count=1
```
...
SkillSet adds a shared GPT-5 prompt contribution for GPT-5-family runs across providers. It applies by model id, so `openai/gpt-5.4`, `openai-codex/gpt-5.4`, `openrouter/openai/gpt-5.4`, `opencode/gpt-5.4`, and other compatible GPT-5 refs receive the same overlay. Older GPT-4.x models do not.
...
The GPT-5 contribution adds a tagged behavior contract for persona persistence, execution safety, tool discipline, output shape, completion checks, and verification. Channel-specific reply and silent-message behavior stays in the shared SkillSet system prompt and outbound delivery policy. The GPT-5 guidance is always enabled for matching models. The friendly interaction-style layer is separate and configurable.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    skillset config set agents.defaults.promptOverlays.gpt5.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="Speech-to-text">
    ...
    SkillSet's media-understanding transcription surface.

    - Default model: `gpt-4o-transcribe`
    - Endpoint: OpenAI REST `/v1/audio/transcriptions`
    - Input path: multipart audio file upload
    - Supported by SkillSet wherever inbound audio transcription uses
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    SkillSet uses WebSocket-first with SSE fallback (`"auto"`) for both `openai/*` and `openai-codex/*`.

    In `"auto"` mode, SkillSet:
  ...
  <Accordion title="WebSocket warm-up">
    SkillSet enables WebSocket warm-up by default for `openai/*` to reduce first-turn latency.
...
<a id="openai-fast-mode"></a>
  ...
  <Accordion title="Fast mode">
    SkillSet exposes a shared fast-mode toggle for both `openai/*` and `openai-codex/*`:

    - **Chat/UI:** `/fast status|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, SkillSet maps fast mode to OpenAI priority processing (`service_tier = "priority"`). Existing `service_tier` values are preserved, and fast mode does not rewrite `reasoning` or `text.verbosity`.
  ...
  <Accordion title="Priority processing (service_tier)">
    OpenAI's API exposes priority processing via `service_tier`. Set it per model in SkillSet:
    ...
    <Warning>
    `serviceTier` is only forwarded to native OpenAI endpoints (`api.openai.com`) and native Codex endpoints (`chatgpt.com/backend-api`). If you route either provider through a proxy, SkillSet leaves `service_tier` untouched.
    </Warning>

  </Accordion>

  <Accordion title="Server-side compaction (Responses API)">
    For direct OpenAI Responses models (`openai/*` on `api.openai.com`), SkillSet auto-enables server-side compaction:
  ...
  <Accordion title="Strict-agentic GPT mode">
    For GPT-5-family runs on `openai/*` and `openai-codex/*`, SkillSet can use a stricter embedded execution contract:
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    SkillSet treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
