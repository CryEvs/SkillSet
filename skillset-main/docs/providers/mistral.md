---
summary: "Use Mistral models and Voxtral transcription with SkillSet"
read_when:
  - You want to use Mistral models in SkillSet
...
SkillSet supports Mistral for both text/image model routing (`mistral/...`) and
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    skillset onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    skillset onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    skillset models list --provider mistral
    ```
  </Step>
</Steps>
...
SkillSet currently ships this bundled Mistral catalog:
...
<Note>
SkillSet defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    SkillSet maps the session **thinking** level to Mistral's API:

    | SkillSet thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
