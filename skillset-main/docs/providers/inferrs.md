---
summary: "Run SkillSet through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run SkillSet against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact SkillSet compat flags for inferrs
title: "inferrs"
---
...
OpenAI-compatible `/v1` API. SkillSet works with `inferrs` through the generic
`openai-completions` path.

`inferrs` is currently best treated as a custom self-hosted OpenAI-compatible
backend, not a dedicated SkillSet provider plugin.
...
<Steps>
  ...
  <Step title="Add an SkillSet provider entry">
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If SkillSet runs fail with an error like:
    ...
    SkillSet will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full SkillSet agent-runtime
    turns.
    ...
    That disables SkillSet's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal SkillSet agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than SkillSet's transport layer.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but skillset infer model run fails">
  ...
  <Accordion title="inferrs still crashes on larger agent turns">
    If SkillSet no longer gets schema errors but `inferrs` still crashes on larger
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running SkillSet against local model servers.
  </Card>
