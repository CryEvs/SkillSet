---
summary: "Use MiniMax models in SkillSet"
read_when:
  - You want MiniMax models in SkillSet
...
SkillSet's MiniMax provider defaults to **MiniMax M2.7**.
    ...
    <Tabs>
      <Tab title="International">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            skillset onboard --auth-choice minimax-global-oauth
            ```
          ...
          <Step title="Verify the model is available">
            ```bash
            skillset models list --provider minimax-portal
            ```
          </Step>
        </Steps>
      </Tab>
      <Tab title="China">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            skillset onboard --auth-choice minimax-cn-oauth
            ```
    ...
    <Tabs>
      <Tab title="International">
        <Steps>
          <Step title="Run onboarding">
            ```bash
            skillset onboard --auth-choice minimax-global-api
            ```
          ...
          <Step title="Verify the model is available">
            ```bash
            skillset models list --provider minimax
            ```
          </Step>
        </Steps>
      </Tab>
...
## Configure via `skillset configure`
...
entries, SkillSet materializes `MiniMax-M2.7` and
...
<AccordionGroup>
  ...
  <Accordion title="Thinking defaults">
    On `api: "anthropic-messages"`, SkillSet injects `thinking: { type: "disabled" }` unless thinking is already explicitly set in params/config.
  ...
  <Accordion title="Coding Plan usage details">
    ...
    - SkillSet normalizes MiniMax coding-plan usage to the same `% left` display used by other providers. MiniMax's raw `usage_percent` / `usagePercent` fields are remaining quota, not consumed quota, so SkillSet inverts them. Count-based fields win when present.
    - When the API returns `model_remains`, SkillSet prefers the chat-model entry, derives the window label from `start_time` / `end_time` when needed, and includes the selected model name in the plan label so coding-plan windows are easier to distinguish.
...
- Use `skillset models list` to confirm the current provider id, then switch with `skillset models set minimax/MiniMax-M2.7` or `skillset models set minimax-portal/MiniMax-M2.7`
...
<AccordionGroup>
  <Accordion title='"Unknown model: minimax/MiniMax-M2.7"'>
    ...
    - Running `skillset configure` and selecting a **MiniMax** auth option, or
