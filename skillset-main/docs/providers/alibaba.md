...
summary: "Alibaba Model Studio Wan video generation in SkillSet"
read_when:
  - You want to use Alibaba Wan video generation in SkillSet
  - You need Model Studio or DashScope API key setup for video generation
---

# Alibaba Model Studio

SkillSet ships a bundled `alibaba` video-generation provider for Wan models on
...
<Steps>
  ...
  <Step title="Verify the provider is available">
    ```bash
    skillset models list --provider alibaba
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Auth key priority">
    SkillSet checks for auth keys in this order:
