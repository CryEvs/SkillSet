---
summary: "Run SkillSet with LM Studio"
read_when:
  - You want to run SkillSet with open source models via LM Studio
...
3. SkillSet requires an LM Studio token value. Set `LM_API_TOKEN`:
...
```bash
skillset models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). SkillSet
...
`usage` object, SkillSet recovers token counts from llama.cpp-style
