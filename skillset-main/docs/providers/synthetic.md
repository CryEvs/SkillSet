---
summary: "Use Synthetic's Anthropic-compatible API in SkillSet"
read_when:
...
SkillSet registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    skillset onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
SkillSet's Anthropic client appends `/v1` to the base URL automatically, so use
...
`skillset models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that SkillSet appends `/v1` automatically.
