...
This is handy when you need to toggle obscure settings without editing `skillset.json`.
...
SkillSet keeps `src/cli/debug-timing.ts` as a small helper for local
...
`skillset models list`, a temporary patch in
...
```bash
SKILLSET_DEBUG_TIMING=1 pnpm skillset models list --all --provider moonshot
```

Example output from a temporary `models list` investigation:

```text
SkillSet CLI debug timing: models list
...
```bash
SKILLSET_DEBUG_TIMING=json pnpm skillset models list --all --provider moonshot \
...
extension `package.json` and `skillset.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.skillset-dev` and
...
```bash
pnpm gateway:dev
SKILLSET_PROFILE=dev skillset tui
```

If you don’t have a global install yet, run the CLI via `pnpm skillset ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `SKILLSET_PROFILE=dev`
   - `SKILLSET_STATE_DIR=~/.skillset-dev`
   - `SKILLSET_CONFIG_PATH=~/.skillset-dev/skillset.json`
   - `SKILLSET_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`SKILLSET_SKIP_CHANNELS=1`).
...
```bash
SKILLSET_PROFILE=dev skillset gateway --dev --reset
```
...
## Raw stream logging (SkillSet)

SkillSet can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.skillset/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
SKILLSET_RAW_STREAM=1
SKILLSET_RAW_STREAM_PATH=~/.skillset/logs/raw-stream.jsonl
```

Default file:

`~/.skillset/logs/raw-stream.jsonl`
