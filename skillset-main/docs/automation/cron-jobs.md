...
read_when:
  ...
  - Wiring external triggers (webhooks, Gmail) into SkillSet
...
# Check your jobs
skillset cron list
skillset cron show <job-id>

# See run history
skillset cron runs --id <job-id>
```

## How cron works

- Cron runs **inside the Gateway** process (not inside the model).
- Job definitions persist at `~/.skillset/cron/jobs.json` so restarts do not lose schedules.
- Runtime execution state persists next to it in `~/.skillset/cron/jobs-state.json`. If you track cron definitions in git, track `jobs.json` and gitignore `jobs-state.json`.
- After the split, older SkillSet versions can read `jobs.json` but may treat jobs as fresh because runtime fields now live in `jobs-state.json`.
...
together`, and similar hints) and no descendant subagent run is still
  responsible for the final answer, SkillSet re-prompts once for the actual
...
This fires ~5–6 times per month instead of 0–1 times per month. SkillSet uses Croner's default OR behavior here. To require both conditions, use Croner's `+` day-of-week modifier (`0 9 15 * +1`) or schedule on one field and guard the other in your job's prompt or command.
...
running, SkillSet suppresses that partial parent update instead of announcing it.
...
agent sends to the configured/current target, SkillSet skips the fallback
...
Wire Gmail inbox triggers to SkillSet via Google PubSub.

**Prerequisites**: `gcloud` CLI, `gog` (gogcli), SkillSet hooks enabled, Tailscale for the public HTTPS endpoint.

### Wizard setup (recommended)

```bash
skillset webhooks gmail setup --account skillset@gmail.com
```
...
When `hooks.enabled=true` and `hooks.gmail.account` is set, the Gateway starts `gog gmail watch serve` on boot and auto-renews the watch. Set `SKILLSET_SKIP_GMAIL_WATCHER=1` to opt out.
...
skillset cron show <jobId>

# Edit a job
skillset cron edit <jobId> --message "Updated prompt" --model "opus"

# Force run a job now
skillset cron run <jobId>

# Run only if due
skillset cron run <jobId> --due

# View run history
skillset cron runs --id <jobId> --limit 50

# Delete a job
skillset cron remove <jobId>

# Agent selection (multi-agent setups)
skillset cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops
skillset cron edit <jobId> --clear-agent
```

Model override note:

- `skillset cron add|edit --model ...` changes the job's selected model.
...
Disable cron: `cron.enabled: false` or `SKILLSET_SKIP_CRON=1`.
...
- Check `cron.enabled` and `SKILLSET_SKIP_CRON` env var.
- Confirm the Gateway is running continuously.
- For `cron` schedules, verify timezone (`--tz`) vs the host timezone.
- `reason: not-due` in run output means manual run was checked with `skillset cron run <jobId> --due` and the job was not due yet.
...
- If the isolated run returns only the silent token (`NO_REPLY` / `no_reply`),
  SkillSet suppresses direct outbound delivery and also suppresses the fallback
