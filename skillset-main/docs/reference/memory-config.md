...
This page lists every configuration knob for SkillSet memory search. For
...
`skillset.json` unless noted otherwise.
...
When `provider` is not set, SkillSet selects the first available:
...
If SkillSet runs on EC2 with a Bedrock-enabled instance role, just set the
provider and model:
...
When sqlite-vec is unavailable, SkillSet falls back to in-process cosine
...
| Key                   | Type     | Default                               | Description                                 |
| --------------------- | -------- | ------------------------------------- | ------------------------------------------- |
| `store.path`          | `string` | `~/.skillset/memory/{agentId}.sqlite` | Index location (supports `{agentId}` token) |
...
SkillSet prefers the current QMD collection and MCP query shapes, but keeps
older QMD releases working by falling back to legacy `--mask` collection flags
and older MCP tool names when needed.

QMD model overrides stay on the QMD side, not SkillSet config. If you need to
