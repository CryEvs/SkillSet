...
This is the full reference for `skillset onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.skillset/skillset.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `skillset doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though SkillSet now prefers Claude CLI reuse when available.
    ...
    - Auth profiles live in `~/.skillset/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.skillset/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.skillset/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$SKILLSET_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.skillset/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `skillset pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `skillset configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `skillset health`.
    - Tip: `skillset status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
  </Step>
...
```bash
export SKILLSET_GATEWAY_TOKEN="your-token"
skillset onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env SKILLSET_GATEWAY_TOKEN
```
...
- Stores it under `~/.skillset/tools/signal-cli/<version>/`.
...
Typical fields in `~/.skillset/skillset.json`:
...
`skillset agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.skillset/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.skillset/agents/<agentId>/sessions/`.
