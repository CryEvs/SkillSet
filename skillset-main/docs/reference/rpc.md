...
SkillSet integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- SkillSet owns lifecycle when `channels.signal.autoStart=true`.
...
- SkillSet spawns `imsg rpc` as a child process (legacy iMessage integration).
