...
This document explains how SkillSet manages sessions end-to-end:
...
SkillSet is designed around a single **Gateway process** that owns session state.
...
SkillSet persists sessions in two layers:
...
- Transcripts: `~/.skillset/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

SkillSet resolves these via `src/config/sessions.ts`.
...
In `mode: "warn"`, SkillSet reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
skillset sessions cleanup --dry-run
skillset sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.skillset/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
SkillSet intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When SkillSet splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, SkillSet
...
- If a trailing tool-result block would otherwise push the chunk over target,
  SkillSet preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (SkillSet consumes the events, but Pi decides when to compact).
...
SkillSet also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, SkillSet bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, SkillSet leaves it alone.
...
- If the provider fails or returns an empty result, SkillSet falls back to built-in LLM summarization automatically.
...
- `skillset status` (CLI)
- `skillset sessions` / `sessions --json`
...
SkillSet supports “silent” turns for background tasks where the user should not see intermediate output.
...
- SkillSet strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, SkillSet also suppresses **draft/typing streaming** when a
...
SkillSet uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but SkillSet’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `skillset status`.
