---
summary: "Get SkillSet installed and run your first chat in minutes."
read_when:
...
Install SkillSet, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.skillset/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.skillset/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run SkillSet as a service account or want custom paths:

- `SKILLSET_HOME` — home directory for internal path resolution
- `SKILLSET_STATE_DIR` — override the state directory
- `SKILLSET_CONFIG_PATH` — override the config file path
