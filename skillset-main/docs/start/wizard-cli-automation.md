---
summary: "Scripted onboarding and agent setup for the SkillSet CLI"
read_when:
...
Use `--non-interactive` to automate `skillset onboard`.
...
Anthropic setup-token remains available as a supported onboarding token path, but SkillSet now prefers Claude CLI reuse when available.
