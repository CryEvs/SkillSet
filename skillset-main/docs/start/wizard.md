...
CLI onboarding is the **recommended** way to set up SkillSet on macOS,
...
`skillset dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`skillset configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.skillset/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `skillset doctor` first.
</Note>
...
Use `skillset agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.skillset/workspace-<agentId>`.
...
- CLI command reference: [`skillset onboard`](/cli/onboard)
