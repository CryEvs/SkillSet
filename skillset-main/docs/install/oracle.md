---
summary: "Host SkillSet on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up SkillSet on Oracle Cloud
  - Looking for free VPS hosting for SkillSet
...
Run a persistent SkillSet Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@skillset`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=skillset --reset` to re-authenticate.

**Gateway will not start** -- Run `skillset doctor --non-interactive` and check logs with `journalctl --user -u skillset-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep SkillSet up to date
