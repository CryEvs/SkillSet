---
summary: "Move (migrate) an SkillSet install from one machine to another"
read_when:
  - You are moving SkillSet to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating SkillSet to a New Machine

This guide moves an SkillSet gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.skillset/` by default) and your **workspace**, you preserve:

- **Config** -- `skillset.json` and all gateway settings
...
<Tip>
Run `skillset status` on the old machine to confirm your state directory path.
Custom profiles use `~/.skillset-<profile>/` or a path set via `SKILLSET_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf skillset-state.tgz .skillset
    ```

    If you use multiple profiles (e.g. `~/.skillset-work`), archive each separately.

  </Step>

  <Step title="Install SkillSet on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.skillset/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `SKILLSET_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `skillset doctor`.
  </Accordion>

  <Accordion title="Copying only skillset.json">
...
- [ ] `skillset status` shows the gateway running
