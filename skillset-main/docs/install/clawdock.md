---
summary: "ClawDock shell helpers for Docker-based SkillSet installs"
read_when:
  - You run SkillSet with Docker often and want shorter day-to-day commands
...
ClawDock is a small shell-helper layer for Docker-based SkillSet installs.
...
| `clawdock-cli <command>`  | Run SkillSet CLI commands in Docker           |
...
- `~/.skillset/.env` for env-backed provider keys and bot tokens
- `~/.skillset/agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `~/.skillset/skillset.json` for behavior config

Use `clawdock-show-config` when you want to inspect the `.env` files and `skillset.json` quickly. It redacts `.env` values in its printed output.
