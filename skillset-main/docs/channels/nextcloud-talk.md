...
Nextcloud Talk ships as a bundled plugin in current SkillSet releases, so
...
```bash
skillset plugins install @skillset/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
skillset plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "SkillSet" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `skillset pairing list nextcloud-talk`
  - `skillset pairing approve nextcloud-talk <CODE>`
