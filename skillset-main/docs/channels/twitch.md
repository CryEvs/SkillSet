...
read_when:
  - Setting up Twitch chat integration for SkillSet
title: "Twitch"
---

# Twitch

Twitch chat support via IRC connection. SkillSet connects as a Twitch user (bot account) to receive and send messages in channels.

## Bundled plugin

Twitch ships as a bundled plugin in current SkillSet releases, so normal
...
```bash
skillset plugins install @skillset/twitch
```

Local checkout (when running from a git repo):

```bash
skillset plugins install ./path/to/local/twitch-plugin
```
...
5. Configure the token:
   - Env: `SKILLSET_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "skillset", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use SKILLSET_TWITCH_ACCESS_TOKEN env var)
...
```bash
SKILLSET_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
