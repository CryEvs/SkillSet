...
read_when:
  - You want to connect SkillSet to LINE
...
LINE connects to SkillSet via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current SkillSet releases, so normal
...
```bash
skillset plugins install @skillset/line
```

Local checkout (when running from a git repo):

```bash
skillset plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so SkillSet applies strict pre-auth body limits and timeout before verification.
- SkillSet processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
skillset pairing list line
skillset pairing approve line <CODE>
```
...
Outbound media URLs must be public HTTPS URLs. SkillSet validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
