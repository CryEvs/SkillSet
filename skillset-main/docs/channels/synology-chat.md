---
summary: "Synology Chat webhook setup and SkillSet config"
read_when:
  - Setting up Synology Chat with SkillSet
...
Synology Chat ships as a bundled plugin in current SkillSet releases, so normal
...
```bash
skillset plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   ...
   - `skillset onboard` now shows Synology Chat in the same channel setup list as `skillset channels add`.
   - Non-interactive setup: `skillset channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your SkillSet gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in SkillSet.
   - Guided: `skillset onboard`
   - Direct: `skillset channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- SkillSet accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `skillset pairing list synology-chat`
  - `skillset pairing approve synology-chat <CODE>`
...
```bash
skillset message send --channel synology-chat --target 123456 --text "Hello from SkillSet"
skillset message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before SkillSet forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. SkillSet now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached SkillSet
- `Rate limit exceeded`:
