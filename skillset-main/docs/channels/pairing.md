...
read_when:
  ...
  - Reviewing SkillSet security posture
title: "Pairing"
---

# Pairing

“Pairing” is SkillSet’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `skillset-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.skillset/credentials/`:
...
3. On your phone, open the SkillSet iOS app → Settings → Gateway.
...
reconnects asking for more scopes or a broader role, SkillSet keeps the
existing approval as-is and creates a fresh pending upgrade request. Use
`skillset devices list` to compare the currently approved access with the newly
requested access before you approve.

### Node pairing state storage

Stored under `~/.skillset/devices/`:
...
- The legacy `node.pair.*` API (CLI: `skillset nodes pending|approve|reject|rename`) is a
