...
Tlon is a decentralized messenger built on Urbit. SkillSet connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current SkillSet releases, so normal packaged
...
```bash
skillset plugins install @skillset/tlon
```

Local checkout (when running from a git repo):

```bash
skillset plugins install ./path/to/local/tlon-plugin
```
...
By default, SkillSet blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `skillset message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, SkillSet replies in-thread.
