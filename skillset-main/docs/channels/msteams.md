...
Microsoft Teams ships as a bundled plugin in current SkillSet releases, so no
...
```bash
skillset plugins install @skillset/msteams
```

Local checkout (when running from a git repo):

```bash
skillset plugins install ./path/to/local/msteams-plugin
```
...
3. Configure SkillSet with those credentials.
...
- Talk to SkillSet via Teams DMs, group chats, or channels.
...
- On startup, SkillSet resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.skillset/skillset.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring SkillSet, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `skillset-msteams` (must be unique) |
...
For production deployments, SkillSet supports **federated authentication** as a more secure alternative to client secrets. Two methods are available:
...
3. At runtime, SkillSet uses `@azure/identity` to acquire tokens from the Azure IMDS endpoint (`169.254.169.254`).
...
**Default behavior:** When `authType` is not set, SkillSet defaults to client secret authentication. Existing configurations continue to work without changes.
...
1. **Ensure the Microsoft Teams plugin is available**
   ...
   - Older/custom installs can add it manually:
     - From npm: `skillset plugins install @skillset/msteams`
     - From a local checkout: `skillset plugins install ./path/to/local/msteams-plugin`
...
SkillSet exposes a Graph-backed `member-info` action for Microsoft Teams so agents and automations can resolve channel member details (display name, email, role) directly from Microsoft Graph.
...
```json5
{
  ...
  description: { short: "SkillSet in Teams", full: "SkillSet in Teams" },
...
SkillSet handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, SkillSet only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/SkillSetShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

SkillSet sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `skillset message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.skillset/msteams-polls.json`.
- The gateway must stay online to record votes.
- Polls do not auto-post result summaries yet (inspect the store file if needed).

## Presentation Cards

Send semantic presentation payloads to Teams users or conversations using the `message` tool or CLI. SkillSet renders them as Teams Adaptive Cards from the generic presentation contract.
...
```bash
skillset message send --channel msteams \
...
skillset message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
skillset message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
skillset message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send a presentation card to a conversation
skillset message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --presentation '{"title":"Hello","blocks":[{"type":"text","text":"Hello"}]}'
```
