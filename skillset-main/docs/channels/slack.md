...
```json
{
  "display_information": {
    "name": "SkillSet",
    "description": "Slack connector for SkillSet"
  },
  "features": {
    "bot_user": {
    ...
    "slash_commands": [
      {
        "command": "/skillset",
        "description": "Send a message to SkillSet",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `skillset pairing approve slack <code>`.
...
- If streaming fails mid-reply, SkillSet falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while SkillSet is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
- The interactive callback values are SkillSet-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, SkillSet falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `skillset channels status --probe --json` shows `botTokenStatus` or
