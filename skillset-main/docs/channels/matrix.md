...
Matrix is a bundled channel plugin for SkillSet.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Bundled plugin

Matrix ships as a bundled plugin in current SkillSet releases, so normal
...
```bash
skillset plugins install @skillset/matrix
```

Install from a local checkout:

```bash
skillset plugins install ./path/to/local/matrix-plugin
```
...
1. Ensure the Matrix plugin is available.
   - Current packaged SkillSet releases already bundle it.
...
```bash
skillset channels add
skillset configure --section channels
```
...
- To resolve room names before saving, use `skillset channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "SkillSet Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.skillset/credentials/matrix/`.
The default account uses `credentials.json`; named accounts use `credentials-<account>.json`.
When cached credentials exist there, SkillSet treats Matrix as configured for setup, doctor, and channel-status discovery even if current auth is not set directly in config.
...
`autoJoin` applies to all Matrix invites, including DM-style invites. SkillSet cannot reliably
...
Set `channels.matrix.streaming` to `"partial"` when you want SkillSet to send a single live preview
...
- `streaming: "off"` is the default. SkillSet waits for the final reply and sends it once.
...
- If the preview no longer fits in one Matrix event, SkillSet stops preview streaming and falls back to normal final delivery.
- Media replies still send attachments normally. If a stale preview can no longer be reused safely, SkillSet redacts it before sending the final media reply.
...
- bot user = the SkillSet Matrix account that sends the reply
...
1. Configure SkillSet to use quiet previews:
...
SkillSet marks finalized text-only preview edits with:

```json
{
  "com.skillset.finalized_preview": true
}
```
...
```bash
curl -sS -X PUT \
  "https://matrix.example.org/_matrix/client/v3/pushrules/global/override/skillset-finalized-preview-botname" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  --data '{
    "conditions": [
      ...
      {
        "kind": "event_property_is",
        "key": "content.com\\.skillset\\.finalized_preview",
        "value": true
      },
...
- `skillset-finalized-preview-botname`: a rule ID unique to this bot for this receiving user
- `@bot:example.org`: your SkillSet Matrix bot MXID, not the receiving user's MXID
...
- If one receiving user should notify for multiple SkillSet Matrix bot accounts, create one rule per bot with a unique rule ID for each sender match.
- A simple pattern is `skillset-finalized-preview-<botname>`, such as `skillset-finalized-preview-ops` or `skillset-finalized-preview-support`.

The rule is evaluated against the event sender:

- authenticate with the receiving user's token
- match `sender` against the SkillSet bot MXID
...
- This only affects text-only preview edits that SkillSet can safely finalize in place. Media fallbacks and stale-preview fallbacks still use normal Matrix delivery.
...
- No special `homeserver.yaml` change is required for finalized SkillSet preview notifications.
...
By default, Matrix messages from other configured SkillSet Matrix accounts are ignored.
...
- SkillSet still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; SkillSet treats "bot-authored" as "sent by another configured Matrix account on this SkillSet gateway".
...
```bash
skillset matrix verify status
```

Verbose status (full diagnostics):

```bash
skillset matrix verify status --verbose
```

Include the stored recovery key in machine-readable output:

```bash
skillset matrix verify status --include-recovery-key --json
```

Bootstrap cross-signing and verification state:

```bash
skillset matrix verify bootstrap
```

Verbose bootstrap diagnostics:

```bash
skillset matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
skillset matrix verify bootstrap --force-reset-cross-signing
```

Verify this device with a recovery key:

```bash
skillset matrix verify device "<your-recovery-key>"
```

Verbose device verification details:

```bash
skillset matrix verify device "<your-recovery-key>" --verbose
```

Check room-key backup health:

```bash
skillset matrix verify backup status
```

Verbose backup health diagnostics:

```bash
skillset matrix verify backup status --verbose
```

Restore room keys from server backup:

```bash
skillset matrix verify backup restore
```

Verbose restore diagnostics:

```bash
skillset matrix verify backup restore --verbose
```
...
```bash
skillset matrix verify backup reset --yes
```
...
```bash
skillset matrix verify status --account assistant
skillset matrix verify backup restore --account assistant
skillset matrix devices list --account assistant
```
...
SkillSet treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `skillset matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for SkillSet to treat the device as fully verified.

### What bootstrap does

`skillset matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, SkillSet tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
backup baseline for future messages, use `skillset matrix verify backup reset --yes`.
Do this only when you accept that unrecoverable old encrypted history will stay
unavailable and that SkillSet may recreate secret storage if the current backup
...
If startup still finds broken bootstrap state, SkillSet can attempt a guarded repair path even when `channels.matrix.password` is not configured.
If the homeserver requires password-based UIA for that repair, SkillSet logs a warning and keeps startup non-fatal instead of aborting the bot.
If the current device is already owner-signed, SkillSet preserves that identity instead of resetting it automatically.
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by SkillSet.
For self-verification flows, SkillSet also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, SkillSet auto-accepts the request and then waits for the SAS flow to proceed normally.
...
SkillSet does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old SkillSet-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
skillset matrix devices list
```

Remove stale SkillSet-managed devices with:

```bash
skillset matrix devices prune-stale
```
...
`~/.skillset/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, SkillSet reuses the best existing
...
```bash
skillset matrix profile set --name "SkillSet Assistant"
skillset matrix profile set --avatar-url https://cdn.example.org/avatar.png
```

Add `--account <id>` when you want to target a named Matrix account explicitly.

Matrix accepts `mxc://` avatar URLs directly. When you pass an `http://` or `https://` avatar URL, SkillSet uploads it to Matrix first and stores the resolved `mxc://` URL back into `channels.matrix.avatarUrl` (or the selected account override).
...
- Same-session DM user-target reuse only kicks in when the current session metadata proves the same DM peer on the same Matrix account; otherwise SkillSet falls back to normal user-scoped routing.
- When SkillSet sees a Matrix DM room collide with another DM room on the same shared Matrix DM session, it posts a one-time `m.notice` in that room with the `/focus` escape hatch when thread bindings are enabled and the `dm.sessionScope` hint.
...
- `threadBindings.spawnAcpSessions` is only required for `/acp spawn --thread auto|here`, where SkillSet needs to create or bind a child Matrix thread.
...
Ack reactions use the standard SkillSet resolution order:
...
- Matrix room history is pending-only: SkillSet buffers room messages that did not trigger a reply yet, then snapshots that window when a mention or other trigger arrives.
...
```bash
skillset pairing list matrix
skillset pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, SkillSet reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.

See [Pairing](/channels/pairing) for the shared DM pairing flow and storage layout.

## Direct room repair

If direct-message state gets out of sync, SkillSet can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
skillset matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
skillset matrix direct repair --user-id @alice:example.org
```
...
Matrix slash commands (for example `/new`, `/reset`, `/model`) work directly in DMs. In rooms, SkillSet also recognizes slash commands that are prefixed with the bot's own Matrix mention, so `@bot:server /new` triggers the command path without needing a custom mention regex. This keeps the bot responsive to room-style `@mention /command` posts that Element and similar clients emit when a user tab-completes the bot before typing the command.
...
Set `defaultAccount` when you want SkillSet to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If multiple Matrix accounts are configured and one account id is `default`, SkillSet uses that account implicitly even when `defaultAccount` is unset.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `skillset matrix verify ...` and `skillset matrix devices ...` when you want to override that implicit selection for one command.
...
By default, SkillSet blocks private/internal Matrix homeservers for SSRF protection unless you
...
SkillSet uses the same proxy setting for runtime Matrix traffic and account status probes.

## Target resolution

Matrix accepts these target forms anywhere SkillSet asks you for a room or user target:
...
- `allowBots`: allow messages from other configured SkillSet Matrix accounts (`true` or `"mentions"`).
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; SkillSet does not trust alias state claimed by the invited room.
- `dm`: DM policy block (`enabled`, `policy`, `allowFrom`, `sessionScope`, `threadReplies`).
- `dm.policy`: controls DM access after SkillSet has joined the room and classified it as a DM. It does not change whether an invite is auto-joined.
