...
You will need to create a new application with a bot, add the bot to your server, and pair it to SkillSet. We recommend adding your bot to your own private server. If you don't have one yet, [create one first](https://support.discord.com/hc/en-us/articles/204849977-How-do-I-create-a-server) (choose **Create My Own > For me and my friends**).

<Steps>
  <Step title="Create a Discord application and bot">
    Go to the [Discord Developer Portal](https://discord.com/developers/applications) and click **New Application**. Name it something like "SkillSet".

    Click **Bot** on the sidebar. Set the **Username** to whatever you call your SkillSet agent.
  ...
  <Step title="Enable Developer Mode and collect your IDs">
    ...
    Save your **Server ID** and **User ID** alongside your Bot Token — you'll send all three to SkillSet in the next step.
  ...
  <Step title="Allow DMs from server members">
    ...
    This lets server members (including bots) send you DMs. Keep this enabled if you want to use Discord DMs with SkillSet. If you only plan to use guild channels, you can disable DMs after pairing.

  </Step>

  <Step title="Set your bot token securely (do not send it in chat)">
    Your Discord bot token is a secret (like a password). Set it on the machine running SkillSet before messaging your agent.

```bash
export DISCORD_BOT_TOKEN="YOUR_BOT_TOKEN"
skillset config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
skillset config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
skillset config set channels.discord.enabled true --strict-json
skillset gateway
```

    If SkillSet is already running as a background service, restart it via the SkillSet Mac app or by stopping and restarting the `skillset gateway run` process.

  </Step>

  <Step title="Configure SkillSet and pair">

    <Tabs>
      <Tab title="Ask your agent">
        Chat with your SkillSet agent on any existing channel (e.g. Telegram) and tell it. If Discord is your first channel, use the CLI / config tab instead.
...
```bash
skillset pairing list discord
skillset pairing approve discord <CODE>
```
...
Discord forum and media channels only accept thread posts. SkillSet supports two ways to create them:
...
- Use `skillset message thread create` to create a thread directly. Do not pass `--message-id` for forum channels.

Example: send to forum parent to create a thread

```bash
skillset message send --channel discord --target channel:<forumId> \
  --message "Topic title\nBody of the post"
```
...
```bash
skillset message thread create --channel discord --target channel:<forumId> \
...
SkillSet supports Discord components v2 containers for agent messages. Use the message tool with a `components` payload. Interaction results are routed back to the agent as normal inbound messages and follow the existing Discord `replyToMode` settings.
...
- SkillSet adds a trigger button automatically
...
<Tabs>
  ...
  <Tab title="Guild policy">
    ...
    - names/tags are supported for `users`, but IDs are safer; `skillset security audit` warns when name/tag entries are used
...
<AccordionGroup>
  ...
  <Accordion title="Copy IDs">
    ...
    Prefer numeric IDs in SkillSet config for reliable audits and probes.
...
- Commands may still be visible in Discord UI for users who are not authorized; execution still enforces SkillSet auth and returns "not authorized".
...
<AccordionGroup>
  ...
  <Accordion title="Live stream preview">
    SkillSet can stream draft replies by sending a temporary message and editing it as text arrives. `channels.discord.streaming` takes `off` (default) | `partial` | `block` | `progress`. `progress` maps to `partial` on Discord; `streamMode` is a legacy alias and is auto-migrated.
...
```
    ...
    Preview streaming is text-only; media replies fall back to normal delivery. When `block` streaming is explicitly enabled, SkillSet skips the preview stream to avoid double-streaming.
...
```
    ...
    - `spawnAcpSessions` is only required when SkillSet needs to create/bind a child thread via `--thread auto|here`.
  ...
  <Accordion title="Ack reactions">
    `ackReaction` sends an acknowledgement emoji while SkillSet is processing an inbound message.
...
```json5
{
  channels: {
    discord: {
      ...
      activityType: 1,
      activityUrl: "https://twitch.tv/skillset",
    },
  },
}
```
...
```
  ...
  <Accordion title="Approvals in Discord">
    ...
    When those buttons are present, they are the primary approval UX; SkillSet
...
SkillSet uses Discord components v2 for exec approvals and cross-context markers. Discord message actions can also accept `components` for custom UI (advanced; requires constructing a component payload via the discord tool), while legacy `embeds` remain available but are not recommended.
...
- SkillSet also watches receive decrypt failures and auto-recovers by leaving/rejoining the voice channel after repeated failures in a short window.
...
Discord voice messages show a waveform preview and require OGG/Opus audio. SkillSet generates the waveform automatically, but needs `ffmpeg` and `ffprobe` on the gateway host to inspect and convert.

- Provide a **local file path** (URLs are rejected).
- Omit text content (Discord rejects text + voice message in the same payload).
- Any audio format is accepted; SkillSet converts to OGG/Opus as needed.
...
```
  ...
  <Accordion title="Voice STT drops with DecryptionFailed(...)">
    ...
    - keep SkillSet current (`skillset update`) so the Discord voice receive recovery logic is present
...
- If command deploy/state is stale, restart gateway and re-check with `skillset channels status --probe`.
