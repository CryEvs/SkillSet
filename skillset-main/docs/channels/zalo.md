...
Zalo ships as a bundled plugin in current SkillSet releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `skillset plugins install @skillset/zalo`
- Or from a source checkout: `skillset plugins install ./path/to/local/zalo-plugin`
...
This page reflects current SkillSet behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `skillset pairing list zalo`
  - `skillset pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in SkillSet.
...
- Example: `skillset message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `skillset channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `skillset logs --follow`
