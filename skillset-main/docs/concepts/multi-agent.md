...
- **Session store** (chat history + routing state) under `~/.skillset/agents/<agentId>/sessions`.
...
`~/.skillset/skills`, then filtered by the effective agent skill allowlist when
...
- State dir: `~/.skillset` (or `SKILLSET_STATE_DIR`)
- Workspace: `~/.skillset/workspace` (or `~/.skillset/workspace-<agentId>`)
- Agent dir: `~/.skillset/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.skillset/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, SkillSet runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.skillset/workspace` (or `~/.skillset/workspace-<profile>` when `SKILLSET_PROFILE` is set).
- State defaults to `~/.skillset/agents/main/agent`.
...
```bash
skillset agents add coding
skillset agents add social
```

Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.skillset/agents/<agentId>`.
...
```bash
skillset channels login --channel whatsapp --account work
```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.skillset/workspace-alex" },
      { id: "mia", workspace: "~/.skillset/workspace-mia" },
    ],
  },
  bindings: [
    {
...
- If you later add the same binding for the same agent with an explicit account id, SkillSet upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
`channels.<channel>.defaultAccount` (optional). When unset, SkillSet falls back
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.skillset/workspace-main" },
      { id: "coding", workspace: "~/.skillset/workspace-coding" },
    ],
  },
  bindings: [
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.skillset/workspace-main" },
      { id: "alerts", workspace: "~/.skillset/workspace-alerts" },
    ],
  },
  bindings: [
...
```bash
skillset channels login --channel whatsapp --account personal
skillset channels login --channel whatsapp --account biz
```

`~/.skillset/skillset.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        default: true,
        name: "Home",
        workspace: "~/.skillset/workspace-home",
        agentDir: "~/.skillset/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/.skillset/workspace-work",
        agentDir: "~/.skillset/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.skillset/credentials/whatsapp/personal
          // authDir: "~/.skillset/credentials/whatsapp/personal",
        },
        biz: {
          // Optional override. Default: ~/.skillset/credentials/whatsapp/biz
    ...
    list: [
      {
        id: "chat",
        name: "Everyday",
        workspace: "~/.skillset/workspace-chat",
        model: "anthropic/claude-sonnet-4-6",
      },
      {
        id: "opus",
        name: "Deep Work",
        workspace: "~/.skillset/workspace-opus",
