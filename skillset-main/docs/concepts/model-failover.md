---
summary: "How SkillSet rotates auth profiles and falls back across models"
read_when:
...
SkillSet handles failures in two stages:
...
For a normal text run, SkillSet evaluates candidates in this order:
...
Inside each candidate, SkillSet tries auth-profile failover before advancing to
...
SkillSet uses **auth profiles** for both API keys and OAuth tokens.

- Secrets live in `~/.skillset/agents/<agentId>/agent/auth-profiles.json` (legacy: `~/.skillset/agent/auth-profiles.json`).
- Runtime auth-routing state lives in `~/.skillset/agents/<agentId>/agent/auth-state.json`.
- Config `auth.profiles` / `auth.order` are **metadata + routing only** (no secrets).
- Legacy import-only OAuth file: `~/.skillset/credentials/oauth.json` (imported into `auth-profiles.json` on first use).
...
Profiles live in `~/.skillset/agents/<agentId>/agent/auth-profiles.json` under `profiles`.

## Rotation order

When a provider has multiple profiles, SkillSet chooses an order like this:

1. **Explicit config**: `auth.order[provider]` (if set).
2. **Configured profiles**: `auth.profiles` filtered by provider.
3. **Stored profiles**: entries in `auth-profiles.json` for the provider.

If no explicit order is configured, SkillSet uses a round‑robin order:
...
SkillSet **pins the chosen auth profile per session** to keep provider caches warm.
...
they are tried first, but SkillSet may rotate to another profile on rate limits/timeouts.
User‑pinned profiles stay locked to that profile; if it fails and model fallbacks
are configured, SkillSet moves to the next model instead of switching profiles.
...
like rate limiting), SkillSet marks it in cooldown and moves to the next profile.
...
returning control to SkillSet. For Stainless-based SDKs such as Anthropic and
OpenAI, SkillSet caps SDK-internal `retry-after-ms` / `retry-after` waits at 60
...
`SKILLSET_SDK_RETRY_MAX_WAIT_SECONDS`; see [/concepts/retry](/concepts/retry).

Rate-limit cooldowns can also be model-scoped:

- SkillSet records `cooldownModel` for rate-limit failures when the failing
  model id is known.
...
Billing/credit failures (for example “insufficient credits” / “credit balance too low”) are treated as failover‑worthy, but they’re usually not transient. Instead of a short cooldown, SkillSet marks the profile as **disabled** (with a longer backoff) and rotates to the next profile/provider.

Not every billing-shaped response is `402`, and not every HTTP `402` lands
here. SkillSet keeps explicit billing text in the billing lane even when a
...
If all profiles for a provider fail, SkillSet moves to the next model in
`agents.defaults.model.fallbacks`. This applies to auth failures, rate limits, and
timeouts that exhausted profile rotation (other errors do not advance fallback).

Overloaded and rate-limit errors are handled more aggressively than billing
cooldowns. By default, SkillSet allows one same-provider auth-profile retry,
...
SkillSet builds the candidate list from the currently requested `provider/model`
...
- If the current run is already on a configured fallback in the same provider
  family, SkillSet keeps using the full configured chain.
- If the current run is on a different provider than config and that current
  model is not already part of the configured fallback chain, SkillSet does not
...
When every auth profile for a provider is already in cooldown, SkillSet does
...
When every candidate fails, SkillSet throws `FallbackSummaryError`. The outer
...
- if the remaining block is a matching model-scoped rate limit, SkillSet
