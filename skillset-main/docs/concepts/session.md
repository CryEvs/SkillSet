---
summary: "How SkillSet manages conversation sessions"
read_when:
...
SkillSet organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `skillset security audit`.
...
SkillSet automatically bounds session storage over time. By default, it runs
...
Preview with `skillset sessions cleanup --dry-run`.

## Inspecting sessions

- `skillset status` -- session store path and recent activity.
- `skillset sessions --json` -- all sessions (filter with `--active <minutes>`).
