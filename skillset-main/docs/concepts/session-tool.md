...
SkillSet gives agents tools to work across sessions, inspect status, and
...
After the target responds, SkillSet can run a **reply-back loop** where the
...
`subagents` is the control-plane helper for already spawned SkillSet
...
the completion origin only identifies a channel SkillSet can still reuse the
