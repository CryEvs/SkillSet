---
summary: "What the SkillSet system prompt contains and how it is assembled"
read_when:
...
SkillSet builds a custom system prompt for every agent run. The prompt is **SkillSet-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by SkillSet and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full SkillSet-owned prompt. The provider runtime can:
...
- **SkillSet Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to SkillSet docs (repo or npm package) and when to read them.
...
SkillSet can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **SkillSet
...
occurs, SkillSet can inject a warning block in Project Context; control this with
...
When eligible skills exist, SkillSet injects a compact **available skills list**
...
local SkillSet docs directory (either `docs/` in the repo workspace or the bundled npm
...
for SkillSet behavior, commands, configuration, or architecture, and to run
`skillset status` itself when possible (asking the user only when it lacks access).
