...
read_when:
  - You want to understand how SkillSet assembles model context
...
A **context engine** controls how SkillSet builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

SkillSet ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.skillset/skillset.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other SkillSet plugin. Install
...
skillset plugins install -l ./my-context-engine
```
...
Every time SkillSet runs a model prompt, the context engine participates at
...
SkillSet currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. SkillSet
...
The built-in `legacy` engine preserves SkillSet's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. SkillSet uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. SkillSet disables Pi's built-in
...
`ownsCompaction: false` does **not** mean SkillSet automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `skillset/plugin-sdk/core` to use
  SkillSet's built-in compaction behavior.
...
SkillSet resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to SkillSet's built-in summarization. Plugin engines can implement
...
- Use `skillset doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, SkillSet
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `skillset plugins install -l ./my-engine` to link a
