...
- `/usage cost` in chats: local cost summary aggregated from SkillSet session logs.
- CLI: `skillset status --usage` prints a full per-provider breakdown.
- CLI: `skillset channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. SkillSet treats
  ...
  quota, so SkillSet inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, SkillSet prefers the
...
can supply plugin-specific usage auth logic; otherwise SkillSet falls back to
