...
read_when:
  - You are building an SkillSet plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (skillset.plugin.json)

This page is for the **native SkillSet plugin manifest** only.
...
SkillSet auto-detects those bundle layouts too, but they are not validated
against the `skillset.plugin.json` schema described here.

For compatible bundles, SkillSet currently reads bundle metadata plus declared
...
SkillSet runtime expectations.

Every native SkillSet plugin **must** ship a `skillset.plugin.json` file in the
**plugin root**. SkillSet uses this manifest to validate configuration
...
`skillset.plugin.json` is the metadata SkillSet reads before it loads your
plugin code.
...
- static capability ownership snapshots used for bundled compat wiring and
  contract coverage
- cheap QA runner metadata that the shared `skillset qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that SkillSet can inspect without loading plugin code.                                                                                                                                           |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                        |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that SkillSet can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                          |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `skillset qa` host before plugin runtime loads.                                                                                                                                    |
...
SkillSet reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, SkillSet falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. SkillSet
...
the shared `skillset qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `skillset qa`, for example `matrix`.    |
...
Use `contracts` only for static capability ownership metadata that SkillSet can
...
Use `modelSupport` when SkillSet should infer your provider plugin from
...
SkillSet applies this precedence:
...
Legacy top-level capability keys are deprecated. Use `skillset doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `skillset.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `skillset` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if SkillSet must know it before loading plugin code, put it in `skillset.plugin.json`
...
`skillset` block instead of `skillset.plugin.json`.

Important examples:

| Field                                                             | What it means                                                                                                                                                                        |
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `skillset.extensions`                                             | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `skillset.runtimeExtensions`                                      | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `skillset.setupEntry`                                             | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `skillset.runtimeSetupEntry`                                      | Declares the built JavaScript setup entrypoint for installed packages. Must stay inside the plugin package directory.                                                                |
| `skillset.channel`                                                | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `skillset.channel.configuredState`                                | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `skillset.channel.persistedAuthState`                             | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `skillset.install.npmSpec` / `skillset.install.localPath`         | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `skillset.install.defaultChoice`                                  | Preferred install path when multiple install sources are available.                                                                                                                  |
| `skillset.install.minHostVersion`                                 | Minimum supported SkillSet host version, using a semver floor like `>=2026.3.22`.                                                                                                    |
| `skillset.install.expectedIntegrity`                              | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `skillset.install.allowInvalidConfigRecovery`                     | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `skillset.startup.deferConfiguredChannelFullLoadUntilAfterListen` | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#skillset.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `skillset.plugin.json`.

`skillset.install.minHostVersion` is enforced during install and manifest
...
`"npmSpec": "@wecom/wecom-skillset-plugin@1.2.3"`. Pair that with
...
Channel plugins should provide `skillset.setupEntry` when status, channel list,
...
entrypoint fields. For example, `skillset.runtimeExtensions` cannot make an
escaping `skillset.extensions` path loadable.

`skillset.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `skillset doctor --fix`.

`skillset.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`skillset.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
SkillSet discovers plugins from several roots (bundled, global install, workspace, explicit config-selected paths). If two discoveries share the same `id`, only the **highest-precedence** manifest is kept; lower-precedence duplicates are dropped instead of loading beside it.

Precedence, highest to lowest:

1. **Config-selected** — a path explicitly pinned in `plugins.entries.<id>`
2. **Bundled** — plugins shipped with SkillSet
3. **Global install** — plugins installed into the global SkillSet plugin root
...
- The manifest is **required for native SkillSet plugins**, including local filesystem loads.
