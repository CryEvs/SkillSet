---
summary: "Install and use Codex, Claude, and Cursor bundles as SkillSet plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how SkillSet maps bundle content into native features
...
SkillSet can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
SkillSet maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native SkillSet plugins. Native plugins run
...
of requiring authors to rewrite them as native SkillSet plugins, SkillSet
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    skillset plugins install ./my-bundle

    # Archive
    skillset plugins install ./my-bundle.tgz
...
## What SkillSet maps from bundles

Not every bundle feature runs in SkillSet today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal SkillSet skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | SkillSet-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
...
- bundle skill roots load as normal SkillSet skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal SkillSet skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal SkillSet hook-pack
...
- SkillSet merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- SkillSet exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, SkillSet uses `sse`
...
SkillSet registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- SkillSet sanitizes shell override keys before applying them
...
- SkillSet loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `skillset plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but SkillSet does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit SkillSet best when they use skill roots and SkillSet-style
...
SkillSet checks for native plugin format first:

1. `skillset.plugin.json` or valid `package.json` with `skillset.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, SkillSet uses the native path. This prevents
dual-format packages from being partially installed as bundles.

## Runtime dependencies and cleanup

- Bundled plugin runtime dependencies ship inside the SkillSet package under
  `dist/*`. SkillSet does **not** run `npm install` at startup for bundled
...
- SkillSet does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `skillset plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. SkillSet does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    SkillSet hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
