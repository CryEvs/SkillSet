---
title: "Codex Harness"
summary: "Run SkillSet embedded agent turns through the bundled Codex app-server harness"
read_when:
...
The bundled `codex` plugin lets SkillSet run embedded agent turns through the
...
SkillSet still owns chat channels, session files, model selection, tools,
...
force `embeddedHarness.runtime: "codex"` or `SKILLSET_AGENT_RUNTIME=codex`.
...
SkillSet has separate routes for OpenAI and Codex-shaped access:

| Model ref              | Runtime path                                 | Use when                                                                |
| ---------------------- | -------------------------------------------- | ----------------------------------------------------------------------- |
| `openai/gpt-5.4`       | OpenAI provider through SkillSet/PI plumbing | You want direct OpenAI Platform API access with `OPENAI_API_KEY`.       |
...
- SkillSet with the bundled `codex` plugin available.
...
SkillSet on the protocol surface it has been tested against.
...
With fallback disabled, SkillSet fails early if the Codex plugin is disabled,
...
SkillSet session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the SkillSet session binding for that thread.
...
By default, SkillSet starts local Codex harness sessions in YOLO mode:
...
`SKILLSET_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=1`. It starts the Codex harness in
...
- `SKILLSET_CODEX_APP_SERVER_BIN`
- `SKILLSET_CODEX_APP_SERVER_ARGS`
- `SKILLSET_CODEX_APP_SERVER_MODE=yolo|guardian`
- `SKILLSET_CODEX_APP_SERVER_APPROVAL_POLICY`
- `SKILLSET_CODEX_APP_SERVER_SANDBOX`

`SKILLSET_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`SKILLSET_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
...
Model switching stays SkillSet-controlled. When an SkillSet session is attached
...
generic and works on any channel that supports SkillSet text commands.
...
- `/codex resume <thread-id>` attaches the current SkillSet session to an existing Codex thread.
...
normal turns. On the next message, SkillSet resumes that Codex thread, passes the
currently selected SkillSet `codex/*` model into app-server, and keeps extended
history enabled.
...
SkillSet still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal SkillSet delivery path.

Codex MCP tool approval elicitations are routed through SkillSet's plugin
...
delegated to Codex app-server. SkillSet keeps a transcript mirror for channel
...
reasoning or plan records when the app-server emits them. Today, SkillSet only
...
**SkillSet uses PI instead of Codex:** if no Codex harness claims the run,
SkillSet may use PI as the compatibility backend. Set
