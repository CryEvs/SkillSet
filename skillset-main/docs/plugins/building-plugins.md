...
summary: "Create your first SkillSet plugin in minutes"
read_when:
  - You want to create a new SkillSet plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to SkillSet
---
...
You do not need to add your plugin to the SkillSet repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`skillset plugins install <package-name>`. SkillSet tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect SkillSet to a messaging platform (Discord, IRC, etc.)
  </Card>
...
`skillset/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json skillset.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to SkillSet",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    skillset plugins install clawhub:@myorg/skillset-my-plugin
    ```

    SkillSet also checks ClawHub before npm for bare package specs like
    `@myorg/skillset-my-plugin`.
...
tool result message is emitted. Prefer regular SkillSet plugin hooks when the
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, SkillSet retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `skillset/plugin-sdk/error-runtime`
...
Always import from focused `skillset/plugin-sdk/<subpath>` paths:
...
import { ... } from "skillset/plugin-sdk";
```
...
package-root seam instead of promoting it into `skillset/plugin-sdk/*`.

Some generated `skillset/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `skillset` metadata</Check>
<Check>**skillset.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [skillset/skillset](https://github.com/skillset/skillset/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official SkillSet X account [@skillset](https://x.com/skillset) for release announcements.
