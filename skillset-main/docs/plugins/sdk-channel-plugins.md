...
summary: "Step-by-step guide to building a messaging channel plugin for SkillSet"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect SkillSet to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects SkillSet to a
...
<Info>
  If you have not built any SkillSet plugin before, read
...
Channel plugins do not need their own send/edit/react tools. SkillSet keeps one
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `skillset/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `skillset/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `skillset/plugin-sdk/approval-auth-runtime`
- `skillset/plugin-sdk/approval-client-runtime`
- `skillset/plugin-sdk/approval-delivery-runtime`
- `skillset/plugin-sdk/approval-gateway-runtime`
- `skillset/plugin-sdk/approval-handler-adapter-runtime`
- `skillset/plugin-sdk/approval-handler-runtime`
- `skillset/plugin-sdk/approval-native-runtime`
- `skillset/plugin-sdk/approval-reply-runtime`
- `skillset/plugin-sdk/channel-runtime-context`

Likewise, prefer `skillset/plugin-sdk/setup-runtime`,
`skillset/plugin-sdk/setup-adapter-runtime`,
`skillset/plugin-sdk/reply-runtime`,
`skillset/plugin-sdk/reply-dispatch-runtime`,
`skillset/plugin-sdk/reply-reference`, and
`skillset/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `skillset/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `skillset/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `skillset/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `skillset.setupEntry` in
...
- use the broader `skillset/plugin-sdk/setup` seam only when you also need the
...
- `skillset/plugin-sdk/account-core`,
  `skillset/plugin-sdk/account-id`,
  `skillset/plugin-sdk/account-resolution`, and
  `skillset/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `skillset/plugin-sdk/inbound-envelope` and
  `skillset/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `skillset/plugin-sdk/messaging-targets` for target parsing/matching
- `skillset/plugin-sdk/outbound-media` and
  `skillset/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `skillset/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `skillset/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `skillset/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `skillset/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `skillset/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `skillset/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`skillset/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`skillset/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#skillset-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/skillset-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "skillset": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect SkillSet to Acme Chat."
        }
      }
    }
    ```

    ```json skillset.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { SkillSetConfig } from "skillset/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "skillset/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so SkillSet
  ...
  <Step title="Add a setup entry">
    ...
    SkillSet loads this instead of the full entry when the channel is disabled
    ...
    `skillset/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    SkillSet. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to SkillSet.
...
├── package.json              # skillset.channel metadata
├── skillset.plugin.json      # Manifest with config schema
