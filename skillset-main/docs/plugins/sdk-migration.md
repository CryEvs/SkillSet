...
read_when:
  - You see the SKILLSET_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the SKILLSET_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external SkillSet plugin
---

# Plugin SDK Migration

SkillSet has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`skillset/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`skillset/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`skillset/plugin-sdk/\<subpath\>`)
is a small, self-contained module with a clear purpose and documented contract.

Legacy provider convenience seams for bundled channels are also gone. Imports
such as `skillset/plugin-sdk/slack`, `skillset/plugin-sdk/discord`,
`skillset/plugin-sdk/signal`, `skillset/plugin-sdk/whatsapp`,
channel-branded helper seams, and
`skillset/plugin-sdk/telegram-core` were private mono-repo shortcuts, not
...
<Steps>
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `skillset/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `skillset/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "skillset/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "skillset/plugin-sdk/compat";
    ...
    import { resolveControlCommandGate } from "skillset/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "skillset/extension-api";
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `SkillSetSchema` |
...
```bash
SKILLSET_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 skillset gateway run
SKILLSET_SUPPRESS_EXTENSION_API_WARNING=1 skillset gateway run
```
