...
read_when:
  - Building or debugging native SkillSet plugins
...
This page covers the internal architecture of the SkillSet plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside SkillSet. Every
native SkillSet plugin registers against one or more capability types:
...
SkillSet classifies every loaded plugin into a shape based on its actual
...
Use `skillset plugins inspect <id>` to see a plugin's shape and capability
...
When you run `skillset doctor` or `skillset plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `skillset status --all` and `skillset plugins doctor`.

## Architecture overview

SkillSet's plugin system has four layers:

1. **Manifest + discovery**
   SkillSet finds candidate plugins from configured paths, workspace roots,
   global plugin roots, and bundled plugins. Discovery reads native
   `skillset.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native SkillSet plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of SkillSet reads the registry to expose tools, channels, provider
...
That keeps plugin-owned CLI code inside the plugin while still letting SkillSet
...
That split lets SkillSet validate config, explain missing/disabled plugins, and
...
normal chat actions. SkillSet keeps one shared `message` tool in core, and
...
SkillSet treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's SkillSet-facing
  surfaces
...
So if SkillSet adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If SkillSet has shared
...
```ts
import type { SkillSetPluginDefinition } from "skillset/plugin-sdk/plugin-entry";
import {
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "skillset/plugin-sdk/media-understanding";

const plugin: SkillSetPluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
SkillSet already treats image/audio/video understanding as one shared
...
`SkillSetPluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   SkillSet can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that SkillSet knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `SkillSetPluginApi` or
  `api.runtime`
...
Native SkillSet plugins run **in-process** with the Gateway. They are not
...
- a malicious native plugin is equivalent to arbitrary code execution inside
  the SkillSet process

Compatible bundles are safer by default because SkillSet currently treats them
...
name: `@skillset/<id>` by default, or an approved typed suffix such as
...
SkillSet exports capabilities, not implementation convenience.
...
At startup, SkillSet does roughly this:
...
The manifest is the control-plane source of truth. SkillSet uses it to:
...
SkillSet keeps short in-process caches for:
...
- Set `SKILLSET_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `SKILLSET_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Tune cache windows with `SKILLSET_PLUGIN_DISCOVERY_CACHE_MS` and
  `SKILLSET_PLUGIN_MANIFEST_CACHE_MS`.
...
SkillSet still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, SkillSet calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | SkillSet tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
that still runs on SkillSet's normal inference loop.
...
- **Pass-through catalog providers** (OpenRouter, Kilocode, Z.AI, xAI) register
  ...
  upstream model ids ahead of SkillSet's static catalog.
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as SkillSet adds those
...
- SkillSet only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "SkillSet plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-skillset-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-skillset-scopes` only when the header is explicitly present
  - if `x-skillset-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-skillset-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `skillset/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `skillset/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `skillset/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `skillset/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `skillset/plugin-sdk/config-schema` | Root `skillset.json` Zod schema (`SkillSetSchema`) |
...
<Info>
`skillset/plugin-sdk/channel-runtime` is deprecated — a compatibility shim for
...
External plugins should only import `skillset/plugin-sdk/*` subpaths. Never
...
`skillset/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape SkillSet writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to SkillSet's
...
- if both `catalog` and `discovery` are registered, SkillSet uses `catalog`
...
- Read-only command paths such as `skillset status`, `skillset status --all`,
  `skillset channels status`, `skillset channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `skillset.extensions`:
...
Security guardrail: every `skillset.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `skillset plugins install` installs plugin dependencies with
...
Optional: `skillset.setupEntry` can point at a lightweight setup-only module.
When SkillSet needs setup surfaces for a disabled channel plugin, or
...
Optional: `skillset.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let SkillSet load the
...
Channel plugins can advertise setup/discovery metadata via `skillset.channel` and
install hints via `skillset.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@skillset/nextcloud-talk",
  "skillset": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@skillset/nextcloud-talk",
...
Useful `skillset.channel` fields beyond the minimal example:
...
SkillSet can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.skillset/mpm/plugins.json`
- `~/.skillset/mpm/catalog.json`
- `~/.skillset/plugins/catalog.json`

Or point `SKILLSET_PLUGIN_CATALOG_PATHS` (or `SKILLSET_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "skillset": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.
...
```ts
import { buildMemorySystemPromptAddition } from "skillset/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `SkillSetPluginApi` and/or `api.runtime` with the smallest useful
...
This is how SkillSet stays opinionated without becoming hardcoded to one
