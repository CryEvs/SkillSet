...
summary: "Step-by-step guide to building a model provider plugin for SkillSet"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to SkillSet
...
(LLM) to SkillSet. By the end you will have a provider with a model catalog,
...
<Tip>
  Provider plugins add models to SkillSet's normal inference loop. If the model
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so SkillSet can detect
    ...
    is optional and lets SkillSet auto-load your provider plugin from shorthand
    model ids like `acme-large` before runtime hooks exist. If you publish the
    provider on ClawHub, those `skillset.compat` and `skillset.build` fields
  ...
  <Step title="Register the provider">
    ...
    import { createProviderApiKeyAuthMethod } from "skillset/plugin-sdk/provider-auth";
    ...
    `skillset onboard --acme-ai-api-key <key>` and select
    `acme-ai/acme-large` as their model.

    If the upstream provider uses different control tokens than SkillSet, add a
    ...
    SkillSet parses its own control markers or channel delivery.
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "skillset/plugin-sdk/provider-entry";
    ...
    `buildProvider` is the live catalog path used when SkillSet can resolve real
    ...
    SkillSet's `models list --all` display currently executes static catalogs
    ...
    `skillset/plugin-sdk/provider-onboard`. The narrowest helpers are
    ...
    `skillset/plugin-sdk/provider-catalog-shared` instead of hardcoding
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    ```typescript
    import { buildProviderReplayFamilyHooks } from "skillset/plugin-sdk/provider-model-shared";
    import { buildProviderStreamFamilyHooks } from "skillset/plugin-sdk/provider-stream";
    import { buildProviderToolCompatFamilyHooks } from "skillset/plugin-sdk/provider-tools";
    ...
    `skillset/plugin-sdk/provider-model-shared` also exports the replay-family
    ...
    `skillset/plugin-sdk/provider-stream` exposes both the family builder and
    ...
    example: `@skillset/anthropic-provider` exports
    ...
    `skillset/plugin-sdk/provider-tools` currently exposes one shared
    ...
    - `@skillset/openai-provider`: `api.ts` exports provider builders,
      default-model helpers, and realtime provider builders
    - `@skillset/openrouter-provider`: `api.ts` exports the provider builder
    ...
    <Accordion title="All available provider hooks">
      SkillSet calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    SkillSet classifies this as a **hybrid-capability** plugin. This is the
    ...
    `skillset/plugin-sdk/provider-http` together with the provider HTTP request
...
├── package.json              # skillset.providers metadata
├── skillset.plugin.json      # Manifest with provider auth metadata
