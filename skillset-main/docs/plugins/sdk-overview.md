...
read_when:
  ...
  - You want a reference for all registration methods on SkillSetPluginApi
...
prefer `skillset/plugin-sdk/channel-core`; keep `skillset/plugin-sdk/core` for
...
`skillset/plugin-sdk/slack`, `skillset/plugin-sdk/discord`,
`skillset/plugin-sdk/signal`, `skillset/plugin-sdk/whatsapp`, or
...
| `plugin-sdk/config-schema`  | `SkillSetSchema`                                                                                                                       |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `skillset.json` Zod schema export (`SkillSetSchema`) |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, SkillSet tool bridge helpers, and attempt result utilities |
...
The `register(api)` callback receives an `SkillSetPluginApi` object with these
methods:
...
event timing during SkillSet embedded runs, for example async `tool_result`
...
`skillset.plugin.json`. Keep normal SkillSet plugin hooks for everything that
...
- User config still wins. SkillSet merges `agents.defaults.cliBackends.<id>` over the
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `skillset/plugin-sdk/memory-host-core` instead of reaching into a specific
...
| `api.config`             | `SkillSetConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `skillset/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when SkillSet is already running. If no runtime
...
- `@skillset/openrouter-provider`: `api.ts` exports the provider builder plus
  onboarding/config helpers

<Warning>
  Extension production code should also avoid `skillset/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `skillset/plugin-sdk/speech`, `.../provider-model-shared`, or another
