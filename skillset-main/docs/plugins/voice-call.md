...
read_when:
  - You want to place an outbound voice call from SkillSet
...
Voice calls for SkillSet via a plugin. Supports outbound notifications and
...
- Use `skillset voicecall ...` or the `voice_call` tool
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `skillset doctor --fix` to rewrite them.
...
- Runtime fallback still accepts those old voice-call keys for now, but the rewrite path is `skillset doctor --fix` and the compat shim is temporary.
...
Legacy keys are still auto-migrated by `skillset doctor --fix`:
...
```bash
skillset voicecall call --to "+15555550123" --message "Hello from SkillSet"
skillset voicecall start --to "+15555550123"   # alias for call
skillset voicecall continue --call-id <id> --message "Any questions?"
skillset voicecall speak --call-id <id> --message "One moment"
skillset voicecall end --call-id <id>
skillset voicecall status --call-id <id>
skillset voicecall tail
skillset voicecall latency                     # summarize turn latency from logs
