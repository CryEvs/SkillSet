...
`runEmbeddedAgent(...)` is the neutral helper for starting a normal SkillSet
...
Bind a Task Flow runtime to an existing SkillSet session key or trusted tool
...
trusted SkillSet session key from your own binding layer. Do not bind from raw
user input.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: { query: "SkillSet plugin SDK", count: 5 },
});
```
