...
read_when:
  ...
  - You are defining plugin config schemas or package.json skillset metadata
---

# Plugin Setup and Config

Reference for plugin packaging (`package.json` metadata), manifests
(`skillset.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `skillset` field that tells the plugin system what
...
```json
{
  "name": "@myorg/skillset-my-channel",
  "version": "1.0.0",
  "type": "module",
  "skillset": {
    "extensions": ["./index.ts"],
...
```json skillset-clawhub-package.json
{
  "name": "@myorg/skillset-my-plugin",
  "version": "1.0.0",
  "type": "module",
  "skillset": {
    "extensions": ["./index.ts"],
...
`skillset.channel` is cheap package metadata for channel discovery and setup
...
`skillset.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported SkillSet version in the form `>=x.y.z`.                        |
...
Interactive onboarding also uses `skillset.install` for install-on-demand
...
to show" metadata in `skillset.plugin.json` and the "how to install it"
...
```json
{
  "skillset": {
    "install": {
      "npmSpec": "@wecom/wecom-skillset-plugin@1.2.3",
...
still fails closed and tells the operator to run `skillset doctor --fix`.
...
When enabled, SkillSet loads only `setupEntry` during the pre-listen startup
...
Every native plugin must ship an `skillset.plugin.json` in the package root.
SkillSet uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to SkillSet",
  "configSchema": {
    "type": "object",
...
SkillSet loads when it only needs setup surfaces (onboarding, config repair,
...
`skillset/plugin-sdk/channel-entry-contract` instead of
...
**When SkillSet uses `setupEntry` instead of the full entry:**
...
Use `buildChannelConfigSchema` from `skillset/plugin-sdk/core` to convert a
Zod schema into the `ChannelConfigSchema` wrapper that SkillSet validates:

```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "skillset/plugin-sdk/core";
...
Channel plugins can provide interactive setup wizards for `skillset onboard`.
The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "skillset/plugin-sdk/channel-setup";
...
helpers from `skillset/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`,
...
`skillset/plugin-sdk/setup` instead of hand-rolling the same `status` object in
each plugin.

For optional setup surfaces that should only appear in certain contexts, use
`createOptionalChannelSetupSurface` from `skillset/plugin-sdk/channel-setup`:

```typescript
import { createOptionalChannelSetupSurface } from "skillset/plugin-sdk/channel-setup";

const setupSurface = createOptionalChannelSetupSurface({
  channel: "my-channel",
  label: "My Channel",
  npmSpec: "@myorg/skillset-my-channel",
  docsPath: "/channels/my-channel",
});
...
```bash
skillset plugins install @myorg/skillset-my-plugin
```

SkillSet tries ClawHub first and falls back to npm automatically. You can also
force ClawHub explicitly:

```bash
skillset plugins install clawhub:@myorg/skillset-my-plugin   # ClawHub only
```
...
```bash
skillset plugins install <package-name>
```

<Info>
  For npm-sourced installs, `skillset plugins install` runs
  `npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
  trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>

Bundled SkillSet-owned plugins are the only startup repair exception: when a
