...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in SkillSet
...
Running SkillSet via Node with `tsx` fails at startup with:

```
[skillset] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `skillset status` and `gateway:watch` worked.
