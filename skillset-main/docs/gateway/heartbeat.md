...
- During heartbeat runs, SkillSet treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `skillset sessions --json` or the [sessions CLI](/cli/sessions)).
...
- If only alert delivery is disabled, SkillSet can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
- If the resolved heartbeat target supports typing, SkillSet shows typing while
...
If **all three** are false, SkillSet skips the heartbeat run entirely (no model call).
...
- SkillSet parses the `tasks:` block and checks each task against its own `interval`.
...
```bash
skillset system event --text "Check for urgent follow-ups" --mode now
```
