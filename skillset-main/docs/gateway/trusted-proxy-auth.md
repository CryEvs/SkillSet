...
read_when:
  - Running SkillSet behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of SkillSet
...
- You run SkillSet behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. SkillSet checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. SkillSet extracts the user identity from the configured header
...
- For same-host loopback proxy setups, use token/password auth instead, or route through a non-loopback trusted proxy address that SkillSet can verify.
...
- SkillSet can stay on loopback HTTP behind the proxy.
...
If SkillSet itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://skillset.example.com
    to: http://skillset-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
skillset.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy skillset:18789 {
...
location / {
    ...
    proxy_pass http://skillset:18789;
...
SkillSet rejects ambiguous configurations where both a `gateway.auth.token` (or `SKILLSET_GATEWAY_TOKEN`) and `trusted-proxy` mode are active at the same time. Mixed token configs can cause loopback requests to silently authenticate on the wrong auth path.
...
optionally declare operator scopes with `x-skillset-scopes`.

Examples:

- `x-skillset-scopes: operator.read`
- `x-skillset-scopes: operator.read,operator.write`
- `x-skillset-scopes: operator.admin,operator.write`

Behavior:

- When the header is present, SkillSet honors the declared scope set.
...
- Gateway-auth **plugin HTTP routes** are narrower by default: when `x-skillset-scopes` is absent, their runtime scope falls back to `operator.write`.
...
- Send `x-skillset-scopes` explicitly when you want a trusted-proxy request to
...
`skillset security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
SkillSet rejected a loopback-source trusted-proxy request.
...
3. Update SkillSet config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `skillset security audit` and review findings
