...
SkillSet supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `SKILLSET_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, SkillSet runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). SkillSet validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/SkillSet QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/skillset"],
...
- SkillSet resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__SKILLSET_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `skillset.json` refs)
...
When reload-time activation fails after a healthy state, SkillSet enters degraded secrets state.
...
- Strict command paths (for example `skillset memory` remote-memory paths and `skillset qr --remote` when it needs remote shared-secret refs) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `skillset status`, `skillset status --all`, `skillset channels status`, `skillset channels resolve`, `skillset security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `skillset secrets reload`.
...
```bash
skillset secrets audit --check
skillset secrets configure
skillset secrets audit --check
```
...
- plaintext values at rest (`skillset.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `skillset.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `skillset secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `skillset.json` plus `auth-profiles.json` for one agent scope
...
- `skillset secrets configure --providers-only`
- `skillset secrets configure --skip-provider-setup`
- `skillset secrets configure --agent <id>`
...
```bash
skillset secrets apply --from /tmp/skillset-secrets-plan.json
skillset secrets apply --from /tmp/skillset-secrets-plan.json --allow-exec
skillset secrets apply --from /tmp/skillset-secrets-plan.json --dry-run
skillset secrets apply --from /tmp/skillset-secrets-plan.json --dry-run --allow-exec
```
...
SkillSet intentionally does not write rollback backups containing historical plaintext secret values.
