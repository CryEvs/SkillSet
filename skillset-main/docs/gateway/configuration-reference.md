...
summary: "Gateway config reference for core SkillSet keys, defaults, and links to dedicated subsystem references"
read_when:
...
Core config reference for `~/.skillset/skillset.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

This page covers the main SkillSet config surfaces and links out when a subsystem has its own deeper reference. It does **not** try to inline every channel/plugin-owned command catalog or every deep memory/QMD knob on one page.

Code truth:

- `skillset config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — SkillSet uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.skillset/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `skillset doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `skillset doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["skillset-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-skillset",
          requireMention: false,
...
- SkillSet additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `skillset plugins install @skillset/mattermost`.
...
- `commands.callbackUrl` must resolve to the SkillSet gateway endpoint and be reachable from the Mattermost server.
- Native slash callbacks are authenticated with the per-command tokens returned
  ...
  commands are activated, SkillSet rejects callbacks with
...
SkillSet spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `skillset channels add` (or channel onboarding) while still on a single-account top-level channel config, SkillSet promotes account-scoped top-level single-account values into the channel account map first so the original account keeps working. Most channels move them into `channels.<channel>.accounts.default`; Matrix can preserve an existing matching named/default target instead.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `skillset doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into the promoted account chosen for that channel. Most channels use `accounts.default`; Matrix can preserve an existing matching named/default target instead.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@skillset", "skillset"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@skillset"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `skillset.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
- `mcp: true` enables `/mcp` for SkillSet-managed MCP server config under `mcp.servers`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, SkillSet auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/skillset" } },
}
```
...
SkillSet has multiple high-volume prompt/context budgets, and they are
...
- `model.primary`: format `provider/model` (e.g. `openai/gpt-5.4`). If you omit the provider, SkillSet tries an alias first, then a unique configured-provider match for that exact model id, and only then falls back to the configured default provider (deprecated compatibility behavior, so prefer explicit `provider/model`). If that provider no longer exposes the configured default model, SkillSet falls back to the first configured provider/model instead of surfacing a stale removed-provider default.
- `models`: the configured model catalog and allowlist for `/model`. Each entry can include `alias` (shortcut) and `params` (provider-specific, for example `temperature`, `maxTokens`, `cacheRetention`, `context1m`).
  - Safe edits: use `skillset config set agents.defaults.models '<json>' --strict-json --merge` to add entries. `config set` refuses replacements that would remove existing allowlist entries unless you pass `--replace`.
...
- Environment overrides: `SKILLSET_AGENT_RUNTIME=<id|auto|pi>` overrides `runtime`; `SKILLSET_AGENT_HARNESS_FALLBACK=none` disables PI fallback for that process.
...
Replace the entire SkillSet-assembled system prompt with a fixed string. Set at the default level (`agents.defaults.systemPromptOverride`) or per agent (`agents.list[].systemPromptOverride`). Per-agent values take precedence; an empty or whitespace-only value is ignored. Useful for controlled prompt experiments.
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before SkillSet aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.skillset/sandboxes",
        docker: {
          image: "skillset-sandbox:bookworm-slim",
          containerPrefix: "skillset-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "skillset-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/skillset-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "skillset-sandbox-browser:bookworm-slim",
          network: "skillset-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that SkillSet materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.skillset/sandboxes`
...
In `remote` mode, host-local edits made outside SkillSet are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `skillset.json`.
noVNC observer access uses VNC auth by default and SkillSet emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `skillset-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
...
- Launch defaults are defined in `scripts/sandbox-browser-entrypoint.sh` and tuned for container hosts:
  ...
  - `--remote-debugging-port=<derived from SKILLSET_BROWSER_CDP_PORT>`
  ...
  - `--disable-3d-apis`, `--disable-software-rasterizer`, and `--disable-gpu` are
    ...
    `SKILLSET_BROWSER_DISABLE_GRAPHICS_FLAGS=0` if WebGL/3D usage requires it.
  - `SKILLSET_BROWSER_DISABLE_EXTENSIONS=0` re-enables extensions if your workflow
    depends on them.
  - `--renderer-process-limit=2` can be changed with
    `SKILLSET_BROWSER_RENDERER_PROCESS_LIMIT=<N>`; set `0` to use Chromium's
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.skillset/workspace",
        agentDir: "~/.skillset/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@skillset"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.skillset/workspace-home" },
      { id: "work", workspace: "~/.skillset/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, SkillSet resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.skillset/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.skillset/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.skillset/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.skillset/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, SkillSet starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.skillset/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, SkillSet treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:skillset`   | All built-in tools (excludes provider plugins)                                                                          |
...
- Files are materialized into the child workspace at `.skillset/attachments/<uuid>/` with a `.manifest.json`.
...
SkillSet uses the built-in model catalog. Add custom providers via `models.providers` in config or `~/.skillset/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `SKILLSET_AGENT_DIR` (or `PI_CODING_AGENT_DIR`, a legacy environment variable alias).
...
- `models.providers`: custom provider map keyed by provider id.
  - Safe edits: use `skillset config set models.providers.<id> '<json>' --strict-json --merge` or `skillset config set models.providers.<id>.models '<json-array>' --strict-json --merge` for additive updates. `config set` refuses destructive replacements unless you pass `--replace`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), SkillSet forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
- `models.providers.*.models.*.compat.requiresStringContent`: optional compatibility hint for string-only OpenAI-compatible chat endpoints. When `true`, SkillSet flattens pure text `messages[].content` arrays into plain strings before sending the request.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `skillset onboard --auth-choice opencode-zen` or `skillset onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `skillset onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `skillset onboard --auth-choice moonshot-api-key-cn`.

Native Moonshot endpoints advertise streaming usage compatibility on the shared
`openai-completions` transport, and SkillSet keys that off endpoint capabilities
...
Anthropic-compatible, built-in provider. Shortcut: `skillset onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `skillset onboard --auth-choice synthetic-api-key`.
...
`skillset onboard --auth-choice minimax-global-api` or
`skillset onboard --auth-choice minimax-cn-api`.
The model catalog defaults to M2.7 only.
On the Anthropic-compatible streaming path, SkillSet disables MiniMax thinking
...
- `install.nodeManager`: node installer preference for `metadata.skillset.install`
...
- Loaded from `~/.skillset/extensions`, `<workspace>/.skillset/extensions`, plus `plugins.load.paths`.
- Discovery accepts native SkillSet plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native SkillSet plugin schema when available).
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; SkillSet applies those as sanitized agent settings, not as raw SkillSet config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `skillset plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      skillset: { cdpPort: 18800, color: "#FF4500" },
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want SkillSet to discover `/json/version`; use WS(S)
...
- Local managed `skillset` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or SKILLSET_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `SKILLSET_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `SKILLSET_ALLOW_INSECURE_PRIVATE_WS=1`: client-side break-glass override that allows plaintext `ws://` to trusted private-network IPs; default remains loopback-only for plaintext.
...
- `SKILLSET_APNS_RELAY_BASE_URL` / `SKILLSET_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `SKILLSET_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
SKILLSET_CONFIG_PATH=~/.skillset/a.json \
SKILLSET_STATE_DIR=~/.skillset-a \
skillset gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.skillset-dev` + port `19001`), `--profile <name>` (uses `~/.skillset-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/skillset/tls/server.crt",
      keyPath: "/etc/skillset/tls/server.key",
      caPath: "/etc/skillset/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.skillset/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-skillset-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `SKILLSET_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.skillset/workspace/canvas",
    liveReload: true,
    // enabled: false, // or SKILLSET_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__skillset__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__skillset__/a2ui/`
...
- Also serves A2UI at `/__skillset__/a2ui/`.
...
- Hostname defaults to `skillset`. Override with `SKILLSET_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.skillset/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `skillset dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.skillset/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${SKILLSET_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `skillset.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.skillset/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/skillset-vault-resolver",
...
- Legacy OAuth imports from `~/.skillset/credentials/oauth.json`.
...
- Default log file: `/tmp/skillset/skillset-YYYY-MM-DD.log`.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.skillset/logs/cache-trace.jsonl",
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$SKILLSET_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one SkillSet.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `SKILLSET_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `skillset doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `skillset.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
- SkillSet-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `skillset.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for SkillSet-owned writes; those writes fail closed instead of flattening the config.
