...
- Default rolling log file is under `/tmp/skillset/` (one file per day): `skillset-YYYY-MM-DD.log`
  - Date uses the gateway host's local timezone.
- The log file path and level can be configured via `~/.skillset/skillset.json`:
  - `logging.file`
  - `logging.level`
...
`skillset gateway` supports a per-gateway style switch:
