...
read_when:
  - Setting up SkillSet for the first time
...
SkillSet reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.skillset/skillset.json`.
The active config path must be a regular file. Symlinked `skillset.json`
layouts are unsupported for SkillSet-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `SKILLSET_CONFIG_PATH` directly at the real file.

If the file is missing, SkillSet uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `skillset onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    skillset onboard       # full onboarding flow
    skillset configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    skillset config get agents.defaults.workspace
    skillset config set agents.defaults.heartbeat.every "2h"
    skillset config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.skillset/skillset.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
SkillSet only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`skillset config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`skillset doctor`, `skillset logs`, `skillset health`, `skillset status`)
- Run `skillset doctor` to see exact issues
- Run `skillset doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup.
If `skillset.json` later fails validation (or drops `gateway.mode`, shrinks
sharply, or has a stray log line prepended), SkillSet preserves the broken file
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `skillset config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@skillset", "skillset"],
            },
          },
        ],
      },
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `skillset.json`.
    ...
    ```bash
    skillset config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `SKILLSET_APNS_RELAY_BASE_URL` and `SKILLSET_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `SKILLSET_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-skillset-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **SkillSet-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      SkillSet updates that included file and leaves `skillset.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for SkillSet-owned writes instead of
...
The Gateway watches `~/.skillset/skillset.json` and applies changes automatically — no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits by restoring the last-known-good config. SkillSet-owned
...
`.clobbered.*` file next to `skillset.json`, fix the rejected payload, then run
`skillset config validate`. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-restored-last-known-good-config)
...
When you edit a source file that is referenced through `$include`, SkillSet plans
...
```bash
skillset gateway call config.get --params '{}'  # capture payload.hash
skillset gateway call config.patch --params '{
...
SkillSet reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.skillset/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, SkillSet runs your login shell and imports only the missing keys:
...
Env var equivalent: `SKILLSET_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${SKILLSET_GATEWAY_TOKEN}" } },
