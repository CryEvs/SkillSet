---
summary: "CLI reference for `skillset update` (safe-ish source update + gateway auto-restart)"
read_when:
...
Safely update SkillSet and switch between stable/beta/dev channels.
...
- `--tag <dist-tag|version|spec>`: override the package target for this update only. For package installs, `main` maps to `github:skillset/skillset#main`.
...
skillset update status --timeout 10
```
...
When you switch channels explicitly (`--channel ...`), SkillSet also keeps the
install method aligned:

- `dev` → ensures a git checkout (default: `~/skillset`, override with `SKILLSET_GIT_DIR`),
...
For package-manager installs, `skillset update` resolves the target package
...
8. Runs `skillset doctor` as the final “safe update” check.
...
differs from the stored install record, `skillset update` aborts that plugin
...
`skillset --update` rewrites to `skillset update` (useful for shells and launcher scripts).

## See also

- `skillset doctor` (offers to run update first on git checkouts)
