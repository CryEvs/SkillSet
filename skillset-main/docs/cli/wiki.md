---
summary: "CLI reference for `skillset wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `skillset wiki`
title: "wiki"
---
...
Use `skillset wiki` when you want a compiled knowledge vault with:
...
- `.skillset-wiki/cache/agent-digest.json`
- `.skillset-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `skillset memory search` when the
...
```bash
skillset wiki get entity.alpha
skillset wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`skillset wiki` behavior is shaped by:
