---
summary: "SkillSet Gateway CLI (`skillset gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is SkillSet’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `skillset gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.skillset/skillset.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
- `skillset onboard --mode local` and `skillset setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
- `--token <token>`: token override (also sets `SKILLSET_GATEWAY_TOKEN` for the process).
...
- Set `SKILLSET_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup.
...
```bash
skillset gateway health --url ws://127.0.0.1:18789
```
...
```bash
skillset gateway usage-cost
skillset gateway usage-cost --days 7
skillset gateway usage-cost --json
```
...
```bash
skillset gateway stability
skillset gateway stability --type payload.large
skillset gateway stability --bundle latest
skillset gateway stability --bundle latest --export
skillset gateway stability --json
```
...
- On fatal Gateway exits, shutdown timeouts, and restart startup failures, SkillSet writes the same diagnostic snapshot to `~/.skillset/logs/stability/skillset-stability-*.json` when the recorder has events. Inspect the newest bundle with `skillset gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
```bash
skillset gateway diagnostics export
skillset gateway diagnostics export --output skillset-diagnostics.zip
skillset gateway diagnostics export --json
```
...
skillset gateway status --require-rpc
```
...
```bash
skillset gateway call status
skillset gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
- For password auth on `gateway run`, prefer `SKILLSET_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
- In inferred auth mode, shell-only `SKILLSET_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
- If both `gateway.auth.token` and `gateway.auth.password` are configured and `gateway.auth.mode` is unset, install is blocked until mode is set explicitly.
- Lifecycle commands accept `--json` for scripting.

## Discover gateways (Bonjour)

`gateway discover` scans for Gateway beacons (`_skillset-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `skillset.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
...
```bash
skillset gateway discover --timeout 4000
skillset gateway discover --json | jq '.beacons[].wsUrl'
```
