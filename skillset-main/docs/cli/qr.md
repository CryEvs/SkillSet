---
summary: "CLI reference for `skillset qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
skillset qr
skillset qr --setup-code-only
skillset qr --json
skillset qr --remote
skillset qr --url wss://gateway.example/ws
```
...
- With `--remote`, SkillSet requires either `gateway.remote.url` or
