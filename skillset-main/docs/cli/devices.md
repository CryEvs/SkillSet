---
summary: "CLI reference for `skillset devices` (device pairing + token rotation/revocation)"
read_when:
...
### `skillset devices remove <deviceId>`
...
```
skillset devices remove <deviceId>
skillset devices remove <deviceId> --json
```

### `skillset devices clear --yes [--pending]`

Clear paired devices in bulk.

```
skillset devices clear --yes
skillset devices clear --yes --pending
skillset devices clear --yes --pending --json
```

### `skillset devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, SkillSet only prints the selected pending
...
key), SkillSet supersedes the previous pending entry and issues a new
`requestId`. Run `skillset devices list` right before approval to use the
current ID.

If the device is already paired and asks for broader scopes or a broader role,
SkillSet keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `skillset devices list`
or use `skillset devices approve --latest` to preview the exact upgrade before
approving it.

```
skillset devices approve
skillset devices approve <requestId>
skillset devices approve --latest
```
...
### `skillset devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
skillset devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```

Returns the new token payload as JSON.

### `skillset devices revoke --device <id> --role <role>`
...
```
skillset devices revoke --device <deviceId> --role node
```
...
```bash
skillset devices rotate --device <deviceId> --role operator
```
