---
summary: "CLI reference for `skillset daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `skillset daemon ...` in scripts
...
`skillset daemon ...` maps to the same service control surface as `skillset gateway ...` service commands.

## Usage

```bash
skillset daemon status
skillset daemon install
skillset daemon start
skillset daemon stop
skillset daemon restart
skillset daemon uninstall
```
...
Use [`skillset gateway`](/cli/gateway) for current docs and examples.
