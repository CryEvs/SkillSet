---
summary: "CLI reference for `skillset approvals` and `skillset exec-policy`"
read_when:
...
## `skillset exec-policy`

`skillset exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.skillset/exec-approvals.json`

Examples:

```bash
skillset exec-policy show
skillset exec-policy show --json

skillset exec-policy preset yolo
skillset exec-policy preset cautious --json

skillset exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `skillset exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `skillset approvals set --gateway`
or `skillset approvals set --node <id|name|ip>`.

## Common commands

```bash
skillset approvals get
skillset approvals get --node <id|name|ip>
skillset approvals get --gateway
```

`skillset approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
skillset approvals set --file ./exec-approvals.json
skillset approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
skillset approvals set --node <id|name|ip> --file ./exec-approvals.json
skillset approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested SkillSet policy aligned, also set:
...
```bash
skillset approvals allowlist add "~/Projects/**/bin/rg"
skillset approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
skillset approvals allowlist add --agent "*" "/usr/bin/uname"

skillset approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `skillset nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.skillset/exec-approvals.json`.
