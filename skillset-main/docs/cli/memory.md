---
summary: "CLI reference for `skillset memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
skillset memory status
skillset memory status --deep
skillset memory status --fix
skillset memory index --force
skillset memory search "meeting notes"
skillset memory search --query "deployment" --max-results 20
skillset memory promote --limit 10 --min-score 0.75
skillset memory promote --apply
skillset memory promote --json --min-recall-count 0 --min-unique-queries 0
skillset memory promote-explain "router vlan"
skillset memory promote-explain "router vlan" --json
skillset memory rem-harness
skillset memory rem-harness --json
skillset memory status --json
skillset memory status --deep --index
skillset memory status --deep --index --verbose
skillset memory status --agent main
skillset memory index --agent main --verbose
```
...
```bash
skillset memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `skillset cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
skillset memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
skillset memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
