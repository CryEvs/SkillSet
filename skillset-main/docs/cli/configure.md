---
summary: "CLI reference for `skillset configure` (interactive configuration prompts)"
read_when:
...
Tip: `skillset config` without a subcommand opens the same wizard. Use
`skillset config get|set|unset` for non-interactive edits.

For web search, `skillset configure --section web` lets you choose a provider
...
skillset configure --section model --section channels
skillset configure --section gateway --section daemon
```
