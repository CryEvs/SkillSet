---
summary: "CLI reference for `skillset secrets` (reload, audit, configure, apply)"
read_when:
...
Use `skillset secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
skillset secrets reload
skillset secrets reload --json
skillset secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan SkillSet state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `skillset.json` refs)
...
```bash
skillset secrets audit
skillset secrets audit --check
skillset secrets audit --json
skillset secrets audit --allow-exec
```
...
```bash
skillset secrets configure
skillset secrets configure --plan-out /tmp/skillset-secrets-plan.json
skillset secrets configure --apply --yes
skillset secrets configure --providers-only
skillset secrets configure --skip-provider-setup
skillset secrets configure --agent ops
skillset secrets configure --json
```
...
- `configure` targets secret-bearing fields in `skillset.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, SkillSet fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
skillset secrets apply --from /tmp/skillset-secrets-plan.json --json
```
...
- `skillset.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.skillset/.env` known secret keys whose values were migrated
