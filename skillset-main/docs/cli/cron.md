---
summary: "CLI reference for `skillset cron` (schedule and run background jobs)"
read_when:
...
Tip: run `skillset cron --help` for the full command surface.

Note: `skillset cron list` and `skillset cron show <job-id>` preview the
...
Note: `skillset cron run` now returns as soon as the manual run is queued for execution. Successful responses include `{ ok: true, enqueued: true, runId }`; use `skillset cron runs --id <job-id>` to follow the eventual outcome.

Note: `skillset cron run <job-id>` force-runs by default. Use `--due` to keep the
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.skillset/cron/runs/<jobId>.jsonl`.

Upgrade note: if you have older cron jobs from before the current delivery/store format, run
`skillset doctor --fix`. Doctor now normalizes legacy cron fields (`jobId`, `schedule.cron`,
...
```bash
skillset cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
skillset cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
skillset cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
skillset cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```
...
skillset cron run <job-id>
skillset cron run <job-id> --due
skillset cron runs --id <job-id> --limit 50
```
...
```bash
skillset cron edit <job-id> --agent ops
skillset cron edit <job-id> --clear-agent
skillset cron edit <job-id> --session current
skillset cron edit <job-id> --session "session:daily-brief"
```
...
```bash
skillset cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
skillset cron edit <job-id> --best-effort-deliver
skillset cron edit <job-id> --no-best-effort-deliver
skillset cron edit <job-id> --no-deliver
```
