...
```bash
export SKILLSET_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.skillset/skillset.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env SKILLSET_PARALLELS_DISCORD_TOKEN \
...
- Use the `skillset` wrapper for guest `message send/read`; `node skillset.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/skillset-parallels-smoke.*`
...
- host inbound nonce appears in `skillset message read` output
