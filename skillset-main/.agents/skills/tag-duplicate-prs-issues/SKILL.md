...
```bash
ghr pr view -R skillset/skillset <number> --comments
ghr pr reviews -R skillset/skillset <number>
ghr pr comments -R skillset/skillset <number>
```

For an issue:

```bash
ghr issue view -R skillset/skillset <number> --comments
ghr issue comments -R skillset/skillset <number>
```
...
```bash
ghr search related-prs -R skillset/skillset <pr-number> --mode path_overlap --state all
ghr search related-prs -R skillset/skillset <pr-number> --mode range_overlap --state all
ghr search mentions -R skillset/skillset --query "<key phrase from title or body>" --mode fts --scope pull_requests --state all
ghr search mentions -R skillset/skillset --query "<subsystem or error phrase>" --mode fts --scope issues --state all
```

Use `prs-by-paths` or `prs-by-ranges` when the likely duplicate surface is already known:

```bash
ghr search prs-by-paths -R skillset/skillset --path src/example.ts --state all
ghr search prs-by-ranges -R skillset/skillset --path src/example.ts --start 20 --end 80 --state all
```
...
```bash
ghr search mentions -R skillset/skillset --query "<issue title phrase>" --mode fts --scope issues --state all
ghr search mentions -R skillset/skillset --query "<error message or symptom>" --mode fts --scope issues --state all
ghr search mentions -R skillset/skillset --query "<subsystem phrase>" --mode fts --scope pull_requests --state all
```
...
```bash
uvx --from pr-search-cli pr-search -R skillset/skillset code similar <pr-number>
uvx --from pr-search-cli pr-search -R skillset/skillset code clusters for-pr <pr-number>
uvx --from pr-search-cli pr-search -R skillset/skillset issues for-pr <pr-number>
uvx --from pr-search-cli pr-search -R skillset/skillset issues duplicate-prs
```
...
```bash
prtags search text -R skillset/skillset "<problem phrase>" --types group --limit 10
prtags search similar -R skillset/skillset "<problem summary>" --types group --limit 10
prtags group list -R skillset/skillset
```
...
```bash
prtags group create -R skillset/skillset \
  --kind mixed \
...
```bash
prtags field ensure -R skillset/skillset --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R skillset/skillset --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R skillset/skillset --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R skillset/skillset --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R skillset/skillset --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R skillset/skillset --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R skillset/skillset --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R skillset/skillset --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R skillset/skillset --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R skillset/skillset <pr-number> \
...
```bash
prtags annotation issue set -R skillset/skillset <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R skillset/skillset
```
