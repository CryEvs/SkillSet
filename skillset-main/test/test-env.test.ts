...
function createTempHome(): string {
  return makeTempDir(tempDirs, "skillset-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-skillset.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.skillset/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.skillset/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".skillset", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".skillset", "external-plugins", "glueclaw", "skillset.plugin.json"),
      '{"id":"glueclaw"}\n',
    );
    writeFile(
      path.join(realHome, ".skillset", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.SKILLSET_LIVE_TEST_QUIET = "1";
    process.env.SKILLSET_CONFIG_PATH = "~/custom-skillset.json5";
    process.env.SKILLSET_TEST_HOME = priorIsolatedHome;
    process.env.SKILLSET_STATE_DIR = path.join(priorIsolatedHome, ".skillset");
    ...
    expect(process.env.SKILLSET_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".skillset", "skillset.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".skillset", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".skillset", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.SKILLSET_LIVE_TEST = "1";
    process.env.SKILLSET_LIVE_USE_REAL_HOME = "1";
    process.env.SKILLSET_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.SKILLSET_LIVE_TEST;
    delete process.env.SKILLSET_LIVE_GATEWAY;
    delete process.env.SKILLSET_LIVE_USE_REAL_HOME;
    delete process.env.SKILLSET_LIVE_TEST_QUIET;
