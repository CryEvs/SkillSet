...
describe("gh-read helpers", () => {
  it("finds repo from gh args", () => {
    expect(parseRepoArg(["pr", "view", "42", "-R", "skillset/skillset"])).toBe("skillset/skillset");
    expect(parseRepoArg(["run", "list", "--repo=skillset/docs"])).toBe("skillset/docs");
    expect(parseRepoArg(["pr", "view", "42"])).toBeNull();
  });

  it("normalizes repo strings from common git formats", () => {
    expect(normalizeRepo("skillset/skillset")).toBe("skillset/skillset");
    expect(normalizeRepo("github.com/skillset/skillset")).toBe("skillset/skillset");
    expect(normalizeRepo("https://github.com/skillset/skillset.git")).toBe("skillset/skillset");
    expect(normalizeRepo("git@github.com:skillset/skillset.git")).toBe("skillset/skillset");
