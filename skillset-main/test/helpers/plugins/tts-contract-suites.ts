...
import type { SkillSetConfig } from "../../../src/config/config.js";
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@skillset/speech-core",
...
function asLegacyTtsConfig(value: unknown): SkillSetConfig {
  return value as SkillSetConfig;
}

function asLegacySkillSetConfig(value: Record<string, unknown>): SkillSetConfig {
  return value as unknown as SkillSetConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): SkillSetConfig {
...
function createResolvedSummarizationConfig(cfg: SkillSetConfig): ResolvedTtsConfig {
  const rawConfig =
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withEnv(
          ...
          () => {
            const cfg = asLegacySkillSetConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: SkillSetConfig = asLegacySkillSetConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.SKILLSET_TTS_PREFS;
      process.env.SKILLSET_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.SKILLSET_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: SkillSetConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
