...
import type { ModelApi } from "skillset/plugin-sdk/provider-onboard";
...
export function expectProviderOnboardAllowlistAlias(params: {
  applyProviderConfig: (config: SkillSetConfig) => SkillSetConfig;
  modelRef: string;
  alias: string;
}) {
...
export function expectProviderOnboardPrimaryAndFallbacks(params: {
  applyConfig: (config: SkillSetConfig) => SkillSetConfig;
  modelRef: string;
}) {
