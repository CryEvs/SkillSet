    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-catalog-"));
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@skillset/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(os.tmpdir(), "skillset-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-catalog-state-"));
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-catalog-rich-"));
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
            JSON.stringify({
              ...
              schemaVersion: 1,
              description:
                "Extension manifest. Declares plugin packages that SkillSet can discover during onboarding and install on demand via `skillset plugins install`.",
              entries: [
                {
                  name: "@wecom/wecom-skillset-plugin",
                  description:
                    "SkillSet WeCom (企业微信) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  skillset: {
                    channel: {
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-catalog-home-"));
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@skillset/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              SKILLSET_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              SKILLSET_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@skillset/default-env-demo",
