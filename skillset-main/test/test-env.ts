...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.SKILLSET_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "SKILLSET_TEST_FAST", value: process.env.SKILLSET_TEST_FAST },
    {
      key: "SKILLSET_STRICT_FAST_REPLY_CONFIG",
      value: process.env.SKILLSET_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "SKILLSET_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.SKILLSET_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "SKILLSET_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.SKILLSET_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "SKILLSET_STATE_DIR", value: process.env.SKILLSET_STATE_DIR },
    { key: "SKILLSET_CONFIG_PATH", value: process.env.SKILLSET_CONFIG_PATH },
    { key: "SKILLSET_GATEWAY_PORT", value: process.env.SKILLSET_GATEWAY_PORT },
    { key: "SKILLSET_BRIDGE_ENABLED", value: process.env.SKILLSET_BRIDGE_ENABLED },
    { key: "SKILLSET_BRIDGE_HOST", value: process.env.SKILLSET_BRIDGE_HOST },
    { key: "SKILLSET_BRIDGE_PORT", value: process.env.SKILLSET_BRIDGE_PORT },
    { key: "SKILLSET_CANVAS_HOST_PORT", value: process.env.SKILLSET_CANVAS_HOST_PORT },
    { key: "SKILLSET_TEST_HOME", value: process.env.SKILLSET_TEST_HOME },
    { key: "SKILLSET_AGENT_DIR", value: process.env.SKILLSET_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.SKILLSET_TEST_HOME = tempHome;
  process.env.SKILLSET_TEST_FAST = "1";
  process.env.SKILLSET_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.SKILLSET_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.SKILLSET_BRIDGE_ENABLED;
  delete process.env.SKILLSET_BRIDGE_HOST;
  delete process.env.SKILLSET_BRIDGE_PORT;
  delete process.env.SKILLSET_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.SKILLSET_STATE_DIR = path.join(tempHome, ".skillset");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.SKILLSET_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.SKILLSET_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".skillset");
  const priorIsolatedHome = params.env.SKILLSET_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".skillset")
  ) {
    realStateDir = path.join(params.realHome, ".skillset");
  }
  const tempStateDir = path.join(params.tempHome, ".skillset");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.SKILLSET_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.SKILLSET_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "skillset.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "skillset.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.SKILLSET_LIVE_TEST === "1" ||
    process.env.SKILLSET_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.SKILLSET_LIVE_USE_REAL_HOME);
