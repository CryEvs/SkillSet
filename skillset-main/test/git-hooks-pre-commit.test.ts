...
describe("git-hooks/pre-commit (integration)", () => {
  it("does not treat staged filenames as git-add flags (e.g. --all)", () => {
    const dir = makeTempRepoRoot(tempDirs, "skillset-pre-commit-");
  ...
  it("runs changed-scope check for non-doc staged changes", () => {
    const dir = makeTempRepoRoot(tempDirs, "skillset-pre-commit-check-changed-");
    run(dir, "git", ["init", "-q", "--initial-branch=main"]);
  ...
  it("skips changed-scope check when FAST_COMMIT is enabled", () => {
    const dir = makeTempRepoRoot(tempDirs, "skillset-pre-commit-yolo-");
    run(dir, "git", ["init", "-q", "--initial-branch=main"]);
