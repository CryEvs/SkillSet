...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when SKILLSET_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".skillset", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when SKILLSET_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", SKILLSET_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".skillset-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers SKILLSET_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        SKILLSET_PROFILE: "rescue",
        SKILLSET_STATE_DIR: "C:\\State\\skillset",
      },
      expected: path.join("C:\\State\\skillset", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", SKILLSET_PROFILE: "default" },
      expected: path.join("/home/test", ".skillset", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem SkillSet Gateway",
          "cd /d C:\\Projects\\skillset",
          "set NODE_ENV=production",
          "set SKILLSET_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\skillset",
          environment: {
            NODE_ENV: "production",
            SKILLSET_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\skillset\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\SkillSet Share\\node.exe" "\\\\fileserver\\SkillSet Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\SkillSet Share\\node.exe",
            "\\\\fileserver\\SkillSet Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from SKILLSET_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ SKILLSET_STATE_DIR: path.join(tmpDir, "custom-state") }),
