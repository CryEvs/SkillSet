...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.SKILLSET_PROFILE;
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    SKILLSET_PROFILE: profile,
    SKILLSET_GATEWAY_PORT: String(port),
    SKILLSET_LAUNCHD_LABEL: resolvedLaunchdLabel,
    SKILLSET_SYSTEMD_UNIT: systemdUnit,
    SKILLSET_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    SKILLSET_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    SKILLSET_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    SKILLSET_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.SKILLSET_GATEWAY_TOKEN);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    SKILLSET_GATEWAY_TOKEN: gatewayToken,
    SKILLSET_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    SKILLSET_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    SKILLSET_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    SKILLSET_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    SKILLSET_LOG_PREFIX: "node",
    SKILLSET_SERVICE_MARKER: NODE_SERVICE_MARKER,
    SKILLSET_SERVICE_KIND: NODE_SERVICE_KIND,
    SKILLSET_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    SKILLSET_STATE_DIR: sharedEnv.stateDir,
    SKILLSET_CONFIG_PATH: sharedEnv.configPath,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.SKILLSET_STATE_DIR;
  const configPath = env.SKILLSET_CONFIG_PATH;
