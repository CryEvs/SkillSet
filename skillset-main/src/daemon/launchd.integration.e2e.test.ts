...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `skillset-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      SKILLSET_LAUNCHD_LABEL: `ai.skillset.launchd-int-${testId}`,
      SKILLSET_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
