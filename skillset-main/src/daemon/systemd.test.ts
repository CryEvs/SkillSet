...
const TEST_MANAGED_HOME = "/tmp/skillset-test-home";
const GATEWAY_SERVICE = "skillset-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/skillset gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/skillset", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/skillset-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", "skillset-gateway.service"]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", "skillset-gateway.service"]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/skillset-test-home" } }),
  ...
  it("returns false when systemctl is-enabled exits with code 4 (not-found)", async () => {
    ...
    execFileMock.mockImplementationOnce((_cmd, _args, _opts, cb) => {
      ...
      const err = new Error(
        "Command failed: systemctl --user is-enabled skillset-gateway.service",
      ) as Error & { code?: number };
      err.code = 4;
      cb(err, "not-found\n", "");
    });
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error("Command failed: systemctl --user is-enabled skillset-gateway.service"),
      ),
    ).toBe(true);
  });
...
describe("resolveSystemdUserUnitPath", () => {
  it.each([
    {
      name: "uses default service name when SKILLSET_PROFILE is unset",
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/skillset-gateway.service",
    },
    {
      name: "uses profile-specific service name when SKILLSET_PROFILE is set to a custom value",
      env: { HOME: "/home/test", SKILLSET_PROFILE: "jbphoenix" },
      expected: "/home/test/.config/systemd/user/skillset-gateway-jbphoenix.service",
    },
    {
      name: "prefers SKILLSET_SYSTEMD_UNIT over SKILLSET_PROFILE",
      env: {
        HOME: "/home/test",
        SKILLSET_PROFILE: "jbphoenix",
        SKILLSET_SYSTEMD_UNIT: "custom-unit",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "handles SKILLSET_SYSTEMD_UNIT with .service suffix",
      env: {
        HOME: "/home/test",
        SKILLSET_SYSTEMD_UNIT: "custom-unit.service",
      },
      expected: "/home/test/.config/systemd/user/custom-unit.service",
    },
    {
      name: "trims whitespace from SKILLSET_SYSTEMD_UNIT",
      env: {
        HOME: "/home/test",
        SKILLSET_SYSTEMD_UNIT: "  custom-unit  ",
      },
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/skillset gateway start --name "My Bot"')).toEqual([
      "/usr/bin/skillset",
      "gateway",
      "start",
      "--name",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('skillset --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["skillset", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('skillset --path "C:\\\\Program Files\\\\SkillSet"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["skillset", "--path", "C:\\\\Program Files\\\\SkillSet"]);

    expect(
      splitArgsPreservingQuotes('skillset --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["skillset", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/skillset gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads SKILLSET_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/skillset gateway run", "EnvironmentFile=%h/.skillset/.env"],
      { [`${TEST_SERVICE_HOME}/.skillset/.env`]: "SKILLSET_GATEWAY_TOKEN=env-file-token\n" },
    );

    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.SKILLSET_GATEWAY_TOKEN).toBe("env-file-token");
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/skillset gateway run",
        "EnvironmentFile=%h/.skillset/.env",
        'Environment="SKILLSET_GATEWAY_TOKEN=inline-token"',
      ],
      { [`${TEST_SERVICE_HOME}/.skillset/.env`]: "SKILLSET_GATEWAY_TOKEN=env-file-token\n" },
    );
    ...
    const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
    expect(command?.environment?.SKILLSET_GATEWAY_TOKEN).toBe("env-file-token");
    expect(command?.environmentValueSources?.SKILLSET_GATEWAY_TOKEN).toBe("file");
  });

  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.skillset/missing.env");
  });
  ...
  it("supports multiple EnvironmentFile entries and quoted paths", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      const pathValue = pathLikeToString(pathname);
      if (pathValue.endsWith("/skillset-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/skillset gateway run",
          'EnvironmentFile=%h/.skillset/first.env "%h/.skillset/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.skillset/first.env") {
        return "SKILLSET_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.skillset/second env.env") {
        return 'SKILLSET_GATEWAY_PASSWORD="second password"\n'; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    expect(command?.environment).toEqual({
      SKILLSET_GATEWAY_TOKEN: "first-token",
      SKILLSET_GATEWAY_PASSWORD: "second password", // pragma: allowlist secret
    });
  });
  ...
  it("resolves relative EnvironmentFile paths from the unit directory", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/.config/systemd/user/gateway.env")) {
        return [
          "SKILLSET_GATEWAY_TOKEN=relative-token", // pragma: allowlist secret
          "SKILLSET_GATEWAY_PASSWORD=relative-password", // pragma: allowlist secret
        ].join("\n");
      }
      if (pathValue.endsWith("/.config/systemd/user/override.env")) {
        return "SKILLSET_GATEWAY_TOKEN=override-token\n"; // pragma: allowlist secret
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      SKILLSET_GATEWAY_TOKEN: "override-token",
      SKILLSET_GATEWAY_PASSWORD: "relative-password", // pragma: allowlist secret
    });
  });
  ...
  it("parses EnvironmentFile content with comments and quoted values", async () => {
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith("/skillset-gateway.service")) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/skillset gateway run",
          "EnvironmentFile=%h/.skillset/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.skillset/gateway.env") {
        return [
          "# comment",
          "; another comment",
          'SKILLSET_GATEWAY_TOKEN="quoted token"', // pragma: allowlist secret
          "SKILLSET_GATEWAY_PASSWORD=quoted-password", // pragma: allowlist secret
        ].join("\n");
      }
      throw new Error(`unexpected readFile path: ${pathValue}`);
    });
    ...
    const command = await readSystemdServiceExecStart({ HOME: "/home/test" });
    expect(command?.environment).toEqual({
      SKILLSET_GATEWAY_TOKEN: "quoted token",
      SKILLSET_GATEWAY_PASSWORD: "quoted-password", // pragma: allowlist secret
    });
    expect(command?.environmentValueSources).toEqual({
      SKILLSET_GATEWAY_TOKEN: "file",
      SKILLSET_GATEWAY_PASSWORD: "file", // pragma: allowlist secret
    });
  });
});
...
describe("stageSystemdService", () => {
  async function withStageFixture(
    ...
    const tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-systemd-stage-"));
    const home = path.join(tempHomeRoot, "home");
    const stateDir = path.join(home, ".skillset");
    const env = {
      HOME: home,
      SKILLSET_STATE_DIR: stateDir,
      SKILLSET_SYSTEMD_UNIT: "skillset-gateway-stage-test",
    };
  ...
  it("writes dotenv-backed values to a separate env file and keeps inline env minimal", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        ...
        ["SKILLSET_GATEWAY_TOKEN=dotenv-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      await stageSystemdService({
        ...
        programArguments: ["/usr/bin/skillset", "gateway", "run"],
        workingDirectory: "/tmp",
        environment: {
          SKILLSET_GATEWAY_TOKEN: "dotenv-token",
          LLM_API_KEY: "dotenv-key",
          SKILLSET_GATEWAY_PORT: "18789",
        },
      });
      ...
      expect(unit).toContain("Environment=SKILLSET_GATEWAY_PORT=18789");
      expect(unit).not.toContain("Environment=SKILLSET_GATEWAY_TOKEN=dotenv-token");
      expect(unit).not.toContain("Environment=LLM_API_KEY=dotenv-key");
      expect(envFile).toBe("SKILLSET_GATEWAY_TOKEN=dotenv-token\nLLM_API_KEY=dotenv-key\n");
  ...
  it("keeps inline overrides out of the generated env file", async () => {
    await withStageFixture(async ({ env, stateDir, unitPath, envFilePath }) => {
      await fs.writeFile(
        path.join(stateDir, ".env"),
        ["SKILLSET_GATEWAY_TOKEN=stale-token", "LLM_API_KEY=dotenv-key"].join("\n"),
        "utf8",
      );
      ...
      await stageSystemdService({
        ...
        workingDirectory: "/tmp",
        environment: {
          SKILLSET_GATEWAY_TOKEN: "fresh-token",
          LLM_API_KEY: "dotenv-key",
        },
      });
      ...
      expect(unit).toContain(`EnvironmentFile=-${envFilePath}`);
      expect(unit).toContain("Environment=SKILLSET_GATEWAY_TOKEN=fresh-token");
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "skillset-gateway-work.service");
        cb(null, "", "");
      });
    await assertRestartSuccess({ SKILLSET_PROFILE: "work" });
  });
