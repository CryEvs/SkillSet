...
describe("resolveInitialTuiAgentId", () => {
  const cfg: SkillSetConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/skillset" },
        { id: "ops", workspace: "/tmp/skillset/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/skillset/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run skillset devices list");
    expect(state.pairingHint).toContain("skillset devices list");
  });
...
describe("resolveLocalAuthCliInvocation", () => {
  ...
  it("uses the packaged wrapper when dist is available", () => {
    expect(
      ...
      command: "/usr/bin/node",
      args: ["/repo/skillset.mjs", "models", "auth", "login"],
    });
  });
});
