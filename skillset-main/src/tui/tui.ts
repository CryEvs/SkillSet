...
import { loadConfig, type SkillSetConfig } from "../config/config.js";
...
const SKILLSET_CLI_WRAPPER_PATH = fileURLToPath(new URL("../../skillset.mjs", import.meta.url));
const SKILLSET_RUN_NODE_SCRIPT_PATH = fileURLToPath(
  new URL("../../scripts/run-node.mjs", import.meta.url),
);
const SKILLSET_DIST_ENTRY_JS_PATH = fileURLToPath(new URL("../../dist/entry.js", import.meta.url));
const SKILLSET_DIST_ENTRY_MJS_PATH = fileURLToPath(
...
export function resolveLocalAuthCliInvocation(params?: {
  ...
  const hasDistEntry =
    params?.hasDistEntry ??
    (existsSync(SKILLSET_DIST_ENTRY_JS_PATH) || existsSync(SKILLSET_DIST_ENTRY_MJS_PATH));
  const hasRunNodeScript = params?.hasRunNodeScript ?? existsSync(SKILLSET_RUN_NODE_SCRIPT_PATH);
  const command = params?.execPath ?? process.execPath;
  const wrapperPath = params?.wrapperPath ?? SKILLSET_CLI_WRAPPER_PATH;
  const runNodePath = params?.runNodePath ?? SKILLSET_RUN_NODE_SCRIPT_PATH;
...
export function resolveGatewayDisconnectState(reason?: string): {
  ...
  if (/pairing required/i.test(reasonLabel)) {
    return {
      ...
      activityStatus: "pairing required: run skillset devices list",
      pairingHint:
        "Pairing required. Run `skillset devices list`, approve your request ID, then reconnect.",
    };
  }
  return {
    connectionStatus: `gateway disconnected: ${reasonLabel}`,
...
export async function runTui(opts: TuiOptions) {
  ...
  const previousConsoleSubsystemFilter = isLocalMode
    ...
    : null;
  if (isLocalMode) {
    setConsoleSubsystemFilter(["__skillset_tui_quiet__"]);
  }
  ...
  const updateHeader = () => {
    ...
    header.setText(
      theme.header(
        `skillset tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`,
      ),
    );
  };
