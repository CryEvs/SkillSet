...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "skillset-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "skillset-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "skillset-bundled");
      const plugin = makeHookEntry("shared", "skillset-plugin");
      const managed = makeHookEntry("shared", "skillset-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("skillset-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "skillset-managed");
      const workspace = makeHookEntry("shared", "skillset-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "skillset-workspace");
      const second = makeHookEntry("shared", "skillset-workspace");
      second.hook.handlerPath = "/tmp/skillset-workspace/shared/handler-2.js";
