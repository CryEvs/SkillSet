...
homepage: https://docs.skillset.ai/automation/hooks#command-logger
metadata:
  {
    "skillset":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.skillset/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.skillset/logs/commands.log ~/.skillset/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/skillset`:
   ```
   /home/username/.skillset/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
