...
This directory contains hooks that ship with SkillSet. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.skillset/workspace`)
...
homepage: https://docs.skillset.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.skillset/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.skillset/skillset.json`:
...
2. Restart gateway: `pkill -9 -f 'skillset.*gateway' && pnpm skillset gateway`
3. Enable the hook: `skillset hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.skillset.ai/automation/hooks
