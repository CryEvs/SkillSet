...
function findBundledPluginMetadataFast(
  ...
  const roots = [
    ...
    path.resolve(SKILLSET_PACKAGE_ROOT, "extensions"),
    path.resolve(SKILLSET_PACKAGE_ROOT, "dist-runtime", "extensions"),
    path.resolve(SKILLSET_PACKAGE_ROOT, "dist", "extensions"),
  ].filter(
...
export function resolveRelativeExtensionPublicModuleId(params: {
  ...
  const fromFilePath = fileURLToPath(params.fromModuleUrl);
  const targetPath = resolveVitestSourceModulePath(
    path.resolve(SKILLSET_PACKAGE_ROOT, "extensions", params.dirName, params.artifactBasename),
  );
  const relativePath = path
    .relative(path.dirname(fromFilePath), targetPath)
    .replaceAll(path.sep, "/");
