...
function summarizeGroupPolicy(cfg: SkillSetConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: SkillSetConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: SkillSetConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: SkillSetConfig): boolean {
...
function isBrowserEnabled(cfg: SkillSetConfig): boolean {
  return cfg.browser?.enabled !== false;
}

export function collectAttackSurfaceSummaryFindings(cfg: SkillSetConfig): SecurityAuditFinding[] {
