...
function stubDiscordPlugin(params: {
  resolveAccount: (cfg: SkillSetConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: SkillSetConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: SkillSetConfig) => boolean;
}): ChannelPlugin {
  return {
    id: "discord",
    meta: {
...
describe("security audit channel source-config fallback discord", () => {
  it("keeps source-configured channel security findings when resolved inspection is incomplete", async () => {
    ...
    const resolvedConfig: SkillSetConfig = {
      commands: { native: true },
      channels: {
        discord: {
          enabled: true,
