...
type ConfiguredBinding = NonNullable<SkillSetConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<SkillSetConfig>,
): SkillSetConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: SkillSetConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/skillset" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
