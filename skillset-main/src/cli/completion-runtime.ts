...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# SkillSet Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# SkillSet Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(skillset completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "skillset") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
