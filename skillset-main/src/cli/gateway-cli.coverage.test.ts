...
describe("gateway-cli coverage", () => {
  ...
  it("prints the latest stability bundle without calling Gateway", async () => {
    callGateway.mockClear();
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-gateway-cli-bundle-"));
    try {
      const bundleDir = path.join(tempDir, "logs", "stability");
      const bundlePath = path.join(
        bundleDir,
        "skillset-stability-2026-04-22T12-00-00-000Z-123-test.json",
      );
      const bundle = {
        version: 1,
      ...
      await withEnvOverride({ SKILLSET_STATE_DIR: tempDir }, async () => {
  ...
  it("writes gateway diagnostics export with a best-effort health snapshot", async () => {
    callGateway.mockClear();
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-gateway-cli-support-"));
    try {
      const outputPath = path.join(tempDir, "diagnostics.zip");
      await withEnvOverride(
        { SKILLSET_STATE_DIR: tempDir, SKILLSET_TEST_FILE_LOG: undefined },
        async () => {
          await runGatewayCommand([
            "gateway",
            "diagnostics",
  ...
  it("registers gateway discover and prints json output", async () => {
    ...
    discoverGatewayBeacons.mockResolvedValueOnce([
      {
        instanceName: "Studio (SkillSet)",
        displayName: "Studio",
        domain: "skillset.internal.",
        host: "studio.skillset.internal",
        port: 18789,
    ...
    await withEnvOverride(
      {
        ...
        SKILLSET_LAUNCHD_LABEL: undefined,
        SKILLSET_SYSTEMD_UNIT: undefined,
        INVOCATION_ID: undefined,
        SYSTEMD_EXEC_PID: undefined,
        JOURNAL_STREAM: undefined,
        SKILLSET_WINDOWS_TASK_NAME: undefined,
        SKILLSET_SERVICE_MARKER: undefined,
        SKILLSET_SERVICE_KIND: undefined,
      },
      async () => {
  ...
  it("keeps exit 1 for gateway lock acquisition failures", async () => {
    ...
    serviceIsLoaded.mockResolvedValue(true);
    startGatewayServer.mockRejectedValueOnce(
      new GatewayLockError("failed to acquire gateway lock at /tmp/skillset/gateway.lock"),
    );
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ SKILLSET_GATEWAY_PORT: "19001" }, async () => {
      runtimeLogs.length = 0;
      runtimeErrors.length = 0;
