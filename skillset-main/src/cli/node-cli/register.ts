    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "skillset node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["skillset node status", "Check node host service status."],
          ["skillset node install", "Install the node host service."],
          ["skillset node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.skillset.ai/cli/node")}\n`,
    );
