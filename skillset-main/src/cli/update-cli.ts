...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update SkillSet and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["skillset update", "Update a source checkout (git)"],
        ["skillset update --channel beta", "Switch to beta channel (git + npm)"],
        ["skillset update --channel dev", "Switch to dev channel (git + npm)"],
        ["skillset update --tag beta", "One-off update to a dist-tag or version"],
        ["skillset update --tag main", "One-off package install from GitHub main"],
        ["skillset update --dry-run", "Preview actions without changing anything"],
        ["skillset update --no-restart", "Update without restarting the service"],
        ["skillset update --json", "Output result as JSON"],
        ["skillset update --yes", "Non-interactive (accept downgrade prompts)"],
        ["skillset update wizard", "Interactive update wizard"],
        ["skillset --update", "Shorthand for skillset update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run skillset update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.skillset.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skillset update status", "Show channel + version status."],
          ["skillset update status --json", "JSON output."],
          ["skillset update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.skillset.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
