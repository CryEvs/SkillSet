...
const serveSkillSetChannelMcp = mocks.serveSkillSetChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveSkillSetChannelMcp: mocks.serveSkillSetChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("skillset-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("skillset-cli-mcp-home-", async () => {
      ...
      expect(serveSkillSetChannelMcp).toHaveBeenCalledWith({
