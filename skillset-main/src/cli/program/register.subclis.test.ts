...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.SKILLSET_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.SKILLSET_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.SKILLSET_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.SKILLSET_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.SKILLSET_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.SKILLSET_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.SKILLSET_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "skillset", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "skillset"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "skillset", "nodes", "list"], "skillset");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "skillset", "infer"], "skillset");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "skillset", "acp", "--help"], "skillset");
