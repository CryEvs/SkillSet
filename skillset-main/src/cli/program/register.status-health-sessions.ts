...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skillset status", "Show channel health + session summary."],
          ["skillset status --all", "Full diagnosis (read-only)."],
          ["skillset status --json", "Machine-readable output."],
          ["skillset status --usage", "Show model provider usage/quota snapshots."],
          [
            "skillset status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["skillset status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.skillset.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.skillset.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skillset sessions", "List all sessions."],
          ["skillset sessions --agent work", "List sessions for one agent."],
          ["skillset sessions --all-agents", "Aggregate sessions across agents."],
          ["skillset sessions --active 120", "Only last 2 hours."],
          ["skillset sessions --json", "Machine-readable output."],
          ["skillset sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.skillset.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skillset sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "skillset sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["skillset sessions cleanup --enforce", "Apply maintenance now."],
          ["skillset sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["skillset sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "skillset sessions cleanup --enforce --store ./tmp/sessions.json",
