...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/skillset-root")).toEqual([
      path.join("/tmp/skillset-root", "dist", "index.js"),
      path.join("/tmp/skillset-root", "dist", "index.mjs"),
      path.join("/tmp/skillset-root", "dist", "entry.js"),
      path.join("/tmp/skillset-root", "dist", "entry.mjs"),
    ]);
  });
});
