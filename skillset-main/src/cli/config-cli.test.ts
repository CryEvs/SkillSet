...
import type { ConfigFileSnapshot, SkillSetConfig } from "../config/types.js";
import { createCliRuntimeCapture, mockRuntimeModule } from "./test-runtime-capture.js";

/**
 * Test for issue #6070:
 * `skillset config set/unset` must update snapshot.resolved (user config after $include/${ENV},
 * but before runtime defaults), so runtime defaults don't leak into the written config.
 */

const mockReadConfigFileSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>();
const mockWriteConfigFile = vi.fn<
  (cfg: SkillSetConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...
    writeConfigFile: (cfg: SkillSetConfig, options?: { unsetPaths?: string[][] }) =>
      mockWriteConfigFile(cfg, options),
    replaceConfigFile: (params: {
      nextConfig: SkillSetConfig;
...
function buildSnapshot(params: {
  resolved: SkillSetConfig;
  config: SkillSetConfig;
}): ConfigFileSnapshot {
  return {
    path: "/tmp/skillset.json",
    exists: true,
...
function setSnapshot(resolved: SkillSetConfig, config: SkillSetConfig) {
...
function withRuntimeDefaults(resolved: SkillSetConfig): SkillSetConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-skillset.json",
    exists: true,
    raw: "{}",
    parsed: {},
    sourceConfig: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: SkillSetConfig = {
    ...
    it("dry-runs nested plugin install updates without dropping sibling fields", async () => {
      const resolved = {
        plugins: {
          installs: {
            "skillset-web-search": {
              source: "npm",
              spec: "@ollama/skillset-web-search",
              installPath: "/tmp/skillset-web-search",
              version: "0.2.2",
              resolvedName: "@ollama/skillset-web-search",
              resolvedVersion: "0.2.2",
              resolvedSpec: "@ollama/skillset-web-search@0.2.2",
      ...
      await runConfigCommand([
        "config",
        "set",
        'plugins.installs["skillset-web-search"].spec',
        '"@ollama/skillset-web-search@0.2.2"',
        "--strict-json",
        "--dry-run",
      ]);
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__SKILLSET_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-skillset.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: SkillSetConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
    ...
    it("shows --strict-json and keeps --json as a legacy alias in help", async () => {
      ...
      expect(helpText).toContain("skillset config set gateway.port 19001 --strict-json");
      expect(helpText).toContain(
        "skillset config set channels.discord.token --ref-provider default --ref-source",
      );
  ...
  describe("config set builders and dry-run", () => {
    ...
    it("rejects --allow-exec without --dry-run", async () => {
      const nonexistentBatchPath = path.join(
        os.tmpdir(),
        `skillset-config-batch-nonexistent-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      await expect(
    ...
    it("supports batch-file mode", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `skillset-config-batch-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
    ...
    it("batch-file nested leaf updates preserve agents defaults and list siblings", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `skillset-config-memory-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify([
    ...
    it("rejects malformed batch-file payloads", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `skillset-config-batch-invalid-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(pathname, '{"path":"gateway.auth.mode","value":"token"}', "utf8");
      try {
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/skillset.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.skillset/skillset.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.skillset/skillset.json");
    });
  });
});
