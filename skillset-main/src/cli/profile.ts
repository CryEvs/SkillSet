...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.skillset${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.SKILLSET_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.SKILLSET_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.SKILLSET_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.SKILLSET_CONFIG_PATH)) {
    env.SKILLSET_CONFIG_PATH = path.join(stateDir, "skillset.json");
  }

  if (profile === "dev" && !env.SKILLSET_GATEWAY_PORT?.trim()) {
    env.SKILLSET_GATEWAY_PORT = "19001";
  }
}
