    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skillset nodes status", "List known nodes with live status."],
          ["skillset nodes pairing pending", "Show pending node pairing requests."],
          [
            'skillset nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["skillset nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.skillset.ai/cli/nodes")}\n`,
    );
