...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skillset security audit", "Run a local security audit."],
          ["skillset security audit --deep", "Include best-effort live Gateway probe checks."],
          ["skillset security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "skillset security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["skillset security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["skillset security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.skillset.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("SkillSet security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("skillset security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("skillset security audit --fix")}`));
        if (!fixResult) {
