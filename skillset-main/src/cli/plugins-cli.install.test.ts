...
const CLI_STATE_ROOT = "/tmp/skillset-state";

function cliInstallPath(pluginId: string): string {
  return installedPluginRoot(CLI_STATE_ROOT, pluginId);
}

function createEnabledPluginConfig(pluginId: string): SkillSetConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): SkillSetConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createPathHookPackInstalledConfig(tmpRoot: string): SkillSetConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): SkillSetConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function primeHookPackNpmFallback() {
  ...
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing skillset.plugin.json",
  });
...
function primeHookPackPathFallback(params: {
  ...
  loadConfig.mockReturnValue({} as SkillSetConfig);
...
describe("plugins cli install", () => {
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/skillset-config.json5",
      exists: true,
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `skillset doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-hook-link-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/skillset/.skillset/extensions/lossless-claw (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing skillset.hooks",
    });
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Use `skillset plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
  ...
  it("passes the install logger to the --link dry-run probe", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-link-plugin-"));
    const cfg = {
      plugins: {
        entries: {},
        load: {
  ...
  it("does not fall back to hook pack for local path when dangerous force unsafe install is set", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-local-plugin-"));
    const cfg = {} as SkillSetConfig;
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-local-hook-pack-"));
    const cfg = {} as SkillSetConfig;
    const installedCfg = {
      hooks: {
        internal: {
    ...
    installPluginFromPath.mockResolvedValue({
      ok: false,
      error: "package.json missing skillset.plugin.json",
      code: "missing_skillset_extensions",
    });
  ...
  it("does not fall back to npm when ClawHub rejects a real package", async () => {
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "skillset skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "skillset skills install demo" instead.');
  });
