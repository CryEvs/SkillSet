...
export function resolveConfigDir(
  ...
  const configPath = env.SKILLSET_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".skillset");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.SKILLSET_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$SKILLSET_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via SKILLSET_STATE_DIR.
