...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-trajectory-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("exportTrajectoryBundle", () => {
  it("sanitizes session ids in default export directory names", () => {
    ...
    expect(outputDir).toBe(
      path.join(
        ...
        ".skillset",
        "trajectory-exports",
        "skillset-trajectory-___evil_-2026-04-22T08-00-00",
      ),
    );
  });
  ...
  it("uses the recorded runtime pointer before current environment overrides", () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        traceSchema: "skillset-trajectory-pointer",
        schemaVersion: 1,
        sessionId: "session-1",
    ...
    const previous = process.env.SKILLSET_TRAJECTORY_DIR;
    process.env.SKILLSET_TRAJECTORY_DIR = envRuntimeDir;
    try {
      const bundle = exportTrajectoryBundle({
        outputDir,
        sessionFile,
      ...
      if (previous === undefined) {
        delete process.env.SKILLSET_TRAJECTORY_DIR;
      } else {
        process.env.SKILLSET_TRAJECTORY_DIR = previous;
      }
    }
  });
  ...
  it("exports merged runtime and transcript events plus convenience files", () => {
    ...
    const runtimeEvents: TrajectoryEvent[] = [
      ...
      },
      {
        ...
        sessionId: "session-1",
        data: {
          harness: { type: "skillset", version: "0.1.0" },
