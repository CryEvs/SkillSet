...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/skillset.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.skillset/skillset.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          SKILLSET_STATE_DIR: "/Users/tester/.skillset",
        },
      ...
      expect(harness.invocation).toEqual([
        ...
        "--api-key",
        "<redacted>",
        "--config=$SKILLSET_STATE_DIR/skillset.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/skillset.js");
