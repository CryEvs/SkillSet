...
export function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = env.SKILLSET_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
...
export function createTrajectoryRuntimeRecorder(
  ...
  // override so operators can still disable recording with SKILLSET_TRAJECTORY=0.
  const enabled = parseBooleanValue(env.SKILLSET_TRAJECTORY) ?? true;
  if (!enabled) {
    return null;
  }
