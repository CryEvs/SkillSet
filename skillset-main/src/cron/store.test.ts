...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses SKILLSET_HOME for tilde expansion", () => {
    vi.stubEnv("SKILLSET_HOME", "/srv/skillset-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/skillset-home", "cron", "jobs.json"));
  });
});
