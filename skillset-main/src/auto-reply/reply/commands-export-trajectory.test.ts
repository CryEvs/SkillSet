...
const hoisted = vi.hoisted(() => ({
  ...
  exportTrajectoryBundleMock: vi.fn(() => ({
    outputDir: "/tmp/workspace/.skillset/trajectory-exports/skillset-trajectory-session",
    manifest: {
      eventCount: 7,
      runtimeEventCount: 3,
  ...
  resolveDefaultTrajectoryExportDirMock: vi.fn(
    () => "/tmp/workspace/.skillset/trajectory-exports/skillset-trajectory-session",
  ),
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-export-command-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("buildExportTrajectoryReply", () => {
  ...
  it("builds a trajectory bundle from the target session", async () => {
    ...
    expect(hoisted.exportTrajectoryBundleMock).toHaveBeenCalledWith(
      expect.objectContaining({
        sessionId: "session-1",
        sessionKey: "agent:target:session",
        workspaceDir: expect.stringContaining("skillset-export-command-"),
      }),
    );
  });
  ...
  it("keeps user-named output paths inside the workspace trajectory export directory", async () => {
    ...
    expect(hoisted.exportTrajectoryBundleMock).toHaveBeenCalledWith(
      expect.objectContaining({
        outputDir: path.join(params.workspaceDir, ".skillset", "trajectory-exports", "my-bundle"),
      }),
    );
  });
  ...
  it("rejects output paths redirected by a symlinked exports directory", async () => {
    ...
    fs.mkdirSync(path.join(workspaceDir, ".skillset"), { recursive: true });
    fs.symlinkSync(outsideDir, path.join(workspaceDir, ".skillset", "trajectory-exports"));
  ...
  it("rejects symlinked state directories before creating export folders", async () => {
    ...
    const workspaceDir = makeTempDir();
    const outsideDir = makeTempDir();
    fs.symlinkSync(outsideDir, path.join(workspaceDir, ".skillset"));
    const params = makeParams(workspaceDir);
    params.command.commandBodyNormalized = "/export-trajectory my-bundle";
