...
const { createSkillSetTools } = await import("../agents/skillset-tools.js");
...
type SessionSendTool = ReturnType<typeof createSkillSetTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createSkillSetTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["SKILLSET_GATEWAY_PORT", "SKILLSET_GATEWAY_TOKEN"]);
  ...
  process.env.SKILLSET_GATEWAY_PORT = String(gatewayPort);
  process.env.SKILLSET_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("SKILLSET_CONFIG_PATH missing in gateway test environment");
      }
      ...
      const tool = createSkillSetTools({
        config: {
          tools: {
            sessions: {
              visibility: "all",
