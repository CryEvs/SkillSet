...
type GatewayReloadHandlerParams = {
  ...
  createHealthMonitor: (config: SkillSetConfig) => ChannelHealthMonitor | null;
};
...
> & {
  ...
  initialConfig: SkillSetConfig;
  initialCompareConfig?: SkillSetConfig;
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: SkillSetConfig) => string | undefined;
  sharedGatewaySessionGenerationState: SharedGatewaySessionGenerationState;
  clients: Iterable<SharedGatewayAuthClient>;
};

export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: SkillSetConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (SKILLSET_SKIP_GMAIL_WATCHER=1)"),
      });
    }

    if (plan.restartChannels.size > 0) {
      if (
        isTruthyEnvValue(process.env.SKILLSET_SKIP_CHANNELS) ||
        isTruthyEnvValue(process.env.SKILLSET_SKIP_PROVIDERS)
      ) {
        params.logChannels.info(
          "skipping channel reload (SKILLSET_SKIP_CHANNELS=1 or SKILLSET_SKIP_PROVIDERS=1)",
        );
      } else {
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: SkillSetConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
