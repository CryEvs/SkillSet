...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "SKILLSET_SKIP_GMAIL_WATCHER",
  "SKILLSET_SKIP_CANVAS_HOST",
  "SKILLSET_BUNDLED_PLUGINS_DIR",
  "SKILLSET_SKIP_CHANNELS",
  "SKILLSET_SKIP_PROVIDERS",
  "SKILLSET_SKIP_CRON",
  "SKILLSET_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.SKILLSET_CONFIG_PATH) {
    configPaths.add(process.env.SKILLSET_CONFIG_PATH);
  }
  if (process.env.SKILLSET_STATE_DIR) {
    configPaths.add(path.join(process.env.SKILLSET_STATE_DIR, "skillset.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.SKILLSET_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "skillset-test-no-bundled-extensions")
    : "skillset-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  const stateDir = process.env.SKILLSET_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".skillset-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".skillset-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".skillset-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>skillset-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__skillsetTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__skillsetTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["SKILLSET_GATEWAY_TOKEN"]);
  const prev = process.env.SKILLSET_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.SKILLSET_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.SKILLSET_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
