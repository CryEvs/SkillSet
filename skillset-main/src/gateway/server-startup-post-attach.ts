...
import type { loadSkillSetPlugins } from "../plugins/loader.js";
...
function shouldStartGatewayMemoryBackend(cfg: SkillSetConfig): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const [
    { resolveSkillSetAgentDir },
    { DEFAULT_MODEL, DEFAULT_PROVIDER },
    { selectAgentHarness },
    { isCliProvider, resolveConfiguredModelRef },
    { ensureSkillSetModelsJson },
  ...
  try {
    await ensureSkillSetModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: SkillSetConfig;
  pluginRegistry: ReturnType<typeof loadSkillSetPlugins>;
  ...
  await measureStartup(params.startupTrace, "sidecars.channels", async () => {
    if (!skipChannels) {
      ...
      params.logChannels.info(
        "skipping channel start (SKILLSET_SKIP_CHANNELS=1 or SKILLSET_SKIP_PROVIDERS=1)",
      );
    }
  });
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: SkillSetConfig;
    pluginRegistry: ReturnType<typeof loadSkillSetPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
