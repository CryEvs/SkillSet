...
const ACP_BIND_LIVE = isTruthyEnvValue(process.env.SKILLSET_LIVE_ACP_BIND);
...
async function prepareCodexHomeForLiveBindTest(): Promise<void> {
  ...
  const model = process.env.SKILLSET_LIVE_ACP_BIND_CODEX_MODEL?.trim() || DEFAULT_LIVE_CODEX_MODEL;
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        port: process.env.SKILLSET_GATEWAY_PORT,
      ...
      const liveAgent = normalizeAcpAgent(process.env.SKILLSET_LIVE_ACP_BIND_AGENT);
      const agentCommandOverride =
        process.env.SKILLSET_LIVE_ACP_BIND_AGENT_COMMAND?.trim() || undefined;
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-live-acp-bind-"));
      const tempStateDir = path.join(tempRoot, "state");
      const tempConfigPath = path.join(tempRoot, "skillset.json");
      ...
      try {
        ...
        if (previous.port === undefined) {
          ...
          process.env.SKILLSET_GATEWAY_PORT = previous.port;
        }
