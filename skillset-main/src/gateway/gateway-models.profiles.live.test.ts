...
import { resolveSkillSetAgentDir } from "../agents/agent-paths.js";
...
import { ensureSkillSetModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { clearRuntimeConfigSnapshot, loadConfig } from "../config/io.js";
import type { ModelsConfig, ModelProviderConfig, SkillSetConfig } from "../config/types.js";
...
const ZAI_FALLBACK = isTruthyEnvValue(process.env.SKILLSET_LIVE_GATEWAY_ZAI_FALLBACK);
const REQUIRE_PROFILE_KEYS = isLiveProfileKeyModeEnabled();
const LIVE_CREDENTIAL_PRECEDENCE = REQUIRE_PROFILE_KEYS ? "profile-first" : "env-first";
const PROVIDERS = parseFilter(process.env.SKILLSET_LIVE_GATEWAY_PROVIDERS);
const GATEWAY_LIVE_SMOKE = isTruthyEnvValue(process.env.SKILLSET_LIVE_GATEWAY_SMOKE);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.SKILLSET_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
const GATEWAY_LIVE_MODEL_TIMEOUT_MS = resolveGatewayLiveModelTimeoutMs();
const GATEWAY_LIVE_HEARTBEAT_MS = Math.max(
  1_000,
  toInt(process.env.SKILLSET_LIVE_GATEWAY_HEARTBEAT_MS, 30_000),
);
...
const QUIET_LIVE_LOGS = process.env.SKILLSET_LIVE_TEST_QUIET !== "0";

const describeLive = isLiveTestEnabled(["SKILLSET_LIVE_GATEWAY"]) ? describe : describe.skip;
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayRaw = process.env.SKILLSET_LIVE_GATEWAY_MAX_MODELS?.trim();
  if (gatewayRaw) {
    return Math.max(0, toInt(gatewayRaw, 0));
  }
  const rawModels = process.env.SKILLSET_LIVE_GATEWAY_MODELS?.trim();
  const useExplicitModels = Boolean(rawModels) && rawModels !== "modern" && rawModels !== "all";
  return resolveHighSignalLiveModelLimit({
    rawMaxModels: process.env.SKILLSET_LIVE_MAX_MODELS,
    useExplicitModels,
...
function resolveGatewayLiveModelTimeoutMs(
  gatewayModelTimeoutRaw = process.env.SKILLSET_LIVE_GATEWAY_MODEL_TIMEOUT_MS,
  liveModelTimeoutRaw = process.env.SKILLSET_LIVE_MODEL_TIMEOUT_MS,
...
describe("resolveGatewayLiveMaxModels", () => {
  const originalGatewayModels = process.env.SKILLSET_LIVE_GATEWAY_MODELS;
  const originalGatewayMax = process.env.SKILLSET_LIVE_GATEWAY_MAX_MODELS;
  const originalSharedMax = process.env.SKILLSET_LIVE_MAX_MODELS;
  ...
  afterEach(() => {
    restoreEnvValue("SKILLSET_LIVE_GATEWAY_MODELS", originalGatewayModels);
    restoreEnvValue("SKILLSET_LIVE_GATEWAY_MAX_MODELS", originalGatewayMax);
    restoreEnvValue("SKILLSET_LIVE_MAX_MODELS", originalSharedMax);
  });

  it("defaults modern gateway sweeps to the curated high-signal cap", () => {
    delete process.env.SKILLSET_LIVE_GATEWAY_MODELS;
    delete process.env.SKILLSET_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.SKILLSET_LIVE_MAX_MODELS;
  ...
  it("keeps explicit gateway model lists uncapped unless a cap is provided", () => {
    process.env.SKILLSET_LIVE_GATEWAY_MODELS = "openai/gpt-5.4,anthropic/claude-opus-4-6";
    delete process.env.SKILLSET_LIVE_GATEWAY_MAX_MODELS;
    delete process.env.SKILLSET_LIVE_MAX_MODELS;

    expect(resolveGatewayLiveMaxModels()).toBe(0);

    process.env.SKILLSET_LIVE_GATEWAY_MAX_MODELS = "2";
...
async function sanitizeAuthConfig(params: {
  cfg: SkillSetConfig;
  agentDir: string;
}): Promise<SkillSetConfig["auth"] | undefined> {
  ...
  let profiles: NonNullable<SkillSetConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const previous = {
    configPath: process.env.SKILLSET_CONFIG_PATH,
    token: process.env.SKILLSET_GATEWAY_TOKEN,
    skipChannels: process.env.SKILLSET_SKIP_CHANNELS,
    skipGmail: process.env.SKILLSET_SKIP_GMAIL_WATCHER,
    skipCron: process.env.SKILLSET_SKIP_CRON,
    skipCanvas: process.env.SKILLSET_SKIP_CANVAS_HOST,
    disableBonjour: process.env.SKILLSET_DISABLE_BONJOUR,
    logLevel: process.env.SKILLSET_LOG_LEVEL,
    agentDir: process.env.SKILLSET_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.SKILLSET_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.SKILLSET_SKIP_CHANNELS = "1";
  process.env.SKILLSET_SKIP_GMAIL_WATCHER = "1";
  process.env.SKILLSET_SKIP_CRON = "1";
  process.env.SKILLSET_SKIP_CANVAS_HOST = "1";
  if (QUIET_LIVE_LOGS) {
    process.env.SKILLSET_DISABLE_BONJOUR = "1";
    process.env.SKILLSET_LOG_LEVEL = "silent";
  }

  const token = `test-${randomUUID()}`;
  process.env.SKILLSET_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveSkillSetAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-live-state-"));
  process.env.SKILLSET_STATE_DIR = tempStateDir;
  ...
  process.env.SKILLSET_AGENT_DIR = tempAgentDir;
  process.env.PI_CODING_AGENT_DIR = tempAgentDir;

  const workspaceDir = resolveAgentWorkspaceDir(params.cfg, agentId);
  await fs.mkdir(workspaceDir, { recursive: true });
  await fs.mkdir(path.join(workspaceDir, ".skillset"), { recursive: true });
  await fs.writeFile(
    path.join(workspaceDir, ".skillset", "workspace-state.json"),
    `${JSON.stringify(
      {
        version: 1,
  ...
  const toolProbePath = path.join(workspaceDir, `.skillset-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveSkillSetAgentDir();
  const sanitizedCfg: SkillSetConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-live-"));
  const tempConfigPath = path.join(tempDir, "skillset.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.SKILLSET_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            (async () => {
              ...
              for (
                ...
                toolText = await requestGatewayAgentText({
                  ...
                  modelKey,
                  message: strictReply
                    ? "SkillSet live tool probe (local, safe): " +
    ...
    process.env.SKILLSET_CONFIG_PATH = previous.configPath;
    process.env.SKILLSET_GATEWAY_TOKEN = previous.token;
    process.env.SKILLSET_SKIP_CHANNELS = previous.skipChannels;
    process.env.SKILLSET_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.SKILLSET_SKIP_CRON = previous.skipCron;
    process.env.SKILLSET_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.SKILLSET_DISABLE_BONJOUR = previous.disableBonjour;
    process.env.SKILLSET_LOG_LEVEL = previous.logLevel;
    process.env.SKILLSET_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.SKILLSET_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () =>
      await withSuppressedGatewayLiveWarnings(async () => {
        ...
        await ensureSkillSetModelsJson(cfg);
        ...
        if (selectedCandidates.length < candidates.length) {
          logProgress(
            `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via SKILLSET_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
          );
        }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.skillset-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
