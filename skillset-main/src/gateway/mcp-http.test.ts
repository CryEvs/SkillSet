  ...
  it("passes session, account, and message channel headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-skillset-account-id": "work",
        "x-skillset-message-channel": "telegram",
      },
    ...
    const sendToolsList = async (senderIsOwner: "true" | "false") =>
      await sendRaw({
        ...
        headers: {
          ...
          "x-skillset-message-channel": "matrix",
        },
        body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
      });
  ...
  it("ignores spoofed owner headers when the bearer token is non-owner scoped", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: {
        ...
        "x-session-key": "agent:main:matrix:dm:test",
        "x-skillset-message-channel": "matrix",
        "x-skillset-sender-is-owner": "true",
      },
      body: JSON.stringify({ jsonrpc: "2.0", id: 1, method: "tools/list" }),
    });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.skillset?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.skillset?.headers?.Authorization).toBe(
      "Bearer ${SKILLSET_MCP_TOKEN}",
    );
    expect(config.mcpServers?.skillset?.headers?.["x-skillset-message-channel"]).toBe(
      "${SKILLSET_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.skillset?.headers?.["x-skillset-sender-is-owner"]).toBeUndefined();
  });
});
