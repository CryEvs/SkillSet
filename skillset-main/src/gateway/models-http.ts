...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([SKILLSET_MODEL_ID, SKILLSET_DEFAULT_MODEL_ID]);
  ids.add(`skillset/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`skillset/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== SKILLSET_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
