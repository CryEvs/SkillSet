...
export function resolveHooksConfig(cfg: SkillSetConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: SkillSetConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-skillset-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-skillset-idempotency-key"]) ||
