...
export async function parseMessageWithAttachments(
  ...
  try {
    for (const [idx, att] of attachments.entries()) {
      ...
      const finalMime =
        sniffedMime ?? providedMime ?? normalizeMime(mime) ?? mime ?? "application/octet-stream";
      const finalIsImage = isImageMime(finalMime);
      ...
      if (!finalIsImage) {
        ...
        try {
          ...
          const rawResult = await saveMediaBuffer(buffer, finalMime, "inbound", maxBytes, labelWithExt);
          const savedMedia = assertSavedMedia(rawResult, label);
          savedMediaIds.push(savedMedia.id);
          const mediaRef = savedMedia.path || `media://inbound/${savedMedia.id}`;
      ...
      if (!supportsImages) {
        log?.warn(`attachment ${label}: dropped image for text-only model`);
        continue;
      }
