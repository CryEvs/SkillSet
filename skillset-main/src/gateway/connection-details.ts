...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: SkillSetConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.SKILLSET_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.SKILLSET_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set SKILLSET_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: skillset doctor --fix",
