...
describe("gateway auth", () => {
  ...
  it("resolves token/password from SKILLSET gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          SKILLSET_GATEWAY_TOKEN: "env-token",
          SKILLSET_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides SKILLSET_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        SKILLSET_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
