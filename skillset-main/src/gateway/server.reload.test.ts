...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.SKILLSET_SKIP_CHANNELS;
    prevSkipGmail = process.env.SKILLSET_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.SKILLSET_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.SKILLSET_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.SKILLSET_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.SKILLSET_SKIP_GMAIL_WATCHER;
    } else {
      process.env.SKILLSET_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.SKILLSET_SKIP_PROVIDERS;
    } else {
      process.env.SKILLSET_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("SKILLSET_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ SKILLSET_TEST_MINIMAL_GATEWAY: undefined }, async () =>
  ...
  it("makes newly available catalog models visible in-process after hot reload", async () => {
    ...
    try {
      await withGatewayServer(async () => {
        ...
        const baseConfig: SkillSetConfig = {
          agents: {
            defaults: {
              model: {
...
`,
    ...
    const previousGatewayTokenEnv = process.env.SKILLSET_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.SKILLSET_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
