...
export function attachSkillSetTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__skillset && typeof record.__skillset === "object" && !Array.isArray(record.__skillset)
      ? (record.__skillset as Record<string, unknown>)
      : {};
  return {
    ...record,
    __skillset: {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachSkillSetTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "skillset" && model === "delivery-mirror";
