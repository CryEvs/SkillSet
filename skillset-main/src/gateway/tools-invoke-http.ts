...
function resolveMemoryToolDisableReasons(cfg: SkillSetConfig): string[] {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-skillset-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-skillset-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-skillset-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-skillset-thread-id"));
