...
function loadGatewayConfig(): SkillSetConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: SkillSetConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.SKILLSET_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as SkillSetConfig) : loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `skillset doctor` for diagnostics.";
  }
  return message;
}
