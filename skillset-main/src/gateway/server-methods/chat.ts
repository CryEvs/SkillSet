...
function resolveEduPlannerSttConfig(cfg: unknown): {
  ...
  const pluginCfg = root.plugins?.entries?.["edu-planner"]?.config;
  if (!pluginCfg || typeof pluginCfg !== "object") {
    return {};
  }
  return {
    sttPythonPath:
      typeof pluginCfg.sttPythonPath === "string" ? pluginCfg.sttPythonPath : undefined,
    sttScriptPath:
      typeof pluginCfg.sttScriptPath === "string" ? pluginCfg.sttScriptPath : undefined,
  };
}
...
export function buildOversizedHistoryPlaceholder(message?: unknown): Record<string, unknown> {
  ...
  return {
    ...
    __skillset: { truncated: true, reason: "oversized" },
  };
}
  ...
  "chat.send": async ({ params, respond, context, client }) => {
    ...
    if (normalizedAttachments.length > 0) {
      ...
      try {
        ...
        const firstAudioPath = offloadedRefs.find((ref) => ref.mimeType.startsWith("audio/"))?.path;
        if (firstAudioPath) {
          ...
          if (transcript && transcript.trim()) {
            parsedMessage = [
              parsedMessage,
              "",
              "Ниже автоматический транскрипт прикрепленного аудио (STT):",
              transcript,
              "",
              "Определи формат материала: лекция, разбор задания, инструкция учителя или смешанный формат.",
              "Если в аудио есть задание (например, написать изложение, сочинение, решить задачи), выдели его отдельно.",
              "Верни структурированный ответ:",
              "1) Кратко о содержании аудио",
              "2) Что было сделано/объяснено",
              "3) Какое задание задано (если есть)",
              "4) Что нужно сделать дальше (план действий, без решения задания)",
            ].join("\n");
          }
        }
      } catch (err) {
