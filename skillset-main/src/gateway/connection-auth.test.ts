...
function cfg(input: Partial<SkillSetConfig>): SkillSetConfig {
  return input as SkillSetConfig;
}
...
const DEFAULT_ENV = {
  SKILLSET_GATEWAY_TOKEN: "env-token",
  SKILLSET_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when SKILLSET env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when SKILLSET env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when SKILLSET env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
