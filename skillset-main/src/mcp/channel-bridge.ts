...
export class SkillSetChannelBridge {
  ...
  constructor(
    private readonly cfg: SkillSetConfig,
  ...
  async start(): Promise<void> {
    ...
    this.gateway = new GatewayClient({
      ...
      clientDisplayName: "SkillSet MCP",
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`skillset mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `skillset mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
