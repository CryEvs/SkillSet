...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before SkillSet consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether SkillSet should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume SkillSet's bundle
   ...
   *
   * Use this when the backend should consume a canonical SkillSet auth profile
   ...
   * `profile-only` treats the selected SkillSet auth profile as the sole auth
