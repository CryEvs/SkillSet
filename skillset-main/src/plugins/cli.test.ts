...
const mocks = vi.hoisted(() => ({
  ...
  loadSkillSetPluginCliRegistry: vi.fn(),
  loadSkillSetPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadSkillSetPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadSkillSetPluginCliRegistry(...args),
  loadSkillSetPlugins: (...args: unknown[]) => mocks.loadSkillSetPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: SkillSetConfig;
  autoEnabledConfig: SkillSetConfig;
  ...
  expect(mocks.loadSkillSetPlugins).toHaveBeenCalledWith(
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadSkillSetPluginCliRegistry.mockReset();
    mocks.loadSkillSetPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadSkillSetPlugins.mockReset();
    mocks.loadSkillSetPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as SkillSetConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as SkillSetConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as SkillSetConfig, env);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadSkillSetPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadSkillSetPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadSkillSetPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("lazy-registers descriptor-backed plugin commands on first invocation", async () => {
    ...
    await registerPluginCliCommands(program, {} as SkillSetConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as SkillSetConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as SkillSetConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
  ...
  it("skips plugin CLI registration from validated config when the snapshot is invalid", async () => {
    ...
    expect(mocks.loadSkillSetPlugins).not.toHaveBeenCalled();
  });
});
