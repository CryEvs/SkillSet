...
const APPROVALS_PATH = "~/.skillset/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "skillset-app-server:thread:",
  "skillset-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("skillset.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("skillset.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to SkillSet instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
