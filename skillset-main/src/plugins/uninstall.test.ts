...
type PluginConfig = NonNullable<SkillSetConfig["plugins"]>;
...
function createPluginConfig(params: {
  ...
  loadPaths?: string[];
  channels?: SkillSetConfig["channels"];
}): SkillSetConfig {
...
function createSinglePluginWithEmptySlotsConfig(): SkillSetConfig {
...
function createSingleNpmInstallConfig(installPath: string): SkillSetConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "skillset-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-skillset-plugin-install/my-plugin",
      },
      extensionsDir,
    });
