...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-runtime-deps-test-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("installBundledRuntimeDeps", () => {
  it("uses a real write probe for runtime dependency roots", () => {
    ...
    expect(isWritableDirectory("/usr/lib/node_modules/skillset")).toBe(false);
    expect(accessSpy).not.toHaveBeenCalled();
    expect(mkdirSpy).toHaveBeenCalledWith(
      path.join("/usr/lib/node_modules/skillset", ".skillset-write-probe-"),
    );
  });
  ...
  it("uses an isolated execution root and copies node_modules back when requested", () => {
    ...
    expect(
      ...
      name: "skillset-runtime-deps-install",
      private: true,
    });
    expect(
      JSON.parse(
  ...
  it("does not fail an isolated runtime deps install when temp cleanup races", () => {
    ...
    vi.spyOn(fs, "rmSync").mockImplementation((target, options) => {
      if (
        !blockedCleanup &&
        path.basename(String(target)).startsWith(".skillset-runtime-deps-copy-")
      ) {
        blockedCleanup = true;
...
describe("ensureBundledPluginRuntimeDeps", () => {
  ...
  it("installs runtime deps into an external stage dir and exposes loader aliases", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "skillset", version: "2026.4.22" }),
    );
    ...
    const env = { SKILLSET_PLUGIN_STAGE_DIR: stageDir };
    const calls: BundledRuntimeDepsInstallParams[] = [];
    const result = ensureBundledPluginRuntimeDeps({
      env,
      installDeps: (params) => {
  ...
  it("installs missing runtime deps for source-checkout bundled plugins", () => {
    ...
    const calls: BundledRuntimeDepsInstallParams[] = [];
    const result = ensureBundledPluginRuntimeDeps({
      env: { SKILLSET_PLUGIN_STAGE_DIR: stageDir },
      installDeps: (params) => {
        calls.push(params);
      },
      pluginId: "tokenjuice",
    ...
    expect(installRoot).not.toBe(pluginRoot);
    expect(
      JSON.parse(fs.readFileSync(path.join(installRoot, ".skillset-runtime-deps.json"), "utf8")),
  ...
  it("keeps source-checkout bundled runtime deps in the plugin root without manifest churn", () => {
    ...
    fs.mkdirSync(pluginRoot, { recursive: true });
    fs.writeFileSync(
      path.join(pluginRoot, ".skillset-runtime-deps.json"),
    ...
    expect(fs.existsSync(path.join(pluginRoot, ".skillset-runtime-deps.json"))).toBe(false);
  });
