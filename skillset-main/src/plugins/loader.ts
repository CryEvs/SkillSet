...
import { discoverSkillSetPlugins } from "./discovery.js";
...
function createGuardedPluginRegistrationApi(api: SkillSetPluginApi): {
  api: SkillSetPluginApi;
  close: () => void;
} {
  let closed = false;
  return {
...
function runPluginRegisterSync(
  ...
  api: Parameters<NonNullable<SkillSetPluginDefinition["register"]>>[0],
): void {
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadSkillSetPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadSkillSetPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: SkillSetPluginDefinition;
  register?: SkillSetPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return {
        register: resolved as SkillSetPluginDefinition["register"],
      };
    }
    if (resolved && typeof resolved === "object") {
      const def = resolved as SkillSetPluginDefinition;
...
function isPluginLoadDebugEnabled(env: NodeJS.ProcessEnv): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(env.SKILLSET_PLUGIN_LOAD_DEBUG);
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.SKILLSET_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverSkillSetPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverSkillSetPlugins>["candidates"][number];
  right: ReturnType<typeof discoverSkillSetPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadSkillSetPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadSkillSetPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  const {
    env,
  ...
  try {
    ...
    const discovery = discoverSkillSetPlugins({
      workspaceDir: options.workspaceDir,
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into SkillSet yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: SkillSetPluginModule | null = null;
      try {
        ...
        mod = withProfile(
          ...
          registrationMode,
          () => getJiti(safeSource)(safeImportSource) as SkillSetPluginModule,
        );
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: SkillSetPluginModule | null = null;
            try {
              runtimeMod = withProfile(
                { pluginId: record.id, source: safeRuntimeSource },
                "load-setup-runtime-entry",
                () => getJiti(safeRuntimeSource)(safeRuntimeImportSource) as SkillSetPluginModule,
              );
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'skillset plugins list' for details.`,
        );
      }
    }
...
export async function loadSkillSetPluginCliRegistry(
  options: PluginLoadOptions = {},
