...
import { clearPluginDiscoveryCache, discoverSkillSetPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("skillset-plugins", tempDirs);
}
...
function buildCachedDiscoveryEnv(
  ...
  return {
    ...
    SKILLSET_PLUGIN_DISCOVERY_CACHE_MS: "5000",
    ...overrides,
  };
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverSkillSetPlugins>[0],
) {
  return discoverSkillSetPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}

function discoverWithCachedEnv(params: Parameters<typeof discoverSkillSetPlugins>[0]) {
  return discoverSkillSetPlugins(params);
}
...
function writePluginManifest(params: { pluginDir: string; id: string }) {
  fs.writeFileSync(
    path.join(params.pluginDir, "skillset.plugin.json"),
    JSON.stringify({
      id: params.id,
...
function expectCandidatePresence(
  result: Awaited<ReturnType<typeof discoverSkillSetPlugins>>,
...
describe("discoverSkillSetPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".skillset", "extensions");
  ...
  it("does not recurse arbitrary workspace directories for plugin auto-discovery", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@skillset/workspace-plugin",
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@skillset/stray-workspace-plugin",
    });

    const result = discoverSkillSetPlugins({
      workspaceDir,
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".skillset", "extensions");
    mkdirSafe(workspaceExt);
  ...
  it("does not treat repo-level live or test files as plugin entrypoints", () => {
    ...
    const { candidates, diagnostics } = discoverSkillSetPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
  ...
  it("uses explicit runtime extension entries for installed package plugins", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@skillset/runtime-pack",
  ...
  it("infers built dist entries for installed TypeScript package plugins", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@skillset/built-peer-pack",
      extensions: ["src/index.ts"],
      setupEntry: "src/setup-entry.ts",
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
  ...
  it("preserves nested entry paths when inferring installed dist entries", async () => {
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@skillset/nested-pack",
  ...
  it("keeps workspace package TypeScript entries unless runtime entries are explicit", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const pluginDir = path.join(workspaceDir, ".skillset", "extensions", "workspace-pack");
    mkdirSafe(path.join(pluginDir, "src"));
    mkdirSafe(path.join(pluginDir, "dist"));

    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@skillset/workspace-pack",
      extensions: ["./src/index.ts"],
    });
    writePluginEntry(path.join(pluginDir, "src", "index.ts"));
    writePluginEntry(path.join(pluginDir, "dist", "index.js"));

    const { candidates } = discoverSkillSetPlugins({
      workspaceDir,
      env: buildDiscoveryEnv(stateDir),
    });
  ...
  it("does not discover nested node_modules copies under installed plugins", async () => {
    const stateDir = makeTempDir();
    const pluginDir = path.join(stateDir, "extensions", "opik-skillset");
    ...
    writePluginPackageManifest({
      packageDir: pluginDir,
      packageName: "@opik/opik-skillset",
      extensions: ["./src/index.ts"],
    });
    writePluginManifest({ pluginDir, id: "opik-skillset" });
    fs.writeFileSync(
    ...
    writePluginPackageManifest({
      packageDir: path.join(pluginDir, "node_modules", "skillset"),
    ...
    const { candidates } = await discoverWithStateDir(stateDir, {});
    expectCandidateOrder(candidates, ["opik-skillset"]);
  });

  it("skips dependency and build directories while scanning workspace roots", () => {
    const stateDir = makeTempDir();
    const workspaceDir = path.join(stateDir, "workspace");
    const workspaceRoot = path.join(workspaceDir, ".skillset", "extensions");
    const workspacePluginDir = path.join(workspaceRoot, "workspace-plugin");
    const nestedNodeModulesDir = path.join(workspaceRoot, "node_modules", "skillset");
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@skillset/voice-call",
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        for (const [dirName, packageName, pluginId] of [
          ["elevenlabs-speech-pack", "@skillset/elevenlabs-speech", "elevenlabs"],
          ["microsoft-speech-pack", "@skillset/microsoft-speech", "microsoft"],
        ] as const) {
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        createPackagePluginWithEntry({
          packageDir,
          packageName: "@skillset/demo-plugin-dir",
          entryPath: "index.js",
        });
  ...
  it.each([
    ...
    expect(legacy?.format).toBe("skillset");
  ...
  it.each([
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@skillset/escape-pack",
    ...
    },
    {
      ...
      setup: (stateDir: string) => {
        ...
        writePluginPackageManifest({
          packageDir: globalExt,
          packageName: "@skillset/missing-entry-pack",
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const result = discoverSkillSetPlugins({
        env: {
          ...process.env,
          SKILLSET_DISABLE_BUNDLED_PLUGINS: undefined,
          SKILLSET_STATE_DIR: stateDir,
          SKILLSET_BUNDLED_PLUGINS_DIR: bundledDir,
        },
      });
  ...
  it("reuses bundled and global discovery across workspace-specific cache misses", () => {
    ...
    createPackagePluginWithEntry({
      ...
      packageName: "@skillset/bundled-plugin",
      pluginId: "bundled-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(globalExt, "global-plugin"),
      packageName: "@skillset/global-plugin",
      pluginId: "global-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceA, ".skillset", "extensions", "workspace-a-plugin"),
      packageName: "@skillset/workspace-a-plugin",
      pluginId: "workspace-a-plugin",
    });
    createPackagePluginWithEntry({
      packageDir: path.join(workspaceB, ".skillset", "extensions", "workspace-b-plugin"),
      packageName: "@skillset/workspace-b-plugin",
