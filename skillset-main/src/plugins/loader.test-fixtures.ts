...
import { clearPluginLoaderCache, loadSkillSetPlugins } from "./loader.js";
import { clearPluginManifestRegistryCache } from "./manifest-registry.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadSkillSetPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadSkillSetPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "skillset-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.SKILLSET_BUNDLED_PLUGINS_DIR;
const prevPluginStageDir = process.env.SKILLSET_PLUGIN_STAGE_DIR;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "skillset.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".skillset", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ SKILLSET_STATE_DIR: stateDir, ...params.env }, () =>
    loadSkillSetPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.SKILLSET_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevPluginStageDir === undefined) {
    delete process.env.SKILLSET_PLUGIN_STAGE_DIR;
  } else {
    process.env.SKILLSET_PLUGIN_STAGE_DIR = prevPluginStageDir;
  }
}
