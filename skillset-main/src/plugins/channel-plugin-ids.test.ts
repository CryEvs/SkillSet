...
describe("resolveGatewayStartupPluginIds", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds.mockReset().mockImplementation((config: SkillSetConfig) => {
    ...
    listPotentialConfiguredChannelPresenceSignals
      .mockReset()
      .mockImplementation((config: SkillSetConfig) => {
        return listPotentialConfiguredChannelIds(config).map((channelId: string) => ({
          channelId,
          source: "config",
        }));
      });
    hasPotentialConfiguredChannels.mockReset().mockImplementation((config: SkillSetConfig) => {
      if (Object.prototype.hasOwnProperty.call(config, "channels")) {
  ...
  it("includes required agent harness owner plugins when env forces the runtime", () => {
    expectStartupPluginIdsCase({
      ...
      env: { SKILLSET_AGENT_RUNTIME: "codex" },
      expected: ["demo-channel", "browser", "codex"],
    });
  });
