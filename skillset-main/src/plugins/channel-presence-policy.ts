...
export function listExplicitConfiguredChannelIdsForConfig(config: SkillSetConfig): string[] {
...
function listDisabledChannelIdsForConfig(config: SkillSetConfig): string[] {
  const channels = config.channels;
  if (!channels || typeof channels !== "object" || Array.isArray(channels)) {
    return [];
  }
