...
const loadSkillSetPluginsMock = vi.fn();
...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadSkillSetPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
  ...
  it.each([
    {
      ...
      expectedLoaderCall: {
        env: { SKILLSET_HOME: "/srv/skillset-home" },
      },
    },
    {
