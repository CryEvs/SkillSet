...
export const PLUGIN_MANIFEST_FILENAME = "skillset.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `skillset qa`, for example `matrix`. */
  commandName: string;
...
// package.json "skillset" metadata (used for setup/catalog)
...
export type SkillSetPackageStartup = {
  /**
...
export type SkillSetPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type SkillSetPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: SkillSetPackageSetupFeatures;
  channel?: PluginPackageChannel;
  install?: PluginPackageInstall;
  startup?: SkillSetPackageStartup;
};
...
export type PackageManifest = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, SkillSetPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): SkillSetPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
