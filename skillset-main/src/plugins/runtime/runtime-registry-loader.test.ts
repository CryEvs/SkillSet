...
const mocks = vi.hoisted(() => ({
  loadSkillSetPlugins: vi.fn<typeof import("../loader.js").loadSkillSetPlugins>(),
...
vi.mock("../loader.js", () => ({
  loadSkillSetPlugins: (...args: Parameters<typeof mocks.loadSkillSetPlugins>) =>
    mocks.loadSkillSetPlugins(...args),
}));
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  it("does not cache scoped loads by explicit plugin ids", () => {
    ...
    expect(mocks.loadSkillSetPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadSkillSetPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not forward empty channel scopes for broad channel loads", () => {
    ...
    expect(
      (mocks.loadSkillSetPlugins.mock.calls[0]?.[0] as { onlyPluginIds?: string[] }).onlyPluginIds,
    ).toBeUndefined();
  });
});
