...
let loadSkillSetPluginsMock: ReturnType<typeof vi.fn>;
...
function createWebSearchEnv(overrides?: Partial<NodeJS.ProcessEnv>) {
  return {
    SKILLSET_HOME: "/tmp/skillset-home",
    ...overrides,
    ...
    plugins: [
      {
        ...
        manifestPath: "/tmp/brave/skillset.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
      ...
      },
      {
        ...
        manifestPath: "/tmp/noise/skillset.plugin.json",
        channels: [],
        providers: [],
        skills: [],
        hooks: [],
...
function expectRuntimeProviderResolution(
  ...
  expect(loadSkillSetPluginsMock).not.toHaveBeenCalled();
}
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadSkillSetPluginsMock = vi
      .spyOn(loaderModule, "loadSkillSetPlugins")
  ...
  it("retains the snapshot cache when config contents change in place", () => {
    ...
    const env = createWebSearchEnv({ SKILLSET_HOME: "/tmp/skillset-home-a" });
  ...
  it("invalidates the snapshot cache when env contents change in place", () => {
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.SKILLSET_HOME = "/tmp/skillset-home-b";
      },
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        SKILLSET_PLUGIN_DISCOVERY_CACHE_MS: "0",
      },
    },
  ])("$title", ({ env }) => {
    expectSnapshotMemoization({
  ...
  it("does not leak host Vitest env into an explicit non-Vitest cache key", () => {
    ...
    expect(loadSkillSetPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("expires web-search snapshot memoization after the shortest plugin cache ttl", () => {
    ...
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      SKILLSET_PLUGIN_DISCOVERY_CACHE_MS: "5",
      SKILLSET_PLUGIN_MANIFEST_CACHE_MS: "20",
    });
    ...
    expect(loadSkillSetPluginsMock).toHaveBeenCalledTimes(2);
  });

  it("invalidates web-search snapshots when cache-control env values change in place", () => {
    const config = createBraveAllowConfig();
    const env = createWebSearchEnv({
      SKILLSET_PLUGIN_DISCOVERY_CACHE_MS: "1000",
    });
    ...
    expectSnapshotLoaderCalls({
      ...
      env,
      mutate: () => {
        env.SKILLSET_PLUGIN_DISCOVERY_CACHE_MS = "5";
      },
      expectedLoaderCalls: 2,
    });
  });
