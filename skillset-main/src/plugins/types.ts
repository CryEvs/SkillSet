...
import type {
  SkillSetPluginHookOptions,
  SkillSetPluginToolContext,
  SkillSetPluginToolFactory,
  SkillSetPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type SkillSetPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<SkillSetConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). SkillSet uses this to expose
 ...
 *
 * Runs after SkillSet resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common SkillSet auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and SkillSet wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before SkillSet builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. SkillSet no
...
/**
 * Provider-owned stream wrapper hook after SkillSet applies its generic
 ...
 *
 * Return `true` or `false` to override SkillSet's built-in provider cache TTL
 ...
 *
 * Runs only after SkillSet exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that SkillSet should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after SkillSet loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, SkillSet falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * SkillSet calls this only from async model resolution paths. After it
   ...
   *
   * Runs after SkillSet performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before SkillSet registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic SkillSet wrappers.
   *
   ...
   *
   * Called after SkillSet resolves the raw configured credential but before the
   ...
   * hint than SkillSet's generic auth-store guidance.
   */
   ...
   * SkillSet's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when SkillSet should surface a provider-specific hint for
   ...
   * merging. SkillSet deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. SkillSet stores the
   ...
   * without replacing the full SkillSet-owned system prompt.
   */
   ...
   * rewrites after SkillSet has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before SkillSet handles
   ...
   *
   * SkillSet uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `skillset doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `skillset doctor` should offer.
   *
   ...
   *
   * SkillSet calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep SkillSet's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * SkillSet falls back to that stored profile.
   */
...
export type SkillSetPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active SkillSet session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current SkillSet configuration */
  config: SkillSetConfig;
 ...
 */
export type SkillSetPluginCommandDefinition = {
...
export type SkillSetPluginHttpRouteAuth = "gateway" | "plugin";
export type SkillSetPluginHttpRouteMatch = "exact" | "prefix";
export type SkillSetPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type SkillSetPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type SkillSetPluginHttpRouteParams = {
  path: string;
  handler: SkillSetPluginHttpRouteHandler;
  auth: SkillSetPluginHttpRouteAuth;
  match?: SkillSetPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: SkillSetPluginGatewayRuntimeScopeSurface;
  replaceExisting?: boolean;
};

export type SkillSetPluginCliContext = {
  program: Command;
  config: SkillSetConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type SkillSetPluginCliRegistrar = (ctx: SkillSetPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want SkillSet to keep a plugin command lazy-loaded while still
 ...
 */
export type SkillSetPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type SkillSetPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type SkillSetPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type SkillSetPluginSecurityAuditContext = {
  config: SkillSetConfig;
  sourceConfig: SkillSetConfig;
  env: NodeJS.ProcessEnv;
  stateDir: string;
  configPath: string;
};

export type SkillSetPluginSecurityAuditCollector = (
  ctx: SkillSetPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

/** Context passed to long-lived plugin services. */
export type SkillSetPluginServiceContext = {
  config: SkillSetConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type SkillSetPluginService = {
  id: string;
  start: (ctx: SkillSetPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: SkillSetPluginServiceContext) => void | Promise<void>;
};

export type SkillSetPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type SkillSetPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  reload?: SkillSetPluginReloadRegistration;
  nodeHostCommands?: SkillSetPluginNodeHostCommand[];
  securityAuditCollectors?: SkillSetPluginSecurityAuditCollector[];
  register?: (api: SkillSetPluginApi) => void;
  activate?: (api: SkillSetPluginApi) => void;
};

export type SkillSetPluginModule = SkillSetPluginDefinition | ((api: SkillSetPluginApi) => void);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime" | "cli-metadata";

export type PluginConfigMigration = (config: SkillSetConfig) =>
  | {
      config: SkillSetConfig;
      changes: string[];
    }
...
export type SkillSetPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | SkillSetPluginToolFactory,
    opts?: SkillSetPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: SkillSetPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: SkillSetPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: SkillSetPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: SkillSetPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, SkillSet can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: SkillSetPluginCliCommandDescriptor[];
    },
  ) => void;
  registerReload: (registration: SkillSetPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: SkillSetPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: SkillSetPluginSecurityAuditCollector) => void;
  registerService: (service: SkillSetPluginService) => void;
   ...
   */
  registerCommand: (command: SkillSetPluginCommandDefinition) => void;
  ...
  /** Register a Pi embedded extension factory for SkillSet embedded runs. Only bundled plugins may use this seam, and `contracts.embeddedExtensionFactories` must include `"pi"`. */
