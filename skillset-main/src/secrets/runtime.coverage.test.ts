...
type SecretRegistryEntry = {
  id: string;
  configFile: "skillset.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.SKILLSET_DEBUG_RUNTIME_COVERAGE === "1";
const COVERAGE_LOADABLE_PLUGIN_ORIGINS =
  buildCoverageLoadablePluginOrigins(COVERAGE_REGISTRY_ENTRIES);
const PLUGIN_OWNED_SKILLSET_COVERAGE_EXCLUSIONS = new Set([
...
function applyConfigForSkillSetTarget(
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every skillset.json registry target when configured as active", async () => {
    const entries = COVERAGE_REGISTRY_ENTRIES.filter(
      (entry) =>
        entry.configFile === "skillset.json" &&
        !PLUGIN_OWNED_SKILLSET_COVERAGE_EXCLUSIONS.has(entry.id),
    );
    for (const batch of buildCoverageBatches(entries)) {
      logCoverageBatch("skillset.json", batch);
      const config = {} as SkillSetConfig;
      const env: Record<string, string> = {};
      for (const [index, entry] of batch.entries()) {
        const envId = `SKILLSET_SECRET_TARGET_${entry.id}`;
        ...
        applyConfigForSkillSetTarget(config, entry, envId, wildcardToken);
      }
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `SKILLSET_AUTH_SECRET_TARGET_${entry.id}`;
      ...
      const snapshot = await prepareAuthCoverageSnapshot({
        ...
        env,
        agentDirs: ["/tmp/skillset-agent-main"],
