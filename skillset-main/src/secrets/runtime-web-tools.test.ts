...
function setConfiguredFetchProviderKey(configTarget: SkillSetConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: SkillSetConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: SkillSetConfig, provider: ProviderUnderTest): unknown {
