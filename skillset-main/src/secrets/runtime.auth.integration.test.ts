...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("skillset-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".skillset", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".skillset", "agents", "ops", "agent");
