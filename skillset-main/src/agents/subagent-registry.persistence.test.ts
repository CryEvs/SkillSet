...
describe("subagent registry persistence", () => {
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-subagent-"));
  ...
  it("uses isolated temp state when SKILLSET_STATE_DIR is unset in tests", async () => {
    delete process.env.SKILLSET_STATE_DIR;
    const registryPath = resolveSubagentRegistryPath();
    expect(registryPath).toContain(path.join(os.tmpdir(), "skillset-test-state"));
  });
});
