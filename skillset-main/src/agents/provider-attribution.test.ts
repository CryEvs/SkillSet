...
describe("provider attribution", () => {
  it("resolves the canonical SkillSet product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in SkillSet runtime wrapper.",
      product: "SkillSet",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { SKILLSET_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "skillset",
        version: "2026.3.22",
        "User-Agent": "skillset/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { SKILLSET_VERSION: "2026.3.22" })).toEqual({
      originator: "skillset",
      version: "2026.3.22",
      "User-Agent": "skillset/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { SKILLSET_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ SKILLSET_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
