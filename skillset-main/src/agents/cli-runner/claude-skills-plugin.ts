...
const SKILLSET_CLAUDE_PLUGIN_NAME = "skillset-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredSkillSetTmpDir(), "skillset-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, SKILLSET_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: SKILLSET_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped SkillSet skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
