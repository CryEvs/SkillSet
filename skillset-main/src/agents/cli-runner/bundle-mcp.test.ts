...
beforeAll(async () => {
  envSnapshot = captureEnv(["SKILLSET_BUNDLED_PLUGINS_DIR"]);
  bundleProbeHomeDir = await tempHarness.createTempDir("skillset-cli-bundle-mcp-home-");
  bundleProbeWorkspaceDir = await tempHarness.createTempDir("skillset-cli-bundle-mcp-workspace-");
  const emptyBundledDir = await tempHarness.createTempDir("skillset-cli-bundle-mcp-bundled-");
  process.env.SKILLSET_BUNDLED_PLUGINS_DIR = emptyBundledDir;
...
function createEnabledBundleProbeConfig(): SkillSetConfig {
  return {
    plugins: {
      entries: {
...
describe("prepareCliBundleMcpConfig", () => {
  it("injects a strict empty --mcp-config overlay for bundle-MCP-enabled backends without servers", async () => {
    const workspaceDir = await tempHarness.createTempDir("skillset-cli-bundle-mcp-empty-");
  ...
  it("loads workspace bundle MCP plugins from the configured workspace root", async () => {
    const workspaceDir = await tempHarness.createTempDir("skillset-cli-bundle-mcp-workspace-root-");
    const pluginRoot = path.join(workspaceDir, ".skillset", "extensions", "workspace-probe");
  ...
  it("merges loopback overlay config with bundle MCP servers", async () => {
    ...
    expect(Object.keys(raw.mcpServers ?? {}).toSorted()).toEqual(["bundleProbe", "skillset"]);
    expect(raw.mcpServers?.skillset?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.skillset?.headers?.Authorization).toBe("Bearer ${SKILLSET_MCP_TOKEN}");

    await prepared.cleanup?.();
  });

  it("stabilizes the resume hash when only the SkillSet loopback port changes", async () => {
  ...
  it("preserves extra env values alongside generated MCP config", async () => {
    const workspaceDir = await tempHarness.createTempDir("skillset-cli-bundle-mcp-env-");
    ...
    const prepared = await prepareCliBundleMcpConfig({
      ...
      config: { plugins: { enabled: false } },
      env: {
        SKILLSET_MCP_TOKEN: "loopback-token-123",
        SKILLSET_MCP_SESSION_KEY: "agent:main:telegram:group:chat123",
      },
    });
  ...
  it("leaves args untouched when bundle MCP is disabled", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/skillset-bundle-mcp-disabled",
    });
  ...
  it("injects codex MCP config overrides with env-backed loopback headers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/skillset-bundle-mcp-codex",
      config: { plugins: { enabled: false } },
      additionalConfig: {
        mcpServers: {
          skillset: {
    ...
    expect(prepared.backend.args).toEqual([
      ...
      "--json",
      "-c",
      'mcp_servers={ skillset = { url = "http://127.0.0.1:23119/mcp", bearer_token_env_var = "SKILLSET_MCP_TOKEN", env_http_headers = { x-session-key = "SKILLSET_MCP_SESSION_KEY" } } }',
    ]);
  ...
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/skillset-bundle-mcp-gemini",
      config: { plugins: { enabled: false } },
      additionalConfig: {
        mcpServers: {
          skillset: {
    ...
    expect(prepared.env?.SKILLSET_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["skillset"]);
    expect(raw.mcpServers?.skillset?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.skillset?.headers?.Authorization).toBe("Bearer loopback-token-123");
