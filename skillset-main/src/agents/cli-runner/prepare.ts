...
const prepareDeps = {
  ...
  createMcpLoopbackServerConfig,
  resolveSkillSetDocsPath: async (
    params: Parameters<typeof import("../docs-path.js").resolveSkillSetDocsPath>[0],
  ) => (await import("../docs-path.js")).resolveSkillSetDocsPath(params),
};
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          ...
          SKILLSET_MCP_AGENT_ID: sessionAgentId ?? "",
          SKILLSET_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          SKILLSET_MCP_SESSION_KEY: params.sessionKey ?? "",
          SKILLSET_MCP_MESSAGE_CHANNEL: params.messageProvider ?? "",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  const docsPath = await prepareDeps.resolveSkillSetDocsPath({
    workspaceDir,
