...
let createSkillSetCodingTools: typeof import("./pi-tools.js").createSkillSetCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      SKILLSET_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "skillset-test-no-bundled-extensions"),
    },
    async () => {
      ({ createSkillSetCodingTools } = await import("./pi-tools.js"));
    },
  );
});
...
async function withSafeBinsExecTool(
  ...
  try {
    await withEnvAsync(
      {
        SKILLSET_SHELL_ENV_TIMEOUT_MS: "1",
        SHELL: "/bin/sh",
      },
      async () => {
        await run(ctx);
...
describe("createSkillSetCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "skillset-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "skillset-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "skillset-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "skillset-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "skillset-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "skillset-safe-bins-grep-",
        safeBins: ["grep"],
