...
type AgentEntry = NonNullable<NonNullable<SkillSetConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: SkillSetConfig): AgentEntry[] {
...
export function listAgentIds(cfg: SkillSetConfig): string[] {
...
export function resolveDefaultAgentId(cfg: SkillSetConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: SkillSetConfig, agentId: string): AgentEntry | undefined {
