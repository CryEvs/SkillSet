...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousSkillSetAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousSkillSetAgentDir = process.env.SKILLSET_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousSkillSetAgentDir === undefined) {
      delete process.env.SKILLSET_AGENT_DIR;
    } else {
      process.env.SKILLSET_AGENT_DIR = previousSkillSetAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "SKILLSET_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.SKILLSET_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.SKILLSET_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.SKILLSET_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: SkillSetConfig = {
  models: {
    providers: {
      "custom-proxy": {
