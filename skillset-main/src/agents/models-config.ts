...
import { planSkillSetModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: SkillSetConfig): {
  config: SkillSetConfig;
  sourceConfigForSecrets: SkillSetConfig;
} {
...
export async function ensureSkillSetModelsJson(
  config?: SkillSetConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveSkillSetAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planSkillSetModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
