...
const GOOGLE_LIVE_SINGLE_KEY = "SKILLSET_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "SKILLSET_LIVE_ANTHROPIC_KEY",
    listVar: "SKILLSET_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "SKILLSET_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `SKILLSET_LIVE_${base}_KEY`;
