...
function isLikelySkillSetCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "skillset" ||
    entryBasename === "skillset.mjs" ||
...
export function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real SkillSet CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureSkillSetModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelySkillSetCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): SkillSetConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureSkillSetModelsJson(cfg);
    } catch {
