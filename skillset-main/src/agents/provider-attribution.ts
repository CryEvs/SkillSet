...
const SKILLSET_ATTRIBUTION_PRODUCT = "SkillSet";
const SKILLSET_ATTRIBUTION_ORIGINATOR = "skillset";
...
function formatSkillSetUserAgent(version: string): string {
  return `${SKILLSET_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: SKILLSET_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: SKILLSET_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatSkillSetUserAgent(identity.version),
    },
  };
}
