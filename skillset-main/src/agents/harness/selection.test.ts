...
const originalRuntime = process.env.SKILLSET_AGENT_RUNTIME;
const originalHarnessFallback = process.env.SKILLSET_AGENT_HARNESS_FALLBACK;

afterEach(() => {
  clearAgentHarnesses();
  piRunAttempt.mockClear();
  if (originalRuntime == null) {
    delete process.env.SKILLSET_AGENT_RUNTIME;
  } else {
    process.env.SKILLSET_AGENT_RUNTIME = originalRuntime;
  }
  if (originalHarnessFallback == null) {
    delete process.env.SKILLSET_AGENT_HARNESS_FALLBACK;
  } else {
    process.env.SKILLSET_AGENT_HARNESS_FALLBACK = originalHarnessFallback;
  }
});

function createAttemptParams(config?: SkillSetConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
describe("runAgentHarnessAttemptWithFallback", () => {
  ...
  it("falls back to the PI harness in auto mode when no plugin harness matches", async () => {
    process.env.SKILLSET_AGENT_RUNTIME = "auto";
