...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## SkillSet CLI Quick Reference");
    expect(prompt).toContain("skillset gateway restart");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      runtimeInfo: {
        channel: "webchat",
        canvasRootDir: "/Users/example/.skillset-dev/canvas",
      },
    });
    ...
    expect(prompt).toContain(
      '[embed url="/__skillset__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
      "Never use local filesystem paths or `file://...` URLs in `[embed ...]`.",
    );
    expect(prompt).toContain(
      "The active hosted embed root for this session is: `/Users/example/.skillset-dev/canvas`.",
    );
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List SkillSet agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("SkillSet docs: /tmp/skillset/docs");
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## SkillSet Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/skillset. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`skillset ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for SkillSet subagents");
