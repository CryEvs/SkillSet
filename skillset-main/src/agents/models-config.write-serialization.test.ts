...
const planSkillSetModelsJsonMock = vi.fn();
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planSkillSetModelsJson: (...args: unknown[]) => planSkillSetModelsJsonMock(...args),
  }));
  ({ ensureSkillSetModelsJson } = await import("./models-config.js"));
});

beforeEach(() => {
  planSkillSetModelsJsonMock
    .mockReset()
...
describe("models-config write serialization", () => {
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      try {
        await Promise.all([ensureSkillSetModelsJson(first), ensureSkillSetModelsJson(second)]);
      } finally {
