...
import { markSkillSetExecEnv } from "../../infra/skillset-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "skillset.sandbox=1");
  args.push("--label", `skillset.sessionKey=${params.scopeKey}`);
  args.push("--label", `skillset.createdAtMs=${createdAtMs}`);
  args.push("--label", `skillset.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `skillset.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markSkillSetExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "skillset.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`skillset sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`skillset sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("skillset sandbox recreate --all");
}
