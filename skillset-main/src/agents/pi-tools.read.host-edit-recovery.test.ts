...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when SKILLSET_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const skillsetHome = path.join(tmpDir, "skillset-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(skillsetHome, { recursive: true });

    const previousHome = process.env.HOME;
    const previousUserProfile = process.env.USERPROFILE;
    const previousOpenclawHome = process.env.SKILLSET_HOME;
    process.env.HOME = osHome;
    process.env.USERPROFILE = osHome;
    process.env.SKILLSET_HOME = skillsetHome;
    ...
    try {
      ...
      await expect(fs.access(path.join(skillsetHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
      ...
      if (previousOpenclawHome === undefined) {
        ...
        process.env.SKILLSET_HOME = previousOpenclawHome;
      }
    }
  });
