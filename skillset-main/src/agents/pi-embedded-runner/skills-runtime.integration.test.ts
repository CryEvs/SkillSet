...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("skillset-bundled-");
  const workspaceDir = await createTempDir("skillset-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: SkillSetConfig) {
