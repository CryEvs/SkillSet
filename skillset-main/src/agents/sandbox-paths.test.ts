...
async function withOutsideHardlinkInSkillSetTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside skillset tmp root",
      media: path.join(os.tmpdir(), "outside-skillset", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked SkillSet tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked SkillSet tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSkillSetTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked SkillSet tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInSkillSetTmp(
