...
function resolveAnthropicOpusThinking(cfg: SkillSetConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: SkillSetConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function createProviderWithModelsConfig(provider: string, models: Array<Record<string, unknown>>) {
  ...
  } as Partial<SkillSetConfig>;
}

function resolveConfiguredRefForTest(cfg: Partial<SkillSetConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as SkillSetConfig,
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    ...
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("claude-cli", {} as SkillSetConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as SkillSetConfig)).toBe(false);
    });
  });
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<SkillSetConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("sanitizes control characters in providerless-model warnings", () => {
      const warnLogs = createWarnLogCapture("skillset-model-selection-test");
      try {
        const cfg: Partial<SkillSetConfig> = {
          agents: {
            defaults: {
