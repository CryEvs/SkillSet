...
import { createSkillSetTools } from "./skillset-tools.js";
...
function resolveExecConfig(params: { cfg?: SkillSetConfig; agentId?: string }) {
...
export function createSkillSetCodingTools(options?: {
  agentId?: string;
  ...
  const base = (createCodingTools(workspaceRoot) as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === "read") {
      ...
      const wrapped = createSkillSetReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
