...
type CoreToolDefinition = {
  ...
  includeInSkillSetGroup?: boolean;
};
...
function buildCoreToolGroupMap() {
  ...
  const skillsetTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInSkillSetGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:skillset": skillsetTools,
