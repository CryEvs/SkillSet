...
function resolveAttachmentLimits(config: SkillSetConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".skillset", "attachments");
  const relDir = path.posix.join(".skillset", "attachments", attachmentId);
