...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("skillset-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("skillset-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("skillset-sbx-");
