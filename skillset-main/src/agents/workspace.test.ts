...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses SKILLSET_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      SKILLSET_HOME: "/srv/skillset-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/skillset-home"), ".skillset", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".skillset", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("skillset-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("skillset-workspace-");
    await fs.mkdir(path.join(tempDir, ".skillset"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-workspace-hardlink-"));
    try {
