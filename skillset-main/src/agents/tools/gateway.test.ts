...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    skillset: process.env.SKILLSET_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.skillset === undefined) {
      delete process.env.SKILLSET_GATEWAY_TOKEN;
    } else {
      process.env.SKILLSET_GATEWAY_TOKEN = envSnapshot.skillset;
    }
  });
  ...
  it("uses SKILLSET_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.SKILLSET_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
