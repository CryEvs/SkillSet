...
import { isSkillSetOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `skillset security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as SkillSetConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as SkillSetConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isSkillSetOwnerOnlyCoreToolName("gateway"),
    description:
