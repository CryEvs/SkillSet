...
export function buildHeadlessExecApprovalDeniedMessage(params: {
  ...
  return [
    ...
    "Stricter values from tools.exec and ~/.skillset/exec-approvals.json both apply.",
    "Fix one of these:",
    ...
    'Tip: run "skillset doctor" and "skillset approvals get --gateway" to inspect the effective policy.',
  ].join("\n");
}
