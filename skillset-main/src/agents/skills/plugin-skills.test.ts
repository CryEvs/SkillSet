    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "skillset.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "skillset.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "skillset.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("skillset-");
  const acpxRoot = await tempDirs.make("skillset-acpx-plugin-");
  const helperRoot = await tempDirs.make("skillset-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("skillset-");
  const pluginRoot = await tempDirs.make("skillset-plugin-");
  const outsideDir = await tempDirs.make("skillset-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("skillset-");
    const pluginRoot = await tempDirs.make("skillset-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("skillset-");
    const pluginRoot = await tempDirs.make("skillset-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
