...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseSkillSetManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applySkillSetManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): SkillSetSkillMetadata | undefined {
