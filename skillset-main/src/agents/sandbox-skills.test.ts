...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "skillset-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-sandbox-skills-"));
  });
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    ...
    process.env.SKILLSET_BUNDLED_SKILLS_DIR = bundledDir;
