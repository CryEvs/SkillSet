...
const TEST_ENV_VAR = "SKILLSET_MODELS_CONFIG_TEST_ENV";
...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/skillset-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): SkillSetConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: SkillSetConfig) {
