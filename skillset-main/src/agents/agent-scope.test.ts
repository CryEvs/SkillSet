...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: SkillSetConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/skillset" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: SkillSetConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: SkillSetConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: SkillSetConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgDisable: SkillSetConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: SkillSetConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: SkillSetConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: SkillSetConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/skillset");
  });

  it("uses SKILLSET_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "skillset-home");
    vi.stubEnv("SKILLSET_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as SkillSetConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".skillset", "workspace"));
  });

  it("uses SKILLSET_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "skillset-home");
    vi.stubEnv("SKILLSET_HOME", home);
    // Clear state dir so it falls back to SKILLSET_HOME
    vi.stubEnv("SKILLSET_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as SkillSetConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".skillset", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/skillset-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/skillset-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-agent-scope-"));
