...
import "./test-helpers/fast-skillset-tools-sessions.js";
...
function createSkillSetTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("uses number (not integer) in tool schemas for Gemini compatibility", () => {
    const tools = createSkillSetTools();
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createSkillSetTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createSkillSetTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
