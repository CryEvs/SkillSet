...
describe("handleSystemRunInvoke mac app exec host routing", () => {
  ...
  let sharedSkillSetHome = "";
  let sharedRuntimeBinDir = "";
  let sharedFixtureId = 0;
  let previousSkillSetHome: string | undefined;
  const sharedRuntimeBins = new Set<string>();

  beforeAll(() => {
    sharedFixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-node-host-fixtures-"));
    sharedSkillSetHome = path.join(sharedFixtureRoot, "skillset-home");
    sharedRuntimeBinDir = path.join(sharedFixtureRoot, "bin");
    fs.mkdirSync(sharedSkillSetHome, { recursive: true });
  ...
  beforeEach(() => {
    previousSkillSetHome = process.env.SKILLSET_HOME;
    process.env.SKILLSET_HOME = sharedSkillSetHome;
  ...
  afterEach(() => {
    clearRuntimeConfigSnapshot();
    if (previousSkillSetHome === undefined) {
      delete process.env.SKILLSET_HOME;
    } else {
      process.env.SKILLSET_HOME = previousSkillSetHome;
    }
  });
  ...
  async function withTempApprovalsHome<T>(params: {
    ...
    const tempHome = sharedSkillSetHome;
    const previousSkillSetHome = process.env.SKILLSET_HOME;
    process.env.SKILLSET_HOME = tempHome;
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      for (const testCase of approvedEnvShellWrapperCases) {
        const tmp = createFixtureDir("skillset-approved-wrapper-");
  ...
  it("denies shell payload carriers in allowlist mode without explicit approval", async () => {
    ...
    > = [
      ...
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "skillset status; id"]
            : ["/bin/sh", "-lc", "skillset status; id"],
        approvalRequired: true,
      },
      {
        ...
        command:
          process.platform === "win32"
            ? ["cmd.exe", "/d", "/s", "/c", "skillset status; cat /etc/passwd"]
            : ["/bin/sh", "-lc", "skillset status; cat /etc/passwd"],
        approvalRequired: true,
      },
      {
      ...
      for (const testCase of [
        {
          ...
          tmpPrefix: "skillset-approval-path-pin-",
        ...
        {
          name: "prepared plan",
          tmpPrefix: "skillset-prepare-run-path-pin-",
          run: async (ctx: { expected: string }) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      ...
      const sendInvokeResult = vi.fn(async () => {});
      await withPathTokenCommand({
        tmpPrefix: "skillset-allowlist-path-pin-",
      ...
      for (const testCase of [
        {
          label: "cwd symlink",
          setup: () => {
            const tmp = createFixtureDir("skillset-approval-cwd-link-");
        ...
        },
        {
          ...
          setup: () => {
            const tmp = createFixtureDir("skillset-approval-cwd-parent-link-");
  ...
  it("uses canonical executable path for approval-based relative command execution", async () => {
    const tmp = createFixtureDir("skillset-approval-cwd-real-");
  ...
  it("denies approval-based execution when cwd identity drifts before execution", async () => {
    const tmp = createFixtureDir("skillset-approval-cwd-drift-");
    const fallback = createFixtureDir("skillset-approval-cwd-drift-alt-");
    const script = path.join(tmp, "run.sh");
    fs.writeFileSync(script, "#!/bin/sh\necho SAFE\n");
    fs.chmodSync(script, 0o755);
    const canonicalCwd = fs.realpathSync(tmp);
  ...
  it("validates approved script operand bindings at dispatch", async () => {
    for (const mutate of [true, false]) {
      const tmp = createFixtureDir(
        mutate ? "skillset-approval-script-drift-" : "skillset-approval-script-stable-",
      );
  ...
  it("validates approved runtime script operand bindings at dispatch", async () => {
    await withFakeRuntimeOnPath({
      runtime: "tsx",
      run: async () => {
        const tmp = createFixtureDir("skillset-approval-tsx-script-drift-");
        ...
        const stableTmp = createFixtureDir("skillset-approval-tsx-script-stable-");
        ...
        const missingBindingTmp = createFixtureDir("skillset-approval-tsx-missing-binding-");
  ...
  it("denies ./sh wrapper spoof in allowlist on-miss mode before execution", async () => {
    const marker = path.join(os.tmpdir(), `skillset-wrapper-spoof-${process.pid}-${Date.now()}`);
    const runCommand = vi.fn(async () => {
  ...
  it("does not persist allow-always approvals for strict inline-eval carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("skillset-inline-eval-bin-");
          for (const testCase of cases) {
  ...
  it("persists benign awk allow-always approvals in strict inline-eval mode without reopening inline carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("skillset-inline-eval-awk-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "awk",
          });
  ...
  it("does not persist allow-always approvals for strict inline-eval make carriers", async () => {
    ...
    try {
      await withTempApprovalsHome({
        approvals: createAllowlistOnMissApprovals(),
        run: async () => {
          const tempDir = createFixtureDir("skillset-inline-eval-make-");
          const executablePath = createTempExecutable({
            dir: tempDir,
            name: "make",
          });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempDir = createFixtureDir("skillset-shell-wrapper-inner-");
  ...
  it("keeps cmd.exe transport wrappers approval-gated on Windows", async () => {
    ...
    try {
      for (const testCase of [
        ...
        const tempDir = createFixtureDir("skillset-cmd-wrapper-allow-");
  ...
  it("reuses exact-command durable trust for shell-wrapper reruns", async () => {
    ...
    const tempDir = createFixtureDir("skillset-shell-wrapper-allow-");
    const prepared = buildSystemRunApprovalPlan({
