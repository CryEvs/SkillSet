...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/skillset/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/skillset/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/skillset/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/skillset/nickserv-password.txt");
