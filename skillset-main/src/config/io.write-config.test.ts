...
import type { ConfigFileSnapshot } from "./types.skillset.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "skillset-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { SKILLSET_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("keeps writes inside an SKILLSET_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".skillset", "skillset.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "skillset.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "skillset.json"), "utf-8"),
      ) as {
  ...
  it("does not mutate caller config when unsetPaths is applied on first write", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".skillset", "skillset.json");
      const io = createConfigIO({
        env: {} as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            $schema: "https://skillset.ai/config.json",
            gateway: { mode: "local" },
          },
          null,
          2,
      ...
      expect(persisted.$schema).toBe("https://skillset.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/skillset-main" }] },
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".skillset", "skillset.json");
      const includePath = path.join(home, ".skillset", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://skillset.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/skillset-test-required-plugin",
          source: "/tmp/skillset-test-required-plugin/index.ts",
          manifestPath: "/tmp/skillset-test-required-plugin/skillset.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  it("writes runtime-derived edits back to source SecretRef markers", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".skillset", "skillset.json");
      const previousConfigPath = process.env.SKILLSET_CONFIG_PATH;
      ...
      try {
        ...
        if (previousConfigPath === undefined) {
          ...
          process.env.SKILLSET_CONFIG_PATH = previousConfigPath;
        }
      }
    });
  });
