...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-plugin-prefer-over-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin auto-enable preferOver", () => {
  it("tolerates bundled channel id metadata drift during auto-enable", async () => {
    ...
    vi.stubEnv("SKILLSET_BUNDLED_PLUGINS_DIR", rootDir);
    const { normalizeChatChannelId } = await import("../channels/ids.js");
    expect(normalizeChatChannelId(channelId)).toBe(channelId);

    vi.stubEnv("SKILLSET_BUNDLED_PLUGINS_DIR", path.join(rootDir, "missing"));
    ...
    const result = materializePluginAutoEnableCandidates({
      ...
      env: {
        SKILLSET_STATE_DIR: path.join(rootDir, "state"),
        SKILLSET_BUNDLED_PLUGINS_DIR: path.join(rootDir, "missing"),
      },
