...
function createCustomRootCfg(customRoot: string, defaultAgentId = "ops"): SkillSetConfig {
  return {
    session: {
...
const discoveryResolvers = [
  {
    label: "async",
    resolve: async (cfg: SkillSetConfig, env: NodeJS.ProcessEnv) =>
...
describe("resolveAllAgentSessionStoreTargets", () => {
  ...
  it("respects the caller env when resolving configured and discovered store roots", async () => {
    await withTempHome(async (home) => {
      ...
      const env = {
        ...process.env,
        SKILLSET_STATE_DIR: envStateDir,
      };
      const cfg: SkillSetConfig = {};
