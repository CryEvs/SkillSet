...
// Keep integration tests deterministic: never read a real skillset.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "skillset-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.SKILLSET_SESSION_CACHE_TTL_MS;
    process.env.SKILLSET_SESSION_CACHE_TTL_MS = "0";
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.SKILLSET_SESSION_CACHE_TTL_MS;
    } else {
      process.env.SKILLSET_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
