...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.skillset) || !isRecord(entry.skillset.channel)) {
      continue;
    }
    const channel = entry.skillset.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: SkillSetConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: SkillSetConfig, pluginId: string) => boolean;
