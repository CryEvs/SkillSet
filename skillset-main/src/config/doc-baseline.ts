...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.SKILLSET_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveSkillSetPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    SKILLSET_STATE_DIR: path.join(os.tmpdir(), "skillset-config-doc-baseline-state"),
    SKILLSET_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
