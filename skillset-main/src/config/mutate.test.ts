...
import type { ConfigFileSnapshot, SkillSetConfig } from "./types.js";
...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/skillset.json",
    exists: true,
    raw: "{}",
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "skillset-config-mutate-" });
  ...
  it("writes through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".skillset", "config", "plugins.json5");
