...
function collectModelRefs(cfg: SkillSetConfig): string[] {
  const refs: string[] = [];
...
function hasConfiguredEmbeddedHarnessRuntime(cfg: SkillSetConfig, env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: SkillSetConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: SkillSetConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: SkillSetConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: SkillSetConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: SkillSetConfig, env: NodeJS.ProcessEnv): string[] {
  return listPotentialConfiguredChannelIds(cfg, env).map(
    (channelId) => normalizeChatChannelId(channelId) ?? channelId,
  );
}

function hasConfiguredWebSearchPluginEntry(cfg: SkillSetConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: SkillSetConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: SkillSetConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: SkillSetConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: SkillSetConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: SkillSetConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: SkillSetConfig): boolean {
  const entries = cfg.plugins?.entries;
...
function hasPluginEntries(cfg: SkillSetConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithEntries(cfg: SkillSetConfig): boolean {
  return Array.isArray(cfg.plugins?.allow) && cfg.plugins.allow.length > 0 && hasPluginEntries(cfg);
}

function hasConfiguredProviderModelOrHarness(cfg: SkillSetConfig, env: NodeJS.ProcessEnv): boolean {
...
function configMayNeedPluginManifestRegistry(cfg: SkillSetConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: SkillSetConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: SkillSetConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isBuiltInChannelAlreadyEnabled(cfg: SkillSetConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
function registerPluginEntry(cfg: SkillSetConfig, pluginId: string): SkillSetConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
