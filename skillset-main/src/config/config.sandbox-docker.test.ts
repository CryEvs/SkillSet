...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("skillset-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "skillset-sandbox-browser-global" },
      agentBrowser: { network: "skillset-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("skillset-sandbox-browser-agent");
  });
