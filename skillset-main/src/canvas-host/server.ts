...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.skillsetCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.skillsetCanvasA2UIAction &&
        typeof window.skillsetCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.skillsetSendUserAction === "function";
  ...
  window.addEventListener("skillset:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android SkillSet node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.skillsetSendUserAction === "function"
        ? window.skillsetSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SKILLSET_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.SKILLSET_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>SkillSet Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
