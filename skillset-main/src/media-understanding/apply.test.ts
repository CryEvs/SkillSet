...
const TEMP_MEDIA_PREFIX = "skillset-media-";
...
function createGroqAudioConfig(): SkillSetConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): SkillSetConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): SkillSetConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: SkillSetConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredSkillSetTmpDir();
