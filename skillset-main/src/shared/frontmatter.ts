...
export function resolveSkillSetManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type SkillSetManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveSkillSetManifestRequires(
  metadataObj: Record<string, unknown>,
): SkillSetManifestRequires | undefined {
  const requiresRaw =
...
export function resolveSkillSetManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveSkillSetManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedSkillSetManifestInstallBase = {
...
export function parseSkillSetManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedSkillSetManifestInstallBase | undefined {
  ...
  const spec: ParsedSkillSetManifestInstallBase = {
    raw,
    kind,
  };
...
export function applySkillSetManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedSkillSetManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
