...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "skillset.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["skillset", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              SKILLSET_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).toContain("$SKILLSET_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/skillset-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.SKILLSET_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\skillset`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$SKILLSET_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\skillset\\dist\\index.js`,
              "--config",
              `${stateDir}\\skillset.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\skillset\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\skillset\\\\dist\\\\index.js");
    expect(serialized).toContain("$SKILLSET_STATE_DIR\\\\skillset.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\skillset\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/skillset.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/skillset-sanitized.jsonl");
    ...
    try {
      await writeDiagnosticSupportExport({
        env: {
          ...process.env,
          HOME: tempDir,
          SKILLSET_CONFIG_PATH: configPath,
          SKILLSET_STATE_DIR: tempDir,
        },
        stateDir: tempDir,
        outputPath,
