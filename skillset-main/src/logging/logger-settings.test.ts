...
let originalSkillSetLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.SKILLSET_TEST_FILE_LOG;
  originalSkillSetLogLevel = process.env.SKILLSET_LOG_LEVEL;
  delete process.env.SKILLSET_TEST_FILE_LOG;
  delete process.env.SKILLSET_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.SKILLSET_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalSkillSetLogLevel === undefined) {
    delete process.env.SKILLSET_LOG_LEVEL;
  } else {
    process.env.SKILLSET_LOG_LEVEL = originalSkillSetLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
