...
const logPathTracker = createSuiteLogPathTracker("skillset-test-env-log-level-");

describe("SKILLSET_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    originalEnv = process.env.SKILLSET_LOG_LEVEL;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.SKILLSET_LOG_LEVEL = originalEnv;
    }
    loggingState.invalidEnvLogLevelValue = null;
    resetLogger();
    setLoggerOverride(null);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.SKILLSET_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.SKILLSET_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("SKILLSET_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid SKILLSET_LOG_LEVEL="nope"');
  });
});
