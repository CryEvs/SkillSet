...
const SUPPORT_EXPORT_PREFIX = "skillset-diagnostics-";
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$SKILLSET_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/skillset-sanitized.jsonl`: sanitized log summaries and metadata",
