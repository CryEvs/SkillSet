...
let cachedSkillSetPackageRoot: string | undefined;
...
function getSkillSetPackageRoot() {
  if (cachedSkillSetPackageRoot) {
    return cachedSkillSetPackageRoot;
  }
  cachedSkillSetPackageRoot =
...
function resolveFacadeModuleLocationUncached(params: {
  ...
  return resolveBundledFacadeModuleLocation({
    ...
    packageRoot: getSkillSetPackageRoot(),
    bundledPluginsDir,
  });
}
...
export function loadFacadeModuleAtLocationSync<T extends object>(params: {
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      params.location.boundaryRoot === getSkillSetPackageRoot()
        ? "SkillSet package root"
        : (() => {
...
export async function loadBundledPluginPublicSurfaceModule<T extends object>(params: {
  ...
  const opened = openBoundaryFileSync({
    ...
    boundaryLabel:
      location.boundaryRoot === getSkillSetPackageRoot() ? "SkillSet package root" : "plugin root",
    rejectHardlinks: false,
  });
  if (!opened.ok) {
...
export function resetFacadeLoaderStateForTest(): void {
  ...
  cachedSkillSetPackageRoot = undefined;
}
