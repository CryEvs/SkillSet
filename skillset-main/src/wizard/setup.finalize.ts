...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      try {
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.skillset.ai/gateway/health",
            "https://docs.skillset.ai/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("skillset gateway run")}`,
          `Or rerun with: ${formatCliCommand("skillset onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("skillset onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.skillset.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
    ...
    if (gatewayProbe.ok) {
      await prompter.note(
        [
          ...
          "Stored in: $SKILLSET_CONFIG_PATH (default: ~/.skillset/skillset.json) under gateway.auth.token, or in SKILLSET_GATEWAY_TOKEN.",
          `View token: ${formatCliCommand("skillset config get gateway.auth.token")}`,
          `Generate token: ${formatCliCommand("skillset doctor --generate-gateway-token")}`,
          "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
          `Open the dashboard anytime: ${formatCliCommand("skillset dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control SkillSet."
            : "Copy/paste this URL in a browser on this machine to control SkillSet.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("skillset dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.skillset.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.skillset.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("skillset configure --section web")}`,
          "",
          "Docs: https://docs.skillset.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("skillset configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.skillset.ai/tools/web"}`,
          "Docs: https://docs.skillset.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("skillset configure --section web")}`,
          "",
          "Docs: https://docs.skillset.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://skillset.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control SkillSet."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control SkillSet.",
  );
