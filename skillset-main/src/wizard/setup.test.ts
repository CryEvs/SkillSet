...
const resolveGatewayPort = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, env?: NodeJS.ProcessEnv) => {
    const raw = env?.SKILLSET_GATEWAY_PORT ?? process.env.SKILLSET_GATEWAY_PORT;
...
const readConfigFileSnapshot = vi.hoisted(() =>
  vi.fn(async () => ({
    path: "/tmp/.skillset/skillset.json",
    exists: false,
...
describe("runSetupWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-onboard-suite-"));
  });
  ...
  it("resolves gateway.auth.password SecretRef for local setup probe", async () => {
    const previous = process.env.SKILLSET_GATEWAY_PASSWORD;
    process.env.SKILLSET_GATEWAY_PASSWORD = "gateway-ref-password"; // pragma: allowlist secret
    ...
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.SKILLSET_GATEWAY_PASSWORD = previous;
      }
    }
  ...
  it("shows the resolved gateway port in quickstart for fresh envs", async () => {
    const previousPort = process.env.SKILLSET_GATEWAY_PORT;
    process.env.SKILLSET_GATEWAY_PORT = "18791";
    const note: WizardPrompter["note"] = vi.fn(async () => {});
    const prompter = buildWizardPrompter({ note });
    const runtime = createRuntime();
    ...
    try {
      ...
      if (previousPort === undefined) {
        ...
        process.env.SKILLSET_GATEWAY_PORT = previousPort;
      }
    }
