...
vi.mock("../channels/plugins/bundled.js", () => {
  ...
  function resolveChatAppAccountId(cfg: SkillSetConfig): string {
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: SkillSetConfig; env: NodeJS.ProcessEnv }) => {
        const root = env.SKILLSET_STATE_DIR;
        if (!root) {
          return [];
        }
...
const createTempDir = () => tempDirs.make("skillset-state-migrations-test-");
