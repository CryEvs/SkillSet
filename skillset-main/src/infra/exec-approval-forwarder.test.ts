...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): SkillSetConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): SkillSetConfig {
  return {
