...
export function triggerSkillSetRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.SKILLSET_SYSTEMD_UNIT,
      process.env.SKILLSET_PROFILE,
    );
  ...
  const label =
    process.env.SKILLSET_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.SKILLSET_PROFILE);
