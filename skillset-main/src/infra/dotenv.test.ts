...
const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "SKILLSET_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "SKILLSET_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "SKILLSET_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "SKILLSET_BUNDLED_SKILLS_DIR=./attacker-skills",
  "SKILLSET_SKIP_BROWSER_CONTROL_SERVER=1",
] as const;
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-dotenv-test-"));
  const cwdDir = path.join(base, "cwd");
  const stateDir = path.join(base, "state");
  process.env.SKILLSET_STATE_DIR = stateDir;
...
describe("loadDotEnv", () => {
  it("loads ~/.skillset/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.skillset/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".skillset");
        process.env.SKILLSET_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "skillset", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "SKILLSET_STATE_DIR=./evil-state",
            "SKILLSET_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.SKILLSET_STATE_DIR).toBe(stateDir);
        expect(process.env.SKILLSET_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "SKILLSET_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "SKILLSET_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "SKILLSET_LIVE_GEMINI_KEY=sk-gemini-live",
            "SKILLSET_LIVE_OPENAI_KEY=sk-openai-live",
            "SKILLSET_GATEWAY_TOKEN=attacker-token",
            "SKILLSET_GATEWAY_PASSWORD=attacker-password",
            "SKILLSET_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks SKILLSET_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.SKILLSET_STATE_DIR).toBeUndefined();
        expect(process.env.SKILLSET_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (SKILLSET_AGENT_DIR, SKILLSET_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, SKILLSET_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "SKILLSET_AGENT_DIR=./evil-agent",
            `SKILLSET_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "SKILLSET_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );

        delete process.env.SKILLSET_AGENT_DIR;
        delete process.env.SKILLSET_BUNDLED_PLUGINS_DIR;
        delete process.env.PI_CODING_AGENT_DIR;
        delete process.env.SKILLSET_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.SKILLSET_AGENT_DIR).toBeUndefined();
        expect(process.env.SKILLSET_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.SKILLSET_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks SKILLSET_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "SKILLSET_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.SKILLSET_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.SKILLSET_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "SKILLSET_PINNED_PYTHON=./attacker-python",
            "SKILLSET_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.SKILLSET_PINNED_PYTHON;
        delete process.env.SKILLSET_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.SKILLSET_PINNED_PYTHON).toBeUndefined();
        expect(process.env.SKILLSET_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "SKILLSET_PINNED_PYTHON=/trusted/python",
            "SKILLSET_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.SKILLSET_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.SKILLSET_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "SKILLSET_LIVE_GEMINI_KEY=sk-gemini-live",
            "SKILLSET_LIVE_OPENAI_KEY=sk-openai-live",
            "SKILLSET_GATEWAY_TOKEN=trusted-token",
            "SKILLSET_GATEWAY_PASSWORD=trusted-password",
            "SKILLSET_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.SKILLSET_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.SKILLSET_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.SKILLSET_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.SKILLSET_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.SKILLSET_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.SKILLSET_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.SKILLSET_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "SKILLSET_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  ...
  it("does not load gateway.env when SKILLSET_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.SKILLSET_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "skillset", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.SKILLSET_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
      const legacyStateDir = path.join(base, ".clawdbot");
      process.env.HOME = base;
      delete process.env.SKILLSET_STATE_DIR;
      delete process.env.SKILLSET_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "SKILLSET_MDNS_HOSTNAME",
          "SKILLSET_SESSION_CACHE_TTL_MS",
          "SKILLSET_UPDATE_PACKAGE_SPEC",
          "SKILLSET_GATEWAY_PORT",
          "SKILLSET_GATEWAY_URL",
          "SKILLSET_CLAWHUB_URL",
          "CLAWHUB_URL",
          "SKILLSET_CLAWHUB_TOKEN",
          "CLAWHUB_TOKEN",
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
          "SKILLSET_DISABLE_BUNDLED_PLUGINS",
          ...
          "SKILLSET_SKIP_CHANNELS",
          "SKILLSET_SKIP_PROVIDERS",
          "SKILLSET_SKIP_CRON",
          "SKILLSET_RAW_STREAM",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("skillset/skillset");
