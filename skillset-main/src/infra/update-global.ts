...
const PRIMARY_PACKAGE_NAME = "skillset";
const ALL_PACKAGE_NAMES = [PRIMARY_PACKAGE_NAME] as const;
const GLOBAL_RENAME_PREFIX = ".";
export const SKILLSET_MAIN_PACKAGE_SPEC = "github:skillset/skillset#main";
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "skillset.plugin.json")))
      ) {
        expectedFiles.add(relativePath);
      }
    }),
...
async function resolvePortableGitPathPrepend(
  ...
  const portableGitRoot = path.join(localAppData, "SkillSet", "deps", "portable-git");
  const candidates = [
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.SKILLSET_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.SKILLSET_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return SKILLSET_MAIN_PACKAGE_SPEC;
  }
