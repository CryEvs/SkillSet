...
import { resolvePreferredSkillSetTmpDir } from "../tmp-skillset-dir.js";
...
const matrixChunkConfig: SkillSetConfig = {
  channels: { matrix: { textChunkLimit: 4000 } } as SkillSetConfig["channels"],
};

const expectedPreferredTmpRoot = resolvePreferredSkillSetTmpDir();
...
async function runChunkedMatrixDelivery(params?: {
  ...
  const cfg: SkillSetConfig = {
    channels: { matrix: { textChunkLimit: 2 } } as SkillSetConfig["channels"],
  };
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks direct adapter text and preserves delivery overrides across sends", async () => {
    ...
    const results = await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as SkillSetConfig,
      channel: "matrix",
      to: "!room",
      accountId: "default",
  ...
  it("uses adapter-provided formatted senders and scoped media roots when available", async () => {
    ...
    const textResults = await deliverOutboundPayloads({
      cfg: { channels: { line: {} } } as SkillSetConfig,
      channel: "line",
      to: "U123",
      accountId: "default",
    ...
    expect(
      sendFormattedMediaCall?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".skillset", "workspace-work")),
      ),
    ).toBe(true);
    expect(sendMedia).not.toHaveBeenCalled();
  });

  it("includes SkillSet tmp root in plugin mediaLocalRoots", async () => {
    const sendMatrix = vi.fn().mockResolvedValue({ messageId: "m-media", roomId: "!room" });

    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as SkillSetConfig,
      channel: "matrix",
      to: "!room:example",
  ...
  it("bails out without sending when a concurrent drain already claimed the queue entry", async () => {
    // Regression for skillset/skillset#70386: if a reconnect or startup drain
