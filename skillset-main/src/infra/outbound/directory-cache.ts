...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: SkillSetConfig | null = null;
  ...
  get(key: string, cfg: SkillSetConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: SkillSetConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: SkillSetConfig): void {
