...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred SkillSet tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideSkillSet = path.join(os.tmpdir(), "outside-skillset", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
