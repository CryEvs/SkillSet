...
describe("isGatewayArgv", () => {
  ...
  it("matches known entrypoints across slash and case variants", () => {
    expect(isGatewayArgv(["NODE", "C:\\SkillSet\\DIST\\ENTRY.JS", "gateway"])).toBe(true);
    expect(isGatewayArgv(["bun", "/srv/skillset/scripts/run-node.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["node", "/srv/skillset/skillset.mjs", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/skillset/src/entry.ts", "gateway"])).toBe(true);
    expect(isGatewayArgv(["tsx", "/srv/skillset/src/index.ts", "gateway"])).toBe(true);
  });

  it("matches the skillset executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\skillset.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/skillset-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/skillset-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\skillset-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/skillset/custom.js", "gateway"])).toBe(false);
