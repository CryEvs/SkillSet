...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.SKILLSET_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*skillset-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "skillset";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "skillset");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "skillset-gw");
  ...
  records.push(`_skillset-gw._tcp IN PTR ${instanceLabel}._skillset-gw._tcp`);
  records.push(`${instanceLabel}._skillset-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._skillset-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; skillset-content-hash: ${contentHash}\n${contentBody}`;
}
