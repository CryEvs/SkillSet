...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers SKILLSET_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for SKILLSET_HOME",
      env: { SKILLSET_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when SKILLSET_HOME is tilde-only and no fallback home exists",
      env: { SKILLSET_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores SKILLSET_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          SKILLSET_HOME: "/srv/skillset-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { SKILLSET_HOME: "/srv/skillset-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/skillset-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/skillset-home " },
      expected: "/srv/skillset-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/skillset-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of SKILLSET_HOME", () => {
    expect(
