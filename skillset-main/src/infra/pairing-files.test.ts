...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/skillset-state", "devices")).toEqual({
      dir: path.join("/tmp/skillset-state", "devices"),
      pendingPath: path.join("/tmp/skillset-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/skillset-state", "devices", "paired.json"),
    });
  });
