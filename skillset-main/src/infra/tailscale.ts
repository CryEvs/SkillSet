...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.SKILLSET_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for SkillSet. You can keep running the web gateway without it: \`${formatCliCommand("skillset gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
