...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.SKILLSET_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.SKILLSET_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.SKILLSET_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.SKILLSET_GATEWAY_PASSWORD)),
  });
