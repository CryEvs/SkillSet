...
const SEARCH_TOOL = "https://docs.skillset.ai/mcp.SearchSkillSet";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.skillset.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('skillset docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.skillset.ai/");
      runtime.log(`Search: ${formatCliCommand('skillset docs "your query"')}`);
    }
    return;
  }
