...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.SKILLSET_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: SkillSetConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: SkillSetConfig) => {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: SkillSetConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: SkillSetConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.SKILLSET_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<SkillSetConfig> {
