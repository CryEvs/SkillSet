...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as SkillSetConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as SkillSetConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<SkillSetConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<SkillSetConfig["models"]>>;
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<SkillSetConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as SkillSetConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<SkillSetConfig["tools"]>["media"],
    },
  };
}
...
export function normalizeLegacyMistralModelMaxTokens(
  ...
  return {
    ...
    models: {
      ...cfg.models,
      providers: nextProviders as NonNullable<SkillSetConfig["models"]>["providers"],
    },
  };
}
