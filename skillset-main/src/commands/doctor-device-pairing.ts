...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["skillset", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["skillset", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["skillset", "devices", "remove", pending.deviceId]),
    };
  }
