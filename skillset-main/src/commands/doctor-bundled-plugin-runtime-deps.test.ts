...
function writeBundledChannelPlugin(root: string, id: string, dependencies: Record<string, string>) {
  ...
  writeJson(path.join(root, "dist", "extensions", id, "skillset.plugin.json"), {
    id,
    channels: [id],
...
describe("doctor bundled plugin runtime deps", () => {
  it("skips source checkouts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-doctor-bundled-"));
  ...
  it("reports missing deps and conflicts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-doctor-bundled-"));
    writeJson(path.join(root, "package.json"), { name: "skillset" });
  ...
  it("reports default-enabled bundled plugin deps", () => {
    ...
    writeJson(path.join(root, "dist", "extensions", "openai", "skillset.plugin.json"), {
      id: "openai",
  ...
  it("repairs missing deps into an external stage dir when configured", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-doctor-bundled-"));
    const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-doctor-bundled-stage-"));
    writeJson(path.join(root, "package.json"), { name: "skillset", version: "2026.4.22" });
