...
export async function loadValidConfigOrThrow(): Promise<SkillSetConfig> {
...
export async function updateConfig(
  mutator: (cfg: SkillSetConfig) => SkillSetConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: SkillSetConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: SkillSetConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("skillset agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
