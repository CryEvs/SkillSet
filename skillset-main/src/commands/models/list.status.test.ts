...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveSkillSetAgentDir: vi.fn().mockReturnValue("/tmp/skillset-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/skillset-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/skillset-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/skillset-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/skillset-dev/skillset.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveSkillSetAgentDir: mocks.resolveSkillSetAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/skillset-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveSkillSetAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/skillset-dev/skillset.json");
    expect(payload.auth.storePath).toBe("/tmp/skillset-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/skillset-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/skillset-agent-custom");
