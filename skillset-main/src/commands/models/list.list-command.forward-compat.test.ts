...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    ensureSkillSetModelsJson: vi.fn(),
    ensureAuthProfileStore: vi.fn(),
    resolveSkillSetAgentDir: vi.fn(),
...
function resetMocks() {
  ...
  mocks.ensureSkillSetModelsJson.mockResolvedValue({ wrote: false });
  mocks.ensureAuthProfileStore.mockReturnValue({ version: 1, profiles: {}, order: {} });
  mocks.resolveSkillSetAgentDir.mockReturnValue("/tmp/skillset-agent");
...
function installModelsListCommandForwardCompatMocks() {
  ...
  vi.doMock("./list.runtime.js", () => ({
    ensureSkillSetModelsJson: mocks.ensureSkillSetModelsJson,
...
describe("modelsListCommand forward-compat", () => {
  ...
  describe("--all catalog supplementation", () => {
    it("uses the provider catalog fast path for Codex provider lists", async () => {
      ...
      expect(mocks.ensureSkillSetModelsJson).not.toHaveBeenCalled();
