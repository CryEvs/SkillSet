...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("skillset doctor --fix")} or ${formatCliCommand("skillset memory status --fix")}`
    : `Verify: ${formatCliCommand("skillset memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: SkillSetConfig): Promise<void> {
  try {
 ...
 * Runs as part of `skillset doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("skillset config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("skillset config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("skillset memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`skillset config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("skillset configure --section model")}`,
        `- To disable: ${formatCliCommand("skillset config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("skillset memory status --deep")}`,
      ].join("\n"),
      "Memory search",
