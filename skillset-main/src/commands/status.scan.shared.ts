...
export function hasExplicitMemorySearchConfig(cfg: SkillSetConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: SkillSetConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: SkillSetConfig, agentId: string) => { store: { path: string } } | null;
