...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".skillset");
  const targetPath = path.join(targetDir, "skillset.json");
  try {
...
export async function runDoctorConfigPreflight(
  ...
  if (
    ...
    note("Removed non-JSON prefix from skillset.json; original saved as .clobbered.*.", "Config");
