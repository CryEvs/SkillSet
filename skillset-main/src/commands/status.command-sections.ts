...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("skillset security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("skillset security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.skillset.ai/faq",
    "Troubleshooting: https://docs.skillset.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("skillset status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("skillset logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("skillset node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("skillset status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("skillset gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`skillset devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("skillset devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("skillset devices list")}`),
  ];
}
