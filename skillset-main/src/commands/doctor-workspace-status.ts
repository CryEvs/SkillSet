...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("skillset tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("skillset tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: SkillSetConfig) {
