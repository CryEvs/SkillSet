...
const MANAGED_SERVICE_ENV_KEYS_VAR = "SKILLSET_SERVICE_MANAGED_ENV_KEYS";
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      process.platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(params.env.SKILLSET_PROFILE)
        : undefined,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("skillset gateway install")}\` after fixing the error.`;
}
