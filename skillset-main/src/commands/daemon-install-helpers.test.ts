...
describe("resolveGatewayDevMode", () => {
  it("detects dev mode for src ts entrypoints", () => {
    expect(resolveGatewayDevMode(["node", "/Users/me/skillset/src/cli/index.ts"])).toBe(true);
    expect(resolveGatewayDevMode(["node", "C:\\Users\\me\\skillset\\src\\cli\\index.ts"])).toBe(
      true,
    );
    expect(resolveGatewayDevMode(["node", "/Users/me/skillset/dist/cli/index.js"])).toBe(false);
  });
});
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { SKILLSET_PORT: "3000" },
  } = params;
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.skillset/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ SKILLSET_PORT: "3000" });
  ...
  it("merges safe config env while dropping unsafe values and keeping service precedence", async () => {
    ...
    expect(plan.environment.SKILLSET_PORT).toBe("3000");
    expect(plan.environment.SKILLSET_SERVICE_MANAGED_ENV_KEYS).toBe(
      "CUSTOM_VAR,GOOGLE_API_KEY,SKILLSET_PORT,SAFE_KEY",
    );
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("merges .env vars with config and service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".skillset"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { SKILLSET_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        SKILLSET_SERVICE_MARKER: "skillset",
      },
    });
    ...
    expect(plan.environment.SKILLSET_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        SKILLSET_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
    ...
    expect(plan.environment.BLOGWATCHER_HOME).toBe("/Users/test/.blogwatcher");
    expect(plan.environment.GOPATH).toBeUndefined();
    expect(plan.environment.SKILLSET_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:skillset|skillset)( --profile isolated)? gateway install/,
    );
  });
});
