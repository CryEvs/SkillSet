...
describe("agents delete command", () => {
  ...
  it("purges deleted agent entries from the session store", async () => {
    await withStateDirEnv("skillset-agents-delete-", async ({ stateDir }) => {
      const now = Date.now();
  ...
  it("purges legacy main-alias entries owned by the deleted default agent", async () => {
    await withStateDirEnv("skillset-agents-delete-main-alias-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: SkillSetConfig = {
        agents: {
  ...
  it("preserves shared-store legacy default keys when deleting another agent", async () => {
    await withStateDirEnv("skillset-agents-delete-shared-store-", async ({ stateDir }) => {
      const now = Date.now();
      const cfg: SkillSetConfig = {
