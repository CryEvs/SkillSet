...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: SkillSetConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => SkillSetConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: SkillSetConfig, sourceConfig: SkillSetConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      ...
      effectiveConfig: SkillSetConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "skillset-agent-" });
}

function mockConfig(home: string, storePath: string): SkillSetConfig {
  const cfg = {
    agents: {
      defaults: {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteMock.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as SkillSetConfig },
    writeOptions: {},
  });
});
...
describe("agentCommand runtime config", () => {
  ...
  it("includes channel secret targets when delivery is requested", async () => {
    await withTempHome(async (home) => {
      ...
      } as unknown as SkillSetConfig["channels"];
      resolveCommandConfigWithSecretsMock.mockResolvedValueOnce({
