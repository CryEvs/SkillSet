...
- `SKILLSET_QA_CONVEX_SITE_URL` must use `https://` in normal use.
- Local development may use loopback `http://` only when `SKILLSET_QA_ALLOW_INSECURE_HTTP=1`.
...
```bash
pnpm skillset qa credentials add \
  --kind telegram \
...
Admin endpoints require `SKILLSET_QA_CONVEX_SECRET_MAINTAINER`.
