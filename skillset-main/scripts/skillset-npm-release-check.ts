...
const EXPECTED_REPOSITORY_URL = "https://github.com/skillset/skillset";
...
const skipPackValidationEnv = "SKILLSET_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "skillset") {
    errors.push(`package.json name must be "skillset"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.skillset !== "skillset.mjs") {
    errors.push(
      `package.json bin.skillset must be "skillset.mjs"; found "${pkg.bin?.skillset ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`skillset-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `skillset-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
