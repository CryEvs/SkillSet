...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[skillset] warning: detected ${packageManager} for install lifecycle.`,
    "[skillset] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[skillset] prefer: corepack pnpm install",
  ].join("\n");
}
