...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[skillset] ${message}\n`);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.SKILLSET_WATCH_MODE = "1";
  childEnv.SKILLSET_WATCH_SESSION = watchSession;
  ...
  childEnv.SKILLSET_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.SKILLSET_WATCH_COMMAND = deps.args.join(" ");
  }
