#!/usr/bin/env bash
# ClawDock - Docker helpers for SkillSet
# Inspired by Simon Willison's "Running SkillSet in Docker"
# https://til.simonwillison.net/llms/skillset-docker
#
# Installation:
...
# Common paths to check for SkillSet
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/skillset"
  "${HOME}/workspace/skillset"
  "${HOME}/projects/skillset"
  "${HOME}/dev/skillset"
  "${HOME}/code/skillset"
  "${HOME}/src/skillset"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found SkillSet at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/skillset"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ SkillSet not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/skillset/skillset.git ~/skillset"
    echo "  cd ~/skillset && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^SKILLSET_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d skillset-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart skillset-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f skillset-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.skillset"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show skillset.json
  if [[ -f "${config_dir}/skillset.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/skillset.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/skillset.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No skillset.json found${_CLR_RESET}"
  fi
  echo ""
...
clawdock-workspace() {
  cd ~/.skillset/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec skillset-gateway \
    bash -c 'echo "alias skillset=\"./skillset.mjs\"" > /tmp/.bashrc_skillset && bash --rcfile /tmp/.bashrc_skillset'
}
...
clawdock-cli() {
  _clawdock_compose run --rm skillset-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build skillset-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d skillset-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build skillset-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "SKILLSET_GATEWAY_TOKEN=$token" skillset-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" skillset-gateway \
    bash -c './skillset.mjs config set gateway.remote.token "$TOKEN" && ./skillset.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec skillset-gateway \
    bash -c "./skillset.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart skillset-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm skillset-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec skillset-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'skillset config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for SkillSet${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (skillset alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to skillset project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.skillset)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'skillset channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'skillset status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.skillset.ai${_CLR_RESET}"
  echo ""
}
