#!/usr/bin/env bash
# Rootless SkillSet in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-skillset-podman.sh launch        # Start gateway
#   ./scripts/run-skillset-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   skillset --container skillset dashboard --no-open
#   skillset --container skillset channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      SKILLSET_GATEWAY_TOKEN|SKILLSET_PODMAN_CONTAINER|SKILLSET_PODMAN_IMAGE|SKILLSET_IMAGE|SKILLSET_PODMAN_PULL|SKILLSET_PODMAN_GATEWAY_HOST_PORT|SKILLSET_GATEWAY_PORT|SKILLSET_PODMAN_BRIDGE_HOST_PORT|SKILLSET_BRIDGE_PORT|SKILLSET_GATEWAY_BIND|SKILLSET_PODMAN_USERNS|SKILLSET_BIND_MOUNT_OPTIONS|SKILLSET_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-skillset-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${SKILLSET_CONFIG_DIR:-$EFFECTIVE_HOME/.skillset}"
ENV_FILE="${SKILLSET_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${SKILLSET_CONFIG_DIR:-$EFFECTIVE_HOME/.skillset}"
ENV_FILE="${SKILLSET_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${SKILLSET_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${SKILLSET_PODMAN_CONTAINER:-skillset}"
SKILLSET_IMAGE="${SKILLSET_PODMAN_IMAGE:-${SKILLSET_IMAGE:-skillset:local}}"
PODMAN_PULL="${SKILLSET_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${SKILLSET_PODMAN_GATEWAY_HOST_PORT:-${SKILLSET_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${SKILLSET_PODMAN_BRIDGE_HOST_PORT:-${SKILLSET_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${SKILLSET_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$SKILLSET_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v skillset >/dev/null 2>&1; then
    return 0
  fi
  SKILLSET_CONTAINER="" SKILLSET_CONFIG_DIR="$config_dir" \
    skillset config get gateway.bind 2>/dev/null || true
}
...
# SKILLSET_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${SKILLSET_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'SKILLSET_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v skillset >/dev/null 2>&1; then
    echo "Warning: skillset not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    SKILLSET_CONTAINER="" SKILLSET_CONFIG_DIR="$config_dir" \
      skillset config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    SKILLSET_CONTAINER="" SKILLSET_CONFIG_DIR="$config_dir" \
      skillset config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  SKILLSET_CONTAINER="" SKILLSET_CONFIG_DIR="$config_dir" \
    skillset config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${SKILLSET_GATEWAY_TOKEN:-}" ]]; then
  export SKILLSET_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "SKILLSET_GATEWAY_TOKEN" "$SKILLSET_GATEWAY_TOKEN"
  echo "Generated SKILLSET_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/skillset.json"
...
PODMAN_USERNS="${SKILLSET_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported SKILLSET_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (SKILLSET_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${SKILLSET_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${SKILLSET_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$SKILLSET_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.skillset/.npm \
    -e SKILLSET_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.skillset:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.skillset/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$SKILLSET_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME skillset dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME skillset devices approve --latest  # if pairing required"
