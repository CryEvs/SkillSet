...
APP_BUNDLE="${1:-dist/SkillSet.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t skillset-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t skillset-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t skillset-entitlements-runtime.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/SkillSet" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/SkillSet" "$APP_ENTITLEMENTS"
fi

# Sign bundled helper binaries before signing the app bundle.
MLX_TTS_HELPER="$APP_BUNDLE/Contents/MacOS/skillset-mlx-tts"
