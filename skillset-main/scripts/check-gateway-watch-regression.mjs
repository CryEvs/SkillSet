...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  SKILLSET_SKIP_ACPX_RUNTIME: "1",
  SKILLSET_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$SKILLSET_WATCH_PID_FILE"',
    'mkdir -p "$SKILLSET_HOME/.skillset"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$SKILLSET_HOME/.skillset/skillset.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    SKILLSET_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    SKILLSET_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-gateway-watch-"));
