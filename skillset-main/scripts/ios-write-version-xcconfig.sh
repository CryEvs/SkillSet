...
- SKILLSET_IOS_VERSION = exact canonical iOS version
- SKILLSET_MARKETING_VERSION = short iOS/App Store version
- SKILLSET_BUILD_VERSION = explicit build number or local numeric fallback
EOF
}
...
while IFS='=' read -r key value; do
  case "${key}" in
    SKILLSET_IOS_VERSION)
      IOS_VERSION="${value}"
      ;;
    SKILLSET_MARKETING_VERSION)
...
SKILLSET_IOS_VERSION = ${IOS_VERSION}
SKILLSET_MARKETING_VERSION = ${MARKETING_VERSION}
SKILLSET_BUILD_VERSION = ${BUILD_NUMBER}
EOF
