...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${SKILLSET_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$SKILLSET_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.SKILLSET_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `SKILLSET_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${SKILLSET_INSTALL_SMOKE_IMAGE:-skillset-install-smoke:local}"
NONROOT_IMAGE="${SKILLSET_INSTALL_NONROOT_IMAGE:-skillset-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${SKILLSET_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${SKILLSET_INSTALL_URL:-https://skillset.bot/install.sh}"
CLI_INSTALL_URL="${SKILLSET_INSTALL_CLI_URL:-https://skillset.bot/install-cli.sh}"
PACKAGE_NAME="${SKILLSET_INSTALL_PACKAGE:-skillset}"
SKIP_NONROOT="${SKILLSET_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${SKILLSET_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${SKILLSET_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${SKILLSET_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${SKILLSET_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${SKILLSET_INSTALL_SMOKE_UPDATE_BASELINE:-2026.4.10}"
UPDATE_PACKAGE_SPEC="${SKILLSET_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${SKILLSET_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${SKILLSET_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${SKILLSET_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${SKILLSET_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${SKILLSET_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (SKILLSET_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  start_update_server
  ...
  docker run --rm -t \
    ...
    -e SKILLSET_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SKILLSET_INSTALL_METHOD=npm \
    -e SKILLSET_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e SKILLSET_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e SKILLSET_INSTALL_LATEST_OUT="/out/latest" \
    -e SKILLSET_NO_ONBOARD=1 \
    -e SKILLSET_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    ${UPDATE_DOCKER_HOST_ARGS[@]+"${UPDATE_DOCKER_HOST_ARGS[@]}"} \
    -e SKILLSET_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SKILLSET_INSTALL_SMOKE_MODE=update \
    -e SKILLSET_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e SKILLSET_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e SKILLSET_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e SKILLSET_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e SKILLSET_NO_ONBOARD=1 \
    -e SKILLSET_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (SKILLSET_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      ${UPDATE_DOCKER_HOST_ARGS[@]+"${UPDATE_DOCKER_HOST_ARGS[@]}"} \
      -e SKILLSET_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e SKILLSET_INSTALL_SMOKE_MODE=npm-global \
      -e SKILLSET_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e SKILLSET_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e SKILLSET_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e SKILLSET_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (SKILLSET_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e SKILLSET_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e SKILLSET_INSTALL_METHOD=npm \
    -e SKILLSET_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e SKILLSET_NO_ONBOARD=1 \
    -e SKILLSET_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${SKILLSET_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (SKILLSET_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e SKILLSET_INSTALL_URL="$INSTALL_URL" \
  -e SKILLSET_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e SKILLSET_NO_ONBOARD=1 \
  -e SKILLSET_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
