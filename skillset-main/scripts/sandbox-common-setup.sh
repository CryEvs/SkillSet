...
BASE_IMAGE="${BASE_IMAGE:-skillset-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-skillset-sandbox-common:bookworm-slim}"
...
SKILLSET_DOCKER_BUILD_USE_BUILDX="${SKILLSET_DOCKER_BUILD_USE_BUILDX:-0}"
SKILLSET_DOCKER_BUILD_CACHE_FROM="${SKILLSET_DOCKER_BUILD_CACHE_FROM:-}"
SKILLSET_DOCKER_BUILD_CACHE_TO="${SKILLSET_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${SKILLSET_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${SKILLSET_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${SKILLSET_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${SKILLSET_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${SKILLSET_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=skillset.sandbox=1)
NOTE
