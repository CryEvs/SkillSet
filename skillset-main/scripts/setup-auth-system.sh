#!/bin/bash
# Setup SkillSet Auth Management System
...
echo "=== SkillSet Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'skillset-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  SkillSet message: Send warning via SkillSet itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/skillset-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/skillset-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/skillset-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now skillset-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status skillset-auth-monitor.timer"
echo ""
echo "Quick commands:"
