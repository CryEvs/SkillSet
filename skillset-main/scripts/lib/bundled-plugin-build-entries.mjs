...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.skillset?.extensions)
    ? packageJson.skillset.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.skillset?.setupEntry === "string" &&
    packageJson.skillset.setupEntry.trim().length > 0
      ? packageJson.skillset.setupEntry
      : undefined;
  if (setupEntry) {
...
function shouldStageBundledPluginRuntimeDependencies(packageJson) {
  return packageJson?.skillset?.bundle?.stageRuntimeDependencies === true;
}
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasManifest, hasPackageJson, sourceEntries } of entries) {
    if (hasManifest) {
      artifacts.add(bundledDistPluginFile(id, "skillset.plugin.json"));
    }
    if (hasPackageJson) {
