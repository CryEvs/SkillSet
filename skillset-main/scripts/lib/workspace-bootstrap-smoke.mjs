...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "skillset-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "skillset.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".skillset", "workspace");
