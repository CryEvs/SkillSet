# SkillSet Installer for Windows (PowerShell)
# Usage: iwr -useb https://skillset.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://skillset.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\skillset",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 SkillSet Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one SkillSet.$NC" -Level info
    Write-Host ""
}
...
function Install-SkillSetNpm {
    param([string]$Target = "latest")

    $installSpec = Resolve-PackageInstallSpec -Target $Target
    
    Write-Host "Installing SkillSet ($installSpec)..." -Level info
    ...
    try {
        ...
        Write-Host "SkillSet installed" -Level success
        return $true
    } catch {
...
function Install-SkillSetGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing SkillSet from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/skillset/skillset.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\skillset\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\skillset.cmd" -Encoding ASCII -Force
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install SkillSet from git to $GitDir" -Level info
        } else {
            Install-SkillSetGit -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install SkillSet via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-SkillSetNpm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'skillset onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 SkillSet installed successfully!" -Level success
}
