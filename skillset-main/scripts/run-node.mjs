...
const extensionRestartMetadataFiles = new Set(["skillset.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.SKILLSET_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.SKILLSET_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.SKILLSET_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by SKILLSET_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by SKILLSET_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "SKILLSET_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "SKILLSET_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "SKILLSET_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "SKILLSET_RUN_NODE_BUILD_LOCK_STALE_MS";
...
const logRunner = (message, deps) => {
  if (deps.env.SKILLSET_RUNNER_LOG === "0") {
    return;
  }
  const line = `[skillset] ${message}\n`;
...
const runSkillSet = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["skillset.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[skillset] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const shouldSkipCleanWatchRuntimeSync = (deps) => deps.env.SKILLSET_WATCH_MODE === "1";
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.SKILLSET_BUILD_PRIVATE_QA = "1";
    deps.env.SKILLSET_ENABLE_PRIVATE_QA_CLI = "1";
  }
  ...
  try {
    ...
    if (!buildRequirement.shouldBuild) {
      ...
      exitCode = await runSkillSet(deps);
