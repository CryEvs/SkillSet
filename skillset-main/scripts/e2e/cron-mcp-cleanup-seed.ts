...
process.title = "skillset-cron-mcp-cleanup-probe";
...
async function main() {
  const stateDir = process.env.SKILLSET_STATE_DIR?.trim() || path.join(os.homedir(), ".skillset");
  const configPath =
    process.env.SKILLSET_CONFIG_PATH?.trim() || path.join(stateDir, "skillset.json");
