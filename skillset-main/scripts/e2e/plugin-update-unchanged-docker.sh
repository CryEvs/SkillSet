...
IMAGE_NAME="$(docker_e2e_resolve_image "skillset-plugin-update-e2e" SKILLSET_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${SKILLSET_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
...
mkdir -p \"\$HOME/.skillset/extensions/lossless-claw\"
cat > \"\$HOME/.skillset/extensions/lossless-claw/package.json\" <<'JSON'
{
  \"name\": \"@example/lossless-claw\",
  \"version\": \"0.9.0\"
}
JSON
cat > \"\$HOME/.skillset/skillset.json\" <<'JSON'
{
  \"plugins\": {
    \"installs\": {
      \"lossless-claw\": {
        ...
        \"installPath\": \"~/.skillset/extensions/lossless-claw\",
...
cat > /tmp/skillset-e2e-registry.mjs <<'NODE'
...
NODE
node /tmp/skillset-e2e-registry.mjs >/tmp/skillset-e2e-registry.log 2>&1 &
registry_pid=\$!
...
if [ \"\$registry_ready\" -ne 1 ]; then
  ...
  cat /tmp/skillset-e2e-registry.log || true
  exit 1
fi
...
before_hash=\$(node --input-type=module -e '
  ...
  const file = path.join(os.homedir(), \".skillset\", \"skillset.json\");
