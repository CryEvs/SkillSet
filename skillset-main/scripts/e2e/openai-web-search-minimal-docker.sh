...
IMAGE_NAME="$(docker_e2e_resolve_image "skillset-openai-web-search-minimal-e2e" SKILLSET_OPENAI_WEB_SEARCH_MINIMAL_E2E_IMAGE)"
SKIP_BUILD="${SKILLSET_OPENAI_WEB_SEARCH_MINIMAL_E2E_SKIP_BUILD:-0}"
PORT="18789"
MOCK_PORT="19191"
...
run_logged openai-web-search-minimal docker run --rm \
  -e "SKILLSET_GATEWAY_TOKEN=$TOKEN" \
  -e "OPENAI_API_KEY=sk-skillset-web-search-minimal-e2e" \
  -e "BRAVE_API_KEY=brave-skillset-web-search-minimal-e2e" \
  -e "PORT=$PORT" \
  -e "MOCK_PORT=$MOCK_PORT" \
  -i "$IMAGE_NAME" bash -s <<'EOF'
set -euo pipefail

export HOME="$(mktemp -d "/tmp/skillset-openai-web-search-minimal.XXXXXX")"
export SKILLSET_STATE_DIR="$HOME/.skillset"
export SKILLSET_SKIP_CHANNELS=1
export SKILLSET_SKIP_GMAIL_WATCHER=1
export SKILLSET_SKIP_CRON=1
export SKILLSET_SKIP_CANVAS_HOST=1

PORT="${PORT:?missing PORT}"
MOCK_PORT="${MOCK_PORT:?missing MOCK_PORT}"
TOKEN="${SKILLSET_GATEWAY_TOKEN:?missing SKILLSET_GATEWAY_TOKEN}"
SUCCESS_MARKER="SKILLSET_SCHEMA_E2E_OK"
RAW_SCHEMA_ERROR="400 The following tools cannot be used with reasoning.effort 'minimal': web_search."
MOCK_REQUEST_LOG="/tmp/skillset-openai-web-search-minimal-requests.jsonl"
GATEWAY_LOG="/tmp/skillset-openai-web-search-minimal-gateway.log"
mock_pid=""
gateway_pid=""
...
dump_debug_logs() {
  ...
  for file in \
    "$GATEWAY_LOG" \
    /tmp/skillset-openai-web-search-minimal-mock.log \
    /tmp/skillset-openai-web-search-minimal-client-success.log \
    /tmp/skillset-openai-web-search-minimal-client-reject.log \
    "$MOCK_REQUEST_LOG" \
    "$SKILLSET_STATE_DIR/skillset.json"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
cat >"$SKILLSET_STATE_DIR/skillset.json" <<JSON
{
  "agents": {
    "defaults": {
...
cat >/tmp/skillset-openai-web-search-minimal-mock.mjs <<'NODE'
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/v1/models") {
    writeJson(res, 200, {
      object: "list",
      data: [{ id: "gpt-5", object: "model", owned_by: "skillset-e2e" }],
    });
    return;
  }
...
node /tmp/skillset-openai-web-search-minimal-mock.mjs >/tmp/skillset-openai-web-search-minimal-mock.log 2>&1 &
mock_pid="$!"
...
cat >/tmp/skillset-openai-web-search-minimal-client.mjs <<'NODE'
import { execFileSync } from "node:child_process";

const entry = process.env.SKILLSET_ENTRY;
const port = process.env.PORT;
const token = process.env.SKILLSET_GATEWAY_TOKEN;
const mode = process.argv[2];
const message =
  mode === "reject"
    ? "FORCE_SCHEMA_REJECT"
    : "Return exactly SKILLSET_SCHEMA_E2E_OK.";
const id = mode === "reject" ? "schema-reject" : "schema-success";

if (!entry || !port || !token) throw new Error("missing SKILLSET_ENTRY/PORT/SKILLSET_GATEWAY_TOKEN");
...
while (Date.now() < deadline) {
  ...
  if (history.ok && JSON.stringify(history.value).includes("SKILLSET_SCHEMA_E2E_OK")) {
    process.exit(0);
  }
  await new Promise((resolve) => setTimeout(resolve, 250));
}
throw new Error("timed out waiting for SKILLSET_SCHEMA_E2E_OK in chat history");
NODE

SKILLSET_ENTRY="$entry" PORT="$PORT" SKILLSET_GATEWAY_TOKEN="$TOKEN" node /tmp/skillset-openai-web-search-minimal-client.mjs success >/tmp/skillset-openai-web-search-minimal-client-success.log 2>&1
...
const success = responseEntries.find((entry) => JSON.stringify(entry.body).includes("SKILLSET_SCHEMA_E2E_OK"));
if (!success) {
...
SKILLSET_ENTRY="$entry" PORT="$PORT" SKILLSET_GATEWAY_TOKEN="$TOKEN" node /tmp/skillset-openai-web-search-minimal-client.mjs reject >/tmp/skillset-openai-web-search-minimal-client-reject.log 2>&1
