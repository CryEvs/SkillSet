...
IMAGE_NAME="$(docker_e2e_resolve_image "skillset-mcp-channels-e2e" SKILLSET_IMAGE)"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="skillset-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t skillset-mcp-client-log.XXXXXX)"
...
docker run --rm \
  ...
  -e "SKILLSET_STATE_DIR=/tmp/skillset-state" \
  -e "SKILLSET_CONFIG_PATH=/tmp/skillset-state/skillset.json" \
  -e "GW_URL=ws://127.0.0.1:$PORT" \
  -e "GW_TOKEN=$TOKEN" \
  -e "SKILLSET_ALLOW_INSECURE_PRIVATE_WS=1" \
  "$IMAGE_NAME" \
