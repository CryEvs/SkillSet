...
IMAGE_NAME="$(docker_e2e_resolve_image "skillset-config-reload-e2e" SKILLSET_CONFIG_RELOAD_E2E_IMAGE)"
SKIP_BUILD="${SKILLSET_CONFIG_RELOAD_E2E_SKIP_BUILD:-0}"
PORT="18789"
TOKEN="reload-e2e-token"
CONTAINER_NAME="skillset-config-reload-e2e-$$"
...
docker run -d \
  ...
  -e SKILLSET_SKIP_GMAIL_WATCHER=1 \
  -e SKILLSET_SKIP_CRON=1 \
  -e SKILLSET_SKIP_CANVAS_HOST=1 \
  "$IMAGE_NAME" \
...
const configPath = path.join(os.homedir(), '.skillset', 'skillset.json');
