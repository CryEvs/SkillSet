...
UPDATE_TARGET=""
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/skillset-parallels-npm-update.XXXXXX)"
MAIN_TGZ_DIR="$(mktemp -d)"
MAIN_TGZ_PATH=""
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/skillset-parallels-build.lock"
...
PROGRESS_STALE_S=60
TIMEOUT_UPDATE_S="${SKILLSET_PARALLELS_NPM_UPDATE_TIMEOUT_S:-1200}"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: skillset@latest
  --update-target <target>    Target passed to guest 'skillset update --tag'.
                             Default: host-served tgz packed from current checkout.
                             Examples: latest, beta, 2026.4.10, http://host/skillset.tgz
...
resolve_latest_version() {
  npm view skillset version --userconfig "$(mktemp)"
}
...
pack_main_tgz() {
  ...
  [[ $rc -eq 0 ]] || return "$rc"
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/skillset-main-$CURRENT_HEAD_SHORT.tgz"
...
write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/skillset-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/skillset warnings do not
...
function Wait-GatewayRpcReady {
  param(
    [Parameter(Mandatory = $true)][string]$SkillSetPath,
    [int]$Attempts = 20,
    [int]$SleepSeconds = 3
  )
  ...
  for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
    ...
    try {
      $statusOutput = Invoke-CaptureLogged 'skillset gateway status' { & $SkillSetPath gateway status --deep --require-rpc }
...
function Stop-GatewayScheduledTaskIfPresent {
  ...
  try {
    $PSNativeCommandUseErrorActionPreference = $false
    schtasks /End /TN 'SkillSet Gateway' 2>$null | Out-Null
  } catch {
  } finally {
    $PSNativeCommandUseErrorActionPreference = $previousNativeErrorPreference
  }
...
function Stop-SkillSetUpdateProcesses {
...
function Invoke-SkillSetUpdateWithTimeout {
  param(
    [Parameter(Mandatory = $true)][string]$SkillSetPath,
    [Parameter(Mandatory = $true)][string]$UpdateTarget,
  ...
  } -ArgumentList $SkillSetPath, $UpdateTarget
  ...
  if ($null -ne $completed) {
    ...
    if ($result.ExitCode -ne 0) {
      throw "skillset update failed with exit code $($result.ExitCode)"
    }
    return
  }
  ...
  Remove-Job $updateJob -Force -ErrorAction SilentlyContinue
  Write-ProgressLog 'update.skillset-update.timeout'
  'skillset update timed out after package install window; killing stale update/completion processes and verifying installed version' | Tee-Object -FilePath $LogPath -Append | Out-Null
  Stop-SkillSetUpdateProcesses
}
...
function Start-GatewayRunFallback {
  ...
  Stop-SkillSetGatewayProcesses
  $entry = Join-Path $env:APPDATA 'npm\node_modules\skillset\dist\index.js'
  if (-not (Test-Path $entry)) {
    throw "skillset dist entry missing: $entry"
  }
  $node = (Get-Command node.exe -ErrorAction Stop).Source
  $stdout = Join-Path $env:TEMP 'skillset-parallels-npm-update-gateway.log'
  $stderr = Join-Path $env:TEMP 'skillset-parallels-npm-update-gateway.err.log'
  ...
  if (-not (Wait-GatewayRpcReady -SkillSetPath $SkillSetPath -Attempts 20 -SleepSeconds 3)) {
    if (Test-Path $stdout) {
...
function Complete-WorkspaceSetup {
  $workspace = $env:SKILLSET_WORKSPACE_DIR
  if (-not $workspace) {
    $workspace = Join-Path $env:USERPROFILE '.skillset\workspace'
  }
  $stateDir = Join-Path $workspace '.skillset'
...
function Restart-GatewayWithRecovery {
  ...
  if ($null -ne $restartCompleted) {
    ...
    if ($restartResult.ExitCode -ne 0) {
      ...
      "skillset gateway restart failed with exit code $($restartResult.ExitCode)" | Tee-Object -FilePath $LogPath -Append | Out-Null
    }
  } else {
    $restartFailed = $true
    Stop-Job $restartJob -ErrorAction SilentlyContinue
    Write-ProgressLog 'update.restart-gateway.timeout'
    'skillset gateway restart timed out after 20s; continuing to RPC readiness checks' | Tee-Object -FilePath $LogPath -Append | Out-Null
  }
  Remove-Job $restartJob -Force -ErrorAction SilentlyContinue

  Write-ProgressLog 'update.gateway-status'
  if (Wait-GatewayRpcReady -SkillSetPath $SkillSetPath) {
    return
  }
  Write-ProgressLog 'update.gateway-start-recover'
  Stop-SkillSetGatewayProcesses
  Invoke-Logged 'skillset gateway start' { & $SkillSetPath gateway start }
  Write-ProgressLog 'update.gateway-status-recover'
  if (-not (Wait-GatewayRpcReady -SkillSetPath $SkillSetPath)) {
    Start-GatewayRunFallback -SkillSetPath $SkillSetPath
  }
}

try {
  $env:PATH = "$env:LOCALAPPDATA\SkillSet\deps\portable-git\cmd;$env:LOCALAPPDATA\SkillSet\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\SkillSet\deps\portable-git\usr\bin;$env:PATH"
  ...
  $skillset = Join-Path $env:APPDATA 'npm\skillset.cmd'
  Stop-SkillSetGatewayProcesses
  Write-ProgressLog 'update.skillset-update'
  Invoke-SkillSetUpdateWithTimeout -SkillSetPath $skillset -UpdateTarget $UpdateTarget
  Write-ProgressLog 'update.verify-version'
  $version = Invoke-CaptureLogged 'skillset --version' { & $skillset --version }
  ...
  Invoke-Logged 'skillset update status' { & $skillset update status --json }
  Write-ProgressLog 'update.set-model'
  Invoke-Logged 'skillset models set' { & $skillset models set $ModelId }
  ...
  Restart-GatewayWithRecovery -SkillSetPath $skillset
  Complete-WorkspaceSetup
  Write-ProgressLog 'update.agent-turn'
  Invoke-CaptureLogged 'skillset agent' { & $skillset agent --local --agent main --session-id $SessionId --message 'Reply with exact ASCII text OK only.' --json } | Out-Null
  $exitCode = $LASTEXITCODE
  if ($null -eq $exitCode) {
    $exitCode = 0
  }
...
start_server() {
  ...
  ) >/tmp/skillset-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_macos_update_after_transport_loss() {
  ...
  local script_path="/tmp/skillset-npm-update-macos-recover.sh"
...
busy="\$(/bin/ps -axo command | /usr/bin/egrep 'skillset update|npm install|pnpm install|pnpm run build' | /usr/bin/egrep -v 'egrep|skillset-npm-update-macos-recover' || true)"
if [ -n "\$busy" ]; then
  printf 'update still has active npm/pnpm/skillset processes\n%s\n' "\$busy" >&2
  exit 1
fi
version="\$(/opt/homebrew/bin/skillset --version)"
printf '%s\n' "\$version"
...
/opt/homebrew/bin/skillset gateway status --deep --require-rpc >/dev/null 2>&1 || /opt/homebrew/bin/skillset gateway restart || true
gateway_ready=0
for _ in 1 2 3 4 5 6; do
  if /opt/homebrew/bin/skillset gateway status --deep --require-rpc; then
    gateway_ready=1
    break
  fi
  sleep 2
...
if [ "\$gateway_ready" != "1" ]; then
  /opt/homebrew/bin/skillset gateway start || true
  for _ in 1 2 3 4 5 6; do
    if /opt/homebrew/bin/skillset gateway status --deep --require-rpc; then
      gateway_ready=1
      break
...
workspace="\${SKILLSET_WORKSPACE_DIR:-\$HOME/.skillset/workspace}"
mkdir -p "\$workspace/.skillset"
...
- Purpose: Parallels npm update smoke test assistant.
IDENTITY_EOF
cat > "\$workspace/.skillset/workspace-state.json" <<'STATE_EOF'
{
  "version": 1,
  "setupCompletedAt": "2026-01-01T00:00:00.000Z"
}
...
/opt/homebrew/bin/skillset models set "$MODEL_ID"
/opt/homebrew/bin/skillset agent --agent main --session-id "parallels-npm-update-macos-transport-recovery-$expected_needle" --message "Reply with exact ASCII text OK only." --json
EOF
...
\$ErrorActionPreference = 'Stop'
\$skillset = Join-Path \$env:APPDATA 'npm\\skillset.cmd'
if (-not (Test-Path \$skillset)) {
  throw "skillset shim missing: \$skillset"
}
\$busy = Get-CimInstance Win32_Process |
  Where-Object {
    \$_.CommandLine -and
    (\$_.CommandLine -match 'skillset update|npm install|pnpm install|pnpm run build')
  }
if (\$busy) {
  throw 'update still has active npm/pnpm/skillset processes'
}
...
function Stop-GatewayListeners {
  ...
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object {
      \$_.CommandLine -and (
        \$_.CommandLine -match 'skillset.*gateway --port 18789' -or
        \$_.CommandLine -match 'skillset.*gateway run' -or
...
\$gatewayReady = \$false
for (\$i = 0; \$i -lt 6; \$i++) {
  if (Test-GatewayWritable \$skillset) {
    \$gatewayReady = \$true
    break
  }
...
if (-not \$gatewayReady) {
  ...
  \$entry = Join-Path \$env:APPDATA 'npm\\node_modules\\skillset\\dist\\index.js'
  \$node = (Get-Command node.exe -ErrorAction Stop).Source
  \$stdout = Join-Path \$env:TEMP 'skillset-parallels-npm-update-recover-gateway.log'
  \$stderr = Join-Path \$env:TEMP 'skillset-parallels-npm-update-recover-gateway.err.log'
  Start-Process -FilePath \$node -ArgumentList @(\$entry, 'gateway', 'run', '--bind', 'loopback', '--port', '18789', '--force') -WindowStyle Hidden -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr | Out-Null
  for (\$i = 0; \$i -lt 20; \$i++) {
    if (Test-GatewayWritable \$skillset) {
      \$gatewayReady = \$true
...
'@ | Set-Content -Path (Join-Path \$stateDir 'workspace-state.json') -Encoding UTF8
Remove-Item (Join-Path \$workspace 'BOOTSTRAP.md') -Force -ErrorAction SilentlyContinue
& \$skillset agent --local --agent main --session-id 'parallels-npm-update-windows-transport-recovery-$expected_needle' --message 'Reply with exact ASCII text OK only.' --json
EOF
  )"
  local rc=$?
  set -e
...
text = pathlib.Path(sys.argv[1]).read_text(encoding="utf-8", errors="replace")
matches = re.findall(r"SkillSet [^\r\n]+", text)
matches = [match for match in matches if re.search(r"SkillSet \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="skillset-update-$RANDOM-$RANDOM.ps1"
  log_name="skillset-update-$RANDOM-$RANDOM.log"
  done_name="skillset-update-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/skillset-update-log-state.XXXXXX")"
  : >"$log_state_path"
  provider_key_b64="$(
...
run_macos_update() {
  ...
  cat <<EOF | prlctl exec "$MACOS_VM" /usr/bin/tee /tmp/skillset-main-update.sh >/dev/null
set -euo pipefail
export PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin
...
cd "\$HOME"
stop_skillset_gateway_processes() {
  /opt/homebrew/bin/skillset gateway stop >/dev/null 2>&1 || true
  /usr/bin/pkill -9 -f skillset-gateway || true
  /usr/bin/pkill -9 -f 'skillset gateway run' || true
  /usr/bin/pkill -9 -f 'skillset.mjs gateway' || true
...
stop_skillset_gateway_processes
/opt/homebrew/bin/skillset update --tag "$update_target" --yes --json
...
/opt/homebrew/bin/skillset update status --json
/opt/homebrew/bin/skillset models set "$MODEL_ID"
...
/opt/homebrew/bin/skillset gateway restart || true
gateway_ready=0
for _ in 1 2 3 4 5 6 7 8; do
  if /opt/homebrew/bin/skillset gateway status --deep --require-rpc >/dev/null 2>&1; then
    gateway_ready=1
    break
  fi
  sleep 2
...
if [ "\$gateway_ready" != "1" ]; then
  stop_skillset_gateway_processes
  /opt/homebrew/bin/skillset gateway run --bind loopback --port 18789 --force >/tmp/skillset-parallels-npm-update-macos-gateway.log 2>&1 </dev/null &
  for _ in 1 2 3 4 5 6 7 8; do
    if /opt/homebrew/bin/skillset gateway status --deep --require-rpc >/dev/null 2>&1; then
      gateway_ready=1
      break
...
if [ "\$gateway_ready" != "1" ]; then
  tail -n 120 /tmp/skillset-parallels-npm-update-macos-gateway.log 2>/dev/null || true
fi
/opt/homebrew/bin/skillset gateway status --deep --require-rpc
workspace="\${SKILLSET_WORKSPACE_DIR:-\$HOME/.skillset/workspace}"
mkdir -p "\$workspace/.skillset"
cat > "\$workspace/IDENTITY.md" <<'IDENTITY_EOF'
# Identity
...
STATE_EOF
rm -f "\$workspace/BOOTSTRAP.md"
/opt/homebrew/bin/skillset agent --agent main --session-id parallels-npm-update-macos-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  macos_desktop_user_exec /bin/bash /tmp/skillset-main-update.sh
}
...
run_linux_update() {
  local update_target="$1"
  local expected_needle="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/skillset-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
stop_skillset_gateway_processes() {
  skillset gateway stop >/dev/null 2>&1 || true
  pkill -9 -f skillset-gateway || true
  pkill -9 -f 'skillset gateway run' || true
  pkill -9 -f 'skillset.mjs gateway' || true
...
stop_skillset_gateway_processes
skillset update --tag "$update_target" --yes --json
...
STATE_EOF
rm -f "\$workspace/BOOTSTRAP.md"
skillset agent --local --agent main --session-id parallels-npm-update-linux-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/skillset-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="skillset@$LATEST_VERSION"
fi
resolve_current_head
...
say "Run same-guest skillset update to $UPDATE_TARGET_EFFECTIVE"
