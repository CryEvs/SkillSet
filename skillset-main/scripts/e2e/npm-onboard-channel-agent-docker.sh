...
IMAGE_NAME="$(docker_e2e_resolve_image "skillset-npm-onboard-channel-agent-e2e" SKILLSET_NPM_ONBOARD_E2E_IMAGE)"
DOCKER_TARGET="${SKILLSET_NPM_ONBOARD_DOCKER_TARGET:-e2e-runner}"
HOST_BUILD="${SKILLSET_NPM_ONBOARD_HOST_BUILD:-1}"
PACKAGE_TGZ="${SKILLSET_NPM_ONBOARD_PACKAGE_TGZ:-}"
CHANNEL="${SKILLSET_NPM_ONBOARD_CHANNEL:-telegram}"

case "$CHANNEL" in
  telegram | discord) ;;
  *)
    echo "SKILLSET_NPM_ONBOARD_CHANNEL must be telegram or discord, got: $CHANNEL" >&2
    exit 1
    ;;
esac
...
prepare_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "SKILLSET_NPM_ONBOARD_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ "$HOST_BUILD" != "0" ]; then
    ...
    echo "Skipping host build (SKILLSET_NPM_ONBOARD_HOST_BUILD=0)"
  fi
  ...
  local pack_dir
  pack_dir="$(mktemp -d "${TMPDIR:-/tmp}/skillset-npm-onboard-pack.XXXXXX")"
...
run_log="$(mktemp "${TMPDIR:-/tmp}/skillset-npm-onboard-channel-agent.XXXXXX.log")"

echo "Running npm tarball onboard/channel/agent Docker E2E ($CHANNEL)..."
if ! docker run --rm \
  -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 \
  -e SKILLSET_NPM_ONBOARD_CHANNEL="$CHANNEL" \
  -e SKILLSET_CURRENT_PACKAGE_TGZ="$DOCKER_PACKAGE_TGZ" \
  -v "$PACKAGE_TGZ:$DOCKER_PACKAGE_TGZ:ro" \
  -i "$IMAGE_NAME" bash -s >"$run_log" 2>&1 <<'EOF'
set -euo pipefail

export HOME="$(mktemp -d "/tmp/skillset-npm-onboard.XXXXXX")"
export NPM_CONFIG_PREFIX="$HOME/.npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export OPENAI_API_KEY="sk-skillset-npm-onboard-e2e"
export SKILLSET_GATEWAY_TOKEN="npm-onboard-channel-agent-token"

CHANNEL="${SKILLSET_NPM_ONBOARD_CHANNEL:?missing SKILLSET_NPM_ONBOARD_CHANNEL}"
PORT="18789"
MOCK_PORT="44080"
SUCCESS_MARKER="SKILLSET_AGENT_E2E_OK_ASSISTANT"
MOCK_REQUEST_LOG="/tmp/skillset-mock-openai-requests.jsonl"
mock_pid=""

case "$CHANNEL" in
  telegram)
    CHANNEL_TOKEN="123456:skillset-npm-onboard-token"
    DEP_SENTINEL="grammy"
    ;;
  discord)
    CHANNEL_TOKEN="skillset-npm-onboard-discord-token"
...
dump_debug_logs() {
  ...
  for file in \
    /tmp/skillset-install.log \
    /tmp/skillset-onboard.json \
    /tmp/skillset-channel-add.log \
    /tmp/skillset-doctor.log \
    /tmp/skillset-agent.combined \
    /tmp/skillset-agent.err \
    /tmp/skillset-agent.json \
    /tmp/skillset-mock-openai.log \
    "$MOCK_REQUEST_LOG"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
assert_dep_absent() {
  local sentinel="$1"
  if find "$package_root" "$HOME/.skillset" -path "*/node_modules/$sentinel/package.json" -print -quit 2>/dev/null | grep -q .; then
    echo "$sentinel should not be installed before channel activation repair" >&2
    find "$package_root" "$HOME/.skillset" -path "*/node_modules/$sentinel/package.json" -print 2>/dev/null >&2 || true
    exit 1
  fi
}
...
assert_dep_present() {
  ...
  if ! find "$package_root" "$HOME/.skillset" -path "*/node_modules/$sentinel/package.json" -print -quit 2>/dev/null | grep -q .; then
    ...
    find "$package_root" "$HOME/.skillset" -maxdepth 6 -type d -name node_modules -print 2>/dev/null >&2 || true
    exit 1
  fi
}

cat >/tmp/skillset-mock-openai.mjs <<'NODE'
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/v1/models") {
    writeJson(res, 200, {
      object: "list",
      data: [{ id: "gpt-5.4", object: "model", owned_by: "skillset-e2e" }],
    });
    return;
  }
...
MOCK_PORT="$MOCK_PORT" SUCCESS_MARKER="$SUCCESS_MARKER" MOCK_REQUEST_LOG="$MOCK_REQUEST_LOG" node /tmp/skillset-mock-openai.mjs >/tmp/skillset-mock-openai.log 2>&1 &
mock_pid="$!"
...
skillset onboard --non-interactive --accept-risk \
  --mode local \
  ...
  --skip-skills \
  --skip-health \
  --json >/tmp/skillset-onboard.json
...
if (!fs.existsSync(configPath)) {
  throw new Error("onboard did not write skillset.json");
}
...
if (authRaw.includes("sk-skillset-npm-onboard-e2e")) {
...
skillset channels add --channel "$CHANNEL" --token "$CHANNEL_TOKEN" >/tmp/skillset-channel-add.log 2>&1
node - "$CHANNEL" "$CHANNEL_TOKEN" <<'NODE'
const fs = require("node:fs");
const path = require("node:path");
const channel = process.argv[2];
const token = process.argv[3];
const cfg = JSON.parse(fs.readFileSync(path.join(process.env.HOME, ".skillset", "skillset.json"), "utf8"));
...
skillset doctor --non-interactive >/tmp/skillset-doctor.log 2>&1
assert_dep_present "$DEP_SENTINEL"

echo "Running local agent turn against mocked OpenAI..."
skillset agent --local \
  --agent main \
  --session-id npm-onboard-channel-agent \
  --message "Return the success marker from the test server." \
  --thinking off \
  --json >/tmp/skillset-agent.combined 2>&1

node - "$SUCCESS_MARKER" "$MOCK_REQUEST_LOG" <<'NODE'
const fs = require("node:fs");
const marker = process.argv[2];
const logPath = process.argv[3];
const output = fs.readFileSync("/tmp/skillset-agent.combined", "utf8");
