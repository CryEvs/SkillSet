...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/skillset-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...process.env,
      SKILLSET_ALLOW_INSECURE_PRIVATE_WS: "1",
      SKILLSET_STATE_DIR: "/tmp/skillset-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[skillset mcp] ${String(chunk)}`);
  });
