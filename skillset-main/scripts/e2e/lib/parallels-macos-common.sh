...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/skillset-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/skillset-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${SKILLSET_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${SKILLSET_SECRET_ENV_NAME:-}" ] && [ -n "${SKILLSET_SECRET_FILE:-}" ] && [ -f "$SKILLSET_SECRET_FILE" ]; then
  secret_value="$(cat "$SKILLSET_SECRET_FILE")"
  rm -f "$SKILLSET_SECRET_FILE"
  export "${SKILLSET_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "SKILLSET_SECRET_ENV_NAME=$api_key_env" \
      "SKILLSET_SECRET_FILE=$secret_path" \
      "SKILLSET_WRAPPER_FILE=$wrapper_path" \
