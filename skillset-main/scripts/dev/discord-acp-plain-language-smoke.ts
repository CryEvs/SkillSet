...
type DriverMode = "token" | "webhook" | "skillset";
...
function resolveStateDir(): string {
  const override = process.env.SKILLSET_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.SKILLSET_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".skillset");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver skillset] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) SkillSet spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|skillset> Driver transport mode (default: token)\n" +
    ...
    "  --skillset-bin <path>        SkillSet CLI binary for driver=skillset (default: skillset)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  SKILLSET_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  SKILLSET_DISCORD_SMOKE_DRIVER\n" +
    "  SKILLSET_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  SKILLSET_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  SKILLSET_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  SKILLSET_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  SKILLSET_DISCORD_SMOKE_AGENT\n" +
    "  SKILLSET_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  SKILLSET_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  SKILLSET_DISCORD_SMOKE_POLL_MS\n" +
    "  SKILLSET_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  SKILLSET_DISCORD_SMOKE_SKILLSET_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.SKILLSET_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.SKILLSET_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "skillset"
        ? "skillset"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.SKILLSET_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.SKILLSET_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.SKILLSET_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.SKILLSET_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.SKILLSET_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.SKILLSET_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.SKILLSET_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.SKILLSET_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.SKILLSET_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.SKILLSET_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const skillsetBin =
    resolveArg("--skillset-bin") || process.env.SKILLSET_DISCORD_SMOKE_SKILLSET_BIN || "skillset";
...
async function skillsetCliJson<T>(params: { skillsetBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.skillsetBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`skillset ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await skillsetCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    skillsetBin: params.skillsetBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "skillset") {
    return await readMessagesWithOpenclaw({
      skillsetBin: params.args.skillsetBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `skillset-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await skillsetCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("skillset message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from SkillSet.`,
        diagnostics: {
          bindingCandidates: [
            {
