...
This document describes how the SkillSet ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`skillset acp` exposes an ACP agent over stdio and forwards prompts to a running
SkillSet Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing SkillSet Gateway
...
- ACP client spawns `skillset acp` and speaks ACP messages over stdio.
...
`skillset acp` resolves the Gateway URL and auth from CLI flags or config:
