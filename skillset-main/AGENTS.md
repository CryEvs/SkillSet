...
- Repo: `https://github.com/skillset/skillset`
...
- Docs: `docs/`
- Apps: `apps/`, `Swabble/`
- Installers served from `skillset.ai`: sibling `../skillset.ai`
...
- Extensions cross into core only via `skillset/plugin-sdk/*`, manifest metadata, injected runtime helpers, and documented local barrels (`api.ts`, `runtime-api.ts`).
...
- Dev CLI: `pnpm skillset ...` or `pnpm dev`.
- Build: `pnpm build`
...
- Shard timing artifact: `.artifacts/vitest-shard-timings.json`; auto-used for balanced shard ordering. Disable with `SKILLSET_TEST_PROJECTS_TIMINGS=0`.
...
- Local heavy-check behavior: `SKILLSET_LOCAL_CHECK=1` default; `SKILLSET_LOCAL_CHECK_MODE=throttled|full`; `SKILLSET_LOCAL_CHECK=0` for CI/shared runs.
- Local validation is local-first. Do not default to Blacksmith/Testbox for routine SkillSet iteration; it burns warm caches and startup time. Use repo `pnpm` lanes first, then reach for remote CI/Testbox only for parity-only failures, secrets/services, or when explicitly requested.
...
- Product naming: **SkillSet** product/docs; `skillset` CLI/package/path/config.
...
- Avoid broad `importOriginal()` / broad `skillset/plugin-sdk/*` partial mocks in hot tests. Add narrow local `*.runtime.ts` seam and mock it.
...
- Do not set test workers above 16. For memory pressure: `SKILLSET_VITEST_MAX_WORKERS=1 pnpm test`.
- Live: `SKILLSET_LIVE_TEST=1 pnpm test:live`; full logs `SKILLSET_LIVE_TEST_QUIET=0`.
...
- PR/issue workflows: use `$skillset-pr-maintainer`.
- `/landpr`: use `~/.codex/prompts/landpr.md`.

## Security / Release

- Never commit real phone numbers, videos, credentials, live config.
- Secrets: channel/provider credentials under `~/.skillset/credentials/`; model auth profiles under `~/.skillset/agents/<agentId>/agent/auth-profiles.json`.
...
- Release docs: `docs/reference/RELEASING.md`; use `$skillset-release-maintainer`.
- GHSA/advisories: use `$skillset-ghsa-maintainer`.
...
- mac gateway: use app or `skillset gateway restart/status --deep`; avoid ad-hoc tmux gateway sessions. Rebuild mac app locally, not over SSH.
...
- Mobile LAN pairing: plaintext `ws://` is loopback-only by default. Trusted private-network `ws://` needs `SKILLSET_ALLOW_INSECURE_PRIVATE_WS=1`; Tailscale/public use `wss://` or a tunnel.
...
- Parallels smoke: `$skillset-parallels-smoke`; Discord roundtrip: `parallels-discord-roundtrip`.

## Misc Footguns

- Rebrand/migration/config warnings: run `skillset doctor`.
