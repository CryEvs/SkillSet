          ...
          actions:
            ...
            - call: webEvaluate
              ...
              args:
                - pageId:
                    ref: uiAckPageId
                  expression: "(() => { const app = document.querySelector('skillset-app'); return app ? { sessionKey: app.sessionKey, settingsSessionKey: app.settings?.sessionKey, lastActiveSessionKey: app.settings?.lastActiveSessionKey, chatMessages: Array.isArray(app.chatMessages) ? app.chatMessages.length : null, chatLoading: app.chatLoading, lastError: app.lastError, connected: app.connected } : null; })()"
                  timeoutMs:
                    expr: liveTurnTimeoutMs(env, 15000)
            - throw:
