...
coverage:
  ...
  secondary:
    - models.openai
objective: Verify a live OpenAI GPT model can use OpenAI native web_search when SkillSet web search is enabled in auto mode.
successCriteria:
...
execution:
  kind: flow
  summary: Run with `SKILLSET_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" pnpm skillset qa suite --provider-mode live-frontier --model openai/gpt-5.4 --alt-model openai/gpt-5.4 --scenario openai-native-web-search-live`.
  config:
...
steps:
  - name: confirms live OpenAI GPT-5.4 web search auto mode
    actions:
      ...
      - assert:
          expr: "env.providerMode !== 'live-frontier' || Boolean(env.gateway.runtimeEnv.OPENAI_API_KEY?.trim() || env.gateway.runtimeEnv.SKILLSET_LIVE_OPENAI_KEY?.trim())"
          message: expected OPENAI_API_KEY or SKILLSET_LIVE_OPENAI_KEY for live OpenAI QA
