...
For SkillSet specifically, normal local iteration should stay local:
...
Only use Testbox in SkillSet when the user explicitly wants CI-parity or the
...
## SkillSet full test suite

For SkillSet, use the repo package manager and the measured stable full-suite
...
shard to one worker to avoid worker OOMs on Testbox:
    ...
    blacksmith testbox run --id <ID> "env NODE_OPTIONS=--max-old-space-size=4096 SKILLSET_TEST_PROJECTS_PARALLEL=6 SKILLSET_VITEST_MAX_WORKERS=1 pnpm test"
