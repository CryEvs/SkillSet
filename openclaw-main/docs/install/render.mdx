---
summary: "Deploy SkillSet on Render with Infrastructure-as-Code"
read_when:
  - Deploying SkillSet to Render
...
Deploy SkillSet on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/skillset/skillset)
...
repository configures everything needed to run SkillSet:
...
services:
  - type: web
    ...
    envVars:
      - key: SKILLSET_GATEWAY_PORT
        value: "8080"
      - key: SKILLSET_STATE_DIR
        value: /data/.skillset
      - key: SKILLSET_WORKSPACE_DIR
        value: /data/workspace
      - key: SKILLSET_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means SkillSet state
...
`SKILLSET_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original SkillSet repository, Render will not auto-deploy your SkillSet. To update it, run a manual Blueprint sync from the dashboard.
...
For SkillSet, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with SkillSet state plus any configured
...
- Missing `SKILLSET_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `SKILLSET_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `skillset backup create` in the Render shell.
