---
summary: "Run SkillSet Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want SkillSet running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# SkillSet on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent SkillSet Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "SkillSet 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the SkillSet Gateway in Docker
- Persist `~/.skillset` + `~/.skillset/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.skillset` state includes `skillset.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-skillset-project --name="SkillSet Gateway"
    gcloud config set project my-skillset-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create skillset-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh skillset-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the SkillSet repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    SKILLSET_IMAGE=skillset:latest
    SKILLSET_GATEWAY_TOKEN=
    SKILLSET_GATEWAY_BIND=lan
    SKILLSET_GATEWAY_PORT=18789

    SKILLSET_CONFIG_DIR=/home/$USER/.skillset
    SKILLSET_WORKSPACE_DIR=/home/$USER/.skillset/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.skillset
    ```

    Leave `SKILLSET_GATEWAY_TOKEN` blank unless you explicitly want to
    manage it through `.env`; SkillSet writes a random gateway token to
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `SKILLSET_GATEWAY_TOKEN`.
    ...
    services:
      skillset-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - SKILLSET_GATEWAY_BIND=${SKILLSET_GATEWAY_BIND}
          - SKILLSET_GATEWAY_PORT=${SKILLSET_GATEWAY_PORT}
          - SKILLSET_GATEWAY_TOKEN=${SKILLSET_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${SKILLSET_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`SKILLSET_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm skillset-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh skillset-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop skillset-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type skillset-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start skillset-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create skillset-deploy \
     --display-name="SkillSet Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-skillset-project \
     --member="serviceAccount:skillset-deploy@my-skillset-project.iam.gserviceaccount.com" \
