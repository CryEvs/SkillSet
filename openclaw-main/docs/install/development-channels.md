...
SkillSet ships three update channels:
...
```bash
skillset update --channel stable
...
- **`dev`**: ensures a git checkout (default `~/skillset`, override with
  `SKILLSET_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
skillset update --tag 2026.4.1-beta.1
...
skillset update --tag skillset@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `skillset update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  SkillSet prompts for confirmation (skip with `--yes`).
...
Preview what `skillset update` would do without making changes:

```bash
skillset update --dry-run
skillset update --channel beta --dry-run
skillset update --tag 2026.4.1-beta.1 --dry-run
skillset update --dry-run --json
```
...
When you switch channels with `skillset update`, SkillSet also syncs plugin
sources:
