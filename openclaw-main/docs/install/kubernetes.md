---
summary: "Deploy SkillSet Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run SkillSet on a Kubernetes cluster
  - You want to test SkillSet in a Kubernetes environment
title: "Kubernetes"
---

# SkillSet on Kubernetes

A minimal starting point for running SkillSet on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

SkillSet is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/skillset 18789:18789 -n skillset
...
```bash
kubectl get secret skillset-secrets -n skillset -o jsonpath='{.data.SKILLSET_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: skillset (configurable via SKILLSET_NAMESPACE)
├── Deployment/skillset        # Single pod, init container + gateway
├── Service/skillset           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/skillset-config  # skillset.json + AGENTS.md
└── Secret/skillset-secrets    # Gateway token + API keys
```
...
Edit `skillset.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret skillset-secrets -n skillset \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/skillset -n skillset
```

### Custom namespace

```bash
SKILLSET_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/skillset/skillset:latest # or pin to a specific version from https://github.com/skillset/skillset/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # skillset.json + AGENTS.md
