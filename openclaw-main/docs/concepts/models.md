...
SkillSet selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models SkillSet can use (plus aliases).
...
`skillset config set` protects model/provider maps from accidental clobbers. A
...
Interactive provider setup and `skillset configure --section model` also merge
...
- If you omit the provider, SkillSet resolves the input in this order:
  1. alias match
  ...
  3. deprecated fallback to the configured default provider
     If that provider no longer exposes the configured default model, SkillSet
...
skillset models set <provider/model>
skillset models set-image <provider/model>

skillset models aliases list
skillset models aliases add <alias> <provider/model>
skillset models aliases remove <alias>

skillset models fallbacks list
skillset models fallbacks add <provider/model>
skillset models fallbacks remove <provider/model>
skillset models fallbacks clear

skillset models image-fallbacks list
skillset models image-fallbacks add <provider/model>
skillset models image-fallbacks remove <provider/model>
skillset models image-fallbacks clear
```

`skillset models` (no subcommand) is a shortcut for `models status`.
...
`skillset models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.skillset/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: SkillSet writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever SkillSet regenerates `models.json`, including command-driven paths like `skillset agent`.
