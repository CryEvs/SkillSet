...
Paste this into `skillset.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, SkillSet can show:
...
`config.promptOverride` replaces the default Active Memory prompt. SkillSet
...
5. Verify memory search itself works with `skillset memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, SkillSet auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `skillset memory status --deep` to inspect the memory-search backend
      and index health.
