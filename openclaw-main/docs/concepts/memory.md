...
summary: "How SkillSet remembers things across sessions"
read_when:
...
SkillSet remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.skillset/workspace`).
...
<Info>
SkillSet auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, SkillSet
...
```bash
skillset memory rem-backfill --rollback
skillset memory rem-backfill --rollback-short-term
```

## CLI

```bash
skillset memory status          # Check index status and provider
skillset memory search "query"  # Search from the command line
skillset memory index --force   # Rebuild the index
```
