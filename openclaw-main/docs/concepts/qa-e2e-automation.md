...
The private QA stack is meant to exercise SkillSet in a more realistic,
...
```bash
pnpm skillset qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`SKILLSET_RUN_NODE_OUTPUT_LOG=<path>` to a repo-local log file.
...
disposable server. It requires `SKILLSET_QA_TELEGRAM_GROUP_ID`,
`SKILLSET_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and
`SKILLSET_QA_TELEGRAM_SUT_BOT_TOKEN`, plus two distinct bots in the same
...
```bash
pnpm skillset qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds SkillSet
...
- `mock-openai` is the scenario-aware SkillSet mock. It remains the default
