...
[Honcho](https://honcho.dev) adds AI-native memory to SkillSet. It persists
...
```bash
skillset plugins install @honcho-ai/skillset-honcho
skillset honcho setup
...
Settings live under `plugins.entries["skillset-honcho"].config`:
    ...
    entries: {
      "skillset-honcho": {
        config: {
          ...
          workspaceId: "skillset", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `skillset honcho setup` detects and
...
```bash
skillset honcho setup                        # Configure API key and migrate files
skillset honcho status                       # Check connection status
skillset honcho ask <question>               # Query Honcho about the user
skillset honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/skillset-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho SkillSet integration guide](https://docs.honcho.dev/v3/guides/integrations/skillset)
- [Memory](/concepts/memory) -- SkillSet memory overview
