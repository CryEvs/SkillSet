...
- `skillset tasks list` shows all tasks; `skillset tasks audit` surfaces issues.
...
| CLI operations         | `cli`        | `skillset agent` commands that run through the gateway | `silent`              |
...
| `cancelled` | Stopped by the operator via `skillset tasks cancel`                        |
...
When a task reaches a terminal state, SkillSet notifies you. There are two delivery paths:

**Direct delivery** — if the task has a channel target (the `requesterOrigin`), the completion message goes straight to that channel (Telegram, Discord, Slack, etc.). For subagent completions, SkillSet also preserves bound thread/topic routing when available and can fill a missing `to` / account from the requester session's stored route (`lastChannel` / `lastTo` / `lastAccountId`) before giving up on direct delivery.
...
```bash
skillset tasks list [--runtime <acp|subagent|cron|cli>] [--status <status>] [--json]
```
...
```bash
skillset tasks audit [--json]
```

Surfaces operational issues. Findings also appear in `skillset status` when issues are detected.
...
```bash
skillset tasks maintenance [--json]
...
```bash
skillset tasks flow list [--status <status>] [--json]
...
For the full operator ledger, use the CLI: `skillset tasks list`.

## Status integration (task pressure)

`skillset status` includes an at-a-glance task summary:
...
```
$SKILLSET_STATE_DIR/tasks/runs.sqlite
```
...
[Task Flow](/automation/taskflow) is the flow orchestration layer above background tasks. A single flow may coordinate multiple tasks over its lifetime using managed or mirrored sync modes. Use `skillset tasks` to inspect individual task records and `skillset tasks flow` to inspect the orchestrating flow.

See [Task Flow](/automation/taskflow) for details.

### Tasks and cron

A cron job **definition** lives in `~/.skillset/cron/jobs.json`; runtime execution state lives beside it in `~/.skillset/cron/jobs-state.json`. **Every** cron execution creates a task record — both main-session and isolated. Main-session cron tasks default to `silent` notify policy so they track without generating notifications.
