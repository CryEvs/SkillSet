...
Hooks are small scripts that run when something happens inside the Gateway. They can be discovered from directories and inspected with `skillset hooks`. The Gateway loads internal hooks only after you enable hooks or configure at least one hook entry, hook pack, legacy handler, or extra hook directory.

There are two kinds of hooks in SkillSet:

- **Internal hooks** (this page): run inside the Gateway when agent events fire, like `/new`, `/reset`, `/stop`, or lifecycle events.
- **Webhooks**: external HTTP endpoints that let other systems trigger work in SkillSet. See [Webhooks](/automation/cron-jobs#webhooks).

Hooks can also be bundled inside plugins. `skillset hooks list` shows both standalone hooks and plugin-managed hooks.
...
metadata:
  { "skillset": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
**Metadata fields** (`metadata.skillset`):
...
1. **Bundled hooks**: shipped with SkillSet
2. **Plugin hooks**: hooks bundled inside installed plugins
3. **Managed hooks**: `~/.skillset/hooks/` (user-installed, shared across workspaces). Extra directories from `hooks.internal.load.extraDirs` share this precedence.
...
The Gateway skips internal hook discovery on startup until internal hooks are configured. Enable a bundled or managed hook with `skillset hooks enable <name>`, install a hook pack, or set `hooks.internal.enabled=true` to opt in. When you enable one named hook, the Gateway loads only that hook's handler; `hooks.internal.enabled=true`, extra hook directories, and legacy handlers opt into broad discovery.

### Hook packs

Hook packs are npm packages that export hooks via `skillset.hooks` in `package.json`. Install with:
...
| command-logger        | `command`                      | Logs all commands to `~/.skillset/logs/commands.log`  |
| boot-md               | `gateway:startup`              | Runs `BOOT.md` when the gateway starts                |

Enable any bundled hook:

```bash
skillset hooks enable <hook-name>
```
...
Logs every slash command to `~/.skillset/logs/commands.log`.
...
# Enable/disable
skillset hooks enable <hook-name>
skillset hooks disable <hook-name>
```
...
1. Verify the hook is enabled: `skillset hooks list`
