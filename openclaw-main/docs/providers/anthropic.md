---
summary: "Use Anthropic Claude via API keys or Claude CLI in SkillSet"
read_when:
  - You want to use Anthropic models in SkillSet
title: "Anthropic"
---

# Anthropic (Claude)

Anthropic builds the **Claude** model family. SkillSet supports two auth routes:
...
SkillSet treats Claude CLI reuse and `claude -p` usage as sanctioned unless
    ...
    **Best for:** standard API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        ```bash
        skillset onboard --anthropic-api-key "$ANTHROPIC_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        skillset models list --provider anthropic
        ```
      </Step>
    </Steps>
    ...
    **Best for:** reusing an existing Claude CLI login without a separate API key.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ...
        SkillSet detects and reuses the existing Claude CLI credentials.
      </Step>
      <Step title="Verify the model is available">
        ```bash
        skillset models list --provider anthropic
    ...
    <Tip>
    If you want the clearest billing path, use an Anthropic API key instead. SkillSet also supports subscription-style options from [OpenAI Codex](/providers/openai), [Qwen Cloud](/providers/qwen), [MiniMax](/providers/minimax), and [Z.AI / GLM](/providers/glm).
    </Tip>
...
Claude 4.6 models default to `adaptive` thinking in SkillSet when no explicit thinking level is set.
...
SkillSet supports Anthropic's prompt caching feature for API-key auth.
...
<AccordionGroup>
  <Accordion title="Fast mode">
    SkillSet's shared `/fast` toggle supports direct Anthropic traffic (API-key and OAuth to `api.anthropic.com`).
  ...
  <Accordion title="Media understanding (image and PDF)">
    The bundled Anthropic plugin registers image and PDF understanding. SkillSet
    ...
    When an image or PDF is attached to a conversation, SkillSet automatically
  ...
  <Accordion title="1M context window (beta)">
    ...
    SkillSet maps this to `anthropic-beta: context-1m-2025-08-07` on requests.

    <Warning>
    Requires long-context access on your Anthropic credential. Legacy token auth (`sk-ant-oat-*`) is rejected for 1M context requests — SkillSet logs a warning and falls back to the standard context window.
    </Warning>
...
<AccordionGroup>
  ...
  <Accordion title='No API key found for provider "anthropic"'>
    Auth is **per agent**. New agents don't inherit the main agent's keys. Re-run onboarding for that agent, or configure an API key on the gateway host, then verify with `skillset models status`.
  </Accordion>

  <Accordion title='No credentials found for profile "anthropic:default"'>
    Run `skillset models status` to see which auth profile is active. Re-run onboarding, or configure an API key for that profile path.
  </Accordion>

  <Accordion title="No available auth profile (all in cooldown)">
    Check `skillset models status --json` for `auth.unusableProfiles`. Anthropic rate-limit cooldowns can be model-scoped, so a sibling Anthropic model may still be usable. Add another Anthropic profile or wait for cooldown.
  </Accordion>
</AccordionGroup>
