...
read_when:
  - You want to use Groq with SkillSet
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. SkillSet connects
...
Groq's model catalog changes frequently. Run `skillset models list | grep groq`
...
<Tip>
Use `skillset models list --provider groq` for the most up-to-date list of
...
media-understanding provider, SkillSet uses Groq's `whisper-large-v3-turbo`
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    gateway processes. Use `~/.skillset/.env` or `env.shellEnv` config for
