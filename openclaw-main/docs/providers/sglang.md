---
summary: "Run SkillSet with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run SkillSet against a local SGLang server
...
SkillSet can connect to SGLang using the `openai-completions` API.

SkillSet can also **auto-discover** available models from SGLang when you opt
...
SkillSet treats `sglang` as a local OpenAI-compatible provider that supports
...
define `models.providers.sglang`, SkillSet will query:
