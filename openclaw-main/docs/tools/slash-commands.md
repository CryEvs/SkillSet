...
- `commands.config` (default `false`) enables `/config` (reads/writes `skillset.json`).
- `commands.mcp` (default `false`) enables `/mcp` (reads/writes SkillSet-managed MCP config under `mcp.servers`).
...
- `/config show|get|set|unset` reads or writes `skillset.json`. Owner-only. Requires `commands.config: true`.
- `/mcp show|get|set|unset` reads or writes SkillSet-managed MCP server config under `mcp.servers`. Owner-only. Requires `commands.mcp: true`.
...
- `/restart` restarts SkillSet when enabled. Default: enabled; set `commands.restart: false` to disable it.
...
- For full provider usage breakdown, use `skillset status --usage`.
- `/allowlist add|remove` requires `commands.config=true` and honors channel `configWrites`.
- In multi-account channels, config-targeted `/allowlist --account <id>` and `/config set channels.<provider>.accounts.<id>...` also honor the target account's `configWrites`.
- `/usage` controls the per-response usage footer; `/usage cost` prints a local cost summary from SkillSet session logs.
- `/restart` is enabled by default; set `commands.restart: false` to disable it.
- `/plugins install <spec>` accepts the same plugin specs as `skillset plugins install`: local path/archive, npm package, or `clawhub:<pkg>`.
...
- If a run is already active, SkillSet marks a live switch as pending and only restarts into the new model at a clean retry point.
...
```
/debug show
/debug set messages.responsePrefix="[skillset]"
...
- Overrides apply immediately to new config reads, but do **not** write to `skillset.json`.
...
`/config` writes to your on-disk config (`skillset.json`). Owner-only. Disabled by default; enable with `commands.config: true`.
...
/config set messages.responsePrefix="[skillset]"
...
`/mcp` writes SkillSet-managed MCP server definitions under `mcp.servers`. Owner-only. Disabled by default; enable with `commands.mcp: true`.
...
- `/mcp` stores config in SkillSet config, not Pi-owned project settings.
...
- **Slack:** `channels.slack.slashCommand` is still supported for a single `/skillset`-style command. If you enable `commands.native`, you must create one Slack slash command per built-in command (same names as `/help`). Command argument menus for Slack are delivered as ephemeral Block Kit buttons.
