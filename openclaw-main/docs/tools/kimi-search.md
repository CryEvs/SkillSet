...
SkillSet supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), SkillSet reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.skillset/.env`.

If you omit `baseUrl`, SkillSet defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, SkillSet defaults to `kimi-k2.6`.
