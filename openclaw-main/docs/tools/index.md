---
summary: "SkillSet tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools SkillSet provides
...
SkillSet has three layers that work together:

<Steps>
  <Step title="Tools are what the agent calls">
    A tool is a typed function the agent can invoke (e.g. `exec`, `browser`,
    `web_search`, `message`). SkillSet ships a set of **built-in tools** and
  ...
  <Step title="Plugins package everything together">
    ...
    SkillSet), others are **external** (published on npm by the community).
...
These tools ship with SkillSet and are available without installing any plugins:
...
| `group:skillset`   | All built-in SkillSet tools (excludes plugin tools)                                                       |
