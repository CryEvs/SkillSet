---
summary: "ClawHub guide: public registry, native SkillSet install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **SkillSet skills and plugins**.

- Use native `skillset` commands to search/install/update skills and install
...
```bash
skillset plugins install clawhub:<package>
...
Native `skillset` commands install into your active workspace and persist source
...
`skillset plugins install clawhub:...` only accepts installable plugin families.
If a ClawHub package is actually a skill, SkillSet stops and points you at
`skillset skills install <slug>` instead.

## What ClawHub is

- A public registry for SkillSet skills and plugins.
...
4. Users browse, download, and install skills in SkillSet.
...
If you want to add new capabilities to your SkillSet agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `skillset skills install <skill-slug>`
3. Start a new SkillSet session so it picks up the new skill.
...
## How it fits into SkillSet

Native `skillset skills install` installs into the active workspace `skills/`
directory. `skillset plugins install clawhub:...` records a normal managed
...
Community or other non-official channels can still install, but SkillSet warns
...
current working directory. If an SkillSet workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). SkillSet loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.skillset/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches SkillSet how to perform a
...
Interested in becoming a moderator? Ask in the SkillSet Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to SkillSet workspace).
...
Code plugins must include the required SkillSet metadata in `package.json`:
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/skillset/skills` and `~/.skillset/skills`). This is designed to find older skill installs without extra flags.
