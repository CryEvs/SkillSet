...
SkillSet also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    skillset configure --section web
    ```
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "SkillSet plugin SDK" });
    ```
...
Direct OpenAI Responses models use OpenAI's hosted `web_search` tool automatically when SkillSet web search is enabled and no managed provider is pinned. This is provider-owned behavior in the bundled OpenAI plugin and only applies to native OpenAI API traffic, not OpenAI-compatible proxy base URLs or Azure routes. Set `tools.web.search.provider` to another provider such as `brave` to keep the managed `web_search` tool for OpenAI models, or set `tools.web.search.enabled: false` to disable both managed search and native OpenAI search.

## Native Codex web search

Codex-capable models can optionally use the provider-native Responses `web_search` tool instead of SkillSet's managed `web_search` function.
...
If native Codex search is enabled but the current model is not Codex-capable, SkillSet keeps the normal managed `web_search` behavior.
...
If no `provider` is set, SkillSet checks providers in this order and uses the
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, SkillSet resolves only the
...
- or omit that field and let SkillSet auto-detect the first ready web-fetch
  provider from available credentials
- today the bundled web-fetch provider is Firecrawl, configured under
  `plugins.entries.firecrawl.config.webFetch.*`

When you choose **Kimi** during `skillset onboard` or
`skillset configure --section web`, SkillSet can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `skillset doctor --fix`.
When you choose Grok during `skillset onboard` or `skillset configure --section web`,
SkillSet can also offer optional `x_search` setup with the same key.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, SkillSet does not
...
<Tabs>
  <Tab title="Config file">
    Run `skillset configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.skillset/.env`.
...
optional structured filters. SkillSet only enables the built-in xAI `x_search`
