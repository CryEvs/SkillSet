...
read_when:
  ...
  - Debugging why skillset is interfering with your own Chrome
  - Implementing browser settings + lifecycle in the macOS app
title: "Browser (SkillSet-managed)"
---

# Browser (skillset-managed)

SkillSet can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- The `skillset` profile does **not** touch your personal browser profile.
- The agent can **open tabs, read pages, click, and type** in a safe lane.
- The built-in `user` profile attaches to your real signed-in Chrome session via Chrome MCP.

## What you get

- A separate browser profile named **skillset** (orange accent by default).
- Deterministic tab control (list/open/focus/close).
- Agent actions (click/type/drag/select), snapshots, screenshots, PDFs.
- Optional multi-profile support (`skillset`, `work`, `remote`, ...).
...
```bash
skillset browser --browser-profile skillset status
...
If `skillset browser` is missing entirely, or the agent says the browser tool
...
If you turn off only the plugin, the bundled browser CLI (`skillset browser`),
...
If `skillset browser` suddenly becomes an unknown command after an upgrade, or
...
Set `browser.defaultProfile: "skillset"` if you want managed mode by default.

## Configuration

Browser settings live in `~/.skillset/skillset.json`.
...
- If you override the Gateway port (`gateway.port` or `SKILLSET_GATEWAY_PORT`),
...
- Default profile is `skillset` (SkillSet-managed standalone browser). Use `defaultProfile: "user"` to opt into the signed-in user browser.
- Auto-detect order: system default browser if Chromium-based; otherwise Chrome → Brave → Edge → Chromium → Chrome Canary.
- Local `skillset` profiles auto-assign `cdpPort`/`cdpUrl` — set those only for remote CDP.
...
SkillSet uses it automatically. Set `browser.executablePath` to override
auto-detection:

CLI example:

```bash
skillset config set browser.executablePath "/usr/bin/google-chrome"
```
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, SkillSet will not launch a local browser.

Stopping behavior differs by profile mode:

- local managed profiles: `skillset browser stop` stops the browser process that
  SkillSet launched
- attach-only and remote CDP profiles: `skillset browser stop` closes the active
  ...
  though no browser process was launched by SkillSet
...
SkillSet preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, SkillSet can
...
- If you set `nodeHost.browserProxy.allowProfiles`, SkillSet treats it as a least-privilege boundary: only allowlisted profiles can be targeted, and persistent profile create/delete routes are blocked on the proxy surface.
...
CDP connection URLs over HTTPS and WebSocket. SkillSet can use either form, but
...
- If Browserless gives you an HTTPS base URL, you can either convert it to
  `wss://` for a direct CDP connection or keep the HTTPS URL and let SkillSet
  discover `/json/version`.

## Direct WebSocket CDP providers

Some hosted browser services expose a **direct WebSocket** endpoint rather than
the standard HTTP-based CDP discovery (`/json/version`). SkillSet accepts three
CDP URL shapes and picks the right connection strategy automatically:

- **HTTP(S) discovery** — `http://host[:port]` or `https://host[:port]`.
  SkillSet calls `/json/version` to discover the WebSocket debugger URL, then
...
- **Direct WebSocket endpoints** — `ws://host[:port]/devtools/<kind>/<id>` or
  ...
  path. SkillSet connects directly via a WebSocket handshake and skips
...
- **Bare WebSocket roots** — `ws://host[:port]` or `wss://host[:port]` with no
  ...
  [Browserbase](https://www.browserbase.com)). SkillSet tries HTTP
  `/json/version` discovery first (normalising the scheme to `http`/`https`);
  if discovery returns a `webSocketDebuggerUrl` it is used, otherwise SkillSet
...
- The standalone loopback browser HTTP API uses **shared-secret auth only**:
  gateway token bearer auth, `x-skillset-password`, or HTTP Basic auth with the
...
- If browser control is enabled and no shared-secret auth is configured, SkillSet
  auto-generates `gateway.auth.token` on startup and persists it to config.
- SkillSet does **not** auto-generate that token when `gateway.auth.mode` is
...
SkillSet supports multiple named profiles (routing configs). Profiles can be:

- **skillset-managed**: a dedicated Chromium-based browser instance with its own user data directory + CDP port
- **remote**: an explicit CDP URL (Chromium-based browser running elsewhere)
- **existing session**: your existing Chrome profile via Chrome DevTools MCP auto-connect

Defaults:

- The `skillset` profile is auto-created if missing.
...
SkillSet can also attach to a running Chromium-based browser profile through the
...
3. Keep the browser running and approve the connection prompt when SkillSet attaches.
...
```bash
skillset browser --browser-profile user start
skillset browser --browser-profile user status
skillset browser --browser-profile user tabs
skillset browser --browser-profile user snapshot --format ai
```
...
- `skillset doctor` migrates old extension-based browser config and checks that
...
- This path is higher-risk than the isolated `skillset` profile because it can
  act inside your signed-in browser session.
- SkillSet does not launch the browser for this driver; it attaches to an
  existing session only.
- SkillSet uses the official Chrome DevTools MCP `--autoConnect` flow here. If
  `userDataDir` is set, SkillSet passes it through to target that explicit
...
When launching locally, SkillSet picks the first available:
...
- `x-skillset-password: <gateway password>` or HTTP Basic auth with that password
...
- ARIA snapshots
- Page screenshots for the managed `skillset` browser when a per-tab CDP
...
then restart the gateway. For packaged installs, run `skillset doctor --fix`.
...
```bash
docker compose run --rm skillset-cli \
  node /app/node_modules/playwright-core/cli.js install chromium
```

To persist browser downloads, set `PLAYWRIGHT_BROWSERS_PATH` (for example,
`/home/node/.cache/ms-playwright`) and make sure `/home/node` is persisted via
`SKILLSET_HOME_VOLUME` or a bind mount. See [Docker](/install/docker).
...
- `skillset browser snapshot --interactive --compact --depth 6`
- `skillset browser snapshot --efficient`
- `skillset browser snapshot --labels`
- `skillset browser snapshot --selector "#main" --interactive`
- `skillset browser snapshot --frame "iframe#main" --interactive`
- `skillset browser console --level error`

Lifecycle note:

- For attach-only and remote CDP profiles, `skillset browser stop` is still the
...
- `skillset browser requests --filter api --clear`
- `skillset browser pdf`
- `skillset browser responsebody "**/api" --max-chars 5000`
...
- `skillset browser storage local set theme dark`
- `skillset browser storage session clear`
- `skillset browser set offline on`
- `skillset browser set headers --headers-json '{"X-Debug":"1"}'`
- `skillset browser set credentials user pass`
- `skillset browser set credentials --clear`
- `skillset browser set geo 37.7749 -122.4194 --origin "https://example.com"`
- `skillset browser set geo --clear`
- `skillset browser set media dark`
- `skillset browser set timezone America/New_York`
- `skillset browser set locale en-US`
...
- Download and trace output paths are constrained to SkillSet temp roots:
  - traces: `/tmp/skillset` (fallback: `${os.tmpdir()}/skillset`)
  - downloads: `/tmp/skillset/downloads` (fallback: `${os.tmpdir()}/skillset/downloads`)
- Upload paths are constrained to an SkillSet temp uploads root:
  - uploads: `/tmp/skillset/uploads` (fallback: `${os.tmpdir()}/skillset/uploads`)
...
SkillSet supports two “snapshot” styles:

- **AI snapshot (numeric refs)**: `skillset browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `skillset browser click 12`, `skillset browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright’s `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `skillset browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `skillset browser click e12`, `skillset browser highlight e12`.
...
- Wait for URL (globs supported by Playwright):
  - `skillset browser wait --url "**/dash"`
- Wait for load state:
  - `skillset browser wait --load networkidle`
- Wait for a JS predicate:
  - `skillset browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `skillset browser wait "#main"`
...
1. `skillset browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `skillset browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `skillset browser trace start`
   - reproduce the issue
   - `skillset browser trace stop` (prints `TRACE:<path>`)
...
```bash
skillset browser status --json
skillset browser snapshot --interactive --json
skillset browser requests --filter api --json
skillset browser cookies --json
```
...
- The skillset browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `skillset browser evaluate` and `wait --fn`
...
- **CDP startup or readiness failure** means SkillSet cannot confirm that the browser control plane is healthy.
...
- CDP startup or readiness failure:
  - `Chrome CDP websocket for profile "skillset" is not reachable after start`
...
- For the local loopback `skillset` managed profile, CDP health checks intentionally skip browser SSRF reachability enforcement for SkillSet's own local control plane.
...
- `browser` accepts:
  - `profile` to choose a named browser profile (skillset, chrome, or remote CDP).
