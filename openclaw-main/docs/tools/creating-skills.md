...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.skillset/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so SkillSet picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    skillset agent --message "give me a greeting"
    ```
...
| `metadata.skillset.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.skillset.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.skillset.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `skillset agent --message "..."` to test before sharing
...
| `~/.skillset/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with SkillSet) | Low        | Global                |
