...
- an Ollama host that is reachable from SkillSet
- `ollama signin`
...
If no explicit Ollama base URL is set, SkillSet uses `http://127.0.0.1:11434`.

If your Ollama host expects bearer auth, SkillSet reuses
...
- If the Ollama host is auth-protected, SkillSet reuses the normal Ollama
  provider API key when present.
- SkillSet warns during setup if Ollama is unreachable or not signed in, but
