...
SkillSet supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, SkillSet also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `skillset configure --section web`. It stores the key in
`~/.skillset/skillset.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.skillset/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
