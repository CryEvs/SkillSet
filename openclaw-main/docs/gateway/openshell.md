---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for SkillSet agents"
read_when:
...
OpenShell is a managed sandbox backend for SkillSet. Instead of running Docker
containers locally, SkillSet delegates sandbox lifecycle to the `openshell` CLI,
...
- SkillSet Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, SkillSet creates an OpenShell
...
- You edit files locally outside SkillSet and want those changes visible in the
...
- When the sandbox is first created, SkillSet seeds the remote workspace from
...
- SkillSet does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside SkillSet after the initial seed,
the remote sandbox does **not** see those changes. Use
`skillset sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"skillset"`  | Sandbox source for first-time create                  |
...
1. SkillSet calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. SkillSet calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `skillset sandbox` commands
