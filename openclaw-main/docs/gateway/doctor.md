...
`skillset doctor` is the repair + migration tool for SkillSet. It fixes stale
...
```bash
skillset doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
skillset doctor --repair --force
```

Apply aggressive repairs too (overwrites custom supervisor configs).

```bash
skillset doctor --non-interactive
```
...
- Extra workspace dir detection (`~/skillset`).
...
doctor-style RPC methods, but they are **not** part of `skillset doctor` CLI
repair/migration.
...
```bash
skillset memory rem-backfill --path ./memory --stage-short-term
```
...
you to run `skillset doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.skillset/skillset.json` with the updated schema.

The Gateway also auto-runs doctor migrations on startup when it detects a
legacy config format, so stale configs are repaired without manual intervention.
Cron job store migrations are handled by `skillset doctor --fix`.
...
- Sessions store + transcripts:
  - from `~/.skillset/sessions/` to `~/.skillset/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.skillset/agent/` to `~/.skillset/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.skillset/credentials/*.json` (except `oauth.json`)
  - to `~/.skillset/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `skillset doctor`. Talk provider/provider-map normalization now
...
Doctor also checks the cron job store (`~/.skillset/cron/jobs.json` by default,
...
- **Multiple state dirs**: warns when multiple `~/.skillset` folders exist across
  home directories or when `SKILLSET_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.skillset/skillset.json` is
...
that re-auth is required and prints the exact `skillset models auth login --provider ...`
command to run.
...
`skillset doctor --fix` / `skillset doctor --repair` mode. External plugins still
use `skillset plugins install` / `skillset plugins update`; doctor does not
install dependencies for arbitrary plugin paths.

### 8) Gateway service migrations and cleanup hints

Doctor detects legacy gateway services (launchd/systemd/schtasks) and
offers to remove them and install the SkillSet service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named SkillSet gateway services are considered first-class and are not
flagged as "extra."
...
startup continues. In read-only mode (`skillset doctor` without `--fix`) this check
is skipped entirely.
...
- inspect pending requests with `skillset devices list`
- approve the exact request with `skillset devices approve <requestId>`
- rotate a fresh token with `skillset devices rotate --device <deviceId> --role <role>`
- remove and re-approve a stale record with `skillset devices remove <deviceId>`
...
- **Legacy workspace dirs**: warns when `~/skillset` or other legacy workspace directories
...
- If the shell profile uses a slow dynamic completion pattern
  (`source <(skillset completion ...)`), doctor upgrades it to the faster
  cached file variant.
...
Run `skillset completion --write-state` to regenerate the cache manually.
...
- `skillset doctor --generate-gateway-token` forces generation only when no token SecretRef is configured.
...
- `skillset doctor --fix` now uses the same read-only SecretRef summary model as status-family commands for targeted config repairs.
...
Use `skillset memory status --deep` to verify embedding readiness at runtime.
...
- `skillset doctor` prompts before rewriting supervisor config.
- `skillset doctor --yes` accepts the default repair prompts.
- `skillset doctor --repair` applies recommended fixes without prompts.
- `skillset doctor --repair --force` overwrites custom supervisor configs.
...
- You can always force a full rewrite via `skillset gateway install --force`.
