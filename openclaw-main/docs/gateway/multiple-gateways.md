---
summary: "Run multiple SkillSet Gateways on one host (isolation, ports, and profiles)"
read_when:
...
skillset --profile rescue onboard
skillset --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `skillset --profile rescue onboard`:
...
`skillset --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.skillset/workspace-rescue`)
...
# extra gateway
skillset --profile ops setup
skillset --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
skillset --profile main setup
skillset --profile main gateway --port 18789
...
skillset --profile ops gateway install --port 19789
```
...
- `SKILLSET_CONFIG_PATH` — per-instance config file
- `SKILLSET_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `SKILLSET_GATEWAY_PORT` / `--port`).
...
```bash
SKILLSET_CONFIG_PATH=~/.skillset/main.json \
SKILLSET_STATE_DIR=~/.skillset \
skillset gateway --port 18789

SKILLSET_CONFIG_PATH=~/.skillset/rescue.json \
SKILLSET_STATE_DIR=~/.skillset-rescue \
skillset gateway --port 19789
```
...
skillset --profile rescue gateway status --deep
skillset --profile rescue gateway probe
skillset status
skillset --profile rescue status
skillset --profile rescue browser status
```
