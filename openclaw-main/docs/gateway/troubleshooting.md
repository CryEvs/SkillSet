...
- `skillset gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `skillset doctor` reports no blocking config/service issues.
- `skillset channels status --probe` shows live per-account transport status and,
...
skillset config get agents.defaults.models
```
...
- SkillSet model runs fail only on normal agent turns
...
skillset infer model run --model <provider/model> --prompt "hi" --json
skillset logs --follow
```

Look for:

- direct tiny calls succeed, but SkillSet runs fail only on larger prompts
...
- direct tiny requests succeed, but SkillSet agent runs fail with backend/model
  crashes (for example Gemma on some `inferrs` builds) → SkillSet transport is
...
2. Set `compat.supportsTools: false` for models/backends that cannot handle
   SkillSet's tool schema surface reliably.
3. Lower prompt pressure where possible: smaller workspace bootstrap, shorter
   ...
   support.
4. If tiny direct requests keep passing while SkillSet agent turns still crash
...
skillset status
skillset channels status --probe
skillset pairing list --channel <channel> [--account <id>]
skillset config get channels
skillset logs --follow
```
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `skillset config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `skillset devices list` then `skillset devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `skillset devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the
  caller also has `operator.admin`
- `skillset devices rotate --scope ...` can only request operator scopes that
...
skillset logs --follow
skillset doctor
skillset gateway status --deep   # also scan system-level services
```
...
- `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `skillset onboard --mode local` / `skillset setup` to restamp the expected local-mode config. If you are running SkillSet via Podman, the default config path is `~/.skillset/skillset.json`.
...
Use this when the Gateway starts, but logs say it restored `skillset.json`.
...
- A timestamped `skillset.json.clobbered.*` file beside the active config
...
- SkillSet preserved the rejected payload as `.clobbered.*`.
...
- `.rejected.*` exists → an SkillSet-owned config write failed schema or clobber checks before commit.
...
2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `skillset config set` or `config.patch`.
3. Run `skillset config validate` before restarting.
...
Use this when `skillset gateway probe` reaches something, but still prints a warning block.

```bash
skillset gateway probe
skillset gateway probe --json
skillset gateway probe --ssh user@gateway-host
```
...
```bash
skillset cron status
skillset cron list
skillset cron runs --id <jobId> --limit 20
skillset system heartbeat last
skillset logs --follow
```
...
- `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so SkillSet skips the model call.
...
skillset approvals get --node <idOrNameOrIp>
skillset logs --follow
skillset status
```
...
skillset browser start --browser-profile skillset
...
- stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `skillset browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
```bash
skillset gateway status
skillset config get gateway.mode
skillset config get gateway.remote.url
skillset config get gateway.auth.mode
```
...
```bash
skillset config get gateway.bind
skillset config get gateway.auth.mode
skillset config get gateway.auth.token
skillset gateway status
skillset logs --follow
```
