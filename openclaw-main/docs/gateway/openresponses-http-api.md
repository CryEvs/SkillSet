...
SkillSet’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`skillset agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-skillset-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-skillset-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "skillset"`, `model: "skillset/default"`, `model: "skillset/<agentId>"`, or `x-skillset-agent-id`
- use `x-skillset-model` when you want to override the selected agent's backend model
- use `x-skillset-session-key` for explicit session routing
- use `x-skillset-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: SkillSet reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
SkillSet normalizes common OpenAI-style aliases before those counters reach
