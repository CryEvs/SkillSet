...
Healthy baseline: `Runtime: running`, `Connectivity probe: ok`, and `Capability: ...` that matches what you expect. Use `skillset gateway status --require-rpc` when you need read-scope RPC proof, not just reachability.
...
<Note>
Gateway config reload watches the active config file path (resolved from profile/state defaults, or `SKILLSET_CONFIG_PATH` when set).
...
- Auth is required by default. Shared-secret setups use
  ...
  `SKILLSET_GATEWAY_TOKEN` / `SKILLSET_GATEWAY_PASSWORD`), and non-loopback
  reverse-proxy setups can use `gateway.auth.mode: "trusted-proxy"`.

## OpenAI-compatible endpoints

SkillSet’s highest-leverage compatibility surface is now:
...
- `/v1/models` is agent-first: it returns `skillset`, `skillset/default`, and `skillset/<agentId>`.
- `skillset/default` is the stable alias that always maps to the configured default agent.
- Use `x-skillset-model` when you want a backend provider/model override; otherwise the selected agent's normal model and embedding setup stays in control.
...
| ------------ | ------------------------------------------------------------- |
| Gateway port | `--port` → `SKILLSET_GATEWAY_PORT` → `gateway.port` → `18789` |
...
LaunchAgent labels are `ai.skillset.gateway` (default) or `ai.skillset.<profile>` (named profile). `skillset doctor` audits and repairs service config drift.
...
```bash
skillset gateway install
systemctl --user enable --now skillset-gateway[-<profile>].service
skillset gateway status
```
...
```ini
[Unit]
Description=SkillSet Gateway
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/skillset gateway --port 18789
Restart=always
RestartSec=5
TimeoutStopSec=30
TimeoutStartSec=30
...
Native Windows managed startup uses a Scheduled Task named `SkillSet Gateway`
(or `SkillSet Gateway (<profile>)` for named profiles). If Scheduled Task
creation is denied, SkillSet falls back to a per-user Startup-folder launcher
...
sudo systemctl enable --now skillset-gateway[-<profile>].service
```

Use the same service body as the user unit, but install it under
`/etc/systemd/system/skillset-gateway[-<profile>].service` and adjust
`ExecStart=` if your `skillset` binary lives elsewhere.
...
- Unique `gateway.port`
- Unique `SKILLSET_CONFIG_PATH`
- Unique `SKILLSET_STATE_DIR`
- Unique `agents.defaults.workspace`

Example:

```bash
SKILLSET_CONFIG_PATH=~/.skillset/a.json SKILLSET_STATE_DIR=~/.skillset-a skillset gateway --port 19001
SKILLSET_CONFIG_PATH=~/.skillset/b.json SKILLSET_STATE_DIR=~/.skillset-b skillset gateway --port 19002
```
...
```bash
skillset --dev setup
skillset --dev gateway --allow-unconfigured
skillset --dev status
```
