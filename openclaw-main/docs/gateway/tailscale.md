...
SkillSet can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. SkillSet requires a shared password.
...
- `token` (default when `SKILLSET_GATEWAY_TOKEN` is set)
- `password` (shared secret via `SKILLSET_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. SkillSet verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
SkillSet only treats a request as Serve when it arrives from loopback with
...
Prefer `SKILLSET_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
skillset gateway --tailscale serve
skillset gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want SkillSet to undo `tailscale serve`
