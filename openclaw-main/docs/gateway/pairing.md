...
skillset nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), SkillSet treats
...
Pairing state is stored under the Gateway state directory (default `~/.skillset`):

- `~/.skillset/nodes/paired.json`
- `~/.skillset/nodes/pending.json`

If you override `SKILLSET_STATE_DIR`, the `nodes/` folder moves with it.
