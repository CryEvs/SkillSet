...
SkillSet can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **SkillSet tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
skillset agent --message "hi" --model codex-cli/gpt-5.4
```
...
gateway host, SkillSet now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), SkillSet will
...
2. **Builds a system prompt** using the same SkillSet prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   SkillSet session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us SkillSet-style Claude CLI usage is allowed again, so SkillSet treats
`claude -p` usage as sanctioned for this integration unless Anthropic publishes
a new policy.
</Note>

The bundled OpenAI `codex-cli` backend passes SkillSet's system prompt through
Codex's `model_instructions_file` config override (`-c
model_instructions_file="..."`). Codex does not expose a Claude-style
`--append-system-prompt` flag, so SkillSet writes the assembled prompt to a
temporary file for each fresh Codex CLI session.

The bundled Anthropic `claude-cli` backend receives the SkillSet skills snapshot
two ways: the compact SkillSet skills catalog in the appended system prompt, and
...
resolver sees the same filtered set that SkillSet would otherwise advertise in
the prompt. Skill env/API key overrides are still applied by SkillSet to the
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, SkillSet resumes from the stored Claude session id. Stored session
...
- SkillSet drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, SkillSet lets that CLI enforce resume permissions.
...
SkillSet will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, SkillSet appends the
...
- For Gemini CLI JSON output, SkillSet reads reply text from `response` and
...
- `stats.cached` is normalized into SkillSet `cacheRead`.
- If `stats.input` is missing, SkillSet derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before SkillSet handles
...
CLI backends do **not** receive SkillSet tool calls directly, but a backend can
...
When bundle MCP is enabled, SkillSet:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`SKILLSET_MCP_TOKEN`)
...
If no MCP servers are enabled, SkillSet still injects a strict config when a
backend opts into bundle MCP so background runs stay isolated.

## Limitations

- **No direct SkillSet tool calls.** SkillSet does not inject tool calls into
...
- **Codex CLI sessions** resume via text output (no JSONL), which is less
  structured than the initial `--json` run. SkillSet sessions still work
  normally.
