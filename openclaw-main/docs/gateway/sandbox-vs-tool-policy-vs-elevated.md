...
SkillSet has three related (but different) controls:
...
Use the inspector to see what SkillSet is _actually_ doing:
...
- SkillSet validates bind sources twice: first on the normalized source path, then again after resolving through the deepest existing ancestor. Symlink-parent escapes do not bypass blocked-path or allowed-root checks.
...
- `group:skillset`: all built-in SkillSet tools (excludes provider plugins)
