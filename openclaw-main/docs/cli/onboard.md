---
summary: "CLI reference for `skillset onboard` (interactive onboarding)"
read_when:
...
```bash
skillset onboard
skillset onboard --flow quickstart
skillset onboard --flow manual
skillset onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`SKILLSET_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `skillset gateway run`.
...
- Fastest first chat: `skillset dashboard` (Control UI, no channel setup).
