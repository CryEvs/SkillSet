...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an SkillSet Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`skillset acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to SkillSet channel
conversations instead of hosting an ACP harness session, use
[`skillset mcp serve`](/cli/mcp) instead.
...
- SkillSet acts as an ACP server
- an IDE or ACP client connects to SkillSet
- SkillSet forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where SkillSet runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to SkillSet: use `skillset acp`
- SkillSet should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the SkillSet gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
skillset acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
skillset acp --url wss://gateway-host:18789 --token-file ~/.skillset/gateway.token

# Attach to an existing session key
skillset acp --session agent:main:main

# Attach by label (must already exist)
skillset acp --session-label "support inbox"

# Reset the session key before the first prompt
skillset acp --session agent:main:main --reset-session
```
...
skillset acp client --server-args --url wss://gateway-host:18789 --token-file ~/.skillset/gateway.token

# Override the server command (default: skillset)
skillset acp client --server "node" --server-args skillset.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run SkillSet through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an SkillSet Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `skillset acp` over stdio.

Example config (persisted):

```bash
skillset config set gateway.remote.url wss://gateway-host:18789
skillset config set gateway.remote.token <token>
```
...
```bash
skillset acp --session agent:main:main
skillset acp --session agent:design:main
skillset acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see SkillSet plugin tools or selected
...
[SkillSet tools MCP bridge](/tools/acp-agents#skillset-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
SkillSet bot over ACP, use `acpx` with its built-in `skillset` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx skillset` at `skillset acp`.
3. Target the SkillSet session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default SkillSet ACP session
acpx skillset exec "Summarize the active SkillSet session state."

# Persistent named session for follow-up turns
acpx skillset sessions ensure --name codex-bridge
acpx skillset -s codex-bridge --cwd /path/to/repo \
  "Ask my SkillSet work agent for recent context relevant to this repo."
```

If you want `acpx skillset` to target a specific Gateway and session key every
time, override the `skillset` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "skillset": {
      "command": "env SKILLSET_HIDE_BANNER=1 SKILLSET_SUPPRESS_NOTES=1 skillset acp --url ws://127.0.0.1:18789 --token-file ~/.skillset/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local SkillSet checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env SKILLSET_HIDE_BANNER=1 SKILLSET_SUPPRESS_NOTES=1 node skillset.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an SkillSet agent without scraping a terminal.
...
In Zed, open the Agent panel and select “SkillSet ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`SKILLSET_GATEWAY_TOKEN`, `SKILLSET_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`SKILLSET_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `SKILLSET_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `skillset acp client` sets `SKILLSET_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `skillset`).
