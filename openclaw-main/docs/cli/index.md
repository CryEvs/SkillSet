---
summary: "SkillSet CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `skillset` subcommand
...
`skillset` is the main CLI entry point. Each core command has either a
...
| Flag                    | Purpose                                                               |
| ----------------------- | --------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.skillset-dev` and shift default ports         |
| `--profile <name>`      | Isolate state under `~/.skillset-<name>`                              |
| `--container <name>`    | Target a named container for execution                                |
| `--no-color`            | Disable ANSI colors (`NO_COLOR=1` is also respected)                  |
| `--update`              | Shorthand for [`skillset update`](/cli/update) (source installs only) |
...
```
skillset [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Plugins can add additional top-level commands (for example `skillset voicecall`).
...
`skillset status --usage` and the Control UI surface provider usage/quota when
