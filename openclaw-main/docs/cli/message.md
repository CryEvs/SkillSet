---
summary: "CLI reference for `skillset message` (send + channel actions)"
read_when:
...
```
skillset message <subcommand> [flags]
```
...
- On cache miss, SkillSet will attempt a live directory lookup when the provider supports it.
...
- `skillset message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, SkillSet does not force a `default` account SecretRef scope
...
```
skillset message send --channel discord \
...
```bash
skillset message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
skillset message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
skillset message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
skillset message poll --channel msteams \
...
```
skillset message react --channel slack \
...
```
skillset message react --channel signal \
...
```
skillset message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
skillset message send --channel telegram --target @mychat \
