---
summary: "CLI reference for `skillset models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
skillset models set <model-or-alias>
skillset models scan
```

`skillset models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise SkillSet falls back to matching OAuth/API-key
...
the command uses `SKILLSET_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, SkillSet resolves the input as an alias first, then
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `SKILLSET_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but SkillSet could not resolve a probeable
...
```bash
skillset models auth add
skillset models auth login --provider <id>
skillset models auth setup-token --provider <id>
skillset models auth paste-token
```
...
`skillset plugins list` to see which providers are installed.

Examples:

```bash
skillset models auth login --provider openai-codex --set-default
```
...
- Anthropic `setup-token` / `paste-token` remain available as a supported SkillSet token path, but SkillSet now prefers Claude CLI reuse and `claude -p` when available.
