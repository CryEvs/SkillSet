---
summary: "CLI reference for `skillset health` (gateway health snapshot via RPC)"
read_when:
...
skillset health --timeout 2500
skillset health --verbose
skillset health --debug
```

Notes:

- Default `skillset health` asks the running gateway for its health snapshot. When the
