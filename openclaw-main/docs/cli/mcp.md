---
summary: "Expose SkillSet channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to SkillSet-backed channels
  - Running `skillset mcp serve`
  - Managing SkillSet-saved MCP server definitions
title: "mcp"
---

# mcp

`skillset mcp` has two jobs:

- run SkillSet as an MCP server with `skillset mcp serve`
- manage SkillSet-owned outbound MCP server definitions with `list`, `show`,
  `set`, and `unset`

In other words:

- `serve` is SkillSet acting as an MCP server
- `list` / `show` / `set` / `unset` is SkillSet acting as an MCP client-side
  registry for other MCP servers its runtimes may consume later

Use [`skillset acp`](/cli/acp) when SkillSet should host a coding harness
session itself and route that runtime through ACP.

## SkillSet as an MCP server

This is the `skillset mcp serve` path.

## When to use `serve`

Use `skillset mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to
  SkillSet-backed channel conversations
- you already have a local or remote SkillSet Gateway with routed sessions
- you want one MCP server that works across SkillSet's channel backends instead
  of running separate per-channel bridges

Use [`skillset acp`](/cli/acp) instead when SkillSet should host the coding
runtime itself and keep the agent session inside SkillSet.

## How it works

`skillset mcp serve` starts a stdio MCP server. The MCP client owns that
process. While the client keeps the stdio session open, the bridge connects to a
local or remote SkillSet Gateway over WebSocket and exposes routed channel
conversations over MCP.

Lifecycle:

1. the MCP client spawns `skillset mcp serve`
...
- stdio MCP servers launched by SkillSet (bundled or user-configured) are torn
...
conversations. A conversation appears when SkillSet already has session state
...
# Remote Gateway
skillset mcp serve --url wss://gateway-host:18789 --token-file ~/.skillset/gateway.token

# Remote Gateway with password auth
skillset mcp serve --url wss://gateway-host:18789 --password-file ~/.skillset/gateway.password

# Enable verbose bridge logs
skillset mcp serve --verbose

# Disable Claude-specific push notifications
skillset mcp serve --claude-channel-mode off
```
...
SkillSet equivalent of a Claude Code channel adapter: standard MCP tools remain
...
- sender allowlists, pairing, and channel-level trust still belong to the
  underlying SkillSet channel configuration
...
SkillSet ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `skillset mcp serve`
...
## SkillSet as an MCP client registry

This is the `skillset mcp list`, `show`, `set`, and `unset` path.

These commands do not expose SkillSet over MCP. They manage SkillSet-owned MCP
server definitions under `mcp.servers` in SkillSet config.

Those saved definitions are for runtimes that SkillSet launches or configures
later, such as embedded Pi and other runtime adapters. SkillSet stores the
definitions centrally so those runtimes do not need to keep their own duplicate
MCP server lists.

Important behavior:

- these commands only read or write SkillSet config
...
SkillSet also stores a lightweight MCP server registry in config for surfaces
that want SkillSet-managed MCP definitions.

Commands:

- `skillset mcp list`
- `skillset mcp show [name]`
- `skillset mcp set <name> <json>`
- `skillset mcp unset <name>`
...
```bash
skillset mcp list
skillset mcp show context7 --json
skillset mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
skillset mcp set docs '{"url":"https://mcp.example.com"}'
skillset mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, SkillSet uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |
