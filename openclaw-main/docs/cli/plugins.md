---
summary: "CLI reference for `skillset plugins` (list, install, marketplace, uninstall, enable/disable, doctor)"
read_when:
...
skillset plugins list --enabled
skillset plugins list --verbose
skillset plugins list --json
skillset plugins install <path-or-spec>
skillset plugins inspect <id>
skillset plugins inspect <id> --json
skillset plugins inspect --all
skillset plugins info <id>
skillset plugins enable <id>
skillset plugins disable <id>
skillset plugins uninstall <id>
skillset plugins doctor
skillset plugins update <id-or-npm-spec>
skillset plugins update --all
skillset plugins marketplace list <marketplace>
skillset plugins marketplace list <marketplace> --json
```

Bundled plugins ship with SkillSet. Some are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin); others require `plugins enable`.

Native SkillSet plugins must ship `skillset.plugin.json` with an inline JSON
Schema (`configSchema`, even if empty). Compatible bundles use their own bundle
manifests instead.

`plugins list` shows `Format: skillset` or `Format: bundle`. Verbose list/info
...
```bash
skillset plugins install <package>                      # ClawHub first, then npm
skillset plugins install clawhub:<package>              # ClawHub only
skillset plugins install <package> --force              # overwrite existing install
skillset plugins install <package> --pin                # pin version
skillset plugins install <package> --dangerously-force-unsafe-install
skillset plugins install <path>                         # local path
skillset plugins install <plugin>@<marketplace>         # marketplace
skillset plugins install <plugin> --marketplace <name>  # marketplace (explicit)
skillset plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
```
...
If your `plugins` section is backed by a single-file `$include`, `plugins install/update/enable/disable/uninstall` write through to that included file and leave `skillset.json` untouched. Root includes, include arrays, and includes with sibling overrides fail closed instead of flattening. See [Config includes](/gateway/configuration) for the supported shapes.

If config is invalid, `plugins install` normally fails closed and tells you to
run `skillset doctor --fix` first. The only documented exception is a narrow
bundled-plugin recovery path for plugins that explicitly opt into
`skillset.install.allowInvalidConfigRecovery`.
...
If you run `plugins install` for a plugin id that is already installed, SkillSet
...
override, while `skillset skills install` remains a separate ClawHub skill
download/install flow.

`plugins install` is also the install surface for hook packs that expose
`skillset.hooks` in `package.json`. Use `skillset hooks` for filtered hook
...
those to a prerelease, SkillSet stops and asks you to opt in explicitly with a
prerelease tag such as `@beta`/`@rc` or an exact prerelease version such as
`@1.2.3-beta.4`.

If a bare install spec matches a bundled plugin id (for example `diffs`), SkillSet
...
```bash
skillset plugins install clawhub:skillset-codex-app-server
skillset plugins install clawhub:skillset-codex-app-server@1.2.3
```

SkillSet now also prefers ClawHub for bare npm-safe plugin specs. It only falls
back to npm if ClawHub does not have that package or version:

```bash
skillset plugins install skillset-codex-app-server
```

SkillSet downloads the package archive from ClawHub, checks the advertised
...
```bash
skillset plugins marketplace list <marketplace-name>
skillset plugins install <plugin-name>@<marketplace-name>
```

Use `--marketplace` when you want to pass the marketplace source explicitly:

```bash
skillset plugins install <plugin-name> --marketplace <marketplace-name>
skillset plugins install <plugin-name> --marketplace <owner/repo>
skillset plugins install <plugin-name> --marketplace https://github.com/<owner>/<repo>
skillset plugins install <plugin-name> --marketplace ./my-marketplace
```
...
inside the cloned marketplace repo. SkillSet accepts relative path sources from
that repo and rejects HTTP(S), absolute-path, git, GitHub, and other non-path
plugin sources from remote manifests.

For local paths and archives, SkillSet auto-detects:

- native SkillSet plugins (`skillset.plugin.json`)
...
```bash
skillset plugins install -l ./my-plugin
```
...
```bash
skillset plugins uninstall <id>
skillset plugins uninstall <id> --dry-run
skillset plugins uninstall <id> --keep-files
```
...
skillset plugins update <id-or-npm-spec>
skillset plugins update --all
skillset plugins update <id-or-npm-spec> --dry-run
skillset plugins update @skillset/voice-call@beta
skillset plugins update skillset-codex-app-server --dangerously-force-unsafe-install
```

Updates apply to tracked installs in `plugins.installs` and tracked hook-pack
installs in `hooks.internal.installs`.

When you pass a plugin id, SkillSet reuses the recorded install spec for that
...
or exact version. SkillSet resolves that package name back to the tracked plugin
...
Before a live npm update, SkillSet checks the installed package version against
...
downloading, reinstalling, or rewriting `skillset.json`.

When a stored integrity hash exists and the fetched artifact hash changes,
SkillSet treats that as npm artifact drift. The interactive
`skillset plugins update` command prints the expected and actual hashes and asks
...
with `SKILLSET_PLUGIN_LOAD_DEBUG=1` to include a compact export-shape summary in
the diagnostic output.

### Marketplace

```bash
skillset plugins marketplace list <source>
skillset plugins marketplace list <source> --json
```
