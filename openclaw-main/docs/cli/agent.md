---
summary: "CLI reference for `skillset agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
skillset agent --to +15555550123 --message "status update" --deliver
...
skillset agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: SkillSet persists markers from the active source config snapshot, not from resolved runtime secret values.
