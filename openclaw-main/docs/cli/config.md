---
summary: "CLI reference for `skillset config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `skillset.json`: get/set/unset/file/schema/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `skillset configure`).

Root options:

- `--section <section>`: repeatable guided-setup section filter when you run `skillset config` without a subcommand
...
skillset config --section model
skillset config --section gateway --section daemon
skillset config schema
skillset config get browser.executablePath
...
skillset config set agents.defaults.models '{"openai-codex/gpt-5.4":{}}' --strict-json --merge
skillset config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
skillset config set secrets.providers.vaultfile --provider-source file --provider-path /etc/skillset/secrets.json --provider-mode json
...
skillset config validate --json
```

### `config schema`

Print the generated JSON schema for `skillset.json` to stdout as JSON.
...
```bash
skillset config schema > skillset.schema.json
```
...
skillset config get agents.list[0].id
```
...
skillset config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
skillset config set agents.defaults.heartbeat.every "0m"
skillset config set gateway.port 19001 --strict-json
skillset config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
skillset config set agents.defaults.models '{"openai-codex/gpt-5.4":{}}' --strict-json --merge
skillset config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
`skillset config set` supports four assignment styles:

1. Value mode: `skillset config set <path> <value>`
2. SecretRef builder mode:

```bash
skillset config set channels.discord.token \
...
```bash
skillset config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/skillset-vault \
  --provider-arg read \
...
```bash
skillset config set --batch-json '[
  {
...
```bash
skillset config set --batch-file ./config-set.batch.json --dry-run
```
...
skillset config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/skillset/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `skillset.json`.
...
```json
{
  "ok": true,
  "operations": 1,
  "configPath": "~/.skillset/skillset.json",
  "inputModes": ["builder"],
  "checks": {
    "schema": false,
    "resolvability": true,
...
`skillset config set` and other SkillSet-owned config writers validate the full
...
and the rejected payload is saved beside it as `skillset.json.rejected.*`.
The active config path must be a regular file. Symlinked `skillset.json`
layouts are unsupported for writes; use `SKILLSET_CONFIG_PATH` to point directly
at the real file instead.

Prefer CLI writes for small edits:

```bash
skillset config set gateway.reload.mode hybrid --dry-run
skillset config set gateway.reload.mode hybrid
skillset config validate
```
...
- `config file`: Print the active config file path (resolved from `SKILLSET_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `skillset config validate` is passing, you can use the local TUI to have
an embedded agent compare the active config against the docs while you validate
each change from the same terminal:

If validation is already failing, start with `skillset configure` or
`skillset doctor --fix`. `skillset chat` does not bypass the invalid-config
guard.
...
- Apply targeted edits with `skillset config set` or `skillset configure`.
- Rerun `skillset config validate` after each change.
- If validation passes but the runtime is still unhealthy, run `skillset doctor` or `skillset doctor --fix` for migration and repair help.
