...
when SkillSet loads an installed package and let npm packages avoid runtime
TypeScript compilation. If an installed package only declares a TypeScript
source entry, SkillSet will use a matching built `dist/*.js` peer when one
...
**Import:** `skillset/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `SkillSetPluginConfigSchema \| () => SkillSetPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: SkillSetPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `skillset.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- SkillSet resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `skillset/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: SkillSetPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: SkillSetPluginApi) => void`                               | No       | —                   |
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and SkillSet
...
SkillSet loads this instead of the full entry when a channel is disabled,
...
- `skillset/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `skillset/plugin-sdk/channel-setup` for optional-install setup surfaces
- `skillset/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`skillset/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "skillset/plugin-sdk/channel-entry-contract";
...
- use `descriptors` when the registrar owns one or more root commands and you
  want SkillSet to lazy-load the real CLI module on first invocation
...
SkillSet classifies loaded plugins by their registration behavior:
...
Use `skillset plugins inspect <id>` to see a plugin's shape.
