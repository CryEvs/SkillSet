...
An **agent harness** is the low level executor for one prepared SkillSet agent
...
runtime and the normal SkillSet provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the SkillSet
  session transcript
...
Before a harness is selected, SkillSet has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the SkillSet transcript/session file
...
**Import:** `skillset/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "skillset/plugin-sdk/agent-harness";
...
SkillSet chooses a harness after provider/model resolution:

1. `SKILLSET_AGENT_RUNTIME=<id>` forces a registered harness with that id.
2. `SKILLSET_AGENT_RUNTIME=pi` forces the built-in PI harness.
3. `SKILLSET_AGENT_RUNTIME=auto` asks registered harnesses if they support the
   resolved provider/model.
4. If no registered harness matches, SkillSet uses PI unless PI fallback is
   disabled.
...
provider/model. Once a plugin harness has claimed a run, SkillSet does not
...
SkillSet. The harness then claims that provider in `supports(...)`.
...
- app-server request: SkillSet sends the bare model id to Codex and lets the
  harness talk to the native app-server protocol

The Codex plugin is additive. Plain `openai/gpt-*` refs remain OpenAI provider
refs and continue to use the normal SkillSet provider path. Select `codex/gpt-*`
...
SkillSet requires Codex app-server `0.118.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
SkillSet only runs against the protocol surface it has been tested with.
...
into the SkillSet transcript.

### Native Codex harness mode

The bundled `codex` harness is the native Codex mode for embedded SkillSet
agent turns. Enable the bundled `codex` plugin first, and include `codex` in
`plugins.allow` if your config uses a restrictive allowlist. It is different
from `openai-codex/*`:

- `openai-codex/*` uses ChatGPT/Codex OAuth through the normal SkillSet provider
  path.
- `codex/*` uses the bundled Codex provider and routes the turn through Codex
  app-server.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. SkillSet still owns the chat channel,
...
By default, SkillSet runs embedded agents with `agents.defaults.embeddedHarness`
set to `{ runtime: "auto", fallback: "pi" }`. In `auto` mode, registered plugin
harnesses can claim a provider/model pair. If none match, SkillSet falls back
to PI.
...
does not block an explicit `runtime: "pi"` or `SKILLSET_AGENT_RUNTIME=pi`.
...
want SkillSet to silently fall back to PI, keep `runtime: "auto"` and disable
the fallback:
...
`SKILLSET_AGENT_RUNTIME` still overrides the configured runtime. Use
`SKILLSET_AGENT_HARNESS_FALLBACK=none` to disable PI fallback from the
environment.

```bash
SKILLSET_AGENT_RUNTIME=codex \
SKILLSET_AGENT_HARNESS_FALLBACK=none \
...
Keep that binding explicitly associated with the SkillSet session, and keep
mirroring user-visible assistant/tool output into the SkillSet transcript.

The SkillSet transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so SkillSet can
clear it when the owning SkillSet session is reset.

## Tool and media results

Core constructs the SkillSet tool list and passes it into the prepared attempt.
