...
Message presentation is SkillSet's shared contract for rich outbound chat UI.
...
```bash
skillset message send --channel slack \
  --target channel:C123 \
...
```bash
skillset message send --channel telegram \
...
Use helpers from `skillset/plugin-sdk/interactive-runtime` when bridging older
code:
