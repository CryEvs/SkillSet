...
This guide summarizes a sane workflow for working on the pi integration in SkillSet.
...
```bash
SKILLSET_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm skillset agent --message "Hello" --thinking low`
...
State lives under the SkillSet state directory. Default is `~/.skillset`. If `SKILLSET_STATE_DIR` is set, use that directory instead.

To reset everything:

- `skillset.json` for config
