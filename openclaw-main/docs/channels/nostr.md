...
read_when:
  - You want SkillSet to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables SkillSet to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current SkillSet releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`skillset onboard`) and `skillset channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
skillset plugins install @skillset/nostr
```

Use a local checkout (dev workflows):

```bash
skillset plugins install --link <path-to-local-nostr-plugin>
```
...
skillset channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "skillset@example.com",
        lud16: "skillset@example.com",
      },
    },
  },
}
