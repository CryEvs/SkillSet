...
```
    ...
    Telegram does **not** use `skillset channels login telegram`; configure token in config/env, then start gateway.
...
skillset pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `skillset doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `skillset doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
...
```
    ...
    - or read `chat.id` from `skillset logs --follow`
...
- DM messages can carry `message_thread_id`; SkillSet routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    SkillSet can stream partial replies in real time:
    ...
    - DM: SkillSet keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: SkillSet keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), SkillSet falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, SkillSet skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, SkillSet automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, SkillSet retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**: `/acp spawn <agent> --thread here|auto` binds the current topic to a new ACP session; follow-ups route there directly. SkillSet pins the spawn confirmation in-topic. Requires `channels.telegram.threadBindings.spawnAcpSessions=true`.
...
```
    ...
    - `~/.skillset/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, SkillSet enqueues system events like:
...
```bash
skillset message send --channel telegram --target 123456789 --message "hi"
skillset message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `skillset message poll` and support forum topics:

```bash
skillset message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
skillset message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat; only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, SkillSet preserves the topic for the approval prompt and the follow-up. Exec approvals expire after 30 minutes by default.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `skillset channels status` warns when config expects unmentioned group messages.
    - `skillset channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `skillset logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, SkillSet now retries these as recoverable network errors.
    - If logs include `Polling stall detected`, SkillSet restarts polling and rebuilds the Telegram transport after 120 seconds without completed long-poll liveness by default.
...
```
    ...
    - Environment overrides (temporary):
      - `SKILLSET_TELEGRAM_DISABLE_AUTO_SELECT_FAMILY=1`
      - `SKILLSET_TELEGRAM_ENABLE_AUTO_SELECT_FAMILY=1`
      - `SKILLSET_TELEGRAM_DNS_RESULT_ORDER=ipv4first`
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `skillset doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `skillset doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, SkillSet falls back to the first normalized account ID and `skillset doctor` warns.
