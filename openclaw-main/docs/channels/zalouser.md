...
read_when:
  - Setting up Zalo Personal for SkillSet
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside SkillSet.

> **Warning:** This is an unofficial integration and may result in account suspension/ban. Use at your own risk.

## Bundled plugin

Zalo Personal ships as a bundled plugin in current SkillSet releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `skillset plugins install @skillset/zalouser`
- Or from a source checkout: `skillset plugins install ./path/to/local/zalouser-plugin`
...
- `skillset pairing list zalouser`
- `skillset pairing approve zalouser <code>`
...
- On startup, SkillSet resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, SkillSet stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in SkillSet state. Example:
...
- SkillSet sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, SkillSet sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `skillset channels logout --channel zalouser && skillset channels login --channel zalouser`
...
- The channel now runs fully in SkillSet without external CLI binaries.
