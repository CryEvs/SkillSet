...
- Same path: run the app from a fixed location (for SkillSet, `dist/SkillSet.app`).
...
```bash
sudo tccutil reset Accessibility ai.skillset.mac
sudo tccutil reset ScreenCapture ai.skillset.mac
...
Workaround: move files into the SkillSet workspace (`~/.skillset/workspace`) if you want to avoid per-folder grants.
