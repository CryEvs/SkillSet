...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `skillset.internal.`), **or**
...
Run `skillset devices list` again before approval.

4. Verify connection:

```bash
skillset nodes status
skillset gateway call node.list --params "{}"
```
...
- `SKILLSET_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine SkillSet iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight SkillSet builds.
...
```bash
export SKILLSET_APNS_TEAM_ID="TEAMID"
export SKILLSET_APNS_KEY_ID="KEYID"
export SKILLSET_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.skillset/credentials/apns
chmod 700 ~/.skillset/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.skillset/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.skillset/credentials/apns/AuthKey_KEYID.p8
export SKILLSET_APNS_PRIVATE_KEY_PATH="$HOME/.skillset/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_skillset-gw._tcp` on `local.` and, when configured, the same
...
`skillset.internal.`) and Tailscale split DNS.
...
```bash
skillset nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__skillset__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__skillset__/canvas/` and `/__skillset__/a2ui/`.
...
```bash
skillset nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__skillset; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
skillset nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `skillset devices list` and approve manually.
