---
summary: "How SkillSet builds prompt context and reports token usage + costs"
read_when:
...
SkillSet tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

SkillSet assembles its own system prompt on every run. It includes:
...
For images, SkillSet downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from SkillSet session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `skillset status --usage` and `skillset channels list` show
...
available; otherwise SkillSet falls back to matching OAuth/API-key credentials
...
`cacheWrite`. If pricing is missing, SkillSet shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. SkillSet can
...
Anthropic's 1M context window is currently beta-gated. SkillSet can inject the
...
SkillSet skips the `context-1m-*` beta header because Anthropic currently
