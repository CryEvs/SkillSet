...
SkillSet has three public release lanes:
...
- Release checks now run in a separate manual workflow:
  `SkillSet Release Checks`
- `SkillSet Release Checks` also runs the QA Lab mock parity gate plus the live
...
- Cross-OS install and upgrade runtime validation is dispatched from the
  ...
  `skillset/releases-private/.github/workflows/skillset-cross-os-release-checks.yml`,
  which invokes the reusable public workflow
  `.github/workflows/skillset-cross-os-release-checks-reusable.yml`
...
- `SkillSet NPM Release` validation-only preflight also accepts the current
...
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/skillset-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/skillset-npm-postpublish-verify.ts YYYY.M.D`
...
`SkillSet NPM Release` accepts these operator-controlled inputs:
...
`SkillSet Release Checks` accepts these operator-controlled inputs:
...
- For `SkillSet NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `SkillSet Release Checks` is always validation-only and also accepts the
...
1. Run `SkillSet NPM Release` with `preflight_only=true`
...
3. Run `SkillSet Release Checks` separately with the same tag or the
...
5. Run `SkillSet NPM Release` again with `preflight_only=false`, the same
...
- [`.github/workflows/skillset-npm-release.yml`](https://github.com/skillset/skillset/blob/main/.github/workflows/skillset-npm-release.yml)
- [`.github/workflows/skillset-release-checks.yml`](https://github.com/skillset/skillset/blob/main/.github/workflows/skillset-release-checks.yml)
- [`.github/workflows/skillset-cross-os-release-checks-reusable.yml`](https://github.com/skillset/skillset/blob/main/.github/workflows/skillset-cross-os-release-checks-reusable.yml)
- [`scripts/skillset-npm-release-check.ts`](https://github.com/skillset/skillset/blob/main/scripts/skillset-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/skillset/skillset/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/skillset/skillset/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`skillset/maintainers/release/README.md`](https://github.com/skillset/maintainers/blob/main/release/README.md)
