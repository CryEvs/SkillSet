...
Prompt caching means the model provider can reuse unchanged prompt prefixes (usually system/developer instructions and other stable context) across turns instead of re-processing them every time. SkillSet normalizes provider usage into `cacheRead` and `cacheWrite` where the upstream API exposes those counters directly.
...
- With Anthropic API-key auth profiles, SkillSet seeds `cacheRetention: "short"` for Anthropic model refs when unset.
- Anthropic native Messages responses expose both `cache_read_input_tokens` and `cache_creation_input_tokens`, so SkillSet can show both `cacheRead` and `cacheWrite`.
...
- Prompt caching is automatic on supported recent models. SkillSet does not need to inject block-level cache markers.
- SkillSet uses `prompt_cache_key` to keep cache routing stable across turns and uses `prompt_cache_retention: "24h"` only when `cacheRetention: "long"` is selected on direct OpenAI hosts.
- OpenAI responses expose cached prompt tokens via `usage.prompt_tokens_details.cached_tokens` (or `input_tokens_details.cached_tokens` on Responses API events). SkillSet maps that to `cacheRead`.
...
For `openrouter/anthropic/*` model refs, SkillSet injects Anthropic
...
If you repoint the model at an arbitrary OpenAI-compatible proxy URL, SkillSet
...
- Direct Gemini transport (`api: "google-generative-ai"`) reports cache hits
  through upstream `cachedContentTokenCount`; SkillSet maps that to `cacheRead`.
- When `cacheRetention` is set on a direct Gemini model, SkillSet automatically
...
- This is separate from Anthropic/OpenAI prompt-prefix caching. For Gemini,
  SkillSet manages a provider-native `cachedContents` resource rather than
...
- Gemini CLI JSON output can also surface cache hits through `stats.cached`;
  SkillSet maps that to `cacheRead`.
- If the CLI omits a direct `stats.input` value, SkillSet derives input tokens
  from `stats.input_tokens - stats.cached`.
- This is usage normalization only. It does not mean SkillSet is creating
  Anthropic/OpenAI-style prompt-cache markers for Gemini CLI.

## System-prompt cache boundary

SkillSet splits the system prompt into a **stable prefix** and a **volatile
...
## SkillSet cache-stability guards

SkillSet also keeps several cache-sensitive payload shapes deterministic before
...
SkillSet exposes dedicated cache-trace diagnostics for embedded agent runs.
...
SkillSet keeps one combined live cache regression gate for repeated prefixes, tool turns, image turns, MCP-style tool transcripts, and an Anthropic no-cache control.
...
```sh
SKILLSET_LIVE_TEST=1 SKILLSET_LIVE_CACHE_TEST=1 pnpm test:live:cache
```
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.skillset/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$SKILLSET_STATE_DIR/logs/cache-trace.jsonl`
...
- `SKILLSET_CACHE_TRACE=1` enables cache tracing.
- `SKILLSET_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `SKILLSET_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `SKILLSET_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `SKILLSET_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
...
- If you need request tracing, log request IDs and rate-limit headers separately from cache metrics. SkillSet's current cache-trace output is focused on prompt/session shape and normalized token usage rather than raw provider response headers.
