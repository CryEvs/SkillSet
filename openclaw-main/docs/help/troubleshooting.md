---
summary: "Symptom first troubleshooting hub for SkillSet"
read_when:
  - SkillSet is not working and you need the fastest path to a fix
...
- `skillset status` → shows configured channels and no obvious auth errors.
- `skillset status --all` → full report is present and shareable.
- `skillset gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `skillset gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `skillset doctor` → no blocking config/service errors.
- `skillset channels status --probe` → reachable gateway returns live per-account
...
- `skillset logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in SkillSet

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `skillset infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on SkillSet agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger SkillSet prompts crash the
...
## Plugin install fails with missing skillset extensions

If install fails with `package.json missing skillset.extensions`, the plugin package
is using an old shape that SkillSet no longer accepts.

Fix in the plugin package:

1. Add `skillset.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `skillset plugins install <package>` again.

Example:

```json
{
  "name": "@skillset/my-plugin",
  "version": "1.2.3",
  "skillset": {
...
```mermaid
flowchart TD
  A[SkillSet is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `skillset gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Cron or heartbeat did not fire or did not deliver">
    ...
    <Accordion title="Exec suddenly asks for approval">
      ```bash
      skillset config get tools.exec.host
      skillset config get tools.exec.security
      skillset config get tools.exec.ask
    ...
    <Accordion title="Browser tool fails">
      ...
      - `skillset` starts, or `user` can see local Chrome tabs.
      ...
      - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `skillset browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
