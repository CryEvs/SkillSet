...
vi.mock("skillset/plugin-sdk/qa-channel", async () => await import("../../qa-channel/api.js"));
...
vi.mock("skillset/plugin-sdk/proxy-capture", () => ({
  getDebugProxyCaptureStore: () => captureMock.store,
  resolveDebugProxySettings: () => ({
    dbPath: process.env.SKILLSET_DEBUG_PROXY_DB_PATH ?? "",
    blobDir: process.env.SKILLSET_DEBUG_PROXY_BLOB_DIR ?? "",
    proxyUrl: process.env.SKILLSET_DEBUG_PROXY_URL ?? "",
...
describe("qa-lab server", () => {
  ...
  it("aborts an in-flight runner model catalog when the lab stops", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "  process.exit(0);",
        "});",
        `fs.writeFileSync(${JSON.stringify(markerPath)}, process.env.SKILLSET_CODEX_DISCOVERY_LIVE || "", "utf8");`,
        "setInterval(() => {}, 1000);",
      ].join("\n"),
      "utf8",
    );
  ...
  it("serves proxy capture sessions, events, and query rows", async () => {
    ...
    process.env.SKILLSET_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.SKILLSET_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    const store = captureMock.store;
    store.upsertSession({
      id: "qa-capture-session",
      startedAt: Date.now(),
      mode: "proxy-run",
      sourceScope: "skillset",
    ...
    cleanups.push(async () => {
      delete process.env.SKILLSET_DEBUG_PROXY_DB_PATH;
      delete process.env.SKILLSET_DEBUG_PROXY_BLOB_DIR;
      await lab.stop();
    });
