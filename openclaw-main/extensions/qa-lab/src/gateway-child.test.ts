...
vi.mock("skillset/plugin-sdk/ssrf-runtime", () => ({
  fetchWithSsrFGuard: fetchWithSsrFGuardMock,
}));

vi.mock("skillset/plugin-sdk/temp-path", () => ({
  resolvePreferredSkillSetTmpDir: () => qaTempPathState.preferredTmpDir,
}));
...
function createParams(baseEnv?: NodeJS.ProcessEnv) {
  return {
    configPath: "/tmp/skillset-qa/skillset.json",
    gatewayToken: "qa-token",
    homeDir: "/tmp/skillset-qa/home",
    stateDir: "/tmp/skillset-qa/state",
    xdgConfigHome: "/tmp/skillset-qa/xdg-config",
    xdgDataHome: "/tmp/skillset-qa/xdg-data",
    xdgCacheHome: "/tmp/skillset-qa/xdg-cache",
    bundledPluginsDir: "/tmp/skillset-qa/bundled-plugins",
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("keeps the slow-reply QA opt-out enabled under fast mode", () => {
    ...
    expect(env.SKILLSET_TEST_FAST).toBe("1");
    expect(env.SKILLSET_QA_ALLOW_LOCAL_IMAGE_PROVIDER).toBe("1");
    expect(env.SKILLSET_ALLOW_SLOW_REPLY_TESTS).toBe("1");
    expect(env.SKILLSET_BUNDLED_PLUGINS_DIR).toBe("/tmp/skillset-qa/bundled-plugins");
    expect(env.SKILLSET_COMPATIBILITY_HOST_VERSION).toBe("2026.4.8");
  });

  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        SKILLSET_LIVE_OPENAI_KEY: "openai-live",
        SKILLSET_LIVE_ANTHROPIC_KEY: "anthropic-live",
        SKILLSET_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing SkillSet home", async () => {
    ...
    expect(env.HOME).toBe("/tmp/skillset-qa/home");
    expect(env.SKILLSET_HOME).toBe("/tmp/skillset-qa/home");
    expect(env.CODEX_HOME).toBe(codexHome);
  });

  it("forwards host HOME for live Claude CLI runs while keeping SkillSet home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
    ...
    expect(env.SKILLSET_HOME).toBe("/tmp/skillset-qa/home");
    expect(env.SKILLSET_STATE_DIR).toBe("/tmp/skillset-qa/state");
  });

  it("can forward host HOME for browser-backed QA runs while keeping SkillSet home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        SKILLSET_LIVE_ANTHROPIC_KEY: "anthropic-live",
        SKILLSET_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.SKILLSET_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.SKILLSET_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        SKILLSET_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.SKILLSET_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.SKILLSET_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        SKILLSET_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        SKILLSET_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.SKILLSET_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.SKILLSET_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });

  it("does not pass Convex credential broker secrets to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        SKILLSET_QA_CONVEX_SECRET_CI: "convex-ci-secret",
        SKILLSET_QA_CONVEX_SECRET_MAINTAINER: "convex-maintainer-secret",
      }),
      providerMode: "live-frontier",
    });

    expect(env.SKILLSET_QA_CONVEX_SECRET_CI).toBeUndefined();
    expect(env.SKILLSET_QA_CONVEX_SECRET_MAINTAINER).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          SKILLSET_LIVE_ANTHROPIC_KEY: "anthropic-live",
          SKILLSET_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          SKILLSET_LIVE_GEMINI_KEY: "gemini-live",
          SKILLSET_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.SKILLSET_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.SKILLSET_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.SKILLSET_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.SKILLSET_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await __testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        SKILLSET_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("preserves only sanitized gateway debug artifacts", async () => {
    ...
    await writeFile(
      stdoutLogPath,
      [
        "SKILLSET_GATEWAY_TOKEN=qa-suite-token",
        'OPENAI_API_KEY="openai-live"',
        "SKILLSET_QA_CONVEX_SECRET_CI=convex-ci-secret",
        "SKILLSET_QA_CONVEX_SECRET_MAINTAINER=convex-maintainer-secret",
    ...
    await expect(readFile(path.join(artifactDir, "gateway.stdout.log"), "utf8")).resolves.toBe(
      [
        "SKILLSET_GATEWAY_TOKEN=<redacted>",
        "OPENAI_API_KEY=<redacted>",
        "SKILLSET_QA_CONVEX_SECRET_CI=<redacted>",
        "SKILLSET_QA_CONVEX_SECRET_MAINTAINER=<redacted>",
        "botToken=<redacted>",
  ...
  it("rejects preserved gateway artifacts outside the repo root", async () => {
    await expect(
      __testing.assertQaArtifactDirWithinRepo("/tmp/skillset-repo", "/tmp/outside"),
  ...
  it("cleans startup temp roots when they are not preserved", async () => {
    ...
    await writeFile(path.join(tempRoot, "skillset.json"), "{}", "utf8");
...
describe("qa bundled plugin dir", () => {
  ...
  it("uses a source bundled plugin when the built copy is missing CLI metadata", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "memory-core", "skillset.plugin.json"),
    ...
    await writeFile(
      path.join(repoRoot, "extensions", "memory-core", "skillset.plugin.json"),
      JSON.stringify({ id: "memory-core", kind: "memory" }),
      "utf8",
    );
    await writeFile(
  ...
  it("creates a scoped bundled plugin tree for allowed plugins plus always-allowed runtime facades", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "package.json"),
      JSON.stringify({ name: "@skillset/qa-channel", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("preserves dist-runtime-only root chunks when dist also exists", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "package.json"),
      JSON.stringify({ name: "skillset", type: "module" }, null, 2),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@skillset/runtime-only", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("maps cli backend provider ids to their owning bundled plugin ids", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "openai", "skillset.plugin.json"),
      JSON.stringify({
        id: "openai",
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    await expect(
      __testing.readQaLiveProviderConfigOverrides({
        providerIds: ["custom-openai"],
        env: { SKILLSET_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
      }),
    ).resolves.toEqual({
  ...
  it("raises the QA runtime host version to the highest allowed plugin floor", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ skillset: { install: { minHostVersion: ">=2026.4.8" } } }),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ skillset: { install: { minHostVersion: ">=2026.4.7" } } }),
      "utf8",
    );
  ...
  it("includes always-allowed runtime facade plugins when raising the QA runtime host version", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ skillset: { install: { minHostVersion: ">=2026.4.9" } } }),
      "utf8",
    );
