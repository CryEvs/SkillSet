...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/skillset-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/skillset";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/skillset-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `skillset-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "SKILLSET_CONFIG_PATH" &&
          key !== "SKILLSET_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `SKILLSET_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `SKILLSET_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  await writeFile(
    plan.hostLogPath,
    `# SkillSet QA Multipass host log\nvmName=${plan.vmName}\noutputDir=${plan.outputDir}\n\n`,
    "utf8",
  );
  await writeFile(
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm skillset qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredSkillSetTmpDir(), `${plan.vmName}-qa-suite-`),
  );
