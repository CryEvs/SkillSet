...
function resolvePersistentTranscriptBaseDir(api: SkillSetPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: SkillSetPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: SkillSetConfig): boolean {
...
async function runRecallSubagent(params: {
  ...
  const tempDir = params.config.persistTranscripts
    ? undefined
    : await fs.mkdtemp(path.join(resolvePreferredSkillSetTmpDir(), "skillset-active-memory-"));
