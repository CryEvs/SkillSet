...
import { readJsonFileWithFallback, writeJsonFileAtomically } from "skillset/plugin-sdk/json-store";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  // Explicit SKILLSET_STATE_DIR overrides take precedence (including
  // per-test mkdtemp dirs in this module's test suite).
  if (env.SKILLSET_STATE_DIR?.trim()) {
    return resolveStateDir(env);
  }
  // Default test isolation: per-pid tmpdir, no bleed into real ~/.skillset.
  // Use resolvePreferredSkillSetTmpDir + string concat (mirrors
  ...
  if (env.VITEST || env.NODE_ENV === "test") {
    const name = "skillset-vitest-" + process.pid;
    return path.join(resolvePreferredSkillSetTmpDir(), name);
  }
  // Canonical SkillSet state dir: honors `~` expansion + legacy/new
