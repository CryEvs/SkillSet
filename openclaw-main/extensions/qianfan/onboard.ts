...
function resolveQianfanPreset(cfg: SkillSetConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: SkillSetConfig) => {
...
export function applyQianfanProviderConfig(cfg: SkillSetConfig): SkillSetConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: SkillSetConfig): SkillSetConfig {
