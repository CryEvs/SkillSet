...
export const SLACK_REPLY_BUTTON_ACTION_ID = "skillset:reply_button";
export const SLACK_REPLY_SELECT_ACTION_ID = "skillset:reply_select";
...
export function buildSlackInteractiveBlocks(interactive?: InteractiveReply): SlackBlock[] {
  ...
  return reduceInteractiveReply(interactive, initialState, (state, block) => {
    ...
    if (block.type === "buttons") {
      ...
      state.blocks.push({
        type: "actions",
        block_id: `skillset_reply_buttons_${++state.buttonIndex}`,
        elements,
      });
      return state;
    }
    ...
    state.blocks.push({
      type: "actions",
      block_id: `skillset_reply_select_${++state.selectIndex}`,
      elements: [
        {
          type: "static_select",
