...
import { patchChannelConfigForAccount } from "skillset/plugin-sdk/setup-runtime";
...
import { getChatChannelMeta, type ChannelPlugin, type SkillSetConfig } from "./channel-api.js";
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "SkillSet";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for SkillSet`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "SkillSet"): string[] {
  return [
