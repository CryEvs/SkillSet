...
const SKILLSET_PLUGIN_SDK_PACKAGE_NAMES = [
  ["skillset", "plugin-sdk"].join("/"),
  ["@skillset", "plugin-sdk"].join("/"),
];
...
function hasTrustedSkillSetRootIndicator(packageRoot, packageJson) {
  ...
  const hasSkillSetBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("skillset")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.skillset === "string");
  const hasSkillSetEntrypoint = fs.existsSync(path.join(packageRoot, "skillset.mjs"));
  return hasCliEntryExport || hasSkillSetBin || hasSkillSetEntrypoint;
}

function findSkillSetPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "skillset" && hasTrustedSkillSetRootIndicator(cursor, pkg)) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findSkillSetPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    for (const packageName of SKILLSET_PLUGIN_SDK_PACKAGE_NAMES) {
  ...
  if (extensionApi) {
    aliasMap["skillset/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findSkillSetPackageRoot(moduleDir);
  if (location) {
