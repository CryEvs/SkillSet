...
import * as jsonStore from "skillset/plugin-sdk/json-store";
...
describe("FileBackedMatrixSyncStore", () => {
  ...
  function createStoragePath(): string {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-matrix-sync-store-"));
