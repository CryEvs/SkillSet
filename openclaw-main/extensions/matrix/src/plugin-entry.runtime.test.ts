...
const matrixWrapperGlobal = globalThis as typeof globalThis & {
  __skillsetMatrixWrapperJitiOptions?: unknown;
};
const PLUGIN_SDK_ROOT = ["skillset", "plugin-sdk"].join("/");
const SCOPED_PLUGIN_SDK_ROOT = ["@skillset", "plugin-sdk"].join("/");
...
function writeCapturingJitiFixture(fixtureRoot: string) {
  writeFixtureFile(
    ...
    [
      ...
      "  globalThis.__skillsetMatrixWrapperJitiOptions = options;",
...
function writeSkillSetPackageFixture(fixtureRoot: string) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
  ...
  writeFixtureFile(fixtureRoot, "skillset.mjs", "export {};\n");
  writeFixtureFile(fixtureRoot, "dist/plugin-sdk/index.js", "export {};\n");
}

function writeSkillSetAliasFixture(fixtureRoot: string, extraExports?: Record<string, string>) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
...
function writeTrustedSkillSetBinFixture(
  fixtureRoot: string,
...
function writeCapturingSourceRuntimeWrapperFixture(fixtureRoot: string) {
  delete matrixWrapperGlobal.__skillsetMatrixWrapperJitiOptions;
  writeSkillSetAliasFixture(fixtureRoot);
...
function expectSourcePluginSdkAliases(fixtureRoot: string) {
  expect(matrixWrapperGlobal.__skillsetMatrixWrapperJitiOptions).toMatchObject({
    alias: {
...
it("loads the source-checkout runtime wrapper through native ESM import", async () => {
  ...
  writeSkillSetPackageFixture(fixtureRoot);
...
it("resolves extension-api aliases through the same source extension family", async () => {
  ...
  expect(matrixWrapperGlobal.__skillsetMatrixWrapperJitiOptions).toMatchObject({
    alias: {
      "skillset/extension-api": path.join(fixtureRoot, "src", "extensionAPI.mts"),
    },
  });
}, 240_000);
...
it("keeps wrapper plugin-sdk aliases deterministic and ignores unsafe subpaths", async () => {
  ...
  const aliasKeys = Object.keys(
    (
      (matrixWrapperGlobal.__skillsetMatrixWrapperJitiOptions ?? {}) as {
...
it("ignores nearby untrusted skillset package stubs when resolving the wrapper root", async () => {
...
it("treats string bin hints case-insensitively when trusting wrapper package roots", async () => {
  ...
  delete matrixWrapperGlobal.__skillsetMatrixWrapperJitiOptions;
  writeTrustedSkillSetBinFixture(fixtureRoot, "SkillSet.MJS");
  writeCapturingJitiFixture(fixtureRoot);
  writeSourceRuntimeWrapperFixture(fixtureRoot);
  await importFixtureModule(fixtureRoot, "extensions/matrix/src/plugin-entry.runtime.js");
