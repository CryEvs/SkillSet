import { type ChannelDoctorAdapter } from "skillset/plugin-sdk/channel-contract";
import type { SkillSetConfig } from "skillset/plugin-sdk/config-runtime";
import {
  detectPluginInstallPathIssue,
  formatPluginInstallPathIssue,
  removePluginFromConfig,
} from "skillset/plugin-sdk/runtime-doctor";
import {
...
function hasConfiguredMatrixChannel(cfg: SkillSetConfig): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
  return isRecord(channels?.matrix);
}

function hasConfiguredMatrixPluginSurface(cfg: SkillSetConfig): boolean {
  return Boolean(
...
function configMayNeedMatrixDoctorSequence(cfg: SkillSetConfig, env: NodeJS.ProcessEnv): boolean {
  return (
...
export function formatMatrixLegacyStatePreview(
  ...
  return [
    ...
    '- Run "skillset doctor --fix" to migrate this Matrix state now.',
  ].join("\n");
}
  ...
  for (const plan of detection.plans) {
    notes.push(
      [
        ...
        '- Run "skillset doctor --fix" to extract any saved backup key now. Backed-up room keys will restore automatically on next gateway start.',
      ].join("\n"),
    );
  }
  return notes;
...
export async function collectMatrixInstallPathWarnings(cfg: SkillSetConfig): Promise<string[]> {
  ...
  return formatPluginInstallPathIssue({
    issue,
    pluginLabel: "Matrix",
    defaultInstallCommand: "skillset plugins install @skillset/matrix",
  }).map((entry) => `- ${entry}`);
}

export async function cleanStaleMatrixPluginConfig(cfg: SkillSetConfig) {
  const issue = await detectPluginInstallPathIssue({
    pluginId: "matrix",
    install: cfg.plugins?.installs?.matrix,
  });
