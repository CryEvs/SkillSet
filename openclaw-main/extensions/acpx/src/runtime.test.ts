...
const DOCUMENTED_SKILLSET_BRIDGE_COMMAND =
  "env SKILLSET_HIDE_BANNER=1 SKILLSET_SUPPRESS_NOTES=1 skillset acp --url ws://127.0.0.1:18789 --token-file ~/.skillset/gateway.token --session agent:main:main";
...
function makeRuntime(
  ...
  const runtime = new AcpxRuntime({
    ...
    agentRegistry: {
      resolve: (agentName: string) => (agentName === "skillset" ? "skillset acp" : agentName),
      list: () => ["codex", "skillset"],
    },
...
describe("AcpxRuntime fresh reset wrapper", () => {
  ...
  it("routes skillset ensureSession through the bridge-safe delegate when MCP servers are configured", async () => {
    const baseStore: TestSessionStore = {
    ...
    const bridgeEnsure = vi.spyOn(bridgeSafeDelegate, "ensureSession").mockResolvedValue({
      sessionKey: "agent:skillset:acp:test",
      backend: "acpx",
  ...
  it("routes non-skillset sessions through the default delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
  ...
  it("routes handle-based follow-up calls for skillset sessions through the bridge-safe delegate", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const handle: Parameters<NonNullable<AcpRuntime["getStatus"]>>[0]["handle"] = {
      sessionKey: "agent:skillset:acp:test",
      backend: "acpx",
      runtimeSessionName: "skillset-session-handle",
    };
  ...
  it("keeps MCP-enabled routing when the skillset agent is overridden to a non-bridge adapter", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "skillset" ? "codex" : agentName),
        list: () => ["codex", "skillset"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for any agent mapped to the skillset bridge command", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      mcpServers: [{ name: "tools", command: "mcp-tools" }] as never,
      agentRegistry: {
        resolve: (agentName: string) => (agentName === "codex" ? "skillset acp" : agentName),
        list: () => ["codex", "skillset"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for documented env-wrapped skillset bridge commands", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "skillset" ? DOCUMENTED_SKILLSET_BRIDGE_COMMAND : agentName,
        list: () => ["codex", "skillset"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("uses the bridge-safe delegate for local node skillset entrypoints", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
    ...
    const { runtime, delegate, bridgeSafeDelegate } = makeRuntime(baseStore, {
      ...
      agentRegistry: {
        resolve: (agentName: string) =>
          agentName === "skillset" ? "env SKILLSET_HIDE_BANNER=1 node skillset.mjs acp" : agentName,
        list: () => ["codex", "skillset"],
      },
    });
    const defaultEnsure = vi.spyOn(delegate, "ensureSession").mockResolvedValue({
  ...
  it("routes follow-up calls by persisted agent command before current config", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => ({
        acpxRecordId: "agent:skillset:acp:test",
        agentCommand: DOCUMENTED_SKILLSET_BRIDGE_COMMAND,
      })),
      save: vi.fn(async () => {}),
    };
    ...
    const status = await runtime.getStatus({
      handle: {
        sessionKey: "agent:skillset:acp:test",
        backend: "acpx",
        runtimeSessionName: "agent:skillset:acp:test",
      },
    });
  ...
  it("probes through the bridge-safe delegate when probeAgent resolves to skillset bridge", async () => {
    const baseStore: TestSessionStore = {
      load: vi.fn(async () => undefined),
      save: vi.fn(async () => {}),
    };
