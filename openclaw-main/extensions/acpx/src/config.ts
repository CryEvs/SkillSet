...
import { formatPluginConfigIssue } from "skillset/plugin-sdk/extension-shared";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "skillset-plugin-tools";
export const ACPX_SKILLSET_TOOLS_MCP_SERVER_NAME = "skillset-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "skillset.plugin.json")) &&
...
function resolveAcpxPluginRootFromSkillSetLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common SkillSet layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromSkillSetLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveSkillSetRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const openClawRoot = resolveSkillSetRoot(pluginRoot);
...
export function resolveSkillSetToolsMcpServerConfig(
  moduleUrl: string = import.meta.url,
): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const openClawRoot = resolveSkillSetRoot(pluginRoot);
  const distEntry = path.join(openClawRoot, "dist", "mcp", "skillset-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(openClawRoot, "src", "mcp", "skillset-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.openClawToolsMcpBridge && resolved[ACPX_SKILLSET_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_SKILLSET_TOOLS_MCP_SERVER_NAME} is reserved when openClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.openClawToolsMcpBridge) {
    resolved[ACPX_SKILLSET_TOOLS_MCP_SERVER_NAME] = resolveSkillSetToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
