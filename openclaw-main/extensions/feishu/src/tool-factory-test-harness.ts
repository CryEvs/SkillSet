...
export function createToolFactoryHarness(cfg: SkillSetPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<SkillSetPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
