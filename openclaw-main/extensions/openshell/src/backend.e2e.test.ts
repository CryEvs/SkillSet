...
const SKILLSET_OPENSHELL_E2E = process.env.SKILLSET_E2E_OPENSHELL === "1";
const SKILLSET_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const SKILLSET_OPENSHELL_COMMAND =
  process.env.SKILLSET_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "skillset-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && SKILLSET_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: SKILLSET_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(SKILLSET_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(SKILLSET_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-openshell-e2e-"));
      ...
      const gatewayName = `skillset-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `skillset-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/skillset-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: SKILLSET_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("skillset-openshell-e2e");
