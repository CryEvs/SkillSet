...
import { sleep } from "skillset/plugin-sdk/text-runtime";
...
vi.mock("skillset/plugin-sdk/text-runtime", async () => {
  const actual = await vi.importActual<typeof import("skillset/plugin-sdk/text-runtime")>(
    "skillset/plugin-sdk/text-runtime",
  );
  return {
    ...actual,
    sleep: vi.fn(async () => {}),
