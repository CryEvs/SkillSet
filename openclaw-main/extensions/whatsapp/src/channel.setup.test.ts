import { DEFAULT_ACCOUNT_ID } from "skillset/plugin-sdk/routing";
  ...
  return {
    ...
    splitSetupEntries: (raw: string) =>
      raw
        ...
        .filter(Boolean),
    setSetupChannelEnabled: (cfg: SkillSetConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<SkillSetConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
...
async function runConfigureWithHarness(params: {
  ...
  const result = await finalizeWhatsAppSetup({
    cfg: params.cfg ?? ({} as SkillSetConfig),
