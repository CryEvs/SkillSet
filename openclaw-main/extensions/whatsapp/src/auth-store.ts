...
import { info, success } from "skillset/plugin-sdk/runtime-env";
import { getChildLogger } from "skillset/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "skillset/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("skillset channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
