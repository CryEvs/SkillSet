...
import { requireRuntimeConfig, type SkillSetConfig } from "skillset/plugin-sdk/config-runtime";
import { resolveMarkdownTableMode } from "skillset/plugin-sdk/config-runtime";
import { generateSecureUuid } from "skillset/plugin-sdk/core";
import { normalizePollInput, type PollInput } from "skillset/plugin-sdk/media-runtime";
import { createSubsystemLogger } from "skillset/plugin-sdk/runtime-env";
import { getChildLogger } from "skillset/plugin-sdk/text-runtime";
import { redactIdentifier } from "skillset/plugin-sdk/text-runtime";
import { convertMarkdownTables } from "skillset/plugin-sdk/text-runtime";
import {
...
function requireOutboundActiveWebListener(params: { cfg: SkillSetConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`skillset channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: SkillSetConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
