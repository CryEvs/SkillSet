...
import { redactIdentifier } from "skillset/plugin-sdk/logging-core";
...
let resetLogger: typeof import("skillset/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("skillset/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: SkillSetConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("skillset/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `skillset-outbound-${crypto.randomUUID()}.log`);
