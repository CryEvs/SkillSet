...
function normalizeDenyList(cfg: SkillSetPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: SkillSetPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: SkillSetPluginApi) {
    ...
    const timerService: SkillSetPluginService = {
