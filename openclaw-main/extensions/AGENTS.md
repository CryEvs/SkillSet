...
- Extension production code should import from `skillset/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `skillset.plugin.json` and the package
  `skillset` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `skillset/plugin-sdk/*`.
