...
import { saveAuthProfileStore } from "skillset/plugin-sdk/agent-runtime";
...
describe("bridgeCodexAppServerStartOptions", () => {
  ...
  async function createAgentDirWithDefaultProfile(
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-codex-app-server-"));
  ...
  it("bridges canonical SkillSet oauth into an isolated CODEX_HOME", async () => {
  ...
  it("hydrates Codex-only auth fields from a matching Codex CLI auth file", async () => {
    const sourceCodexHome = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-codex-source-home-"));
