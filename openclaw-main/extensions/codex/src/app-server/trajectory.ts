...
} from "skillset/plugin-sdk/agent-harness";
import { resolveUserPath } from "skillset/plugin-sdk/agent-harness";
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.SKILLSET_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.SKILLSET_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
