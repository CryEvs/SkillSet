...
import { embeddedAgentLog, SKILLSET_VERSION } from "skillset/plugin-sdk/agent-harness-runtime";
...
export function defaultServerRequestResponse(
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "SkillSet did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
  ...
  if (isCodexAppServerApprovalRequest(request.method)) {
    return {
      decision: "decline",
      reason: "SkillSet codex app-server bridge does not grant native approvals yet.",
    };
  }
...
function assertSupportedCodexAppServerVersion(response: CodexInitializeResponse): void {
  ...
  if (!detectedVersion) {
    throw new Error(
      `Codex app-server ${MIN_CODEX_APP_SERVER_VERSION} or newer is required, but SkillSet could not determine the running Codex version. Upgrade Codex CLI and retry.`,
    );
  }
...
export function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // SkillSet, Codex Desktop, or an env override, so only the slash-delimited
