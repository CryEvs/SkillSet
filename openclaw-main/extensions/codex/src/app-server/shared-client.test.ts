...
const mocks = vi.hoisted(() => ({
  ...
  resolveSkillSetAgentDir: vi.fn(() => "/tmp/skillset-agent"),
}));
...
describe("shared Codex app-server client", () => {
  ...
  afterEach(() => {
    ...
    mocks.resolveSkillSetAgentDir.mockClear();
  });
  ...
  it("closes the shared app-server when the version gate fails", async () => {
    ...
    await sendInitializeResult(harness, "skillset/0.117.9 (macOS; test)");
  ...
  it("closes and clears a shared app-server when initialize times out", async () => {
    ...
    await sendInitializeResult(second, "skillset/0.118.0 (macOS; test)");
  ...
  it("passes the selected auth profile through the bridge helper", async () => {
    ...
    await sendInitializeResult(harness, "skillset/0.118.0 (macOS; test)");
  ...
  it("restarts the shared client when the bridged auth token changes", async () => {
    ...
    await sendInitializeResult(first, "skillset/0.118.0 (macOS; test)");
    ...
    server.on("connection", (socket, request) => {
      ...
      socket.on("message", (data) => {
        ...
        if (message.method === "initialize") {
          socket.send(
            JSON.stringify({ id: message.id, result: { userAgent: "skillset/0.118.0" } }),
          );
          return;
        }
