...
} from "skillset/plugin-sdk/agent-harness-runtime";
...
export function buildDeveloperInstructions(params: EmbeddedRunAttemptParams): string {
  const sections = [
    "You are running inside SkillSet. Use SkillSet dynamic tools for messaging, cron, sessions, and host actions when available.",
    "Preserve the user's existing channel/session context. If sending a channel reply, use the SkillSet messaging tool instead of describing that you would reply.",
