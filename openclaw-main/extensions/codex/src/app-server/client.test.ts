...
describe("CodexAppServerClient", () => {
  ...
  it("initializes with the required client version", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "skillset/0.118.0 (macOS; test)" },
    });
  ...
  it("blocks unsupported app-server versions during initialize", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "skillset/0.117.9 (macOS; test)" },
    });
  ...
  it("reads the Codex version from the app-server user agent", () => {
    ...
    expect(readCodexVersionFromUserAgent("skillset/0.118.0 (macOS; test)")).toBe("0.118.0");
    ...
    expect(readCodexVersionFromUserAgent("skillset/0.118.0abc")).toBeUndefined();
