...
import type { EmbeddedRunAttemptParams } from "skillset/plugin-sdk/agent-harness";
...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-codex-projector-"));
