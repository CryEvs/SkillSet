...
import type { SpeechProviderPlugin, SpeechSynthesisRequest } from "skillset/plugin-sdk/speech-core";
...
vi.mock("skillset/plugin-sdk/channel-targets", () => ({
...
function createTtsConfig(prefsName: string): SkillSetConfig {
  return {
    messages: {
      tts: {
        enabled: true,
...
describe("speech-core native voice-note routing", () => {
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    await expectTtsPayloadResult({
      channel: "discord",
      prefsName: "skillset-speech-core-tts-test",
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    await expectTtsPayloadResult({
      channel: "slack",
      prefsName: "skillset-speech-core-tts-slack-test",
