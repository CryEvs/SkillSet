...
function resolveStoredDreaming(config: SkillSetConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: SkillSetConfig = {}) {
  const registered: { command?: SkillSetPluginCommandDefinition } = {};
  let runtimeConfig: SkillSetConfig = initialConfig;

  const runtime = {
    config: {
      loadConfig: vi.fn(() => runtimeConfig),
      writeConfigFile: vi.fn(async (nextConfig: SkillSetConfig) => {
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: SkillSetPluginCommandDefinition) => {
      registered.command = definition;
    }),
  } as unknown as SkillSetPluginApi;
...
describe("memory-core /dreaming command", () => {
  ...
  it("shows a blocked line directly after enabled when main heartbeat is disabled", async () => {
    ...
    expect(text).toContain(
      '- blocked: dreaming is enabled but will not run because heartbeat is disabled for "main". See https://docs.skillset.ai/concepts/dreaming#troubleshooting',
    );
