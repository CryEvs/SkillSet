...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "skillset/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("skillset/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): SkillSetConfig {
  return {
    agents: {
      defaults: {
...
describe("getMemorySearchManager caching", () => {
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-qmd-workspace-"));
