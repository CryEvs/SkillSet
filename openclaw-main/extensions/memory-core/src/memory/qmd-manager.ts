...
import { withFileLock } from "skillset/plugin-sdk/file-lock";
import {
  ...
  writeFileWithinRoot,
  type SkillSetConfig,
} from "skillset/plugin-sdk/memory-core-host-engine-foundation";
import {
  buildSessionEntry,
...
const QMD_UPDATE_QUEUE_KEY = Symbol.for("skillset.qmdUpdateQueueState");
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so SkillSet search semantics match direct qmd search.
  return trimmed;
}
