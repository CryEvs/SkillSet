...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("skillset.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("skillset.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("skillset.qmdEmbedQueueTail");
...
vi.mock("skillset/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("skillset/plugin-sdk/memory-core-host-engine-foundation")
  >("skillset/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("skillset/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("skillset/plugin-sdk/file-lock")>(
    "skillset/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { SkillSetConfig } from "skillset/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
...
describe("QmdMemoryManager", () => {
  ...
  async function createManager(params?: { mode?: "full" | "status"; cfg?: SkillSetConfig }) {
