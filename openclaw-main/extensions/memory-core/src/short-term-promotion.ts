...
import { formatMemoryDreamingDay } from "skillset/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "skillset/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "skillset-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*skillset-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*skillset-memory-promotion:([^\n]+?)\s*-->/gi);
