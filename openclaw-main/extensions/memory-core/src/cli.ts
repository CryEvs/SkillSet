...
import {
  ...
  formatHelpExamples,
  theme,
} from "skillset/plugin-sdk/memory-core-host-runtime-cli";
import type {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["skillset memory status", "Show index and provider status."],
          [
            "skillset memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["skillset memory status --deep", "Probe embedding provider readiness."],
          ["skillset memory index --force", "Force a full reindex."],
          ['skillset memory search "meeting notes"', "Quick search using positional query."],
          [
            'skillset memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `skillset memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "skillset memory rem-backfill --path ./memory",
          ...
          ["skillset memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.skillset.ai/cli/memory")}\n`,
    );
