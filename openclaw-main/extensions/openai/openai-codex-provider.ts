...
import { buildOauthProviderAuthResult } from "skillset/plugin-sdk/provider-auth";
import { loginOpenAICodexOAuth } from "skillset/plugin-sdk/provider-auth-login";
import {
...
import { fetchCodexUsage } from "skillset/plugin-sdk/provider-usage";
import { normalizeLowercaseStringOrEmpty, readStringValue } from "skillset/plugin-sdk/text-runtime";
import {
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.skillset.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `skillset models auth login --provider openai-codex` or `skillset configure`.";
}
