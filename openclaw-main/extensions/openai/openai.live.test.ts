...
import type { ResolvedTtsConfig } from "skillset/plugin-sdk/agent-runtime";
...
import { encodePngRgba, fillPixel } from "skillset/plugin-sdk/media-runtime";
...
const LIVE_MODEL_ID = process.env.SKILLSET_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.4-nano";
const LIVE_IMAGE_MODEL = process.env.SKILLSET_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.SKILLSET_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.SKILLSET_LIVE_TEST === "1";
...
function createLiveConfig(): SkillSetConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "SkillSet integration test OK.",
      cfg,
  ...
  it("transcribes synthesized speech through the registered media provider", async () => {
    ...
    expect(collapsedText).toContain("skillset");
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing SkillSet OpenAI realtime transcription integration test OK.";
    ...
    expect(compact).toContain("skillset");
