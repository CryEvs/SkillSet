...
} from "skillset/plugin-sdk/provider-web-search";
import {
...
function readLegacyGrokFallbackAuth(cfg?: SkillSetConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: SkillSetConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function readLegacyGrokApiKey(cfg?: SkillSetConfig): string | undefined {
...
export function readPluginXaiWebSearchApiKey(cfg?: SkillSetConfig): string | undefined {
  const resolved = readPluginXaiWebSearchApiKeyResult(cfg);
  return resolved.status === "available" ? resolved.value : undefined;
}

export function resolveFallbackXaiAuth(cfg?: SkillSetConfig): XaiFallbackAuth | undefined {
...
export function resolveFallbackXaiApiKey(cfg?: SkillSetConfig): string | undefined {
