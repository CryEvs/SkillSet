...
import { OPENAI_COMPATIBLE_REPLAY_HOOKS } from "skillset/plugin-sdk/provider-model-shared";
import { defaultToolStreamExtraParams } from "skillset/plugin-sdk/provider-stream-shared";
import { jsonResult, readProviderEnvValue } from "skillset/plugin-sdk/provider-web-search";
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          docs: "https://docs.skillset.ai/tools/code-execution",
        });
      }
