...
function resolveDefaultAgentId(cfg: SkillSetConfig): string {
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  const chosen = (agents.find((agent) => agent?.default) ?? agents[0])?.id;
  return normalizeAgentId(chosen);
}

function listConfiguredAccountIds(cfg: SkillSetConfig): string[] {
...
function listBoundAccountIds(cfg: SkillSetConfig, channelId: string): string[] {
  const ids = new Set<string>();
...
function resolveDefaultAgentBoundAccountId(cfg: SkillSetConfig, channelId: string): string | null {
...
export function resolveDefaultTelegramAccountSelection(cfg: SkillSetConfig): {
  accountId: string;
  accountIds: string[];
