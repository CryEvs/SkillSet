...
import * as runtimeEnvModule from "skillset/plugin-sdk/runtime-env";
import { withEnv } from "skillset/plugin-sdk/testing";
...
describe("resolveTelegramAccount", () => {
  ...
  it("formats debug logs with inspect-style output when debug env is enabled", () => {
    withEnv({ TELEGRAM_BOT_TOKEN: "", SKILLSET_DEBUG_TELEGRAM_ACCOUNTS: "1" }, () => {
...
describe("resolveTelegramAccount groups inheritance (#30673)", () => {
  const createMultiAccountGroupsConfig = (): SkillSetConfig => ({
    channels: {
      telegram: {
  ...
  const createDefaultAccountGroupsConfig = (includeDevAccount: boolean): SkillSetConfig => ({
    channels: {
      telegram: {
