...
import type { GetReplyOptions, MsgContext } from "skillset/plugin-sdk/reply-runtime";
...
type LoadConfigFn = typeof import("skillset/plugin-sdk/config-runtime").loadConfig;
type LoadSessionStoreFn = typeof import("skillset/plugin-sdk/config-runtime").loadSessionStore;
type ResolveStorePathFn = typeof import("skillset/plugin-sdk/config-runtime").resolveStorePath;
...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("skillset/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/skillset-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("skillset/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: SkillSetConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: SkillSetConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: SkillSetConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: SkillSetConfig) => {
