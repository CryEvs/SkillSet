...
import { resolveStorePath, type SkillSetConfig } from "skillset/plugin-sdk/config-runtime";
import { logVerbose } from "skillset/plugin-sdk/runtime-env";

const TTL_MS = 24 * 60 * 60 * 1000;
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("skillset.telegramSentMessagesState");
...
function resolveSentMessageStorePath(cfg?: Pick<SkillSetConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<SkillSetConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<SkillSetConfig, "session">): SentMessageStore {
...
export function recordSentMessage(
  ...
  messageId: number,
  cfg?: Pick<SkillSetConfig, "session">,
): void {
