...
import { AcpRuntimeError } from "skillset/plugin-sdk/acp-runtime";
...
let runtimeEnvModule: typeof import("skillset/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): SkillSetConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("skillset/plugin-sdk/plugin-runtime", async () => {
      const actual = await vi.importActual<typeof import("skillset/plugin-sdk/plugin-runtime")>(
        "skillset/plugin-sdk/plugin-runtime",
      );
      return {
        ...actual,
    ...
    runtimeEnvModule = await import("skillset/plugin-sdk/runtime-env");
  ...
  it("passes SkillSet EventQueue defaults to Carbon Client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
