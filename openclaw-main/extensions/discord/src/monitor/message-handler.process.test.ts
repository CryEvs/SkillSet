import { DEFAULT_EMOJIS } from "skillset/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/skillset-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("skillset/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("skillset/plugin-sdk/conversation-runtime");
...
const sessionStoreRuntimeModule = await import("skillset/plugin-sdk/session-store-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/skillset-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/skillset-discord-process-test-sessions.json" },
      },
    });
