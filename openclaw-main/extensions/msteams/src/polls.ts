...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["skillsetPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["skillset", "pollId"]) ??
    readNestedString(value, ["skillset", "poll", "id"]) ??
    readNestedString(value, ["data", "skillsetPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "skillset", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          skillsetPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "skillset poll vote",
          displayText: "Vote recorded",
          value: { skillsetPollId: pollId, pollId },
        },
      },
    ],
  };
