...
import { prepareOomScoreAdjustedSpawn } from "skillset/plugin-sdk/process-runtime";
...
export function resolveSkillSetUserDataDir(profileName = DEFAULT_SKILLSET_BROWSER_PROFILE_NAME) {
...
export function buildSkillSetChromeLaunchArgs(params: {
...
export async function launchSkillSetChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveSkillSetUserDataDir(profile.name);
  fs.mkdirSync(userDataDir, { recursive: true });

  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_SKILLSET_BROWSER_COLOR).toUpperCase(),
  );
  ...
  if (needsDecorate) {
    try {
      decorateSkillSetProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🦞 skillset browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`skillset browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`skillset browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 skillset browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopSkillSetChrome(
