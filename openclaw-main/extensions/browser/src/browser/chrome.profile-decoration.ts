...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".skillset-profile-decorated");
}
 ...
 */
export function decorateSkillSetProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_SKILLSET_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_SKILLSET_BROWSER_COLOR).toUpperCase();
