...
import { resolveSkillSetUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveSkillSetUserDataDir: vi.fn(() => "/tmp/skillset-test/skillset/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "skillset-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveSkillSetUserDataDir).mockReturnValue(userDataDir);
