...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("skillset");
    expect(profile?.driver).toBe("skillset");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const skillset = resolveProfile(resolved, "skillset");
    expect(skillset?.driver).toBe("skillset");
    expect(skillset?.cdpPort).toBe(18800);
    expect(skillset?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from SKILLSET_GATEWAY_PORT when unset", () => {
    withEnv({ SKILLSET_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      expect(resolved.controlPort).toBe(19003);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const skillset = resolveProfile(resolved, "skillset");
      expect(skillset?.cdpPort).toBe(19012);
      expect(skillset?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ SKILLSET_GATEWAY_PORT: undefined }, () => {
      const resolved = resolveBrowserConfig(undefined, { gateway: { port: 19011 } });
      expect(resolved.controlPort).toBe(19013);
      expect(resolveProfile(resolved, "chrome-relay")).toBe(null);

      const skillset = resolveProfile(resolved, "skillset");
      expect(skillset?.cdpPort).toBe(19022);
      expect(skillset?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(skillset?.cdpPort).toBe(19000);
    expect(skillset?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "skillset");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "skillset")!;
  ...
  describe("default profile preference", () => {
    it("defaults to skillset profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("skillset");
    });

    it("keeps skillset default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("skillset");
    ...
    it("keeps skillset default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("skillset");
    ...
    it("keeps skillset default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
