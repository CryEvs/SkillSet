...
const AGENT_DIGEST_PATH = ".skillset-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".skillset-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- skillset:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- skillset:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- skillset:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- skillset:wiki:${group.dir}:index:end -->`,
