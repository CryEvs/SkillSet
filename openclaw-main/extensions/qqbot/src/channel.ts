import { getExecApprovalReplyMetadata } from "skillset/plugin-sdk/approval-runtime";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped skillset.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // skillset.json). We only restore when both fields are empty so a
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            const configApi = runtime.config as {
              writeConfigFile: (cfg: SkillSetConfig) => Promise<void>;
            };
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe skillset.json mid-flight.
          if (account.appId && account.clientSecret) {
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        ...
        await configApi.writeConfigFile(nextCfg as SkillSetConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as SkillSetConfig, accountId);
