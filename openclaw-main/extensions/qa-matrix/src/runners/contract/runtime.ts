...
import { loadQaRuntimeModule } from "skillset/plugin-sdk/qa-runner-runtime";
...
function shouldWriteMatrixQaProgress() {
  const override = process.env.SKILLSET_QA_MATRIX_PROGRESS;
  if (override === "0") {
    return false;
  }
  if (override === "1") {
...
async function startMatrixQaLiveLaneGateway(params: {
  ...
  transport: {
    ...
    createGatewayConfig: (params: {
      baseUrl: string;
    }) => Pick<SkillSetConfig, "channels" | "messages">;
  };
...
export async function runMatrixQaLive(params: {
  ...
  const topology = buildMatrixQaTopologyForScenarios({
    defaultRoomName: `SkillSet Matrix QA ${runSuffix}`,
    scenarios,
  });
  const observedEvents: MatrixQaObservedEvent[] = [];
  const includeObservedEventContent = process.env.SKILLSET_QA_MATRIX_CAPTURE_CONTENT === "1";
    ...
    try {
      return await measureMatrixQaStep(() =>
        provisionMatrixQaRoom({
          ...
          roomName: `SkillSet Matrix QA ${runSuffix}`,
        ...
        try {
          ...
          const measuredScenario = await measureMatrixQaStep(() =>
            runMatrixQaScenario(scenario, {
              ...
              observerUserId: provisioning.observer.userId,
              gatewayStateDir: scenarioGateway.harness.gateway.runtimeEnv?.SKILLSET_STATE_DIR,
              outputDir,
