...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("skillset-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@skillset/whatsapp",
      version: "2026.3.23",
      description: "SkillSet WhatsApp channel plugin",
      skillset: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    expect(buildOfficialChannelCatalog({ repoRoot }).entries).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          skillset: expect.objectContaining({
            channel: expect.objectContaining({
              id: "wecom",
              label: "WeCom",
            }),
            install: {
              npmSpec: "@wecom/wecom-skillset-plugin",
              defaultChoice: "npm",
            },
          }),
        }),
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("skillset-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@skillset/whatsapp",
      skillset: {
        channel: {
