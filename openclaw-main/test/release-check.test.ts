...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | skillset.install.npmSpec must be a non-empty string",
    ]);
  });
      ...
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            skillset: {
              install: { npmSpec: "@skillset/broken", minHostVersion: "2026.3.14" },
            },
          },
        },
      ]),
    ).toEqual([
      "bundled extension 'broken' manifest invalid | skillset.install.minHostVersion must use a semver floor in the form \">=x.y.z\"",
    ]);
  });
  ...
  it("flags non-object install metadata instead of throwing", () => {
    ...
    ).toEqual(["bundled extension 'broken' manifest invalid | skillset.install must be an object"]);
  });
});
...
describe("bundled plugin root runtime mirrors", () => {
  ...
  it("derives required root mirrors from built root dist imports", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "skillset-root-mirror-"));
  ...
  it("does not derive root mirrors for root chunks sourced from the owning plugin", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "skillset-root-mirror-owned-"));
...
describe("collectForbiddenPackPaths", () => {
  ...
  it("blocks legacy runtime dependency stamps from npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        "dist/index.js",
        "dist/extensions/codex/.skillset-runtime-deps-backup-node_modules-old/zod/index.js",
        "dist/extensions/discord/.skillset-runtime-deps-stamp.json",
      ]),
    ).toEqual([
      "dist/extensions/codex/.skillset-runtime-deps-backup-node_modules-old/zod/index.js",
      "dist/extensions/discord/.skillset-runtime-deps-stamp.json",
  ...
  it("blocks root dist chunks that still reference private qa lab sources", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "skillset-release-private-qa-"));
  ...
  it("allows legacy QA compatibility paths in the generated dist inventory", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "skillset-release-inventory-"));
  ...
  it("requires the shipped channel catalog, control ui, and optional bundled metadata", () => {
    ...
    expect(missing).toEqual(
      expect.arrayContaining([
        ...
        bundledDistPluginFile("matrix", "skillset.plugin.json"),
        ...
        bundledDistPluginFile("whatsapp", "skillset.plugin.json"),
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("skillset-2026.3.14.tgz", 120_354_302)]),
    ).toEqual([]);
  });

  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("skillset-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "skillset-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 211812352 bytes (202.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "skillset-2026.3.14.tgz" },
        { filename: "skillset-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
...
describe("createPackedBundledPluginPostinstallEnv", () => {
  it("keeps packed postinstall on the lazy bundled dependency path", () => {
    expect(createPackedBundledPluginPostinstallEnv({ PATH: "/usr/bin" })).toEqual({
      PATH: "/usr/bin",
      SKILLSET_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK: "1",
    });
  });
});
...
describe("collectInstalledBundledPluginRuntimeDepErrors", () => {
  ...
  it("returns no errors when declared deps are installed at the skillset package root", () => {
    const packageRoot = createPackageRoot();
    try {
      writeBundledPluginPackageJson(packageRoot, "whatsapp", {
        name: "@skillset/whatsapp",
        dependencies: { "@whiskeysockets/baileys": "7.0.0-rc.9" },
        skillset: { bundle: { stageRuntimeDependencies: true } },
      });
