...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @skillset/zalo, @skillset/feishu  @skillset/zalo "),
    ).toEqual(["@skillset/feishu", "@skillset/zalo"]);
  });
});
...
describe("collectPublishablePluginPackages", () => {
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "skillset-plugin-npm-release-");
    ...
    writeJsonFile(join(repoDir, "extensions", "demo-plugin", "package.json"), {
      name: "@skillset/demo-plugin",
      version: "2026.4.10",
      skillset: {
        extensions: ["./index.ts"],
        install: {
          npmSpec: "@skillset/demo-plugin",
        },
        release: {
          publishToNpm: true,
        },
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        packageName: "@skillset/demo-plugin",
        version: "2026.4.10",
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@skillset/demo-plugin",
      },
    ]);
  });
});
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@skillset/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @skillset/missing.");
  });
});
