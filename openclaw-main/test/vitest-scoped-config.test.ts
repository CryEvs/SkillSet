...
describe("resolveVitestIsolation", () => {
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ SKILLSET_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ SKILLSET_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ SKILLSET_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  it("applies the non-isolated runner by default", () => {
    ...
    expect(normalizeConfigPaths(config.test?.setupFiles)).toEqual([
      "test/setup.ts",
      "test/setup-skillset-runtime.ts",
    ]);
  });
  ...
  it("loads scoped include overrides from SKILLSET_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-vitest-scoped-"));
    try {
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the skillset runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the skillset runtime setup", () => {
  ...
  it("keeps the ui lane off both the skillset runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from SKILLSET_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "skillset-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
