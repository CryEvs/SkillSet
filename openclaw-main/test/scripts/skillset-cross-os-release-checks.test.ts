...
} from "../../scripts/skillset-cross-os-release-checks.ts";

describe("scripts/skillset-cross-os-release-checks", () => {
  ...
  it("prefers the freshly installed Windows CLI under npm's prefix before PATH lookup", () => {
    ...
    expect(script).toContain("(Join-Path $npmPrefix 'skillset.cmd')");
    expect(script).toContain("$cmd = Get-Command skillset -ErrorAction Stop");
  });
  ...
  it("prefers workflow-injected runner override env names over legacy ones", () => {
    expect(
      readRunnerOverrideEnv({
        ...
        SKILLSET_RELEASE_CHECKS_UBUNTU_RUNNER: "legacy-linux",
        SKILLSET_RELEASE_CHECKS_WINDOWS_RUNNER: "legacy-windows",
        SKILLSET_RELEASE_CHECKS_MACOS_RUNNER: "legacy-macos",
      }),
    ).toEqual({
  ...
  it("serves installer scripts as UTF-8 text and package payloads as binary", () => {
    ...
    expect(resolveStaticFileContentType("skillset-2026.4.14.tgz")).toBe("application/octet-stream");
  });

  it("uses the published installer URLs for native installer lanes", () => {
    expect(resolvePublishedInstallerUrl("darwin")).toBe("https://skillset.ai/install.sh");
    expect(resolvePublishedInstallerUrl("linux")).toBe("https://skillset.ai/install.sh");
    expect(resolvePublishedInstallerUrl("win32")).toBe("https://skillset.ai/install.ps1");
  });
  ...
  it("derives the installed prefix from resolved CLI paths", () => {
    ...
    expect(
      resolveInstalledPrefixDirFromCliPath("/Users/runner/.npm-global/bin/skillset", "darwin"),
  ...
  it("only treats pinned baseline specs as exact installer version assertions", () => {
    ...
    expect(resolveExplicitBaselineVersion("skillset@latest")).toBe("");
    expect(resolveExplicitBaselineVersion("skillset@2026.4.10")).toBe("2026.4.10");
  ...
  it("reads an installed baseline version without requiring build metadata", () => {
    const prefixDir = mkdtempSync(join(tmpdir(), "skillset-cross-os-installed-version-"));
    try {
      const packageRoot =
  ...
  it("treats missing package scripts as optional in older refs", () => {
    const packageRoot = mkdtempSync(join(tmpdir(), "skillset-cross-os-scripts-"));
    try {
      writeFileSync(
        join(packageRoot, "package.json"),
        JSON.stringify({
