  ...
  it("keeps `update status --json` stdout parseable even with legacy doctor preflight inputs", async () => {
    await withTempHome(
      async (tempHome) => {
        ...
        delete env.SKILLSET_HOME;
        delete env.SKILLSET_STATE_DIR;
        delete env.SKILLSET_CONFIG_PATH;
        delete env.VITEST;

        const entry = path.resolve(process.cwd(), "skillset.mjs");
