...
export function requireSkillSetAgentDir(): string {
  const agentDir = process.env.SKILLSET_AGENT_DIR;
  if (!agentDir) {
    throw new Error("SKILLSET_AGENT_DIR not set");
  }
  return agentDir;
}
