...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked SKILLSET_HOME.
  delete process.env.SKILLSET_HOME;
  process.env.SKILLSET_STATE_DIR = path.join(base, ".skillset");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "skillset-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".skillset", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
