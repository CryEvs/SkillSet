...
import type { SkillSetConfig } from "../../../src/config/types.skillset.js";
...
const piEmbeddedMocks = getSharedMocks("skillset.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("skillset.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("skillset.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("skillset.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    skillsetHome: process.env.SKILLSET_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("SKILLSET_HOME", snapshot.skillsetHome);
  restoreKey("SKILLSET_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.SKILLSET_STATE_DIR = join(home, ".skillset");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "skillset-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".skillset", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): SkillSetConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "skillset"),
