...
function parsePluginSdkSubpath(specifier) {
  ...
  const subpath = specifier.slice("skillset/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced skillset/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced skillset/plugin-sdk/<subpath> must exist in the public package exports.",
  );
