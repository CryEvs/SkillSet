...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/skillset/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "SkillSet Re-Auth" -c "After getting key, run: ssh $SERVER '~/skillset/scripts/mobile-reauth.sh'" --id skillset-auth
        ;;
    *)
