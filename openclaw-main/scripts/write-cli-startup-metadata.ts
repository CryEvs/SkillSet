...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.skillset?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.skillset?.channel?.order;
      const labelRaw = parsed.skillset?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".skillset-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "skillset-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "skillset-build",
    ...
    SKILLSET_DISABLE_PLUGIN_MANIFEST_CACHE: "1",
    SKILLSET_PLUGIN_DISCOVERY_CACHE_MS: "0",
    SKILLSET_PLUGIN_MANIFEST_CACHE_MS: "0",
