...
AUTH_KEY_FLAG=""
MODEL_ID=""
INSTALL_URL="https://skillset.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_SKILLSET_BIN="/opt/homebrew/bin/skillset"
GUEST_SKILLSET_ENTRY="/opt/homebrew/lib/node_modules/skillset/skillset.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/skillset-parallels-smoke.XXXXXX)"
...
TIMEOUT_UPDATE_DEV_S="${SKILLSET_PARALLELS_MACOS_UPDATE_DEV_TIMEOUT_S:-1200}"
TIMEOUT_VERIFY_S=60
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://skillset.ai/install.sh
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: skillset@2026.3.13-beta.1
...
resolve_guest_git_skillset_entry() {
  local guest_home
  guest_home="$(resolve_guest_current_user_home)"
  printf '%s/skillset/skillset.mjs\n' "$guest_home"
}
...
spawn {*}$cmd
send -- "printf '__SKILLSET_READY__\\n'\r"
expect "__SKILLSET_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/skillset-prl.sh <<'__SKILLSET_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__SKILLSET_SCRIPT__\r"
send -- "/bin/bash /tmp/skillset-prl.sh; rc=\$?; rm -f /tmp/skillset-prl.sh; printf '__SKILLSET_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
set saw_rc 0
expect {
  -re {__SKILLSET_RC__:(-?[0-9]+)} {
...
guest_current_user_sh() {
  ...
  script+="$1"
  if headless_guest_fallback; then
    script_path="/tmp/skillset-prl-${BASHPID:-$$}-$RANDOM.sh"
    local guest_home
    guest_home="$(parallels_macos_resolve_desktop_home "$VM_NAME" "$GUEST_CURRENT_USER")"
...
markers = [
    ...
    if line.startswith("__SKILLSET_RC__:")
]
if not markers:
    raise SystemExit(1)
...
printf '__SKILLSET_RC__:%s\n' "\$status"
printf '%s\n' "\$status" > "$done_path"
exit "\$status"
EOF
)"
  write_runner_cmd="/bin/rm -f $(shell_quote "$runner_path")"$'\n'
  write_runner_cmd+="cat > $(shell_quote "$runner_path") <<'__SKILLSET_RUNNER__'"$'\n'
  write_runner_cmd+="$runner_body"$'\n'
  write_runner_cmd+="__SKILLSET_RUNNER__"$'\n'
  ...
  guest_home="$(resolve_guest_current_user_home)"
  guest_log_state_path="$(mktemp "${TMPDIR:-/tmp}/skillset-guest-log-state.XXXXXX")"
  latest_npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/skillset-guest-npm-log-state.XXXXXX")"
  npm_state_path="$(mktemp "${TMPDIR:-/tmp}/skillset-guest-npm-log-path.XXXXXX")"
...
matches = re.findall(r"^__SKILLSET_RC__:(-?\d+)$", text, flags=re.MULTILINE)
if not matches:
    raise SystemExit(1)
print(matches[-1])
PY
...
curl -fsSL $install_url_q -o /tmp/skillset-install.sh
bash /tmp/skillset-install.sh${version_arg_q}
$GUEST_SKILLSET_BIN --version
EOF
)"
}
...
ensure_guest_pnpm_for_dev_update() {
  ...
  bootstrap_root="/tmp/skillset-smoke-pnpm-bootstrap"
...
repair_legacy_dev_source_checkout_if_needed() {
  ...
  bootstrap_bin="/tmp/skillset-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/skillset"
  update_entry="$update_root/skillset.mjs"
  ...
  guest_current_user_exec /usr/bin/git clone --depth 1 --branch main \
    https://github.com/skillset/skillset.git "$update_root"
...
run_dev_channel_update() {
  ...
  bootstrap_bin="/tmp/skillset-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/skillset"
  update_log="/tmp/skillset-smoke-update-dev.log"
  update_done="/tmp/skillset-smoke-update-dev.done"
  update_runner="/tmp/skillset-smoke-update-dev.sh"
  ensure_guest_pnpm_for_dev_update
...
/usr/bin/env NODE_OPTIONS=--max-old-space-size=4096 \
  $GUEST_NODE_BIN $GUEST_SKILLSET_ENTRY update --channel dev --yes --json
EOF
  ...
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_SKILLSET_ENTRY" --version
  printf 'update-dev: git-status\n'
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_SKILLSET_ENTRY" update status --json
}

verify_dev_channel_update() {
  local status_json
  status_json="$(guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_SKILLSET_ENTRY" update status --json)"
  printf '%s\n' "$status_json"
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_SKILLSET_BIN" --version 2>&1
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
pack_main_tgz() {
  ...
  set +e
  {
    SKILLSET_PARALLELS_BUILD_LOCK_HELD=1 ensure_current_build &&
  ...
  release_build_lock
  [[ $rc -eq 0 ]] || return "$rc"
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/skillset-main-$short_head.tgz"
...
ensure_current_build() {
  ...
  lock_owned=0
  if [[ "${SKILLSET_PARALLELS_BUILD_LOCK_HELD:-0}" != "1" ]]; then
    acquire_build_lock
    lock_owned=1
  fi
...
start_server() {
  ...
  ) >/tmp/skillset-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
check_path "\$root/skillset"
check_path "\$root/skillset/extensions"
if [ -d "\$root/skillset/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "\$extension_dir"
  done < <(/usr/bin/find "\$root/skillset/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi
EOF
)"
...
start_manual_gateway_if_needed() {
  ...
  guest_gateway_log="/tmp/skillset-parallels-macos-gateway.log"
  runner_log="/tmp/skillset-parallels-gateway-start.log"
  done_path="/tmp/skillset-parallels-gateway-start.done"
  runner_path="/tmp/skillset-parallels-gateway-start.sh"
  printf 'manual gateway launch transport=%s user=%s\n' "$GUEST_CURRENT_USER_TRANSPORT" "$GUEST_CURRENT_USER"
  launch_cmd="$(cat <<EOF
set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'skillset.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'skillset-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'skillset.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env \\
  ...
  SKILLSET_HOME=$(shell_quote "$guest_home") \\
  SKILLSET_STATE_DIR=$(shell_quote "$guest_home/.skillset") \\
  SKILLSET_CONFIG_PATH=$(shell_quote "$guest_home/.skillset/skillset.json") \\
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_SKILLSET_ENTRY") gateway run --bind loopback --port 18789 --force \\
...
verify_gateway() {
  ...
  for attempt in 1 2 3 4; do
    if guest_current_user_exec "$GUEST_SKILLSET_BIN" gateway status --deep --require-rpc --timeout 5000; then
      return 0
    fi
    if (( attempt < 4 )); then
...
show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_SKILLSET_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_SKILLSET_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_SKILLSET_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_SKILLSET_ENTRY" models set "$MODEL_ID"
  guest_current_user_sh "$(cat <<EOF
export PATH=$(shell_quote "$GUEST_EXEC_PATH")
exec /usr/bin/env $(shell_quote "$API_KEY_ENV=$API_KEY_VALUE") \
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_SKILLSET_ENTRY") agent \
  --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_SKILLSET_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from skillset dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
verify_dashboard_load() {
  ...
  # `skillset dashboard --no-open` can hang under the Tahoe Parallels transport
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 "\$dashboard_http_url" >/tmp/skillset-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>SkillSet Control</title>' /tmp/skillset-dashboard-smoke.html >/dev/null; then
      if grep -F '<skillset-app></skillset-app>' /tmp/skillset-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>SkillSet Control</title>' /tmp/skillset-dashboard-smoke.html >/dev/null
grep -F '<skillset-app></skillset-app>' /tmp/skillset-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/skillset-discord-token <<'__SKILLSET_TOKEN__'
$DISCORD_TOKEN_VALUE
__SKILLSET_TOKEN__
cat >/tmp/skillset-discord-guilds.json <<'__SKILLSET_GUILDS__'
$guilds_json
__SKILLSET_GUILDS__
token="\$(tr -d '\n' </tmp/skillset-discord-token)"
guilds_json="\$(cat /tmp/skillset-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_SKILLSET_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_SKILLSET_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_SKILLSET_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_SKILLSET_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_SKILLSET_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_SKILLSET_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_SKILLSET_ENTRY channels status --probe --json
rm -f /tmp/skillset-discord-token /tmp/skillset-discord-guilds.json
EOF
)"
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "skillset-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "skillset-main-upgrade.tgz"
