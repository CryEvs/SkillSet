...
IMAGE_NAME="$(docker_e2e_resolve_image "skillset-openwebui-e2e" SKILLSET_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${SKILLSET_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${SKILLSET_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${SKILLSET_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${SKILLSET_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${SKILLSET_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${SKILLSET_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="skillset-openwebui-e2e-$$"
GW_NAME="skillset-openwebui-gateway-$$"
OW_NAME="skillset-openwebui-$$"
...
docker run -d \
  ...
  -e "SKILLSET_OPENWEBUI_MODEL=$MODEL" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/skillset-openwebui-config.XXXXXX.json)"
    SKILLSET_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.SKILLSET_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.SKILLSET_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.SKILLSET_OPENWEBUI_MODEL },
];
...
EOF
    cat > "$workspace/.skillset/workspace-state.json" <<'"'"'EOF'"'"'
{
  "version": 1,
...
docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="SkillSet E2E" \
  -e WEBUI_SECRET_KEY="skillset-openwebui-e2e-secret" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="SkillSet E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="skillset/default" \
...
echo "Running Open WebUI -> SkillSet smoke..."
if ! docker exec \
