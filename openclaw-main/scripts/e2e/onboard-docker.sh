...
IMAGE_NAME="$(docker_e2e_resolve_image "skillset-onboard-e2e" SKILLSET_ONBOARD_E2E_IMAGE)"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/skillset-bin:$PATH"
  mkdir -p /tmp/skillset-bin
  cat > /tmp/skillset-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/skillset-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$SKILLSET_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_skillset_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/skillset-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/skillset-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$SKILLSET_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/skillset-e2e-$1.XXXXXX"
  }

  set_isolated_skillset_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export SKILLSET_HOME="$home_dir"
    export SKILLSET_STATE_DIR="$home_dir/.skillset"
  ...
  run_case_logged() {
    local label="$1"
    shift
    local log_path="/tmp/skillset-onboard-${label}.log"
  ...
  run_case_local_basic() {
    ...
    set_isolated_skillset_env "$home_dir"
    run_case_logged local-basic node "$SKILLSET_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$SKILLSET_STATE_DIR/workspace"
    config_path="$SKILLSET_CONFIG_PATH"
    sessions_dir="$SKILLSET_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_skillset_env "$home_dir"
	    ...
	    run_case_logged remote-non-interactive node "$SKILLSET_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
JSON
	    ...
	    run_case_logged reset-config node "$SKILLSET_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$SKILLSET_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$SKILLSET_ENTRY\" configure --section skills" send_skills_flow
