...
IMAGE_NAME="$(docker_e2e_resolve_image "skillset-doctor-install-switch-e2e" SKILLSET_DOCTOR_INSTALL_SWITCH_E2E_IMAGE)"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/skillset-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/skillset-bin/systemctl

  cat > /tmp/skillset-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/skillset-bin/loginctl
  ...
  npm_log="/tmp/skillset-doctor-switch-npm-install.log"
  if ! npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz" >"$npm_log" 2>&1; then
    cat "$npm_log"
    exit 1
  fi

	  npm_bin="/tmp/npm-prefix/bin/skillset"
	  npm_root="/tmp/npm-prefix/lib/node_modules/skillset"
  ...
  run_flow() {
    ...
    local install_log="/tmp/skillset-doctor-switch-${name}-install.log"
    local doctor_log="/tmp/skillset-doctor-switch-${name}-doctor.log"

    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/skillset-switch-${name}.XXXXXX")
    ...
    unit_path="$HOME/.config/systemd/user/skillset-gateway.service"
    if [ ! -f "$unit_path" ]; then
