...
IMAGE_NAME="$(docker_e2e_resolve_image "skillset-bundled-channel-deps-e2e" SKILLSET_BUNDLED_CHANNEL_DEPS_E2E_IMAGE)"
UPDATE_BASELINE_VERSION="${SKILLSET_BUNDLED_CHANNEL_UPDATE_BASELINE_VERSION:-2026.4.20}"
DOCKER_TARGET="${SKILLSET_BUNDLED_CHANNEL_DOCKER_TARGET:-e2e-runner}"
HOST_BUILD="${SKILLSET_BUNDLED_CHANNEL_HOST_BUILD:-1}"
PACKAGE_TGZ="${SKILLSET_BUNDLED_CHANNEL_PACKAGE_TGZ:-}"
RUN_CHANNEL_SCENARIOS="${SKILLSET_BUNDLED_CHANNEL_SCENARIOS:-1}"
RUN_UPDATE_SCENARIO="${SKILLSET_BUNDLED_CHANNEL_UPDATE_SCENARIO:-1}"
RUN_ROOT_OWNED_SCENARIO="${SKILLSET_BUNDLED_CHANNEL_ROOT_OWNED_SCENARIO:-1}"
RUN_SETUP_ENTRY_SCENARIO="${SKILLSET_BUNDLED_CHANNEL_SETUP_ENTRY_SCENARIO:-1}"
RUN_LOAD_FAILURE_SCENARIO="${SKILLSET_BUNDLED_CHANNEL_LOAD_FAILURE_SCENARIO:-1}"
...
prepare_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "SKILLSET_BUNDLED_CHANNEL_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ "$HOST_BUILD" != "0" ]; then
    ...
    echo "Skipping host build (SKILLSET_BUNDLED_CHANNEL_HOST_BUILD=0)"
  fi
  ...
  local pack_dir
  pack_dir="$(mktemp -d "${TMPDIR:-/tmp}/skillset-bundled-channel-pack.XXXXXX")"
  run_logged bundled-channel-deps-pack npm pack --ignore-scripts --pack-destination "$pack_dir"
  PACKAGE_TGZ="$(find "$pack_dir" -maxdepth 1 -name 'skillset-*.tgz' -print -quit)"
  if [ -z "$PACKAGE_TGZ" ]; then
    echo "missing packed SkillSet tarball" >&2
    exit 1
  fi
  PACKAGE_TGZ="$(cd "$(dirname "$PACKAGE_TGZ")" && pwd)/$(basename "$PACKAGE_TGZ")"
}
...
prepare_package_tgz
DOCKER_PACKAGE_TGZ="/tmp/skillset-current.tgz"
PACKAGE_DOCKER_ARGS=(-v "$PACKAGE_TGZ:$DOCKER_PACKAGE_TGZ:ro" -e "SKILLSET_CURRENT_PACKAGE_TGZ=$DOCKER_PACKAGE_TGZ")

run_channel_scenario() {
  local channel="$1"
  local dep_sentinel="$2"
  local run_log
  run_log="$(mktemp "${TMPDIR:-/tmp}/skillset-bundled-channel-deps-$channel.XXXXXX")"

  echo "Running bundled $channel runtime deps Docker E2E..."
  if ! docker run --rm \
    -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 \
    -e SKILLSET_CHANNEL_UNDER_TEST="$channel" \
    -e SKILLSET_DEP_SENTINEL="$dep_sentinel" \
    "${PACKAGE_DOCKER_ARGS[@]}" \
    -i "$IMAGE_NAME" bash -s >"$run_log" 2>&1 <<'EOF'
set -euo pipefail

export HOME="$(mktemp -d "/tmp/skillset-bundled-channel-deps.XXXXXX")"
export NPM_CONFIG_PREFIX="$HOME/.npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export OPENAI_API_KEY="sk-skillset-bundled-channel-deps-e2e"
export SKILLSET_NO_ONBOARD=1

TOKEN="bundled-channel-deps-token"
PORT="18789"
CHANNEL="${SKILLSET_CHANNEL_UNDER_TEST:?missing SKILLSET_CHANNEL_UNDER_TEST}"
DEP_SENTINEL="${SKILLSET_DEP_SENTINEL:?missing SKILLSET_DEP_SENTINEL}"
gateway_pid=""
...
echo "Installing mounted SkillSet package..."
package_tgz="${SKILLSET_CURRENT_PACKAGE_TGZ:?missing SKILLSET_CURRENT_PACKAGE_TGZ}"
npm install -g "$package_tgz" --no-fund --no-audit >/tmp/skillset-install.log 2>&1

command -v skillset >/dev/null
package_root="$(npm root -g)/skillset"
...
const configPath = path.join(process.env.HOME, ".skillset", "skillset.json");
    ...
    entries: {
      ...
      "memory-lancedb": {
        ...
        config: {
          ...
          dbPath: "~/.skillset/memory/lancedb-e2e",
          autoCapture: false,
          autoRecall: false,
        },
      },
...
start_gateway() {
  local log_file="$1"
  : >"$log_file"
  skillset gateway --port "$PORT" --bind loopback --allow-unconfigured >"$log_file" 2>&1 &
  gateway_pid="$!"
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if skillset gateway health --url "ws://127.0.0.1:$PORT" --token "$TOKEN" --json >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.25
  done
...
assert_channel_status() {
  ...
  local out="/tmp/skillset-channel-status-$channel.json"
  skillset gateway call channels.status \
...
start_gateway "/tmp/skillset-$CHANNEL-baseline.log"
wait_for_gateway_health
stop_gateway
...
start_gateway "/tmp/skillset-$CHANNEL-first.log"
wait_for_gateway_health
assert_installed_once "/tmp/skillset-$CHANNEL-first.log" "$CHANNEL" "$DEP_SENTINEL"
...
start_gateway "/tmp/skillset-$CHANNEL-second.log"
wait_for_gateway_health
assert_not_installed "/tmp/skillset-$CHANNEL-second.log" "$CHANNEL"
assert_channel_status "$CHANNEL"
stop_gateway
...
run_root_owned_global_scenario() {
  local run_log
  run_log="$(mktemp "${TMPDIR:-/tmp}/skillset-bundled-channel-root-owned.XXXXXX")"
...
export HOME="/root"
export OPENAI_API_KEY="sk-skillset-bundled-channel-root-owned-e2e"
export SKILLSET_NO_ONBOARD=1
export SKILLSET_PLUGIN_STAGE_DIR="/var/lib/skillset/plugin-runtime-deps"
...
echo "Installing mounted SkillSet package into root-owned global npm..."
package_tgz="${SKILLSET_CURRENT_PACKAGE_TGZ:?missing SKILLSET_CURRENT_PACKAGE_TGZ}"
npm install -g "$package_tgz" --no-fund --no-audit >/tmp/skillset-root-owned-install.log 2>&1

root="$(package_root)"
test -d "$root/dist/extensions/$CHANNEL"
rm -rf "$root/dist/extensions/$CHANNEL/node_modules"
chmod -R a-w "$root"
mkdir -p "$SKILLSET_PLUGIN_STAGE_DIR" /home/appuser/.skillset
chown -R appuser:appuser /home/appuser/.skillset /var/lib/skillset
...
const configPath = "/home/appuser/.skillset/skillset.json";
const config = {
  gateway: {
    port,
    auth: { mode: "token", token },
...
fs.writeFileSync(configPath, `${JSON.stringify(config, null, 2)}\n`, "utf8");
NODE
chown appuser:appuser /home/appuser/.skillset/skillset.json
...
start_gateway() {
  ...
  runuser -u appuser -- env \
    ...
    SKILLSET_PLUGIN_STAGE_DIR="$SKILLSET_PLUGIN_STAGE_DIR" \
    npm_config_cache=/tmp/skillset-root-owned-npm-cache \
    skillset gateway --port "$PORT" --bind loopback --allow-unconfigured >"$log_file" 2>&1 &
  gateway_pid="$!"
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if runuser -u appuser -- env HOME=/home/appuser skillset gateway health --url "ws://127.0.0.1:$PORT" --token "$TOKEN" --json >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.25
  done
...
assert_channel_status() {
  local out="/tmp/skillset-root-owned-channel-status.json"
  runuser -u appuser -- env HOME=/home/appuser skillset gateway call channels.status \
    --url "ws://127.0.0.1:$PORT" \
    --token "$TOKEN" \
    --timeout 30000 \
    --json \
...
NODE
  then
    cat /tmp/skillset-root-owned-gateway.log >&2
    exit 1
  fi
}

start_gateway /tmp/skillset-root-owned-gateway.log
wait_for_gateway_health
assert_channel_status
...
if ! find "$SKILLSET_PLUGIN_STAGE_DIR" -maxdepth 12 -path "*/node_modules/$DEP_SENTINEL/package.json" -type f | grep -q .; then
  echo "missing external staged dependency sentinel for $DEP_SENTINEL" >&2
  find "$SKILLSET_PLUGIN_STAGE_DIR" -maxdepth 12 -type f | sort | head -120 >&2 || true
  cat /tmp/skillset-root-owned-gateway.log >&2
  exit 1
fi
...
run_setup_entry_scenario() {
  local run_log
  run_log="$(mktemp "${TMPDIR:-/tmp}/skillset-bundled-channel-setup-entry.XXXXXX")"
...
export HOME="$(mktemp -d "/tmp/skillset-bundled-channel-setup-entry.XXXXXX")"
export NPM_CONFIG_PREFIX="$HOME/.npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export SKILLSET_NO_ONBOARD=1
export SKILLSET_PLUGIN_STAGE_DIR="$HOME/.skillset/plugin-runtime-deps"
mkdir -p "$SKILLSET_PLUGIN_STAGE_DIR"
...
echo "Installing mounted SkillSet package..."
package_tgz="${SKILLSET_CURRENT_PACKAGE_TGZ:?missing SKILLSET_CURRENT_PACKAGE_TGZ}"
npm install -g "$package_tgz" --no-fund --no-audit >/tmp/skillset-setup-entry-install.log 2>&1
...
if find "$SKILLSET_PLUGIN_STAGE_DIR" -maxdepth 12 -path "*/node_modules/$DEP_SENTINEL/package.json" -type f | grep -q .; then
  ...
  find "$SKILLSET_PLUGIN_STAGE_DIR" -maxdepth 12 -type f | sort | head -160 >&2 || true
  exit 1
fi
...
skillset doctor --non-interactive >/tmp/skillset-setup-entry-doctor.log 2>&1
...
if ! find "$SKILLSET_PLUGIN_STAGE_DIR" -maxdepth 12 -path "*/node_modules/$DEP_SENTINEL/package.json" -type f | grep -q .; then
  ...
  cat /tmp/skillset-setup-entry-doctor.log >&2
  find "$SKILLSET_PLUGIN_STAGE_DIR" -maxdepth 12 -type f | sort | head -160 >&2 || true
  exit 1
fi
...
run_update_scenario() {
  local run_log
  run_log="$(mktemp "${TMPDIR:-/tmp}/skillset-bundled-channel-update.XXXXXX")"

  echo "Running bundled channel runtime deps Docker update E2E..."
  if ! docker run --rm \
    -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 \
    -e SKILLSET_BUNDLED_CHANNEL_UPDATE_BASELINE_VERSION="$UPDATE_BASELINE_VERSION" \
    "${PACKAGE_DOCKER_ARGS[@]}" \
    -i "$IMAGE_NAME" bash -s >"$run_log" 2>&1 <<'EOF'
set -euo pipefail

export HOME="$(mktemp -d "/tmp/skillset-bundled-channel-update.XXXXXX")"
export NPM_CONFIG_PREFIX="$HOME/.npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export OPENAI_API_KEY="sk-skillset-bundled-channel-update-e2e"
export SKILLSET_NO_ONBOARD=1
export SKILLSET_UPDATE_PACKAGE_SPEC=""
    ...
    entries: {
      ...
      "memory-lancedb": {
        ...
        config: {
          ...
          dbPath: "~/.skillset/memory/lancedb-update-e2e",
          autoCapture: false,
          autoRecall: false,
        },
      },
...
const doctor = steps.find((step) => step?.name === "skillset doctor");
if (!doctor) {
  throw new Error("missing skillset doctor step");
}
if (Number(doctor.exitCode ?? 1) !== 0) {
  throw new Error(`skillset doctor step failed: ${JSON.stringify(doctor)}`);
}
NODE
}
...
run_update_and_capture() {
  ...
  local out_file="$2"
  set +e
  skillset update --tag "$update_target" --yes --json >"$out_file" 2>"/tmp/skillset-$label-update.stderr"
  local status=$?
  set -e
  if [ "$status" -ne 0 ]; then
    echo "skillset update failed for $label with exit code $status" >&2
    cat "$out_file" >&2 || true
    cat "/tmp/skillset-$label-update.stderr" >&2 || true
    exit "$status"
  fi
}

echo "Installing current candidate as update baseline..."
npm install -g "$package_tgz" --no-fund --no-audit >/tmp/skillset-update-baseline-install.log 2>&1
command -v skillset >/dev/null
...
set +e
skillset doctor --non-interactive >/tmp/skillset-baseline-doctor.log 2>&1
...
run_update_and_capture telegram /tmp/skillset-update-telegram.json
cat /tmp/skillset-update-telegram.json
assert_update_ok /tmp/skillset-update-telegram.json "$candidate_version"
...
remove_runtime_dep telegram grammy
assert_no_dep_available telegram grammy
if ! SKILLSET_UPDATE_IN_PROGRESS=1 skillset doctor --non-interactive >/tmp/skillset-update-mode-doctor.log 2>&1; then
  echo "update-mode doctor failed while repairing Telegram deps" >&2
  cat /tmp/skillset-update-mode-doctor.log >&2
  exit 1
fi
assert_dep_available telegram grammy
...
run_update_and_capture discord /tmp/skillset-update-discord.json
cat /tmp/skillset-update-discord.json
assert_update_ok /tmp/skillset-update-discord.json "$candidate_version"
...
run_update_and_capture slack /tmp/skillset-update-slack.json
cat /tmp/skillset-update-slack.json
assert_update_ok /tmp/skillset-update-slack.json "$candidate_version"
...
run_update_and_capture feishu /tmp/skillset-update-feishu.json
cat /tmp/skillset-update-feishu.json
assert_update_ok /tmp/skillset-update-feishu.json "$candidate_version"
...
run_update_and_capture memory-lancedb /tmp/skillset-update-memory-lancedb.json
cat /tmp/skillset-update-memory-lancedb.json
assert_update_ok /tmp/skillset-update-memory-lancedb.json "$candidate_version"
...
run_load_failure_scenario() {
  local run_log
  run_log="$(mktemp "${TMPDIR:-/tmp}/skillset-bundled-channel-load-failure.XXXXXX")"
...
export HOME="$(mktemp -d "/tmp/skillset-bundled-channel-load-failure.XXXXXX")"
export NPM_CONFIG_PREFIX="$HOME/.npm-global"
export PATH="$NPM_CONFIG_PREFIX/bin:$PATH"
export SKILLSET_NO_ONBOARD=1
...
echo "Installing mounted SkillSet package..."
package_tgz="${SKILLSET_CURRENT_PACKAGE_TGZ:?missing SKILLSET_CURRENT_PACKAGE_TGZ}"
npm install -g "$package_tgz" --no-fund --no-audit >/tmp/skillset-load-failure-install.log 2>&1
...
{
  "name": "@skillset/load-failure-alpha",
  "version": "2026.4.21",
  "private": true,
  "type": "module",
  "skillset": {
...
JSON
cat >"$plugin_dir/skillset.plugin.json" <<'JSON'
{
...
(
  cd "$root"
  SKILLSET_BUNDLED_PLUGINS_DIR="$root/dist/extensions" node --input-type=module - <<'NODE'
import fs from "node:fs";
import path from "node:path";
import { pathToFileURL } from "node:url";
