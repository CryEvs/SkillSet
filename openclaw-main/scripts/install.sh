...
# SkillSet Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://skillset.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one SkillSet."
...
GUM_VERSION="${SKILLSET_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 SkillSet Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://skillset.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$SKILLSET_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.skillset.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_skillset_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.skillset-* "$npm_root"/skillset 2>/dev/null || true
}

extract_skillset_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_skillset_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/skillset" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/skillset"|"/usr/local/bin/skillset")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/skillset/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale skillset symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing skillset binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing SkillSet package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_skillset_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "SkillSet npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*skillset" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_skillset_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_skillset_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "SkillSet npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an skillset binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${SKILLSET_TAGLINE_INDEX:-}" ]]; then
        if [[ "${SKILLSET_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((SKILLSET_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${SKILLSET_NO_ONBOARD:-0}
NO_PROMPT=${SKILLSET_NO_PROMPT:-0}
DRY_RUN=${SKILLSET_DRY_RUN:-0}
INSTALL_METHOD=${SKILLSET_INSTALL_METHOD:-}
SKILLSET_VERSION=${SKILLSET_VERSION:-latest}
USE_BETA=${SKILLSET_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/skillset"
GIT_DIR=${SKILLSET_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${SKILLSET_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${SKILLSET_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${SKILLSET_VERBOSE:-0}"
VERIFY_INSTALL="${SKILLSET_VERIFY_INSTALL:-0}"
SKILLSET_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
SkillSet installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://skillset.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/skillset)
...
Environment variables:
  SKILLSET_INSTALL_METHOD=git|npm
  SKILLSET_VERSION=latest|next|main|<semver>|<spec>
  SKILLSET_BETA=0|1
  SKILLSET_GIT_DIR=...
  SKILLSET_GIT_UPDATE=0|1
  SKILLSET_NO_PROMPT=1
  SKILLSET_VERIFY_INSTALL=1
  SKILLSET_DRY_RUN=1
  SKILLSET_NO_ONBOARD=1
  SKILLSET_VERBOSE=1
  SKILLSET_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://skillset.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://skillset.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://skillset.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://skillset.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://skillset.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected SkillSet checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a SkillSet source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_skillset_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"skillset"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://skillset.ai/install.sh | bash"
}
...
ensure_skillset_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/skillset" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/skillset" ]]; then
        ln -sf "$npm_root/skillset/dist/entry.js" "${npm_bin}/skillset"
        ui_info "Created skillset bin link at ${npm_bin}/skillset"
    fi
    return 0
}

# Check for existing SkillSet installation
check_existing_skillset() {
    if [[ -n "$(type -P skillset 2>/dev/null || true)" ]]; then
        ui_info "Existing SkillSet installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make skillset show as \"command not found\" in new terminals."
...
warn_skillset_not_found() {
    ui_warn "Installed, but skillset is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t skillset 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named skillset; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_skillset_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P skillset 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/skillset" ]]; then
        echo "${npm_bin}/skillset"
        return 0
    fi
...
install_skillset_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/skillset/skillset.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing SkillSet from git checkout: ${repo_dir}"
    else
        ui_info "Installing SkillSet from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning SkillSet" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building SkillSet" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/skillset" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/skillset"
    ui_success "SkillSet wrapper installed to \$HOME/.local/bin/skillset"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view skillset dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:skillset/skillset#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_skillset() {
    local package_name="skillset"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            SKILLSET_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="skillset"
        else
            SKILLSET_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${SKILLSET_VERSION}" ]]; then
        SKILLSET_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${SKILLSET_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${SKILLSET_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing SkillSet v${resolved_version}"
    else
        ui_info "Installing SkillSet (${SKILLSET_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${SKILLSET_VERSION}")"

    if ! install_skillset_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_skillset_paths
        install_skillset_npm "${install_spec}"
    fi

    if [[ "${SKILLSET_VERSION}" == "latest" && "${package_name}" == "skillset" ]]; then
        if ! resolve_skillset_bin &> /dev/null; then
            ui_warn "npm install skillset@latest failed; retrying skillset@next"
            cleanup_npm_skillset_paths
            install_skillset_npm "skillset@next"
        fi
    fi

    ensure_skillset_bin_link || true

    ui_success "SkillSet installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${SKILLSET_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_skillset_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (skillset not on PATH yet)"
        warn_skillset_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${SKILLSET_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.skillset/workspace-${profile}"
    else
        echo "${HOME}/.skillset/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${SKILLSET_CONFIG_PATH:-$HOME/.skillset/skillset.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run skillset onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but skillset not on PATH; skipping onboarding"
        warn_skillset_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run skillset onboard to retry"
        return
    }
}
...
if ! declare -F extract_skillset_semver >/dev/null 2>&1; then
...
resolve_skillset_version() {
    ...
    local claw="${SKILLSET_BIN:-}"
    if [[ -z "$claw" ]] && command -v skillset &> /dev/null; then
        claw="$(command -v skillset)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_skillset_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/skillset/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/skillset/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: skillset not on PATH yet"
        warn_skillset_not_found
        return 1
    fi

    run_quiet_step "Checking SkillSet version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: skillset gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_skillset_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found SkillSet checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set SKILLSET_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_skillset; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing SkillSet"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g skillset &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g skillset 2>/dev/null || true
        ...
        install_skillset_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/skillset" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/skillset"
    ...
    SKILLSET_BIN="$(resolve_skillset_bin || true)"
    ...
    installed_version=$(resolve_skillset_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 SkillSet installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 SkillSet installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/skillset"
        ui_kv "Update command" "skillset update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://skillset.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running skillset doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                SKILLSET_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                SKILLSET_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                SKILLSET_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run skillset doctor and skillset plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run skillset onboard later"
        else
            local config_path="${SKILLSET_CONFIG_PATH:-$HOME/.skillset/skillset.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (skillset not on PATH yet)"
                    warn_skillset_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run skillset onboard to finish setup"
            return 0
        fi
    fi

    if command -v skillset &> /dev/null; then
        local claw="${SKILLSET_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_skillset_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (skillset daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if SKILLSET_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: skillset daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${SKILLSET_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
