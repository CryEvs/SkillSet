...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.skillset) ? packageJson.skillset : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.skillset?.channel?.id) || left.name;
    const rightId = trimString(right.skillset?.channel?.id) || right.name;
