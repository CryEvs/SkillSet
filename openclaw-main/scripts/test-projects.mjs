...
const TIMINGS_FILE_ENV_KEY = "SKILLSET_TEST_PROJECTS_TIMINGS_PATH";
const TIMINGS_DISABLE_ENV_KEY = "SKILLSET_TEST_PROJECTS_TIMINGS";
...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.SKILLSET_VITEST_MAX_WORKERS || env.SKILLSET_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      SKILLSET_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(process.env.SKILLSET_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        ...
        !process.env.SKILLSET_TEST_PROJECTS_PARALLEL &&
        !process.env.SKILLSET_VITEST_MAX_WORKERS &&
        !process.env.SKILLSET_TEST_WORKERS &&
