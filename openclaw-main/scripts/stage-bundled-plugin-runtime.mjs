...
function removeStaleSkillSetSelfReference(sourcePluginNodeModulesDir, repoRoot) {
  if (!fs.existsSync(sourcePluginNodeModulesDir)) {
    return;
  }

  const selfReferencePath = path.join(sourcePluginNodeModulesDir, "skillset");
  try {
...
function ensureSkillSetExtensionAlias(params) {
  const pluginSdkDir = path.join(params.repoRoot, "dist", "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "skillset");
...
function shouldCopyRuntimeFile(sourcePath) {
  ...
  return (
    ...
    relativePath.endsWith("/skillset.plugin.json") ||
...
function linkPluginNodeModules(params) {
  ...
  removeStaleSkillSetSelfReference(params.sourcePluginNodeModulesDir, params.repoRoot);
  ensureSymlink(
...
export function stageBundledPluginRuntime(params = {}) {
  ...
  ensureSkillSetExtensionAlias({ repoRoot, distExtensionsRoot });
