...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.SKILLSET_BUILD_PRIVATE_QA === "1";
}
...
function isManifestlessBundledRuntimeSupportPackage(params) {
  ...
  if (packageName !== `@skillset/${params.dirName}`) {
    return false;
  }
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "skillset.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "skillset.plugin.json");
    ...
    if (packageJson.skillset && "extensions" in packageJson.skillset) {
      packageJson.skillset = {
        ...packageJson.skillset,
        extensions: rewritePackageExtensions(packageJson.skillset.extensions),
        ...(typeof packageJson.skillset.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.skillset.setupEntry) }
          : {}),
      };
    }
