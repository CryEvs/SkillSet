 ...
 * `skillset update`. Use it to verify the completion installation flow
...
\`skillset update\`, \`skillset doctor\`, and \`skillset onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# SkillSet Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`skillset completion --install\` later to enable.`));
    return;
  }
