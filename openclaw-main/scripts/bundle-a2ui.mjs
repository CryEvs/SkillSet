...
const a2uiAppDir = path.join(rootDir, "apps", "shared", "SkillSetKit", "Tools", "CanvasA2UI");
...
export function getBundleHashRepoInputPaths(repoRoot = rootDir) {
  return [
    ...
    path.join(repoRoot, "apps", "shared", "SkillSetKit", "Tools", "CanvasA2UI"),
  ];
}
...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.SKILLSET_SPARSE_PROFILE || process.env.SKILLSET_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because SKILLSET_A2UI_SKIP_MISSING=1 or SKILLSET_SPARSE_PROFILE is set.",
      );
      return;
    }
