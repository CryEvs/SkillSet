...
#
# Suggested cron: */30 * * * * /home/admin/skillset/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send SkillSet notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., skillset-alerts)
...
STATE_FILE="$HOME/.skillset/auth-monitor-state"
...
send_notification() {
    ...
    # Send via SkillSet if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use skillset
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via SkillSet to $NOTIFY_PHONE..."
            skillset send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! SkillSet is down. Run: ssh l36 '~/skillset/scripts/mobile-reauth.sh'" "urgent"
    exit 1
