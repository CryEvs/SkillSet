...
function makePluginOwnedTempDir(pluginDir, label) {
  return makeTempDir(pluginDir, `.skillset-runtime-deps-${label}-`);
}
...
function replaceDirAtomically(targetPath, sourcePath) {
  ...
  const backupPath = makeTempDir(
    targetParentDir,
    `.skillset-runtime-deps-backup-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function writeJsonAtomically(targetPath, value) {
  ...
  const tempDir = makeTempDir(
    targetParentDir,
    `.skillset-runtime-deps-stamp-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function createRuntimeInstallManifest(pluginId, pinnedGroups) {
  const manifest = {
    name: `skillset-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}`,
    private: true,
    version: "0.0.0",
  };
...
function resolveLegacyRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".skillset-runtime-deps-stamp.json");
}
...
function removeStaleRuntimeDepsTempDirs(pluginDir) {
  ...
  for (const entry of fs.readdirSync(pluginDir, { withFileTypes: true })) {
    if (entry.name.startsWith(".skillset-runtime-deps-")) {
