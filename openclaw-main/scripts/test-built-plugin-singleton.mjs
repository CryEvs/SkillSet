...
const { clearPluginCommands, getPluginCommandSpecs, loadSkillSetPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadSkillSetPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@skillset/build-smoke-plugin",
      type: "module",
      skillset: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "skillset.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'skillset/plugin-sdk';",
...
const registry = loadSkillSetPlugins({
  ...
  env: {
    ...process.env,
    SKILLSET_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
