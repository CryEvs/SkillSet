...
function isSystemThrottleDisabled(env) {
  const normalized = env.SKILLSET_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.SKILLSET_VITEST_MAX_WORKERS ?? env.SKILLSET_TEST_WORKERS);
  if (override !== null) {
