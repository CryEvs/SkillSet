...
// Usage:
// SKILLSET_PROMPT_TRANSPORT=direct|gateway
// SKILLSET_PROMPT_MODE=extra|override
// SKILLSET_PROMPT_TEXT='...'
// SKILLSET_PROMPT_CAPTURE=1
...
import { resolveSkillSetAgentDir } from "../src/agents/agent-paths.js";
...
const TRANSPORT = process.env.SKILLSET_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE =
  process.env.SKILLSET_PROMPT_MODE?.trim() === "override" ? "override" : "extra";
const PROMPT_TEXT = process.env.SKILLSET_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.SKILLSET_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.SKILLSET_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = process.env.SKILLSET_PROMPT_CAPTURE === "1";
const INCLUDE_RAW = process.env.SKILLSET_PROMPT_INCLUDE_RAW === "1";
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.SKILLSET_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = Number(process.env.SKILLSET_PROMPT_TIMEOUT_MS ?? "45000");
const GATEWAY_TIMEOUT_MS = Number(process.env.SKILLSET_PROMPT_GATEWAY_TIMEOUT_MS ?? "120000");
const SETUP_TOKEN_RAW = process.env.SKILLSET_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.SKILLSET_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.SKILLSET_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const DIRECT_CLAUDE_ARGS = ["-p", "--append-system-prompt"];

if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
  throw new Error("missing SKILLSET_PROMPT_TEXT or SKILLSET_PROMPT_LIST_JSON");
}
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set SKILLSET_LIVE_SETUP_TOKEN_VALUE or SKILLSET_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["skillset.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        SKILLSET_CONFIG_PATH: params.configPath,
        SKILLSET_STATE_DIR: params.stateDir,
        SKILLSET_AGENT_DIR: params.agentDir,
        SKILLSET_GATEWAY_TOKEN: params.gatewayToken,
        SKILLSET_SKIP_CHANNELS: "1",
        SKILLSET_SKIP_GMAIL_WATCHER: "1",
        SKILLSET_SKIP_CANVAS_HOST: "1",
        SKILLSET_SKIP_BROWSER_CONTROL_SERVER: "1",
        SKILLSET_DISABLE_BONJOUR: "1",
        SKILLSET_SKIP_CRON: "1",
        SKILLSET_TEST_MINIMAL_GATEWAY: "1",
        SKILLSET_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-gateway-prompt-probe-"));
  const stateDir = path.join(tmpDir, "state");
  const agentDir = path.join(stateDir, "agents", "main", "agent");
  const bundledPluginsDir = path.join(tmpDir, "bundled-plugins-empty");
  const configPath = path.join(tmpDir, "skillset.json");
