...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "skillset.json");
...
function sanitizedEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LOGNAME: process.env.LOGNAME ?? "skillset-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "skillset-bench",
    npm_config_update_notifier: "false",
    SKILLSET_CONFIG: configPath,
    SKILLSET_CONFIG_PATH: configPath,
    SKILLSET_GATEWAY_STARTUP_TRACE: "1",
    SKILLSET_HOME: root,
    SKILLSET_LOCAL_CHECK: "0",
    SKILLSET_STATE_DIR: path.join(root, "state"),
    SKILLSET_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
  return env;
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "skillset-gateway-bench-"));
