...
const PUBLISHED_INSTALLER_BASE_URL = "https://skillset.ai";
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.SKILLSET_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.SKILLSET_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.SKILLSET_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `skillset@${previousVersion}` : "skillset@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.SKILLSET_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.SKILLSET_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        SKILLSET_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `skillset-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".skillset");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    SKILLSET_HOME: lane.homeDir,
    SKILLSET_STATE_DIR: lane.stateDir,
    SKILLSET_CONFIG_PATH: join(lane.stateDir, "skillset.json"),
    SKILLSET_DISABLE_BONJOUR: "1",
    SKILLSET_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
...
export function buildRealUpdateEnv(env) {
  const updateEnv = { ...env };
  delete updateEnv.SKILLSET_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
  return updateEnv;
}

export function resolveExplicitBaselineVersion(baselineSpec) {
  const trimmed = baselineSpec.trim();
  if (!trimmed || trimmed === "skillset@latest") {
    return "";
  }
  if (trimmed.startsWith("skillset@")) {
    return trimmed.slice("skillset@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "skillset@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view skillset@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'skillset.cmd'),
      (Join-Path $npmPrefix 'skillset.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command skillset -ErrorAction Stop
...
Write-Output "__SKILLSET_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__SKILLSET_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed skillset path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v skillset >/dev/null 2>&1",
    'printf "__SKILLSET_PATH__=%s\\n" "$(command -v skillset)"',
    "skillset --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__SKILLSET_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed skillset path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `skillset update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.skillset/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.SKILLSET_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.SKILLSET_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.SKILLSET_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.SKILLSET_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
async function runAgentTurn(params) {
  const sessionId = `cross-os-release-check-${params.label}-${Date.now()}`;
  const result = await runSkillSet({
    lane: params.lane,
    env: params.env,
    args: [
      "agent",
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>SkillSet Control</title>") &&
          html.includes("<skillset-app></skillset-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>SkillSet Control</title>")} app=${html.includes("<skillset-app></skillset-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runSkillSet(params) {
...
function installedPackageRoot(prefixDir) {
  return process.platform === "win32"
    ? join(prefixDir, "node_modules", "skillset")
    : join(prefixDir, "lib", "node_modules", "skillset");
}

function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "skillset.mjs");
}
