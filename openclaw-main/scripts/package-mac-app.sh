...
# Build and bundle SkillSet into a minimal .app we can open.
# Outputs to dist/SkillSet.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/SkillSet.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="SkillSet"
MLX_TTS_HELPER_PRODUCT="skillset-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.skillset.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/skillset/skillset/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/SkillSet/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :SkillSetBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :SkillSetGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/SkillSet"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/SkillSet"
fi
chmod +x "$APP_ROOT/Contents/MacOS/SkillSet"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/SkillSet" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/SkillSet/Resources/SkillSet.icns" "$APP_ROOT/Contents/Resources/SkillSet.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/SkillSet/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying SkillSetKit resources"
SKILLSETKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/SkillSetKit_SkillSetKit.bundle"
if [ -d "$SKILLSETKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/SkillSetKit_SkillSetKit.bundle"
  cp -R "$SKILLSETKIT_BUNDLE" "$APP_ROOT/Contents/Resources/SkillSetKit_SkillSetKit.bundle"
else
  echo "WARN: SkillSetKit resource bundle not found at $SKILLSETKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running SkillSet"
killall -q SkillSet 2>/dev/null || true
