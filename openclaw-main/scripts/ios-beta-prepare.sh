...
- regenerates apps/ios/SkillSet.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${SKILLSET_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid SKILLSET_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid SKILLSET_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid SKILLSET_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid SKILLSET_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing SKILLSET_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(SKILLSET_URL_SLASH)$(SKILLSET_URL_SLASH)#g'
)"
...
SKILLSET_CODE_SIGN_STYLE = Automatic
SKILLSET_DEVELOPMENT_TEAM = ${TEAM_ID}
SKILLSET_IOS_SELECTED_TEAM = ${TEAM_ID}
SKILLSET_APP_BUNDLE_ID = ai.skillset.client
SKILLSET_SHARE_BUNDLE_ID = ai.skillset.client.share
SKILLSET_ACTIVITY_WIDGET_BUNDLE_ID = ai.skillset.client.activitywidget
SKILLSET_WATCH_APP_BUNDLE_ID = ai.skillset.client.watchkitapp
SKILLSET_WATCH_EXTENSION_BUNDLE_ID = ai.skillset.client.watchkitapp.extension
SKILLSET_APP_PROFILE =
SKILLSET_SHARE_PROFILE =
SKILLSET_PUSH_TRANSPORT = relay
SKILLSET_PUSH_DISTRIBUTION = official
SKILLSET_URL_SLASH = /
SKILLSET_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
SKILLSET_PUSH_APNS_ENVIRONMENT = production
EOF
