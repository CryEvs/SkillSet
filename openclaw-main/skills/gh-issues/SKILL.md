...
IMPORTANT — No `gh` CLI dependency. This skill uses curl + the GitHub REST API exclusively. The GH_TOKEN env var is already injected by SkillSet. Pass it as a Bearer token in all API calls:
...
If still empty, check `/data/.clawdbot/skillset.json`:

```
cat /data/.clawdbot/skillset.json | jq -r '.skills.entries["gh-issues"].apiKey // empty'
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the SkillSet dashboard or in the active SkillSet config path (`$SKILLSET_CONFIG_PATH`, default `~/.skillset/skillset.json`) under `skills.entries.gh-issues`."
...
- **Sequential cursor tracking:** Use a cursor file to track which issue to process next:
  ...
  # SOURCE_REPO_SLUG = owner-repo with slashes replaced by hyphens (e.g., skillset-skillset)
  ```
...
CONFIG_PATH="${SKILLSET_CONFIG_PATH:-${SKILLSET_STATE_DIR:-$HOME/.skillset}/skillset.json}"
GH_TOKEN=$(cat "$CONFIG_PATH" 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty') || GH_TOKEN=$(cat /data/.clawdbot/skillset.json 2>/dev/null | jq -r '.skills.entries["gh-issues"].apiKey // empty')
...
export GH_TOKEN=$(node -e "const fs=require('fs'); const c=JSON.parse(fs.readFileSync('/data/.clawdbot/skillset.json','utf8')); console.log(c.skills?.entries?.['gh-issues']?.apiKey || '')")

```
If that fails, also try:
```

export CONFIG_PATH="${SKILLSET_CONFIG_PATH:-${SKILLSET_STATE_DIR:-$HOME/.skillset}/skillset.json}"
