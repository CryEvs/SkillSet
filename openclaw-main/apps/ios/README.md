# SkillSet iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an SkillSet Gateway as a `role: node`.
...
- Beta release uses canonical `ai.skillset.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `SkillSetPushTransport=relay`, `SkillSetPushDistribution=official`, and `SkillSetPushAPNsEnvironment=production`.
...
- `SKILLSET_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
```bash
export SKILLSET_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
7. Expected outputs after a successful run:
   - `apps/ios/build/beta/SkillSet-<version>.ipa`
   - `apps/ios/build/beta/SkillSet-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/SkillSet.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `SkillSetPushTransport=direct` and `SkillSetPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `SKILLSET_APNS_TEAM_ID`, `SKILLSET_APNS_KEY_ID`, and either `SKILLSET_APNS_PRIVATE_KEY_P8` or `SKILLSET_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.skillset/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `SKILLSET_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `SkillSetPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `skillset.json`. `SKILLSET_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine SkillSet official builds and to ensure a
