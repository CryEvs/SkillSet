...
final class RemindersService: RemindersServicing {
    func list(params: SkillSetRemindersListParams) async throws -> SkillSetRemindersListPayload {
        ...
        let payload: [SkillSetReminderPayload] = try await withCheckedThrowingContinuation { cont in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return SkillSetReminderPayload(
        ...
        return SkillSetRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: SkillSetRemindersAddParams) async throws -> SkillSetRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = SkillSetReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return SkillSetRemindersAddPayload(reminder: payload)
    }
