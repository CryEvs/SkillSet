...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: SkillSetWatchNotifyParams) -> SkillSetWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [SkillSetWatchAction]?,
        kind: String?,
        promptId: String?) -> [SkillSetWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> SkillSetWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return SkillSetWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                SkillSetWatchAction(id: "approve", label: "Approve"),
                SkillSetWatchAction(id: "decline", label: "Decline", style: "destructive"),
                SkillSetWatchAction(id: "open_phone", label: "Open iPhone"),
                SkillSetWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            SkillSetWatchAction(id: "done", label: "Done"),
            SkillSetWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            SkillSetWatchAction(id: "open_phone", label: "Open iPhone"),
            SkillSetWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: SkillSetWatchRisk?,
        priority: SkillSetNotificationPriority?) -> SkillSetWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: SkillSetNotificationPriority?,
        risk: SkillSetWatchRisk?) -> SkillSetNotificationPriority?
    {
