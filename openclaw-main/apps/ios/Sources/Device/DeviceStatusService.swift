...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> SkillSetDeviceStatusPayload {
        ...
        return SkillSetDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> SkillSetDeviceInfoPayload {
        ...
        return SkillSetDeviceInfoPayload(
    ...
    private func batteryStatus() -> SkillSetBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: SkillSetBatteryState = switch device.batteryState {
        ...
        return SkillSetBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> SkillSetThermalStatusPayload {
        let state: SkillSetThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return SkillSetThermalStatusPayload(state: state)
    }

    private func storageStatus() -> SkillSetStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return SkillSetStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
