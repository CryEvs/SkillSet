...
private let chatUILogger = Logger(subsystem: "ai.skillset", category: "SkillSetChatUI")

@MainActor
@Observable
public final class SkillSetChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [SkillSetChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [SkillSetChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [SkillSetPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [SkillSetChatPendingToolCall] = []
    public private(set) var sessions: [SkillSetChatSessionEntry] = []
    private let transport: any SkillSetChatTransport
    private var sessionDefaults: SkillSetChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: SkillSetChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any SkillSetChatTransport,
    ...
    public var sessionChoices: [SkillSetChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [SkillSetChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: SkillSetPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [SkillSetChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: SkillSetChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: SkillSetChatMessage) -> SkillSetChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> SkillSetChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return SkillSetChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: SkillSetChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: SkillSetChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: SkillSetChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [SkillSetChatMessage],
        incoming: [SkillSetChatMessage]) -> [SkillSetChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [SkillSetChatMessage]) -> [SkillSetChatMessage] {
        var result: [SkillSetChatMessage] = []
    ...
    private static func dedupeKey(for message: SkillSetChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [SkillSetChatMessageContent] = [
            SkillSetChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> SkillSetChatAttachmentPayload in
            SkillSetChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> SkillSetChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = SkillSetChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: SkillSetChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: SkillSetChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: SkillSetAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = SkillSetChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> SkillSetPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
