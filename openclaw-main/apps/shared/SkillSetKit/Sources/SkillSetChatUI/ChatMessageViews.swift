...
struct ChatMessageBubble: View {
    let message: SkillSetChatMessage
    let style: SkillSetChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? SkillSetChatTheme.userText : SkillSetChatTheme.assistantText
    ...
    private var inlineAttachments: [SkillSetChatMessageContent] {
    ...
    private var toolCalls: [SkillSetChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [SkillSetChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? SkillSetChatTheme.userBubble
        }
        if self.style == .onboarding {
            return SkillSetChatTheme.onboardingAssistantBubble
        }
        return SkillSetChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return SkillSetChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: SkillSetChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? SkillSetChatTheme.userText : SkillSetChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: SkillSetChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(SkillSetChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [SkillSetChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: SkillSetChatTheme.assistantText)
            }
        }
    }
}
