...
private const val NOTIFICATION_CHANNEL_BASE_ID = "skillset.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "SkillSet Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "SkillSet Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "SkillSet Active")
      }
