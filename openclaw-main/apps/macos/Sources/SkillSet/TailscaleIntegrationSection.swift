...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.skillset/skillset.json. Prefer SKILLSET_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.skillset/skillset.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.skillset/skillset.json. Restart the gateway to apply."
        }
