...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: SkillSetProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.skillset", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: SkillSetKit.AnyCodable]()) {
                $0[$1.key] = SkillSetKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: SkillSetProtocol.AnyCodable?) -> [String: SkillSetProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: SkillSetProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: SkillSetKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: SkillSetProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("skillset.control.heartbeat")
    static let controlAgentEvent = Notification.Name("skillset.control.agent")
}
