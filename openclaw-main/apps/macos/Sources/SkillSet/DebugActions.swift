...
enum DebugActions {
    private static let verboseDefaultsKey = "skillset.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "skillset.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = SkillSetPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "SkillSet", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = SkillSetPaths.configURL
        guard
