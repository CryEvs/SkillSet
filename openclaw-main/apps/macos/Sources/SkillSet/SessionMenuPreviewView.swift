...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.skillset", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> SkillSetSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: SkillSetSessionPreviewEntry,
    ...
    private static func cache(payload: SkillSetSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: SkillSetChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [SkillSetKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [SkillSetKit.AnyCodable]) -> [SkillSetChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(SkillSetChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: SkillSetChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: SkillSetChatMessage) -> Bool {
    ...
    private static func toolNames(for message: SkillSetChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: SkillSetChatMessage) -> String? {
