...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.skillset", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        let sshKey = SkillSetConfigFile.hostKey(sshHost)
        let urlKey = SkillSetConfigFile.hostKey(host)
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.skillset.remote.tunnel.port", qos: .utility)
            do {
