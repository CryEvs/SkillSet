...
enum SkillSetLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = SkillSetOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = SkillSetFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        SkillSetLogging.bootstrapIfNeeded()
        let label = SkillSetLogging.makeLabel(subsystem: subsystem, category: category)
...
struct SkillSetOSLogHandler: AppLogLevelBackedHandler {
...
struct SkillSetFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = SkillSetLogging.parseLabel(self.label)
