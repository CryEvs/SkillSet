...
enum CommandResolver {
    private static let projectRootDefaultsKey = "skillset.gatewayProjectRootPath"
    private static let helperName = "skillset"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let skillsetEntry = root.appendingPathComponent("skillset.mjs").path
        if FileManager().isReadableFile(atPath: skillsetEntry) { return skillsetEntry }
        let binEntry = root.appendingPathComponent("bin/skillset.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/skillset")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let skillsetPaths = self.skillsetManagedPaths(home: home)
        if !skillsetPaths.isEmpty {
            extras.insert(contentsOf: skillsetPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + skillsetPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func skillsetManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".skillset"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func skillsetExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectSkillSetExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("skillset.mjs").path,
            root.appendingPathComponent("bin/skillset.js").path,
        ]
    ...
    static func hasAnySkillSetInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.skillsetExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func skillsetNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let skillsetPath = self.projectSkillSetExecutable(projectRoot: root) {
            return [skillsetPath, subcommand] + extraArgs
        }
        if let skillsetPath = self.skillsetExecutable(searchPaths: searchPaths) {
            return [skillsetPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "skillset", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            skillset entrypoint missing (looked for dist/index.js or skillset.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real skillset CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/skillset"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/skillset.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/skillset.mjs"
            node "$PRJ/skillset.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/skillset.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/skillset.js"
            node "$PRJ/bin/skillset.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent skillset"
          pnpm --silent skillset \(quotedArgs);
        else
          echo "skillset CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? SkillSetConfigFile.loadDict()
