...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.skillset.mac`) even if the shipped
    ...
    {
        if environment["SKILLSET_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "skillset.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "skillset.nixMode") { return true }
