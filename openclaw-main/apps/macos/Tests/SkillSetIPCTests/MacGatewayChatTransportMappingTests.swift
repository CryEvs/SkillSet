...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: SkillSetProtocol.AnyCodable(["ok": SkillSetProtocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: SkillSetProtocol.AnyCodable(["ok": SkillSetProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = SkillSetProtocol.AnyCodable([
            "runId": SkillSetProtocol.AnyCodable("run-1"),
            "sessionKey": SkillSetProtocol.AnyCodable("main"),
            "state": SkillSetProtocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: SkillSetProtocol.AnyCodable(["a": SkillSetProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
