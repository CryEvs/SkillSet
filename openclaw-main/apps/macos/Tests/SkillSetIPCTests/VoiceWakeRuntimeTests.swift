...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["skillset"]
        let text = "hey skillset"
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey skillset do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["skillset"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "skillset", start: 0.2, duration: 0.1, range: transcript.range(of: "skillset")),
