...
import Foundation
import Testing
@testable import SkillSetDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("SKILLSET_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("SKILLSET_WIDE_AREA_DOMAIN", "skillset.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("SKILLSET_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("SKILLSET_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._skillset-gw._tcp.skillset.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.skillset.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (SkillSet)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/skillset/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (SkillSet)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/skillset/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._skillset-gw._tcp.skillset.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
