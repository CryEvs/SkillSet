...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[skillset proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[skillset proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`skillset proxy coverage\` for details.`,
  );
}
