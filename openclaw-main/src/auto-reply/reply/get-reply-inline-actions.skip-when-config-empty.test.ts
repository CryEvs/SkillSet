...
const { buildStatusReplyMock, createSkillSetToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createSkillSetToolsMock: vi.fn(),
...
vi.mock("../../agents/skillset-tools.runtime.js", () => ({
  createSkillSetTools: (...args: unknown[]) => createSkillSetToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createSkillSetToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
