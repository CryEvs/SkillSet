...
const mocks = vi.hoisted(() => ({
  ...
  formatDoctorNonInteractiveHint: vi.fn(() => "Run: skillset doctor --non-interactive"),
  ...
  triggerSkillSetRestart: vi.fn(() => ({ ok: true, method: "launchctl" })),
}));
...
vi.mock("../../infra/restart.js", () => ({
  ...
  triggerSkillSetRestart: mocks.triggerSkillSetRestart,
}));
...
describe("handleRestartCommand", () => {
  beforeEach(() => {
    ...
    mocks.triggerSkillSetRestart.mockReset();
    mocks.triggerSkillSetRestart.mockReturnValue({ ok: true, method: "launchctl" });
  });
  ...
  it("writes a routed restart sentinel before restarting from chat", async () => {
    ...
    expect(mocks.triggerSkillSetRestart).toHaveBeenCalledTimes(1);
  });
  ...
  it("prepares the routed sentinel only when SIGUSR1 restart emits", async () => {
    ...
    try {
      ...
      expect(mocks.triggerSkillSetRestart).not.toHaveBeenCalled();
  ...
  it("removes the success sentinel when fallback restart fails", async () => {
    mocks.triggerSkillSetRestart.mockReturnValueOnce({
      ok: false,
      method: "launchctl",
    });
