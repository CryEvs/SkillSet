...
export type ModelAddAdapter = {
  providerId: string;
  bootstrapProviderConfig?: (cfg: SkillSetConfig) => ModelProviderConfig | null;
  detect?: (params: {
    cfg: SkillSetConfig;
...
function canAddProvider(params: { cfg: SkillSetConfig; provider: string }): boolean {
...
function ensureProviderConfig(params: { cfg: SkillSetConfig; provider: string }):
  | {
      ok: true;
      providerKey: string;
      providerConfig: ModelProviderConfig;
...
function upsertModelEntry(params: {
  ...
  providerConfig: ModelProviderConfig;
  model: ModelDefinitionConfig;
}): { nextConfig: SkillSetConfig; existed: boolean } {
...
function maybeAddAllowlistEntry(params: {
  ...
  provider: string;
  modelId: string;
}): { nextConfig: SkillSetConfig; added: boolean } {
...
export async function addModelToConfig(params: {
  ...
  const currentConfig = structuredClone(snapshot.parsed as SkillSetConfig);
  const providerResolution = ensureProviderConfig({
    cfg: currentConfig,
    provider,
  });
