...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop skillset!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@skillset_bot", { botUsername: "skillset_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@skillset_bot", { botUsername: "skillset_bot" })).toBe(true);
