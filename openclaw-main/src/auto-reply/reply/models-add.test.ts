...
describe("models-add", () => {
  ...
  it("adds an ollama model and extends the allowlist when needed", async () => {
    ...
    configMocks.validateConfigObjectWithPlugins.mockImplementation((config: SkillSetConfig) => ({
      ok: true,
      config,
    }));
    ...
    const written = configMocks.replaceConfigFile.mock.calls[0]?.[0]?.nextConfig as SkillSetConfig;
