...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("skillset.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("skillset.reply.full-runtime");

type ReplyConfigWithMarker = SkillSetConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.SKILLSET_ALLOW_SLOW_REPLY_TESTS === "1" || env.SKILLSET_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends SkillSetConfig>(
  config: T,
...
export function withFastReplyConfig<T extends SkillSetConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends SkillSetConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

export function isCompleteReplyConfig(config: unknown): config is SkillSetConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set SKILLSET_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  if (params.isFastTestEnv && isCompleteReplyConfig(configOverride)) {
    return configOverride;
  }
  return applyMergePatch(params.loadConfig(), configOverride) as SkillSetConfig;
}
