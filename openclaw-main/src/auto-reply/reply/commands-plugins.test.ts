...
vi.mock("../../plugins/toggle-config.js", () => ({
  setPluginEnabledInConfig: vi.fn((config: SkillSetConfig, id: string, enabled: boolean) => ({
    ...config,
    plugins: {
      ...config.plugins,
      entries: {
...
function buildCfg(): SkillSetConfig {
  return {
...
function buildPluginsParams(commandBodyNormalized: string, cfg: SkillSetConfig) {
