...
type HomeEnvSnapshot = {
  ...
  SKILLSET_STATE_DIR: string | undefined;
  SKILLSET_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    SKILLSET_AGENT_DIR: process.env.SKILLSET_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".skillset", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.SKILLSET_STATE_DIR = path.join(home, ".skillset");
    process.env.SKILLSET_AGENT_DIR = path.join(home, ".skillset", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".skillset", "agent");
