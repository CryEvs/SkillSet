...
export function makeTempDir(): string {
  return makeTrackedTempDir("skillset-plugin-auto-enable", tempDirs);
}
...
export function writePluginManifestFixture(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "skillset.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function makeRegistry(
  ...
  return {
    plugins: plugins.map((plugin) => ({
      ...
      manifestPath: `/fake/${plugin.id}/skillset.plugin.json`,
    })),
    diagnostics: [],
  };
}
