...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ SKILLSET_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-skillset",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-skillset#general");
  });
  ...
  it("derives session transcripts dir from SKILLSET_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { SKILLSET_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when SKILLSET_STATE_DIR differs from stored paths", () => {
