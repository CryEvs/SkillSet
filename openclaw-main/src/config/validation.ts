...
import type { SkillSetConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.skillset.ai/reference/secretref-credential-surface";
...
function validateIdentityAvatar(config: SkillSetConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: SkillSetConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
...
): { ok: true; config: SkillSetConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = SkillSetSchema.safeParse(raw);
  ...
  const validatedConfig = validated.data as SkillSetConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: SkillSetConfig | null | undefined;
  ...
  const ensureCompatConfig = (): SkillSetConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native SkillSet config schema.
