...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.skillset/skillset.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".skillset", 19001);
      const io = createIoForHome(home);
      expect(io.configPath).toBe(configPath);
    });
  });

  it("defaults to ~/.skillset/skillset.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".skillset", "skillset.json"));
    });
  });

  it("uses SKILLSET_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { SKILLSET_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".skillset", "skillset.json"));
    });
  });

  it("honors explicit SKILLSET_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".skillset", 20002, "custom.json");
      const io = createIoForHome(home, { SKILLSET_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("moves WhatsApp shared access defaults into accounts.default during runtime compat", () => {
    ...
    const next = migrated.next as SkillSetConfig | null;
