...
export function parseSkillSetVersion(raw: string | null | undefined): SkillSetVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeSkillSetVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseSkillSetVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameSkillSetStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseSkillSetVersion(a);
  const parsedB = parseSkillSetVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareSkillSetVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseSkillSetVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseSkillSetVersion(current);
  const parsedTouched = parseSkillSetVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameSkillSetStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareSkillSetVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: SkillSetVersion): number {
