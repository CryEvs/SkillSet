...
describe("config io write prepare", () => {
  ...
  it("preserves authored source-only nested fields during partial writes", () => {
      ...
      nextConfig: {
        plugins: {
          ...
          installs: {
            "skillset-web-search": {
              source: "npm",
              spec: "@ollama/skillset-web-search@0.2.2",
              installPath: "/tmp/skillset-web-search",
              resolvedName: "@ollama/skillset-web-search",
              resolvedVersion: "0.2.2",
            },
    ...
    expect(persisted.plugins?.installs?.["skillset-web-search"]).toEqual({
      source: "npm",
      spec: "@ollama/skillset-web-search@0.2.2",
      installPath: "/tmp/skillset-web-search",
      resolvedName: "@ollama/skillset-web-search",
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("skillset config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('skillset config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/skillset.json",
        expectedConfigPath: "/tmp/skillset.json",
  ...
  it("keeps plugin AJV defaults out of the persisted candidate", () => {
    ...
    const nextConfig: SkillSetConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("preserves normalized nested channel enabled keys during unrelated writes", () => {
    ...
    const nextConfig: SkillSetConfig = {
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://skillset.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
