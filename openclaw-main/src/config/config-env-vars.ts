...
function collectConfigEnvVarsByTarget(cfg?: SkillSetConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: SkillSetConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: SkillSetConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: SkillSetConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.SKILLSET_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
