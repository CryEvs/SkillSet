...
import type { SkillSetConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function coerceConfig(value: unknown): SkillSetConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as SkillSetConfig;
}
...
function stampConfigVersion(cfg: SkillSetConfig): SkillSetConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: SkillSetConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer SkillSet (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as SkillSetConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function finalizeLoadedRuntimeConfig(cfg: SkillSetConfig): SkillSetConfig {
  ...
  function loadConfig(): SkillSetConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as SkillSetConfig, {
        env: deps.env,
        homedir: deps.homedir,
      });
  ...
  async function readConfigFileSnapshotInternal(): Promise<ReadConfigFileSnapshotInternalResult> {
    ...
    let fallbackSourceConfig: SkillSetConfig = {};
  ...
  async function readBestEffortConfig(): Promise<SkillSetConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffort(): Promise<SkillSetConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFile(
  ...
  ): Promise<{ persistedHash: string; persistedConfig: SkillSetConfig }> {
    clearConfigCache();
    ...
    let cfgToWrite = persistCandidate as SkillSetConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as SkillSetConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.SKILLSET_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.SKILLSET_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
...
// module scope. `SKILLSET_CONFIG_PATH` (and friends) are expected to work even
...
function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: SkillSetConfig;
  candidate: SkillSetConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: SkillSetConfig): SkillSetConfig {
...
export function loadConfig(): SkillSetConfig {
  ...
  // reparsing skillset.json on hot code paths.
  return loadPinnedRuntimeConfig(() => createConfigIO().loadConfig());
}

export function getRuntimeConfig(): SkillSetConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<SkillSetConfig> {
  return await createConfigIO().readBestEffortConfig();
}

export async function readSourceConfigBestEffort(): Promise<SkillSetConfig> {
