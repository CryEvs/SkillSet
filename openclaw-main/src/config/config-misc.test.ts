...
import { buildWebSearchProviderConfig, withTempHome, writeSkillSetConfig } from "./test-helpers.js";
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    ...
    if (result.success) {
      expect(result.data.$schema).toBe("https://skillset.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = SkillSetSchema.safeParse({ $schema: 123 });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://skillset.ai/config.json");
    }
  });
});
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@skillset"]);
    }
  });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = SkillSetSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("accepts top-level memorySearch via auto-migration and reports legacyIssues", async () => {
    await withTempHome(async (home) => {
      await writeSkillSetConfig(home, {
        memorySearch: {
          provider: "local",
          fallback: "none",
          query: { maxResults: 7 },
  ...
  it("does not treat resolved-only gateway.bind aliases as source-literal legacy or invalid", async () => {
    await withTempHome(async (home) => {
      await writeSkillSetConfig(home, {
        gateway: { bind: "${SKILLSET_BIND}" },
      });

      const prev = process.env.SKILLSET_BIND;
      process.env.SKILLSET_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(true);
        ...
        if (prev === undefined) {
          delete process.env.SKILLSET_BIND;
        } else {
          process.env.SKILLSET_BIND = prev;
        }
      }
    });
  });
