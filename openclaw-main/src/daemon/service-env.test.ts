...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.SKILLSET_GATEWAY_PORT).toBe("18789");
    expect(env.SKILLSET_GATEWAY_TOKEN).toBeUndefined();
    expect(env.SKILLSET_SERVICE_MARKER).toBe("skillset");
    expect(env.SKILLSET_SERVICE_KIND).toBe("gateway");
    expect(typeof env.SKILLSET_SERVICE_VERSION).toBe("string");
    expect(env.SKILLSET_SYSTEMD_UNIT).toBe("skillset-gateway.service");
    expect(env.SKILLSET_WINDOWS_TASK_NAME).toBe("SkillSet Gateway");
    if (process.platform === "darwin") {
      expect(env.SKILLSET_LAUNCHD_LABEL).toBe("ai.skillset.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", SKILLSET_PROFILE: "work" },
      port: 18789,
    });
    expect(env.SKILLSET_SYSTEMD_UNIT).toBe("skillset-gateway-work.service");
    expect(env.SKILLSET_WINDOWS_TASK_NAME).toBe("SkillSet Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.SKILLSET_LAUNCHD_LABEL).toBe("ai.skillset.work");
    }
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through SKILLSET_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", SKILLSET_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.SKILLSET_GATEWAY_TOKEN).toBe("node-token");
  });

  it("omits SKILLSET_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".skillset"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", SKILLSET_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".skillset-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", SKILLSET_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".skillset"));
  });

  it("uses SKILLSET_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", SKILLSET_STATE_DIR: "/var/lib/skillset" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/skillset"));
  });

  it("expands ~ in SKILLSET_STATE_DIR", () => {
    const env = { HOME: "/Users/test", SKILLSET_STATE_DIR: "~/skillset-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/skillset-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { SKILLSET_STATE_DIR: "C:\\State\\skillset" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\skillset");
  });
});
