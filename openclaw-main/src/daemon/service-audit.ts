...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds SKILLSET_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `skillset gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service SKILLSET_GATEWAY_TOKEN does not match gateway.auth.token in skillset.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.SKILLSET_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.SKILLSET_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `skillset gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
