...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): SkillSetConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as SkillSetConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: SkillSetConfig) => string[];
  input: SkillSetConfig;
  expected: string[];
}) {
  expect(params.resolve(params.input)).toEqual(params.expected);
}

function expectResolvedDefaultAccountCase(input: SkillSetConfig, expected: string) {
