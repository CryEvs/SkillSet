...
let compiledCoreSkillSetTargetState: {
  knownTargetIds: Set<string>;
  openClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  openClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledBundledChannelSkillSetTargets = new Map<
  string,
...
function getCompiledSecretTargetRegistryState() {
  ...
  const openClawCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "skillset.json",
  );
...
function getCompiledCoreSkillSetTargetState() {
  if (compiledCoreSkillSetTargetState) {
    return compiledCoreSkillSetTargetState;
  }
  const openClawCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "skillset.json")
    .map(compileTargetRegistryEntry);
  compiledCoreSkillSetTargetState = {
...
function getCompiledBundledChannelSkillSetTargets(
  channelId: string,
): CompiledTargetRegistryEntry[] | null {
  ...
  if (compiledBundledChannelSkillSetTargets.has(normalizedChannelId)) {
    return compiledBundledChannelSkillSetTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadBundledChannelSecretContractApi(normalizedChannelId)
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "skillset.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledBundledChannelSkillSetTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreSkillSetTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreSkillSetTargetState().openClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitBundledChannelEntries = explicitBundledChannelId
    ? getCompiledBundledChannelSkillSetTargets(explicitBundledChannelId)
    : null;
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreSkillSetTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreSkillSetTargetState()
