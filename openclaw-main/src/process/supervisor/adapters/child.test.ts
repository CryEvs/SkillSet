...
describe("createChildAdapter", () => {
  const originalServiceMarker = process.env.SKILLSET_SERVICE_MARKER;
  ...
  beforeEach(() => {
    ...
    delete process.env.SKILLSET_SERVICE_MARKER;
    vi.useRealTimers();
  });
  ...
  afterAll(() => {
    if (originalServiceMarker === undefined) {
      ...
      process.env.SKILLSET_SERVICE_MARKER = originalServiceMarker;
    }
  });
  ...
  it("disables detached mode in service-managed runtime", async () => {
    process.env.SKILLSET_SERVICE_MARKER = "skillset";
