...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        SKILLSET_BASE_ENV: "base",
        SKILLSET_TO_REMOVE: undefined,
      },
      env: {
        SKILLSET_TEST_ENV: "ok",
      },
    });

    expect(resolved.SKILLSET_BASE_ENV).toBe("base");
    expect(resolved.SKILLSET_TEST_ENV).toBe("ok");
    expect(resolved.SKILLSET_TO_REMOVE).toBeUndefined();
    expect(resolved.SKILLSET_CLI).toBe(SKILLSET_CLI_ENV_VALUE);
  });
