...
const createTempDir = () => tempDirs.make("skillset-acp-client-test-");
...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets SKILLSET_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "skillset",
    });

    expect(env.SKILLSET_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("skillset");
  });

  it("overrides pre-existing SKILLSET_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      SKILLSET_SHELL: "wrong",
    });
    expect(env.SKILLSET_SHELL).toBe("acp-client");
  });
  ...
  it("preserves SKILLSET_SHELL even when stripKeys contains it", () => {
    const openAiApiKeyEnv = envVar("OPENAI", "API", "KEY");
    const env = resolveAcpClientSpawnEnv(
      {
        SKILLSET_SHELL: "skill-overridden",
        [openAiApiKeyEnv]: "openai-leaked", // pragma: allowlist secret
      },
      { stripKeys: new Set(["SKILLSET_SHELL", openAiApiKeyEnv]) },
    );
  ...
  it("strips provider auth env vars for the default SkillSet bridge", () => {
    ...
    expect(env.SKILLSET_API_KEY).toBe("keep-me");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.SKILLSET_SHELL).toBe("acp-client");
  });
...
describe("shouldStripProviderAuthEnvVarsForAcpServer", () => {
  it("strips provider auth env vars for the default bridge", () => {
    ...
    expect(
      shouldStripProviderAuthEnvVarsForAcpServer({
        serverCommand: "skillset",
        serverArgs: ["acp"],
        defaultServerCommand: "skillset",
...
describe("buildAcpClientStripKeys", () => {
  ...
  it("adds provider auth env vars for the default bridge", () => {
    ...
    expect(stripKeys.has("SKILLSET_API_KEY")).toBe(false);
  });
});
...
describe("resolveAcpClientSpawnInvocation", () => {
  it("keeps non-windows invocation unchanged", () => {
    const resolved = resolveAcpClientSpawnInvocation(
      { serverCommand: "skillset", serverArgs: ["acp", "--verbose"] },
      {
        platform: "darwin",
        env: {},
  ...
  it("unwraps .cmd shim entrypoint on windows", async () => {
    ...
    const scriptPath = path.join(dir, "skillset", "dist", "entry.js");
    const shimPath = path.join(dir, "skillset.cmd");
    await mkdir(path.dirname(scriptPath), { recursive: true });
    await writeFile(scriptPath, "console.log('ok')\n", "utf8");
    await writeFile(shimPath, `@ECHO off\r\n"%~dp0\\skillset\\dist\\entry.js" %*\r\n`, "utf8");
...
describe("resolvePermissionRequest", () => {
    ...
    await expectAutoAllowWithoutPrompt({
      request: {
        toolCall: {
          ...
          title: "read: ignored-by-raw-input",
          status: "pending",
          rawInput: { path: "file:///tmp/skillset-acp-cwd/docs/security.md" },
        },
      },
      cwd: "/tmp/skillset-acp-cwd",
    });
  ...
  it("prompts for read when rawInput path escapes cwd via traversal", async () => {
    ...
    const res = await resolvePermissionRequest(
      ...
      { prompt, log: () => {}, cwd: "/tmp/skillset-acp-cwd/workspace" },
    );
    expect(prompt).toHaveBeenCalledTimes(1);
