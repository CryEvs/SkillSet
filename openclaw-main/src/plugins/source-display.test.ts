...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "skillset", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".skillset", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".skillset", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "skillset-home");
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".skillset", "extensions"),
      },
    });
  });
});
