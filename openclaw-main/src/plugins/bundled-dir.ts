...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "skillset-empty-bundled-plugins");

function bundledPluginsDisabled(env: NodeJS.ProcessEnv): boolean {
  const raw = normalizeOptionalLowercaseString(env.SKILLSET_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "skillset.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.SKILLSET_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveSkillSetPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveSkillSetPackageRootSync({ argv1: process.argv[1] }),
      resolveSkillSetPackageRootSync({ cwd: process.cwd() }),
      resolveSkillSetPackageRootSync({ moduleUrl: import.meta.url }),
    ].filter(
