...
import type {
  ...
  SkillSetPluginApi,
  SkillSetPluginChannelRegistration,
  SkillSetPluginCliCommandDescriptor,
  SkillSetPluginCliRegistrar,
  SkillSetPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  SkillSetPluginGatewayRuntimeScopeSurface,
  SkillSetPluginHttpRouteParams,
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("skillset.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerPiEmbeddedExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<SkillSetPluginApi["registerEmbeddedExtensionFactory"]>[0],
  ) => {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<SkillSetPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
    if (record.origin !== "bundled") {
      pushDiagnostic({
        level: "error",
  ...
  const registerTool = (
    ...
    const factory: SkillSetPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: SkillSetPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: SkillSetPluginHookOptions | undefined,
    config: SkillSetPluginApi["config"],
  ) => {
  ...
  const registerHttpRoute = (record: PluginRecord, params: SkillSetPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: SkillSetPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as SkillSetPluginChannelRegistration).plugin === "object"
        ? (registration as SkillSetPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: SkillSetPluginCliCommandDescriptor[] },
  ) => {
  ...
  const registerReload = (record: PluginRecord, registration: SkillSetPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: SkillSetPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: SkillSetPluginNodeHostCommand,
  ) => {
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: SkillSetPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: SkillSetPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: SkillSetPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationMode === "full"
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<SkillSetPluginApi["registerCompactionProvider"]>[0],
              ) => {
