...
function writeBundledPlugin(params: {
  ...
  process.env.SKILLSET_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".skillset", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ SKILLSET_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.SKILLSET_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
      ...
      JSON.stringify(
        {
          ...
          skillset: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.SKILLSET_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadSkillSetPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadSkillSetPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadSkillSetPlugins>[0], "cache" | "config">,
) {
  return loadSkillSetPlugins({
    cache: false,
    ...options,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadSkillSetPlugins>,
  pluginId: string,
) {
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadSkillSetPlugins>;
  loadSecond: () => ReturnType<typeof loadSkillSetPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadSkillSetPlugins>;
  loadVariant: () => ReturnType<typeof loadSkillSetPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const skillsetHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(skillsetHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    SKILLSET_HOME: skillsetHome,
    HOME: ignoredHome,
    SKILLSET_STATE_DIR: stateDir,
    SKILLSET_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadSkillSetPlugins({
    cache: false,
    config: {
      plugins: {
...
module.exports = {
  ...
  register() {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@skillset/discord",
          version: "1.0.0",
          dependencies: {
            "discord-runtime": "1.0.0",
          },
          skillset: { extensions: ["./index.cjs"] },
        },
        null,
        2,
      ),
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "skillset.plugin.json"),
      JSON.stringify(
        {
          id: "discord",
          channels: ["discord"],
...
`,
  ...
  it("loads bundled runtime deps from an external stage dir", () => {
    ...
    process.env.SKILLSET_BUNDLED_PLUGINS_DIR = bundledDir;
    process.env.SKILLSET_PLUGIN_STAGE_DIR = stageDir;
    fs.writeFileSync(
      path.join(plugin.dir, "package.json"),
      JSON.stringify(
        {
  ...
  it("loads dist-runtime wrappers from an external stage dir", () => {
    ...
    fs.writeFileSync(
      path.join(pluginRoot, "skillset.plugin.json"),
      JSON.stringify(
        {
          id: "acpx",
          enabledByDefault: true,
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadSkillSetPlugins>) => {
  ...
  it.each([
    {
      ...
      run: () => {
        process.env.SKILLSET_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
        const plugin = writePlugin({
...
module.exports = { id: "manifest-only-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "skillset.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__skillset_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__skillset_loader_reentry_error";
        const reenterFnMarker = "__skillset_loader_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          reenterFnMarker,
          (options: Parameters<typeof loadSkillSetPlugins>[0]) => loadSkillSetPlugins(options),
        );
        const pluginDir = makeTempDir();
        ...
        } satisfies Parameters<typeof loadSkillSetPlugins>[0];
        writePlugin({
  ...
  register() {
    try {
      ...
      throw error;
        ...
        const registry = loadSkillSetPlugins(nestedOptions);
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__skillset_runtime_registry_reentry_marker";
        const resolverMarker = "__skillset_runtime_registry_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          resolverMarker,
  ...
  register() {
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadSkillSetPlugins(options);
        const scoped = loadSkillSetPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadSkillSetPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadSkillSetPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("replaces prior plugin hook registrations on activating reloads", async () => {
    ...
    loadSkillSetPlugins(loadOptions);
    loadSkillSetPlugins(loadOptions);
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadSkillSetPlugins(options);
    ...
    const second = loadSkillSetPlugins(options);
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadSkillSetPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadSkillSetPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
        ...
        return {
          ...
          loadFirst: () =>
            loadSkillSetPlugins({
              ...
              env: {
                ...process.env,
                SKILLSET_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadSkillSetPlugins({
              ...
              env: {
                ...process.env,
                SKILLSET_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadSkillSetPlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                SKILLSET_HOME: undefined,
                SKILLSET_STATE_DIR: stateDir,
                SKILLSET_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadSkillSetPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(skillsetHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadSkillSetPlugins(options),
          loadVariant: () =>
            loadSkillSetPlugins({
              ...options,
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadSkillSetPlugins({
      env: {
        ...
        HOME: homeDir,
        SKILLSET_HOME: undefined,
        SKILLSET_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers SKILLSET_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const skillsetHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "skillset-home-demo",
      dir: path.join(skillsetHome, "plugins", "skillset-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "skillset-home-demo", register() {} };`,
    });
    ...
    const registry = loadSkillSetPlugins({
      ...
      config: {
        plugins: {
          allow: ["skillset-home-demo"],
          entries: {
            "skillset-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/skillset-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "skillset-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  it("can include plugin export shape when register is missing", () => {
    ...
    const registry = withEnv({ SKILLSET_PLUGIN_LOAD_DEBUG: "1" }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadSkillSetPlugins>) =>
  ...
  api.registerContextEngine("shared-context-engine-loader-test", () => ({}));
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadSkillSetPlugins>) => {
          expect(
  ...
  it("loads bundled channel entries through nested default export wrappers", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@skillset/nested-default-channel",
          skillset: {
            extensions: ["./index.cjs"],
          },
        },
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "skillset.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    const broadSetupRegistry = loadSkillSetPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
    });
    ...
    const scopedSetupRegistry = loadSkillSetPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@skillset/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@skillset/setup-only-bundled-contract-test",
        fullBlurb: "full entry should not run in setup-only mode",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@skillset/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@skillset/setup-runtime-bundled-contract-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@skillset/setup-runtime-bundled-contract-secrets-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@skillset/setup-runtime-bundled-contract-runtime-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@skillset/setup-runtime-bundled-runtime-merge-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@skillset/setup-runtime-not-preferred-test",
  ...
  it("applies the bundled runtime setter before loading the merged setup-runtime plugin", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@skillset/setup-runtime-order-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("records setup runtime setter failures without aborting the full load pass", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@skillset/setup-runtime-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledSetupEntryContract: true,
  ...
  it("rejects mismatched bundled runtime entry ids before applying setup-runtime setters", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@skillset/setup-runtime-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("rejects mismatched bundled setup export ids before loading setup-runtime entry code", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@skillset/setup-export-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("isolates loadSetupPlugin errors as per-plugin diagnostics instead of crashing registry load", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@skillset/setup-entry-throws-test",
          skillset: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
  ...
  it("keeps healthy sibling channel plugins loadable when a setup entry throws", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@skillset/setup-entry-throws-sibling-test",
          skillset: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
    ...
    fs.writeFileSync(
      path.join(brokenDir, "skillset.plugin.json"),
      JSON.stringify(
        {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "skillset.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "skillset.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryCoreDir, "skillset.plugin.json"),
            JSON.stringify(
          ...
          fs.writeFileSync(
            path.join(memoryLanceDir, "skillset.plugin.json"),
            JSON.stringify(
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ SKILLSET_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("uses the source runtime snapshot allowlist for plugin trust checks", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ SKILLSET_STATE_DIR: stateDir }, () => {
  ...
  if (!Object.getOwnPropertyDescriptor(runtime, "channel")) {
    ...
    const registry = withEnv({ SKILLSET_STATE_DIR: stateDir }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("skillset/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ SKILLSET_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadSkillSetPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__skillsetLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("skillset/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("skillset/plugin-sdk");
