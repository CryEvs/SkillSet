...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing skillset.extensions; update the plugin package to include skillset.extensions (for example ["./dist/index.js"]). See https://docs.skillset.ai/help/troubleshooting#plugin-install-fails-with-missing-skillset-extensions';
const PLUGIN_ARCHIVE_ROOT_MARKERS = [
  "package.json",
  "skillset.plugin.json",
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_SKILLSET_EXTENSIONS: "missing_skillset_extensions",
  EMPTY_SKILLSET_EXTENSIONS: "empty_skillset_extensions",
...
function ensureSkillSetExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_SKILLSET_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json skillset.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_SKILLSET_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "skillset security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureSkillSetExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureSkillSetExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from skillset.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-skillset"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json skillset.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires SkillSet >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set SKILLSET_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires SkillSet >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade SkillSet and retry.`,
...
export async function installPluginFromNpmSpec(
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "skillset-npm-pack-",
    spec,
    timeoutMs,
