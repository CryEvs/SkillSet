...
type LoadSkillSetPlugins = typeof import("./loader.js").loadSkillSetPlugins;
...
const loadSkillSetPluginsMock = vi.fn<LoadSkillSetPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/skillset.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadSkillSetPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: SkillSetConfig = {
    plugins: {},
  };
  const autoEnabledConfig: SkillSetConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadSkillSetPlugins: (...args: Parameters<LoadSkillSetPlugins>) =>
        loadSkillSetPluginsMock(...args),
  ...
  beforeEach(() => {
    ...
    loadSkillSetPluginsMock.mockReset();
    ...
    loadSkillSetPluginsMock.mockReturnValue(registry);
