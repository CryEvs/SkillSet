...
import type { SkillSetPluginService, SkillSetPluginServiceContext } from "./types.js";
...
function createRegistry(services: SkillSetPluginService[]) {
...
function expectServiceContext(
  ctx: SkillSetPluginServiceContext,
...
function expectServiceLogger(ctx: SkillSetPluginServiceContext) {
...
function expectServiceContexts(
  contexts: SkillSetPluginServiceContext[],
  config: Parameters<typeof startPluginServices>[0]["config"],
) {
...
async function startTrackingServices(params: {
  services: SkillSetPluginService[];
  config?: Parameters<typeof startPluginServices>[0]["config"];
  workspaceDir?: string;
}) {
...
describe("startPluginServices", () => {
  ...
  it("starts services and stops them in reverse order", async () => {
    const starts: string[] = [];
    const stops: string[] = [];
    const contexts: SkillSetPluginServiceContext[] = [];
