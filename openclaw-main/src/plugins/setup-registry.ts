...
function collectConfiguredPluginEntryIds(config: SkillSetConfig): string[] {
...
function resolveRegister(mod: SkillSetPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: SkillSetPluginModule;
  try {
    mod = getJiti(setupSource)(setupSource) as SkillSetPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: SkillSetPluginModule }).default ?? mod);
