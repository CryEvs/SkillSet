...
const RETAINED_RUNTIME_DEPS_MANIFEST = ".skillset-runtime-deps.json";
...
export function isWritableDirectory(dir: string): boolean {
  ...
  try {
    probeDir = fs.mkdtempSync(path.join(dir, ".skillset-write-probe-"));
...
function resolveBundledRuntimeDepsExternalBaseDir(env: NodeJS.ProcessEnv): string {
  const explicit = env.SKILLSET_PLUGIN_STAGE_DIR?.trim();
  if (explicit) {
...
function resolveExternalBundledRuntimeDepsInstallRoot(params: {
  ...
  const packageKey = `skillset-${version}-${createPathHash(packageRoot)}`;
...
function replaceNodeModulesDir(targetDir: string, sourceDir: string): void {
  ...
  const tempDir = fs.mkdtempSync(path.join(parentDir, ".skillset-runtime-deps-copy-"));
...
function readBundledPluginChannels(pluginDir: string): string[] {
  const manifest = readJsonObject(path.join(pluginDir, "skillset.plugin.json"));
...
function readBundledPluginEnabledByDefault(pluginDir: string): boolean {
  return readJsonObject(path.join(pluginDir, "skillset.plugin.json"))?.enabledByDefault === true;
}
...
export function resolveBundledRuntimeDependencyPackageInstallRoot(
  ...
  if (
    ...
    env.SKILLSET_PLUGIN_STAGE_DIR?.trim() ||
...
export function installBundledRuntimeDeps(params: {
  ...
  if (path.resolve(installExecutionRoot) !== path.resolve(params.installRoot)) {
    fs.writeFileSync(
      ...
      `${JSON.stringify({ name: "skillset-runtime-deps-install", private: true }, null, 2)}\n`,
      "utf8",
    );
  }
