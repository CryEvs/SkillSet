...
import { resolveSkillSetPackageRootSync } from "../infra/skillset-root.js";
...
function hasTrustedSkillSetRootIndicator(params: {
  packageRoot: string;
  ...
  const hasSkillSetBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("skillset")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.skillset === "string");
  const hasSkillSetEntrypoint = fs.existsSync(path.join(params.packageRoot, "skillset.mjs"));
  return hasCliEntryExport || hasSkillSetBin || hasSkillSetEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedSkillSetRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedSkillSetRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveSkillSetPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedSkillSetRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveSkillSetPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedSkillSetRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["skillset/plugin-sdk", "@skillset/plugin-sdk"] as const;
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.SKILLSET_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.prototype.hasOwnProperty.call(aliasMap, `skillset/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "skillset/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
    ...(pluginSdkAlias
      ? Object.fromEntries(
