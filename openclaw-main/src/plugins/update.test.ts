...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-skillset",
    targetDir: params?.targetDir ?? "/tmp/opik-skillset",
...
function createInstalledPackageDir(params: { name?: string; version: string }): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@skillset/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["skillset-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/skillset-codex-app-server",
    version: params.version,
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-skillset",
        spec: "@opik/opik-skillset",
        integrity: "sha512-old",
        installPath: "/tmp/opik-skillset",
      }),
      pluginIds: ["opik-skillset"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-skillset",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "skillset-codex-app-server",
        spec: "skillset-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/skillset-codex-app-server",
      }),
      ...
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "skillset-codex-app-server",
        targetDir: "/tmp/skillset-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "skillset-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-skillset@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-skillset@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-skillset" (@opik/opik-skillset@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-skillset: aborted: npm package integrity drift detected for @opik/opik-skillset@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @skillset/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@skillset/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @skillset/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        ...
        resolvedSpec: "skillset-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "skillset-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "skillset-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "skillset-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
      ...
      expectedSpec: "skillset-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "skillset-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "skillset-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@skillset/voice-call",
      targetDir: "/tmp/skillset-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@skillset/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@skillset/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@skillset/voice-call");
    expect(result.config.plugins?.entries?.["@skillset/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@skillset/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@skillset/voice-call",
      installPath: "/tmp/skillset-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { SKILLSET_HOME: "/srv/skillset-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/skillset-home";
    mockBundledSources(
      createBundledSource({
