...
import type { SkillSetConfig, ConfigFileSnapshot } from "../config/types.skillset.js";
...
vi.mock("../infra/skillset-root.js", () => ({
  resolveSkillSetPackageRoot: vi.fn(),
  resolveSkillSetPackageRootSync: vi.fn(() => process.cwd()),
}));
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/skillset-config",
  };
});
...
const { resolveSkillSetPackageRoot } = await import("../infra/skillset-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/skillset-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as SkillSetConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/skillset-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveSkillSetPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("skillset-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("skillset-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveSkillSetPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("skillset");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/skillset-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "skillset-gateway" }],
      hints: [],
    });
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("skillset-completion-timeout");
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      expect.any(String),
      [path.join(root, "skillset.mjs"), "completion", "--write-state"],
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          SKILLSET_UPDATE_POST_CORE: "1",
          SKILLSET_UPDATE_POST_CORE_CHANNEL: "dev",
        }),
      }),
    );
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    setupUpdatedRootRefresh({
      gatewayUpdateImpl: async () =>
        makeOkUpdateResult({
          mode: "npm",
          root: createCaseDir("skillset-downgraded-root"),
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        SKILLSET_UPDATE_POST_CORE: "1",
        SKILLSET_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@skillset/demo@1.0.0",
        resolvedSpec: "@skillset/demo@1.0.0",
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @skillset/demo@1.0.0"
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("SkillSet update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "skillset", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as SkillSetConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "skillset@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    mockPackageInstallStatus(createCaseDir("skillset-update"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "latest",
      version: "2026.3.23-2",
      nodeEngine: ">=22.14.0",
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g skillset` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:skillset/skillset#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("skillset-update"));
        await updateCommand({ yes: true, tag: "github:skillset/skillset#main" });
      },
      expectedSpec: "github:skillset/skillset#main",
    },
    {
      name: "SKILLSET_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("skillset-update"));
        await withEnvAsync(
          { SKILLSET_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/skillset-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/skillset-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "skillset");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "skillset", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-update-package-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "skillset");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "skillset", version: "2026.4.21" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("skillset-update");
    const localAppData = createCaseDir("skillset-localappdata");
  ...
  it("skips plugin sync in the old process after switching from package to git", async () => {
    ...
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "git",
        root: path.join(tempDir, "..", "skillset"),
        after: { version: "2026.4.10" },
      }),
    );
    serviceLoaded.mockResolvedValue(true);
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `skillset update`.",
    );
    expect(defaultRuntime.exit).toHaveBeenCalledWith(0);
  });
  it.each([
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            SKILLSET_CONFIG_PATH: "./config/skillset.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          cwd: root,
          env: expect.objectContaining({
            SKILLSET_STATE_DIR: path.resolve("./state"),
            SKILLSET_CONFIG_PATH: path.resolve("./config/skillset.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            SKILLSET_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ SKILLSET_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.SKILLSET_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("skillset-update-wizard");
    await withEnvAsync({ SKILLSET_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/skillset as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ SKILLSET_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "skillset"));
    });
