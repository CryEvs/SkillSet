...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-bundled-skills-test-"));
    process.env.SKILLSET_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
