...
${formatHelpExamples([
  ['skillset agent --to +15555550123 --message "status update"', "Start a new session."],
  ['skillset agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'skillset agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'skillset agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['skillset agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'skillset agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.skillset.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.skillset.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['skillset agents set-identity --agent main --name "SkillSet" --emoji "🦞"', "Set name + emoji."],
  ["skillset agents set-identity --agent main --avatar avatars/skillset.png", "Set avatar path."],
  [
    "skillset agents set-identity --workspace ~/.skillset/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "skillset agents set-identity --identity-file ~/.skillset/workspace/IDENTITY.md --agent main",
