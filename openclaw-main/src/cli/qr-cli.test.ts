...
describe("registerQrCli", () => {
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("skillset devices approve <requestId>");
  });
  ...
  it("uses SKILLSET_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("SKILLSET_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
