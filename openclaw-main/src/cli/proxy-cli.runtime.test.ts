...
describe("proxy cli runtime", () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(path.join(os.tmpdir(), "skillset-proxy-cli-runtime-"));
    ...
    process.env.SKILLSET_DEBUG_PROXY_CERT_DIR = path.join(tempDir, "certs");
    delete process.env.SKILLSET_DEBUG_PROXY_ENABLED;
    delete process.env.SKILLSET_DEBUG_PROXY_SESSION_ID;
