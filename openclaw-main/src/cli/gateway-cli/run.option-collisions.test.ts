...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/skillset-stability.json",
  path: "/tmp/skillset-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/skillset-control-ui" as string | null,
}));

const { runtimeErrors, defaultRuntime, resetRuntimeCapture } = createCliRuntimeCapture();

vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/skillset-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/skillset-control-ui";
  ...
  it.each([
    ...
    delete process.env.SKILLSET_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.SKILLSET_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode even if loadConfig still says local", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `skillset onboard --mode local` or `skillset setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("restores last-known-good config before startup when the effective config is invalid", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/skillset-test-missing-config.json",
      config: {},
      parsed: null,
    ...
    expect(gatewayLogMessages).toContain(
      "gateway: restored invalid effective config from last-known-good backup: /tmp/skillset-test-missing-config.json",
    );
    expect(startGatewayServer).toHaveBeenCalledWith(
      19170,
      expect.objectContaining({
  ...
  it("allows password mode preflight when password is configured via SecretRef", async () => {
    configState.cfg = {
      gateway: {
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "SKILLSET_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        defaults: {
