  ...
  plugins: [
    {
      ...
      source: "bundled",
      manifestPath: "/tmp/memory-wiki/skillset.plugin.json",
...
describe("shouldEnsureCliPath", () => {
  it("skips path bootstrap for help/version invocations", () => {
    expect(shouldEnsureCliPath(["node", "skillset", "--help"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "skillset", "-V"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "skillset", "-v"])).toBe(false);
  });

  it("skips path bootstrap for read-only fast paths", () => {
    expect(shouldEnsureCliPath(["node", "skillset", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "skillset", "--log-level", "debug", "status"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "skillset", "sessions", "--json"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "skillset", "config", "get", "update"])).toBe(false);
    expect(shouldEnsureCliPath(["node", "skillset", "models", "status", "--json"])).toBe(false);
  });

  it("keeps path bootstrap for mutating or unknown commands", () => {
    expect(shouldEnsureCliPath(["node", "skillset", "message", "send"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "skillset", "voicecall", "status"])).toBe(true);
    expect(shouldEnsureCliPath(["node", "skillset", "acp", "-v"])).toBe(true);
  });
});

describe("shouldUseRootHelpFastPath", () => {
  it("uses the fast path for root help only", () => {
    expect(shouldUseRootHelpFastPath(["node", "skillset", "--help"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "skillset", "--profile", "work", "-h"])).toBe(true);
    expect(shouldUseRootHelpFastPath(["node", "skillset", "status", "--help"])).toBe(false);
    expect(shouldUseRootHelpFastPath(["node", "skillset", "--help", "status"])).toBe(false);
  });
});
...
describe("resolveMissingPluginCommandMessage", () => {
  ...
  it("explains that dreaming is a runtime slash command, not a CLI command", () => {
    ...
    expect(message).toContain("skillset memory");
  });
