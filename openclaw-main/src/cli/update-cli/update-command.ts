...
const POST_CORE_UPDATE_ENV = "SKILLSET_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "SKILLSET_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "SKILLSET_UPDATE_POST_CORE_RESULT_PATH";
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `Node ${process.versions.node ?? "unknown"} is too old for skillset@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    "Upgrade Node to 22.14+ or Node 24, then rerun `skillset update`.",
    "Bare `npm i -g skillset` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g skillset@latest`.",
  ].join("\n");
}
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("skillset completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function maybeRestartService(params: {
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.SKILLSET_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.SKILLSET_UPDATE_IN_PROGRESS;
        }
      }
      ...
      if (!params.opts.json && restartInitiated) {
        ...
        if (health.healthy) {
          ...
          defaultRuntime.log(
            theme.muted(
              `Run \`${replaceCliName(formatCliCommand("skillset gateway status --deep"), CLI_NAME)}\` for details.`,
            ),
          );
        }
        defaultRuntime.log("");
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("skillset gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("skillset doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("skillset gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("skillset gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir =
    params.opts.json === true
      ? await fs.mkdtemp(path.join(os.tmpdir(), "skillset-update-post-core-"))
      : null;
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  const devTargetRef =
    channel === "dev" ? process.env.SKILLSET_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating SkillSet..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `skillset update`."),
      );
    }
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this SkillSet install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("skillset doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("skillset gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g skillset@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g skillset@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
