  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        SKILLSET_PROFILE: "default",
      });
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("systemctl --user restart 'skillset-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses SKILLSET_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        SKILLSET_PROFILE: "default",
        SKILLSET_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.skillset.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.skillset.gateway'");
      expect(content).toContain("launchctl bootstrap 'gui/501'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("captures macOS launchctl stderr to ~/.skillset/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.skillset/logs/gateway-restart.log' 2>&1");
    ...
    it("uses SKILLSET_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/skillset-state/logs' 2>/dev/null && : >>'/tmp/skillset-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/skillset-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
    ...
    it("returns the final macOS launchctl kickstart failure after logging cleanup", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      const tmpDir = await makeTempDir("skillset-restart-helper-");
...
`,
      ...
      expect(log).toContain("skillset restart attempt source=update target=ai.skillset.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.skillset.gateway");
      expect(log).toContain("skillset restart failed source=update status=42");
      expect(log).not.toContain("skillset restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        SKILLSET_LAUNCHD_LABEL: "ai.skillset.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        SKILLSET_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.skillset.$(echo injected)");
      expect(log).not.toContain("target=ai.skillset.injected");
    });

    it("uses SKILLSET_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        SKILLSET_PROFILE: "default",
        SKILLSET_LAUNCHD_LABEL: "com.custom.skillset",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.skillset'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("skillset restart attempt source=update target=SkillSet Gateway");
      expect(content).toContain('schtasks /End /TN "SkillSet Gateway"');
      expect(content).toContain('schtasks /Run /TN "SkillSet Gateway" >>');
    ...
    it("uses SKILLSET_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        SKILLSET_PROFILE: "default",
        SKILLSET_WINDOWS_TASK_NAME: "SkillSet Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "SkillSet Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "SkillSet Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        SKILLSET_PROFILE: "production",
      });
      expect(content).toContain("skillset-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        SKILLSET_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.skillset.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "SkillSet Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        SKILLSET_LAUNCHD_LABEL: "ai.skillset.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.skillset.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
