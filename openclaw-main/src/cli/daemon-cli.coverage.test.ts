...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      SKILLSET_GATEWAY_PORT: String(params.port),
      ...(params.token ? { SKILLSET_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.SKILLSET_STATE_DIR = "/tmp/skillset-cli-state";
    process.env.SKILLSET_CONFIG_PATH = "/tmp/skillset-cli-state/skillset.json";
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ...
        SKILLSET_STATE_DIR: "/tmp/skillset-daemon-state",
        SKILLSET_CONFIG_PATH: "/tmp/skillset-daemon-state/skillset.json",
        SKILLSET_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.skillset.gateway.plist",
    });
