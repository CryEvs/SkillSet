...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    SKILLSET_STATE_DIR: "/tmp/skillset-daemon",
    SKILLSET_CONFIG_PATH: "/tmp/skillset-daemon/skillset.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.SKILLSET_STATE_DIR ?? "/tmp/skillset-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.SKILLSET_CONFIG_PATH ?? `${stateDir}/skillset.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/skillset-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.SKILLSET_STATE_DIR = "/tmp/skillset-cli";
    process.env.SKILLSET_CONFIG_PATH = "/tmp/skillset-cli/skillset.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/skillset-daemon/skillset.json",
      }),
    );
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/skillset-cli/skillset.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.SKILLSET_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.SKILLSET_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.SKILLSET_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "skillset-gateway" }],
        hints: [],
      },
      healthy: false,
