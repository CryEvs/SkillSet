...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("skillset-daemon-install-int-");
    configPath = path.join(tempHome, "skillset.json");
    process.env.HOME = tempHome;
    process.env.SKILLSET_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.SKILLSET_GATEWAY_TOKEN).toBeUndefined();
  });
});
