...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-completion-home-"));
    ...
    expect(await fs.readdir(cacheDir)).toEqual(
      expect.arrayContaining(["skillset.bash", "skillset.fish", "skillset.ps1", "skillset.zsh"]),
    );
