...
import { z } from "zod";
import { createSkillSetChannelMcpServer, SkillSetChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createSkillSetChannelMcpServer({
...
describe("skillset channel mcp server", () => {
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new SkillSetChannelBridge({} as never, {
