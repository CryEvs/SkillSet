...
vi.mock("./channel-bridge.js", () => ({
  SkillSetChannelBridge: class MockSkillSetChannelBridge {
...
describe("serveSkillSetChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveSkillSetChannelMcp } = await import("./channel-server.js");

    const servePromise = serveSkillSetChannelMcp({ verbose: false });
