...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("SKILLSET_PLUGIN_LOAD_PROFILE", "1");
  ...
  try {
    ...
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-channel-entry-contract-"));
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("can disable source-tree fallback for dist bundled entry checks", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"skillset"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("SKILLSET_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "1");
