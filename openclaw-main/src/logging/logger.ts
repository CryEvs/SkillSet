...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredSkillSetTmpDir() : POSIX_SKILLSET_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "skillset.log")
    : `${POSIX_SKILLSET_TMP_DIR}/skillset.log`;
}
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.SKILLSET_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: SkillSetConfig["logging"] | undefined =
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.SKILLSET_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[skillset] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
