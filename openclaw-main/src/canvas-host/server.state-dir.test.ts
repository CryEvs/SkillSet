...
describe("canvas host state dir defaults", () => {
  ...
  it("uses SKILLSET_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("skillset-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("SkillSet Canvas");
      } finally {
        await handler.close();
      }
    });
