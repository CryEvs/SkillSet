...
export type PromptDefaultModelResult = { model?: string; config?: SkillSetConfig };
...
function resolveConfiguredModelRaw(cfg: SkillSetConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: SkillSetConfig): string[] {
