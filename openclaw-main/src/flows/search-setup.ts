...
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<SkillSetConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: SkillSetConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: SkillSetConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: SkillSetConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: SkillSetConfig, result: SkillSetConfig): SkillSetConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: SkillSetConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as SkillSetConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with skillset configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "SkillSet will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.skillset.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — SkillSet will store a reference instead of the API key.",
