...
function createSkillSetBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "skillset-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createSkillSetBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses skillset-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "skillset-gateway" },
          { pid: process.pid, cmd: "skillset-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: skillset-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "skillset-gateway" },
          { pid: unrelatedStalePid, cmd: "skillset-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "skillset-gateway" },
            { pid: grandparentPid, cmd: "skillset-gateway" },
            { pid: benignStalePid, cmd: "skillset-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "skillset-gateway" },
            { pid: gatewayGrandparentPid, cmd: "skillset-gateway" },
          ]),
          stderr: "",
        });
    ...
    it("excludes pids whose command does not include 'skillset'", () => {
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["skillset", "gateway"]);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple skillset pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "skillset-gateway" },
          { pid: pid2, cmd: "skillset-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-skillset pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks skillset processes — non-skillset listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty skillset stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty skillset stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + skillset pid in stdout — container-restricted lsof reports partial results
          return createSkillSetBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "skillset-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'skillset'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("skillset")
      // inside the mid-loop flush: a non-skillset cmd between two entries must not
      // be pushed, but the following skillset entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-skillset entry first, then skillset entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid skillset entry must still be found.
      const stdout = `p0\ncskillset-gateway\np${stalePid}\ncskillset-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero skillset pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-skillset stdout (line 145)", () => {
    it("treats status 1 + non-skillset stdout as port-free (not an skillset process)", () => {
      // status 1 + non-empty stdout where no skillset pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill skillset pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-skillset output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no skillset pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
