...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__skillset_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "skillset-root");
...
function setPackageRoot(root: string, name = "skillset") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./skillset-root.js").resolveSkillSetPackageRootSync,
  asyncResolver: typeof import("./skillset-root.js").resolveSkillSetPackageRoot,
  opts: Parameters<typeof import("./skillset-root.js").resolveSkillSetPackageRootSync>[0],
...
vi.mock("./skillset-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveSkillSetPackageRoot", () => {
  let resolveSkillSetPackageRoot: typeof import("./skillset-root.js").resolveSkillSetPackageRoot;
  let resolveSkillSetPackageRootSync: typeof import("./skillset-root.js").resolveSkillSetPackageRootSync;

  beforeAll(async () => {
    ({ resolveSkillSetPackageRoot, resolveSkillSetPackageRootSync } =
      await import("./skillset-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "skillset");
        const pkgRoot = path.join(project, "node_modules", "skillset");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "skillset");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "skillset.mjs")));
    ...
    {
      name: "falls through from a non-skillset moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-skillset");
    ...
    {
      name: "returns null for non-skillset package roots",
      setup: () => {
        const pkgRoot = fx("not-skillset");
        setPackageRoot(pkgRoot, "not-skillset");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "skillset.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "skillset");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveSkillSetPackageRootSync,
      resolveSkillSetPackageRoot,
      opts,
      expected,
    );
  });
