...
async function writeGlobalPackageJson(packageRoot: string, version = "1.0.0") {
  await fs.writeFile(
    ...
    JSON.stringify({ name: "skillset", version }),
    "utf-8",
  );
}
...
describe("update global helpers", () => {
  ...
  it("prefers explicit package spec overrides", () => {
    envSnapshot = captureEnv(["SKILLSET_UPDATE_PACKAGE_SPEC"]);
    process.env.SKILLSET_UPDATE_PACKAGE_SPEC = "file:/tmp/skillset.tgz";

    expect(resolveGlobalInstallSpec({ packageName: "skillset", tag: "latest" })).toBe(
      "file:/tmp/skillset.tgz",
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "skillset",
        tag: "beta",
        env: { SKILLSET_UPDATE_PACKAGE_SPEC: "skillset@next" },
      }),
    ).toBe("skillset@next");
  });
  ...
  it("resolves global roots and package roots from runner output", async () => {
    ...
    await expect(resolveGlobalPackageRoot("npm", runCommand, 1000)).resolves.toBe(
      path.join("/tmp/npm-root", "skillset"),
    );
  });

  it("maps main and explicit install specs for global installs", () => {
    expect(resolveGlobalInstallSpec({ packageName: "skillset", tag: "main" })).toBe(
      SKILLSET_MAIN_PACKAGE_SPEC,
    );
    expect(
      resolveGlobalInstallSpec({
        packageName: "skillset",
        tag: "github:skillset/skillset#feature/my-branch",
      }),
    ).toBe("github:skillset/skillset#feature/my-branch");
    expect(
      resolveGlobalInstallSpec({
        packageName: "skillset",
        tag: "https://example.com/skillset-main.tgz",
      }),
    ).toBe("https://example.com/skillset-main.tgz");
  });
  ...
  it("classifies main and raw install specs separately from registry selectors", () => {
    ...
    expect(isExplicitPackageInstallSpec("github:skillset/skillset#main")).toBe(true);
    expect(isExplicitPackageInstallSpec("https://example.com/skillset-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("file:/tmp/skillset-main.tgz")).toBe(true);
    expect(isExplicitPackageInstallSpec("beta")).toBe(false);

    expect(canResolveRegistryVersionForPackageTarget("latest")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("2026.3.22")).toBe(true);
    expect(canResolveRegistryVersionForPackageTarget("main")).toBe(false);
    expect(canResolveRegistryVersionForPackageTarget("github:skillset/skillset#main")).toBe(false);
  });

  it("detects install managers from resolved roots and on-disk presence", async () => {
    await withTempDir({ prefix: "skillset-update-global-" }, async (base) => {
      const npmRoot = path.join(base, "npm-root");
      const pnpmRoot = path.join(base, "pnpm-root");
      const bunRoot = path.join(base, ".bun", "install", "global", "node_modules");
      const pkgRoot = path.join(pnpmRoot, "skillset");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.mkdir(path.join(npmRoot, "skillset"), { recursive: true });
      await fs.mkdir(path.join(bunRoot, "skillset"), { recursive: true });
      ...
      await fs.rm(path.join(npmRoot, "skillset"), { recursive: true, force: true });
      await fs.rm(path.join(pnpmRoot, "skillset"), { recursive: true, force: true });
  ...
  it("prefers the owning npm prefix when PATH npm points at a different global root", async () => {
    ...
    try {
      await withTempDir({ prefix: "skillset-update-npm-prefix-" }, async (base) => {
        ...
        const pkgRoot = path.join(brewRoot, "skillset");
        ...
        expect(globalInstallArgs("npm", "skillset@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "skillset@latest",
        ...
        expect(globalInstallFallbackArgs("npm", "skillset@latest", pkgRoot)).toEqual([
          brewNpm,
          "i",
          "-g",
          "skillset@latest",
  ...
  it("does not infer npm ownership from path shape alone when the owning npm binary is absent", async () => {
    await withTempDir({ prefix: "skillset-update-npm-missing-bin-" }, async (base) => {
  ...
  it("prefers npm.cmd for win32-style global npm roots", async () => {
    ...
    try {
      await withTempDir({ prefix: "skillset-update-win32-npm-prefix-" }, async (base) => {
        const npmPrefix = path.join(base, "Roaming", "npm");
        const npmRoot = path.join(npmPrefix, "node_modules");
        const pkgRoot = path.join(npmRoot, "skillset");
  ...
  it("builds install argv and npm fallback argv", () => {
    ...
    expect(globalInstallArgs("npm", "skillset@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "skillset@latest",
    ...
    expect(globalInstallArgs("pnpm", "skillset@latest")).toEqual([
      "pnpm",
      "add",
      "-g",
      "skillset@latest",
    ]);
    expect(globalInstallArgs("bun", "skillset@latest")).toEqual([
      "bun",
      "add",
      "-g",
      "skillset@latest",
    ...
    expect(globalInstallFallbackArgs("npm", "skillset@latest")).toEqual([
      "npm",
      "i",
      "-g",
      "skillset@latest",
    ...
    expect(globalInstallFallbackArgs("pnpm", "skillset@latest")).toBeNull();
    expect(
      globalInstallArgs({ manager: "pnpm", command: "/opt/homebrew/bin/pnpm" }, "skillset@latest"),
    ).toEqual(["/opt/homebrew/bin/pnpm", "add", "-g", "skillset@latest"]);
  });

  it("cleans only renamed package directories", async () => {
    await withTempDir({ prefix: "skillset-update-cleanup-" }, async (root) => {
      await fs.mkdir(path.join(root, ".skillset-123"), { recursive: true });
      await fs.mkdir(path.join(root, ".skillset-456"), { recursive: true });
      await fs.writeFile(path.join(root, ".skillset-file"), "nope", "utf8");
      await fs.mkdir(path.join(root, "skillset"), { recursive: true });
      ...
      await expect(
        ...
        removed: [".skillset-123", ".skillset-456"],
      });
      await expect(fs.stat(path.join(root, "skillset"))).resolves.toBeDefined();
      await expect(fs.stat(path.join(root, ".skillset-file"))).resolves.toBeDefined();
    });
  });

  it("checks installed dist against the packaged inventory", async () => {
    await withTempDir({ prefix: "skillset-update-global-pkg-" }, async (packageRoot) => {
  ...
  it("does not require private QA sidecars when the inventory is missing", async () => {
    await withTempDir({ prefix: "skillset-update-global-legacy-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeCompatSidecars(packageRoot);
  ...
  it("fails closed on newer installs when the inventory is missing", async () => {
    await withTempDir(
      { prefix: "skillset-update-global-missing-inventory-new-" },
      async (packageRoot) => {
  ...
  it("rejects invalid inventory files during global verify", async () => {
    await withTempDir(
      { prefix: "skillset-update-global-invalid-inventory-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
  ...
  it("verifies legacy sidecars for installed bundled plugins without inventory", async () => {
    await withTempDir({ prefix: "skillset-update-global-legacy-plugin-" }, async (packageRoot) => {
      await writeGlobalPackageJson(packageRoot);
      await writeBundledPluginPackageJson(packageRoot, "matrix", "@skillset/matrix");
  ...
  it("still enforces critical sidecars when the inventory omits them", async () => {
    await withTempDir(
      { prefix: "skillset-update-global-critical-sidecars-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeCompatSidecars(packageRoot);
        await writeBundledPluginPackageJson(packageRoot, "matrix", "@skillset/matrix");
  ...
  it("ignores stale metadata for non-packaged private QA plugins during inventory verify", async () => {
    await withTempDir(
      { prefix: "skillset-update-global-stale-private-qa-" },
      async (packageRoot) => {
        await writeGlobalPackageJson(packageRoot, "2026.4.15");
        await writeCompatSidecars(packageRoot);
        await writeBundledPluginPackageJson(packageRoot, "qa-lab", "@skillset/qa-lab");
        await writePackageDistInventory(packageRoot);
