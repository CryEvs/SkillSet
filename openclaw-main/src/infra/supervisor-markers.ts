const SUPERVISOR_HINTS = {
  launchd: ["LAUNCH_JOB_LABEL", "LAUNCH_JOB_NAME", "XPC_SERVICE_NAME", "SKILLSET_LAUNCHD_LABEL"],
  systemd: ["SKILLSET_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["SKILLSET_WINDOWS_TASK_NAME"],
} as const;
...
export function detectRespawnSupervisor(
  ...
  if (platform === "win32") {
    ...
    const marker = env.SKILLSET_SERVICE_MARKER?.trim();
    const serviceKind = env.SKILLSET_SERVICE_KIND?.trim();
