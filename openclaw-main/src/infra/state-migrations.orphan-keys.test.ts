...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".skillset");
...
function sharedMainOpsConfig(sharedStorePath: string): SkillSetConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: SkillSetConfig = OPS_WORK_CONFIG) {
...
describe("migrateOrphanedSessionKeys", () => {
  ...
  it("is idempotent — running twice produces same result", async () => {
    await withStateFixture(async ({ stateDir }) => {
      ...
      const env = { SKILLSET_STATE_DIR: stateDir };
