...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: SkillSetConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: SkillSetConfig) {
...
function resolveHeartbeatAgents(cfg: SkillSetConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: SkillSetConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: SkillSetConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: SkillSetConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: SkillSetConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: SkillSetConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
