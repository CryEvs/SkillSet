...
const EXTENSION_MANIFEST = bundledPluginFile("demo", "skillset.plugin.json");
const EXTENSION_PACKAGE = bundledPluginFile("demo", "package.json");
const EXTENSION_README = bundledPluginFile("demo", "README.md");
const DIST_EXTENSION_MANIFEST = bundledDistPluginFile("demo", "skillset.plugin.json");
...
const BASE_PROJECT_FILES = {
  [ROOT_TSCONFIG]: "{}\n",
  [ROOT_PACKAGE]: '{"name":"skillset-test"}\n',
...
function statusCommandSpawn() {
  return [process.execPath, "skillset.mjs", "status"];
}
...
describe("run-node script", () => {
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "skillset-run-node-" }, async (tmp) => {
        ...
        expect(nodeCalls).toEqual([
          ...
          [process.execPath, "skillset.mjs", "--version"],
        ]);
      });
    },
  );
    ...
    await withTempDir({ prefix: "skillset-run-node-" }, async (tmp) => {
      ...
      const spawn = (_cmd: string, args: string[], options?: unknown) => {
        ...
        return createPipedExitedProcess({
          stdout: args[0] === "skillset.mjs" ? "child stdout\n" : "",
          stderr: args[0] === "skillset.mjs" ? "child stderr\n" : "",
        });
      };
      const mutedStream = {
        write: () => true,
      ...
      const exitCode = await runNodeMain({
        ...
        env: {
          ...process.env,
          SKILLSET_FORCE_BUILD: "1",
          SKILLSET_RUNNER_LOG: "1",
          SKILLSET_RUN_NODE_OUTPUT_LOG: outputPath,
        },
        spawn,
        stderr: mutedStream,
        stdout: mutedStream,
      ...
      await expect(fs.readFile(outputPath, "utf-8")).resolves.toContain("[skillset]");
      expect(spawnCalls.at(-1)?.args).toEqual(["skillset.mjs", "status"]);
      expect(spawnCalls.at(-1)?.env.SKILLSET_RUN_NODE_OUTPUT_LOG).toBe(outputPath);
  ...
  it("does not mutate Matrix QA args when no generic output log is requested", async () => {
    await withTempDir({ prefix: "skillset-run-node-" }, async (tmp) => {
      ...
      expect(childArgs).toEqual(["skillset.mjs", "qa", "matrix"]);
      expect(spawnCalls.at(-1)?.env.SKILLSET_RUN_NODE_OUTPUT_LOG).toBeUndefined();
    });
  });
      ...
      expect(runRuntimePostBuild).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            SKILLSET_BUILD_PRIVATE_QA: "1",
            SKILLSET_ENABLE_PRIVATE_QA_CLI: "1",
          }),
        }),
      );
    });
    ...
    await withTempDir({ prefix: "skillset-run-node-" }, async (tmp) => {
      ...
      const requirement = resolveBuildRequirement(
        createBuildRequirementDeps(tmp, {
          env: { SKILLSET_BUILD_PRIVATE_QA: "1" },
          gitHead: "abc123\n",
          gitStatus: "",
        }),
      );
      ...
      await expect(
        Promise.all([
          runStatusCommand({
            ...
            spawnSync,
            env: {
              SKILLSET_RUN_NODE_BUILD_LOCK_POLL_MS: "1",
            },
            runRuntimePostBuild,
          }),
          runStatusCommand({
  ...
  it("forwards wrapper SIGTERM to the active skillset child and returns 143", async () => {
    await withTempDir({ prefix: "skillset-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [ROOT_SRC]: "export const value = 1;\n",
  ...
  it("skips rebuilding when extension package metadata is newer than the build stamp", async () => {
    await withTempDir({ prefix: "skillset-run-node-" }, async (tmp) => {
      await setupTrackedProject(tmp, {
        files: {
          [EXTENSION_MANIFEST]: '{"id":"demo","configSchema":{"type":"object"}}\n',
          [EXTENSION_PACKAGE]: '{"name":"demo","skillset":{"extensions":["./index.ts"]}}\n',
          [ROOT_TSDOWN]: "export default {};\n",
          [DIST_EXTENSION_PACKAGE]: '{"name":"demo","skillset":{"extensions":["./stale.js"]}}\n',
        },
