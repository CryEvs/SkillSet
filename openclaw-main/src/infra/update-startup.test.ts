...
vi.mock("./skillset-root.js", async () => {
  const actual = await vi.importActual<typeof import("./skillset-root.js")>("./skillset-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "skillset-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["SKILLSET_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.SKILLSET_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveSkillSetPackageRoot } = await import("./skillset-root.js"));
    ...
    vi.mocked(resolveSkillSetPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveSkillSetPackageRoot).mockResolvedValue("/opt/skillset");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/skillset/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
