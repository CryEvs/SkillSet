 ...
 * returning control to the caller (typically `triggerSkillSetRestart` →
...
/**
 * Parse skillset gateway PIDs from lsof -Fpc stdout, excluding the current
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (
        currentPid != null &&
        currentCmd &&
        normalizeLowercaseStringOrEmpty(currentCmd).includes("skillset")
      ) {
...
/**
 * Windows: find listening PIDs on the port, then verify each is an skillset
 ...
 * Returns only PIDs that belong to skillset gateway processes (not the current process).
 */
export function findGatewayPidsOnPortSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only skillset gateway processes.
