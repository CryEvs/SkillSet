...
import { resolveSkillSetPackageRoot } from "./skillset-root.js";
...
function resolveAutoUpdatePolicy(cfg: SkillSetConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: SkillSetConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const root = await resolveSkillSetPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("skillset update")}`,
      );
