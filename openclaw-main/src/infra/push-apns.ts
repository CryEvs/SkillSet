...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open SkillSet to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "skillset.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.SKILLSET_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.SKILLSET_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set SKILLSET_APNS_TEAM_ID and SKILLSET_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.SKILLSET_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.SKILLSET_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.SKILLSET_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set SKILLSET_APNS_PRIVATE_KEY_P8 or SKILLSET_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading SKILLSET_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
