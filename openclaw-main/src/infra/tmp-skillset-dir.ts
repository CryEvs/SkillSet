...
export const POSIX_SKILLSET_TMP_DIR = "/tmp/skillset";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredSkillSetTmpDirOptions = {
...
export function resolvePreferredSkillSetTmpDir(
  options: ResolvePreferredSkillSetTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "skillset" : `skillset-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[skillset] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback SkillSet temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback SkillSet temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_SKILLSET_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_SKILLSET_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_SKILLSET_TMP_DIR)) {
      return POSIX_SKILLSET_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_SKILLSET_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_SKILLSET_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_SKILLSET_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_SKILLSET_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_SKILLSET_TMP_DIR;
