...
vi.mock("./skillset-root.js", () => ({
  resolveSkillSetPackageRoot: vi.fn(async () => null),
  resolveSkillSetPackageRootSync: vi.fn(() => null),
}));
...
let skillsetRoot: typeof import("./skillset-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    skillsetRoot = await import("./skillset-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/skillset");
    const wrapperArgv1 = abs("fixtures/bin/skillset");
  ...
  it("uses resolveSkillSetPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/skillset");
    (
      skillsetRoot.resolveSkillSetPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/skillset"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "skillset" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "skillset.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-skillset");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/skillset-bundle");
    (
      skillsetRoot.resolveSkillSetPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/SkillSet.app/Contents/MacOS/SkillSet");
    const bundledUiDir = abs("fixtures/SkillSet.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/skillset-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
