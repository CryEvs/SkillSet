...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevSkillSetHome = process.env.SKILLSET_HOME;

    try {
      process.env.SKILLSET_HOME = dir;
      const approvalsPath = path.join(dir, ".skillset", "exec-approvals.json");
      ...
      if (prevSkillSetHome === undefined) {
        delete process.env.SKILLSET_HOME;
      } else {
        process.env.SKILLSET_HOME = prevSkillSetHome;
      }
    }
  });
});
