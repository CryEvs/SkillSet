...
function candidateBinDirs(opts: EnsureSkillSetPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current SkillSet process is on.
  try {
  ...
  // Bundled macOS app: `skillset` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "skillset");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.SKILLSET_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "skillset"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `skillset`
...
/**
 * Best-effort PATH bootstrap so skills that require the `skillset` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureSkillSetCliOnPath(opts: EnsureSkillSetPathOpts = {}) {
  if (isTruthyEnvValue(process.env.SKILLSET_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.SKILLSET_PATH_BOOTSTRAPPED = "1";
