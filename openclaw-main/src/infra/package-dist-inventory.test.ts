...
describe("package dist inventory", () => {
  it("tracks missing and stale dist files", async () => {
    await withTempDir({ prefix: "skillset-dist-inventory-" }, async (packageRoot) => {
  ...
  it("keeps npm-omitted dist artifacts out of the inventory", async () => {
    await withTempDir({ prefix: "skillset-dist-inventory-pack-" }, async (packageRoot) => {
      ...
      const omittedRuntimeDepsStamp = path.join(
        ...
        "extensions",
        "discord",
        ".skillset-runtime-deps-stamp.json",
      );
      const omittedRuntimeDepsTempFile = path.join(
        ...
        "extensions",
        "discord",
        ".skillset-runtime-deps-backup-node_modules-old",
        "left-pad",
        "index.js",
      );
  ...
  it("fails closed when the inventory is missing", async () => {
    await withTempDir({ prefix: "skillset-dist-inventory-missing-" }, async (packageRoot) => {
  ...
  it("rejects symlinked dist entries", async () => {
    await withTempDir({ prefix: "skillset-dist-inventory-symlink-" }, async (packageRoot) => {
