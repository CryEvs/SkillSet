...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.SKILLSET_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-current-bindings-"));
    process.env.SKILLSET_STATE_DIR = testStateDir;
