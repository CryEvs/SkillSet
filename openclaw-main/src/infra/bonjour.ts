...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.SKILLSET_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "SkillSet";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(SkillSet\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw = process.env.SKILLSET_MDNS_HOSTNAME?.trim() || "skillset";
    const hostname =
      hostnameRaw
        .replace(/\.local$/i, "")
        .split(".")[0]
