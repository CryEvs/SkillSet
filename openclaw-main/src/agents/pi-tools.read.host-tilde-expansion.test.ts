...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("skillset-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("skillset-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("skillset-tilde-test-mkdir-");
  ...
  it("ignores SKILLSET_HOME for write operations", async () => {
    const skillsetHome = await createTempDir("skillset-home-override-", os.tmpdir());
    const dir = await createTempDir("skillset-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("SKILLSET_HOME", skillsetHome);

    createHostWorkspaceWriteTool(skillsetHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(skillsetHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores SKILLSET_HOME for mkdir operations", async () => {
    const skillsetHome = await createTempDir("skillset-home-override-", os.tmpdir());
    const dir = await createTempDir("skillset-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(skillsetHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores SKILLSET_HOME for readFile operations", async () => {
    const skillsetHome = await createTempDir("skillset-home-override-", os.tmpdir());
    const dir = await createTempDir("skillset-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(skillsetHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores SKILLSET_HOME for access operations", async () => {
    const skillsetHome = await createTempDir("skillset-home-override-", os.tmpdir());
    const dir = await createTempDir("skillset-tilde-test-edit-");
