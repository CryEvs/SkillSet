...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  const envSnapshot = captureEnv(["SKILLSET_STATE_DIR"]);
  let stateDir = "";

  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to resolveSkillSetAgentDir when agentDir is omitted", () => {
    // Omitting agentDir exercises the `agentDir ?? resolveSkillSetAgentDir()`
    // nullish branch. With SKILLSET_STATE_DIR set to our tempdir, the
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-skillset-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-path-ensure-"));
    process.env.SKILLSET_STATE_DIR = stateDir;
  });
