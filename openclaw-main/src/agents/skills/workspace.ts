...
import { resolveSkillSetMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
function resolveSkillsLimits(config?: SkillSetConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "skillset-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "skillset-bundled") {
    return {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "skillset-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "skillset-managed",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values())
    ...
    .map((record) => {
      ...
      return {
        skill,
        frontmatter,
        metadata: resolveSkillSetMetadata(frontmatter),
        invocation,
        exposure: {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`skillset skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`skillset skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
