...
describe("getBlockedBindReason", () => {
  ...
  it("still blocks OS-home credential paths when SKILLSET_HOME points elsewhere", () => {
    vi.stubEnv("HOME", "/home/tester");
  ...
  it("blocks canonical OS-home aliases for credential paths", () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), "skillset-home-"));
...
describe("validateBindMounts", () => {
  ...
  it("allows legitimate project directory mounts", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "skillset-sbx-safe-"));
    expect(() =>
      validateBindMounts([
  ...
  it("blocks symlink escapes into blocked directories", () => {
    if (process.platform === "win32") {
      ...
      const dir = mkdtempSync(join(tmpdir(), "skillset-sbx-"));
  ...
  it("blocks symlink-parent escapes into blocked paths when leaf does not exist", () => {
    ...
    const missingLeaf = join(link, "skillset-not-created");
    expect(() =>
  ...
  it("blocks bind sources outside allowed roots when allowlist is configured", () => {
    const allowedRoot = mkdtempSync(join(tmpdir(), "skillset-sbx-allowed-root-"));
    const externalRoot = mkdtempSync(join(tmpdir(), "skillset-sbx-external-"));
    expect(() =>
  ...
  it("allows bind sources in allowed roots when allowlist is configured", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "skillset-sbx-allowed-"));
    expect(() =>
  ...
  it("blocks reserved container target paths by default", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "skillset-sbx-reserved-default-"));
    expect(() =>
  ...
  it("allows reserved container target paths with explicit dangerous override", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "skillset-sbx-reserved-"));
    expect(() =>
...
describe("validateApparmorProfile", () => {
  it("allows named profile/undefined", () => {
    expect(() => validateApparmorProfile("skillset-sandbox")).not.toThrow();
...
describe("validateSandboxSecurity", () => {
  it("passes with safe config", () => {
    const projectRoot = mkdtempSync(join(tmpdir(), "skillset-sbx-safe-config-"));
    expect(() =>
      validateSandboxSecurity({
