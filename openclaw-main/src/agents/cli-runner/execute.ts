...
const CLI_BACKEND_PRESERVE_ENV = "SKILLSET_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.SKILLSET_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.SKILLSET_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.SKILLSET_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.SKILLSET_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.SKILLSET_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.SKILLSET_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (
            env.SKILLSET_MCP_TOKEN ||
            env.SKILLSET_MCP_SESSION_KEY ||
            env.SKILLSET_MCP_SENDER_IS_OWNER
          ) {
