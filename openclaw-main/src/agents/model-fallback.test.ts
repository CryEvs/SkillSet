...
function makeFallbacksOnlyCfg(): SkillSetConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): SkillSetConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "skillset-auth-suite-mock");
...
// https://github.com/skillset/skillset/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("skillset-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
