...
import { resolveSkillSetDocsPath } from "../../docs-path.js";
...
import { createSkillSetCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveSkillSetAgentDir();
    const toolsRaw = params.disableTools
      ? []
      : (() => {
          const allTools = createSkillSetCodingTools({
            agentId: sessionAgentId,
    ...
    const docsPath = await resolveSkillSetDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop SkillSet
      ...
      // exact SkillSet-managed registrations so custom tools survive startup and
      ...
      try {
        ...
        // double-compaction. See: https://github.com/skillset/skillset/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("skillset:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/skillset/skillset/issues/8643
