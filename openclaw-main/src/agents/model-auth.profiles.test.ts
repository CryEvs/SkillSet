...
async function expectVertexAdcEnvApiKey(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "skillset-adc-"));
...
function buildDemoLocalProviderCfg(apiKey: string): SkillSetConfig {
  return {
    models: {
      providers: {
        "demo-local": {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-oauth-"));
    ...
    try {
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          const store = ensureAuthProfileStore(process.env.SKILLSET_AGENT_DIR, {
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-auth-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts ADC credentials from the provided env snapshot", async () => {
    await expectVertexAdcEnvApiKey({
      ...
      credentialsJson: "{}",
      tempPrefix: "skillset-google-adc-",
      env: {
