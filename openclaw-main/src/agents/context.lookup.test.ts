...
const contextTestState = vi.hoisted(() => {
  const state = {
    ...
    ensureSkillSetModelsJson: vi.fn(async () => {}),
...
vi.mock("./models-config.runtime.js", () => ({
  ensureSkillSetModelsJson: contextTestState.ensureSkillSetModelsJson,
}));

vi.mock("./agent-paths.js", () => ({
  resolveSkillSetAgentDir: () => "/tmp/skillset-agent",
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureSkillSetModelsJson.mockClear();
  return { ensureSkillSetModelsJson: contextTestState.ensureSkillSetModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real skillset startup commands that need model metadata", async () => {
    const { shouldEagerWarmContextWindowCache } = await importContextModule();

    expect(shouldEagerWarmContextWindowCache(["node", "skillset", "chat"])).toBe(true);
    expect(
    ...
    expect(shouldEagerWarmContextWindowCache(["node", "skillset", "logs", "--limit", "5"])).toBe(
      false,
    );
    expect(shouldEagerWarmContextWindowCache(["node", "skillset", "status", "--json"])).toBe(false);
    expect(
