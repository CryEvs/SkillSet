...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: SkillSetConfig) {
    return createSkillSetCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<SkillSetConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<SkillSetConfig["agents"]>["list"]>[number]["tools"];
  }): SkillSetConfig {
    return {
      tools: params.tools,
      agents: {
