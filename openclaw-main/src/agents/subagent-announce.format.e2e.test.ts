...
let configOverride: SkillSetConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: SkillSetConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/skillset/skillset/issues/31298
    previousFastTestEnv = process.env.SKILLSET_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.SKILLSET_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // SKILLSET_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("SkillSet runtime context (internal):");
