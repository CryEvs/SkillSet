...
export type SkillSetStdioServerParameters = {
  command: string;
  args?: string[];
...
export class SkillSetStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: SkillSetStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "SkillSetStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
