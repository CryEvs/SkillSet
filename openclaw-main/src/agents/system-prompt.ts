...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__skillset__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__skillset__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__skillset__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; SkillSet handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `SkillSet docs: ${docsPath}`,
    "Mirror: https://docs.skillset.ai",
    "Source: https://github.com/skillset/skillset",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    "For SkillSet behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `skillset status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running SkillSet process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List SkillSet agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List SkillSet agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside SkillSet.";
  }

  const lines = [
    "You are a personal assistant running inside SkillSet.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control SkillSet's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    ...safetySection,
    "## SkillSet CLI Quick Reference",
    "SkillSet is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- skillset gateway status",
    "- skillset gateway start",
    "- skillset gateway stop",
    "- skillset gateway restart",
    "If unsure, ask the user to run `skillset help` (or `skillset gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## SkillSet Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, SkillSet pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by SkillSet and included below in Project Context.",
    "",
