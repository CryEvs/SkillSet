...
interface EmbeddedGatewayRuntime {
  resolveSessionAgentId: (opts: { sessionKey: string; config: SkillSetConfig }) => string;
  ...
  resolveEffectiveChatHistoryMaxChars: (cfg: SkillSetConfig) => number;
  ...
  loadCombinedSessionStoreForGateway: (cfg: SkillSetConfig) => {
    storePath: string;
    store: unknown;
  };
