...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.SKILLSET_PROFILE?.trim();
  if (profile && normalizeOptionalLowercaseString(profile) !== "default") {
    return path.join(home, ".skillset", `workspace-${profile}`);
  }
  return path.join(home, ".skillset", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".skillset";
