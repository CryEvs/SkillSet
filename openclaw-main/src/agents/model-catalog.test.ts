...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    vi.doMock("./models-config.js", () => ({
      ensureSkillSetModelsJson: vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false }),
    }));
    vi.doMock("./agent-paths.js", () => ({
      resolveSkillSetAgentDir: () => "/tmp/skillset",
    }));
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    try {
      ...
      const result = await loadModelCatalog({ config: {} as SkillSetConfig });
