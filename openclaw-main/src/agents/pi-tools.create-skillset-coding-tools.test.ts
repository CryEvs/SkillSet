...
import "./test-helpers/fast-skillset-tools.js";
import { createSkillSetCodingTools } from "./pi-tools.js";
...
describe("createSkillSetCodingTools", () => {
  const testConfig: SkillSetConfig = {};

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createSkillSetCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createSkillSetCodingTools({
      config: testConfig,
    ...
    const codexTools = createSkillSetCodingTools({
      config: testConfig,
    ...
    const disabledConfig: SkillSetConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createSkillSetCodingTools({
    ...
    const anthropicTools = createSkillSetCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: SkillSetConfig = {
      tools: {
        exec: {
    ...
    const allowed = createSkillSetCodingTools({
    ...
    const denied = createSkillSetCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createSkillSetCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createSkillSetCodingTools({ config: testConfig });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createSkillSetCodingTools({ messageProvider: "discord" });
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createSkillSetCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      ...
      const persistedEnvelopeTools = createSkillSetCodingTools({
      ...
      const restrictedTools = createSkillSetCodingTools({
      ...
      const ancestryTools = createSkillSetCodingTools({
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      const mainStorePath = storeTemplate.replaceAll("{agentId}", "main");
      const writerStorePath = storeTemplate.replaceAll("{agentId}", "writer");
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createSkillSetCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(codingTools.some((tool) => tool.name === "message")).toBe(false);

    const cronTools = createSkillSetCodingTools({
      config: { tools: { profile: "coding" } },
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createSkillSetCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createSkillSetCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createSkillSetCodingTools({
      modelProvider: "xai",
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createSkillSetCodingTools();
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-read-"));
    try {
      ...
      const contents = "Hello from skillset read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "skillset-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "skillset-workspace"),
    ...
    const tools = createSkillSetCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-canonical-"));
    try {
      const tools = createSkillSetCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-legacy-alias-"));
    try {
      const tools = createSkillSetCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-structured-write-"));
    try {
      const tools = createSkillSetCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-structured-edit-"));
    try {
