...
declare const __SKILLSET_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "skillset";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["SKILLSET_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["SKILLSET_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["SKILLSET_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.SKILLSET_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current SkillSet version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __SKILLSET_VERSION__ === "string" ? __SKILLSET_VERSION__ : undefined,
  bundledVersion: process.env.SKILLSET_BUNDLED_VERSION,
});
