...
export function hasConfiguredInternalHooks(config: SkillSetConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: SkillSetConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
