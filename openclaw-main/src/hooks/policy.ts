import type { SkillSetConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "skillset-bundled": {
    ...
    canOverride: ["skillset-bundled"],
    canBeOverriddenBy: ["skillset-managed", "skillset-plugin"],
  },
  "skillset-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["skillset-bundled", "skillset-plugin"],
    canBeOverriddenBy: ["skillset-managed"],
  },
  "skillset-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["skillset-bundled", "skillset-managed", "skillset-plugin"],
    canBeOverriddenBy: ["skillset-managed"],
  },
  "skillset-workspace": {
    precedence: 40,
    ...
    canOverride: ["skillset-workspace"],
    canBeOverriddenBy: ["skillset-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "skillset-plugin") {
    return { enabled: true };
  }
