...
`;
    ...
    expect(parsed.skillset.emoji).toBe("💾");
    expect(parsed.skillset.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.skillset.emoji).toBe("📝");
    expect(parsed.skillset.events).toEqual(["command"]);
    expect(parsed.skillset.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.skillset.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"skillset": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"skillset": {"events": ["test"]}}');
  });
...
describe("resolveSkillSetMetadata", () => {
  it("extracts skillset metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveSkillSetMetadata(frontmatter);
  ...
  it("returns undefined when skillset key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        skillset: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with SkillSet" },
            { id: "npm", kind: "npm", package: "@skillset/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@skillset/hook");
  });
...
`;
    ...
    const skillset = resolveSkillSetMetadata(frontmatter);
    expect(skillset).toBeDefined();
    expect(skillset?.emoji).toBe("💾");
    expect(skillset?.events).toEqual(["command:new", "command:reset"]);
    expect(skillset?.requires?.config).toEqual(["workspace.dir"]);
    expect(skillset?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const skillset = resolveSkillSetMetadata(frontmatter);
    expect(skillset?.emoji).toBe("disk");
    expect(skillset?.events).toEqual(["command:new"]);
  });
});
