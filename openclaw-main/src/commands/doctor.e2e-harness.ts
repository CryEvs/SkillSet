...
export const resolveSkillSetPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadSkillSetPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../infra/skillset-root.js", () => ({
  ...
  resolveSkillSetPackageRootSync: vi.fn(() => "/tmp/skillset"),
}));
...
beforeEach(() => {
  ...
  resolveSkillSetPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.SKILLSET_STATE_DIR;
  originalUpdateInProgress = process.env.SKILLSET_UPDATE_IN_PROGRESS;
  process.env.SKILLSET_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-doctor-state-"));
