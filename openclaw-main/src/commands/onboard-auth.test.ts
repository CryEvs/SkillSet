...
vi.mock("../agents/agent-paths.js", () => ({
  resolveSkillSetAgentDir: () => process.env.SKILLSET_AGENT_DIR ?? "/tmp/skillset-agent",
}));

vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.SKILLSET_STATE_DIR ?? "/tmp/skillset-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  return {
    upsertAuthProfile: (params: { profileId: string; credential: unknown; agentDir?: string }) => {
      const agentDir = params.agentDir ?? process.env.SKILLSET_AGENT_DIR ?? "/tmp/skillset-agent";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under SKILLSET_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("skillset-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-oauth-sync-"));
    ...
    process.env.SKILLSET_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-oauth-nosync-"));
    process.env.SKILLSET_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside SKILLSET_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-oauth-external-"));
    process.env.SKILLSET_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* SKILLSET_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("skillset-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("skillset-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to SKILLSET_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("skillset-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
