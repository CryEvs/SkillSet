...
type EnvSnapshot = {
  HOME?: string;
  SKILLSET_HOME?: string;
  SKILLSET_STATE_DIR?: string;
  SKILLSET_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    SKILLSET_HOME: process.env.SKILLSET_HOME,
    SKILLSET_STATE_DIR: process.env.SKILLSET_STATE_DIR,
    SKILLSET_OAUTH_DIR: process.env.SKILLSET_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: SkillSetConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("SKILLSET_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: SkillSetConfig) {
...
async function runStateIntegrityText(cfg: SkillSetConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "skillset-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.SKILLSET_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when SKILLSET_OAUTH_DIR is explicitly configured", async () => {
    process.env.SKILLSET_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: SkillSetConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.SKILLSET_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it("prints skillset-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: SkillSetConfig = {};
    ...
    expect(text).toMatch(/skillset sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/skillset sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /skillset sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
