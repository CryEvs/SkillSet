...
export function normalizeLegacyTalkConfig(cfg: SkillSetConfig, changes: string[]): SkillSetConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as SkillSetConfig["talk"]) ?? {};
