...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.SKILLSET_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("skillset gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
