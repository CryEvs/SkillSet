...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:skillset security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:skillset security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:skillset status --all",
      "  Need to debug live? cmd:skillset logs --follow",
      "  Fix reachability first: cmd:skillset gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:skillset devices approve req-123)",
      "muted(Fallback: cmd:skillset devices approve --latest)",
      "muted(Inspect: cmd:skillset devices list)",
    ]);
  });
