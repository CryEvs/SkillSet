...
beforeAll(() => {
  envSnapshot = captureEnv(["SKILLSET_PROFILE"]);
  process.env.SKILLSET_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedBySkillSet: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/skillset/pnpm-lock.yaml",
        markerPath: "/tmp/skillset/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.SKILLSET_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.skillset.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.skillset.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.SKILLSET_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/skillset-root.js", () => ({
  resolveSkillSetPackageRoot: vi.fn().mockResolvedValue("/tmp/skillset"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("skillset status --all") ||
          line.includes("skillset --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("skillset --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("SKILLSET_GATEWAY_TOKEN", "abcd1234", async () => {
