...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/skillset-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = SkillSetConfig>(
  overrides: T = {} as T,
): SkillSetConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): SkillSetConfig {
