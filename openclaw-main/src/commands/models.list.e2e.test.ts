...
const ensureSkillSetModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveSkillSetAgentDir = vi.fn().mockReturnValue("/tmp/skillset-agent");
...
beforeEach(() => {
  ...
  ensureSkillSetModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureSkillSetModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the source snapshot config when provided", async () => {
    ...
    expect(ensureSkillSetModelsJson).toHaveBeenCalled();
    expect(ensureSkillSetModelsJson.mock.calls[0]?.[0]).toEqual(sourceConfig);
  });
