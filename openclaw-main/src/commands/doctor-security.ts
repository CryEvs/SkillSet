...
import type { SkillSetConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: SkillSetConfig): string[] {
...
function collectExecPolicyConflictWarnings(cfg: SkillSetConfig): string[] {
  const warnings: string[] = [];
  const approvals = loadExecApprovals();
  const defaultRequestedSecuritySource = "SkillSet default (full)";
  const defaultRequestedAskSource = "SkillSet default (off)";
  ...
  const maybeWarn = (params: {
    ...
    warnings.push(
      [
        ...
        `  Inspect with: ${formatCliCommand("skillset approvals get --gateway")}`,
      ].join("\n"),
    );
  };
...
function collectDurableExecApprovalWarnings(cfg: SkillSetConfig): string[] {
  void cfg;
  return [];
}

export async function noteSecurityWarnings(cfg: SkillSetConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("skillset security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.skillset/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("skillset approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.skillset.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("skillset configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("skillset config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("skillset doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("skillset config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('skillset config set session.dmScope "per-channel-peer"') +
