...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "SkillSet", ".skillset");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/skillset-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".skillset");
  ...
  it("anchors cloud detection to OS homedir when SKILLSET_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".skillset");
    const originalSkillSetHome = process.env.SKILLSET_HOME;
    process.env.SKILLSET_HOME = "/tmp/skillset-home-override";
    ...
    try {
      ...
      if (originalSkillSetHome === undefined) {
