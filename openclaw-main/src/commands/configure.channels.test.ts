...
function expectUnknownChannelRemovalPrompt(unsafeChannel: string, label: string) {
  ...
  expect(confirm).toHaveBeenCalledWith(
    expect.objectContaining({
      message: `Delete ${label} configuration from ~/.skillset/skillset.json?`,
    }),
  );
...
describe("removeChannelConfigWizard", () => {
  ...
  it("lists configured channels from skillset.json even when no plugins are loaded", async () => {
  ...
  it("deletes the selected channel block from skillset.json", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram configuration from ~/.skillset/skillset.json?",
      }),
    );
  ...
  it("deletes a real channel block named done", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete done configuration from ~/.skillset/skillset.json?",
      }),
    );
  ...
  it("sanitizes known channel labels before rendering prompts", async () => {
    ...
    expect(confirm).toHaveBeenCalledWith(
      expect.objectContaining({
        message: "Delete Telegram\\nBot configuration from ~/.skillset/skillset.json?",
      }),
    );
    expect(note).toHaveBeenCalledWith(
