...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: SkillSetConfig },
  primary: string,
) {
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: SkillSetConfig) => config,
      applyDefaultConfig: (config: SkillSetConfig) => config,
      noteAgentModel,
...
describe("applyOpencodeZenModelDefault", () => {
  it("sets defaults when model is unset", () => {
    const cfg: SkillSetConfig = { agents: { defaults: {} } };
