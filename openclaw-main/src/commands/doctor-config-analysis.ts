...
import { SkillSetSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: SkillSetConfig): {
  config: SkillSetConfig;
  removed: string[];
} {
  const parsed = SkillSetSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: SkillSetConfig): void {
