...
export async function runSetupWizard(
  ...
  await prompter.intro("SkillSet setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: SkillSetConfig = snapshot.valid
    ? snapshot.exists
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("skillset doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("skillset doctor")}`,
        `Inspect: ${formatCliCommand("skillset plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("skillset configure")}.`;
  ...
  let localGatewayToken = process.env.SKILLSET_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.SKILLSET_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: SkillSetConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
