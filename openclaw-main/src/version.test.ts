...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "skillset-version-" });
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "skillset", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "skillset", version: "2.3.4" });
  ...
  it("ignores non-skillset package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers SKILLSET_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        SKILLSET_VERSION: "9.9.9",
        SKILLSET_SERVICE_VERSION: "2.2.2",
        npm_package_version: "1.1.1",
      }),
    ).toBe("9.9.9");
  });

  it("prefers runtime VERSION over stale SKILLSET_VERSION for compatibility checks", () => {
    const previous = process.env.SKILLSET_VERSION;
    const previousService = process.env.SKILLSET_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.SKILLSET_VERSION = "2026.3.25";
      process.env.SKILLSET_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      process.env.SKILLSET_VERSION = previous;
      process.env.SKILLSET_SERVICE_VERSION = previousService;
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        SKILLSET_VERSION: "2026.3.99",
        SKILLSET_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        SKILLSET_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        SKILLSET_VERSION: "2026.3.99",
        SKILLSET_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
  ...
  it("prefers runtime VERSION over service/package markers and ignores blank env values", () => {
    expect(
      resolveRuntimeServiceVersion({
        SKILLSET_VERSION: "   ",
        SKILLSET_SERVICE_VERSION: "  2.0.0  ",
