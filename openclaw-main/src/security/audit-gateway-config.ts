...
type CollectDangerousConfigFlags = (cfg: SkillSetConfig) => string[];
...
function collectCoreInsecureOrDangerousFlags(cfg: SkillSetConfig): string[] {
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.SKILLSET_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.SKILLSET_GATEWAY_PASSWORD);
