...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("skillset status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("skillset security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set SKILLSET_GATEWAY_TOKEN/SKILLSET_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("skillset security audit --deep")}".`,
    });
  }
  return findings;
}
