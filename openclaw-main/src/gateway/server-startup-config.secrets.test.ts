...
function gatewayTokenConfig(config: SkillSetConfig): SkillSetConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function asConfig(value: unknown): SkillSetConfig {
  return value as SkillSetConfig;
}

function buildSnapshot(config: SkillSetConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/skillset-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: SkillSetConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.SKILLSET_SKIP_CHANNELS;
  const previousSkipProviders = process.env.SKILLSET_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.SKILLSET_SKIP_CHANNELS;
    } else {
      process.env.SKILLSET_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.SKILLSET_SKIP_PROVIDERS;
    } else {
      process.env.SKILLSET_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
