...
export function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("skillset doctor --fix")}" to repair, then retry.`
    : "";
...
function hasActiveGatewayAuthSecretRef(config: SkillSetConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: SkillSetConfig): SkillSetConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: SkillSetConfig): void {
