...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "skillset.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "skillset.pluginSubagentOverridePolicyState",
);
...
export function setPluginSubagentOverridePolicies(cfg: SkillSetConfig): void {
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.skillset.ai/tools/plugin#runtime-helpers and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadSkillSetPlugins({
    config: resolvedConfig,
