...
function listConfiguredAgentIds(cfg: SkillSetConfig): string[] {
...
export function listAgentsForGateway(cfg: SkillSetConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
export function loadCombinedSessionStoreForGateway(cfg: SkillSetConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: SkillSetConfig): GatewaySessionsDefaults {
