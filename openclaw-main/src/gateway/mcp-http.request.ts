...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.SKILLSET_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.SKILLSET_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: SkillSetConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-skillset-message-channel")) ?? undefined,
    accountId: normalizeOptionalString(getHeader(req, "x-skillset-account-id")),
