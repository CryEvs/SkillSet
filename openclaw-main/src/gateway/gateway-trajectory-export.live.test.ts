...
const CODEX_HARNESS_LIVE = process.env.SKILLSET_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.SKILLSET_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    trajectory: process.env.SKILLSET_TRAJECTORY,
    trajectoryDir: process.env.SKILLSET_TRAJECTORY_DIR,
  };
}
...
function restoreEnv(snapshot: EnvSnapshot): void {
  ...
  restoreEnvVar("SKILLSET_TRAJECTORY", snapshot.trajectory);
  restoreEnvVar("SKILLSET_TRAJECTORY_DIR", snapshot.trajectoryDir);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-skillset-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.SKILLSET_TRAJECTORY = "1";
      process.env.SKILLSET_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".skillset", "trajectory-exports", "bundle");
