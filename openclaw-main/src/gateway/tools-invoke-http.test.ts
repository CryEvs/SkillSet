...
let lastCreateSkillSetToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/skillset-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateSkillSetToolsContext?.agentTo,
          agentThreadId: lastCreateSkillSetToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createSkillSetTools: (ctx: Record<string, unknown>) => {
      lastCreateSkillSetToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateSkillSetToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-skillset-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-skillset-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateSkillSetToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateSkillSetToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateSkillSetToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateSkillSetToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateSkillSetToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateSkillSetToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-skillset-message-to": "channel:24514",
        "x-skillset-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
