...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("SKILLSET_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.SKILLSET_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.SKILLSET_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "SKILLSET_LIVE_CLI_BACKEND_ARGS",
      process.env.SKILLSET_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `SKILLSET_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "SKILLSET_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.SKILLSET_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.SKILLSET_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.SKILLSET_SKIP_PROVIDERS,
    ...
    bundledPluginsDir: process.env.SKILLSET_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.SKILLSET_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.SKILLSET_SKIP_PROVIDERS = "1";
  ...
  process.env.SKILLSET_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  ...
  restoreEnvVar("SKILLSET_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("SKILLSET_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("SKILLSET_SKIP_PROVIDERS", snapshot.skipProviders);
  ...
  restoreEnvVar("SKILLSET_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("SKILLSET_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
