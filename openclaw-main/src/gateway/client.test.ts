...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("skillset doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with SKILLSET_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with SKILLSET_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.SKILLSET_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://skillset-gateway.ai:18789",
      onConnectError,
    });
