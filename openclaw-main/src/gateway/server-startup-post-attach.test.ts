...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/skillset-state",
    resolveConfigPath: vi.fn(() => "/tmp/skillset-state/skillset.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/skillset-state"),
  };
});
  ...
  it("dispatches registered gateway startup internal hooks without configured hook packs", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/skillset-workspace",
        deps,
