...
import { attachSkillSetTranscriptMeta, readSessionMessages } from "./session-utils.js";
...
export type SessionHistoryMessage = Record<string, unknown> & {
  __skillset?: SessionHistoryTranscriptMeta;
};
...
export function resolveMessageSeq(message: SessionHistoryMessage | undefined): number | undefined {
  const seq = message?.__skillset?.seq;
...
export class SessionHistorySseState {
  ...
  appendInlineMessage(update: {
    ...
    const nextMessage = attachSkillSetTranscriptMeta(update.message, {
