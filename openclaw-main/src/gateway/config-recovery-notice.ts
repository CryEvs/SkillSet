...
export function formatConfigRecoveryNotice(params: {
  ...
  const configName = path.basename(params.configPath) || "skillset.json";
  return [
    `Config recovery warning: SkillSet restored ${configName} from the last-known-good backup during ${params.phase} (${params.reason}).`,
