...
export function resolveGatewayReloadSettings(cfg: SkillSetConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: SkillSetConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: SkillSetConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: SkillSetConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: SkillSetConfig, nextCompareConfig: SkillSetConfig) => {
