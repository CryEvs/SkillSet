...
type GatewayInstallAuthMode = NonNullable<NonNullable<SkillSetConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: SkillSetConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    normalizeOptionalString(durableServiceEnv.SKILLSET_GATEWAY_PASSWORD) ||
