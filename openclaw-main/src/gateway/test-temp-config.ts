...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "skillset-test-owner-display-secret",
    },
  };
}
...
export async function withTempConfig(params: {
  ...
  const prevConfigPath = process.env.SKILLSET_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as SkillSetConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "skillset-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.SKILLSET_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
