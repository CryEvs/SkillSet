...
describe("gateway startup websocket readiness", () => {
  it("does not bind the websocket port until websocket handlers are attached", async () => {
    ...
    const previousMinimal = process.env.SKILLSET_TEST_MINIMAL_GATEWAY;
    process.env.SKILLSET_TEST_MINIMAL_GATEWAY = "0";
    ...
    try {
      ...
      if (previousMinimal === undefined) {
        ...
        process.env.SKILLSET_TEST_MINIMAL_GATEWAY = previousMinimal;
      }
    }
  });
