...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.SKILLSET_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: SkillSetConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: SkillSetConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: SkillSetConfig, requestedAgentId: string) => {
