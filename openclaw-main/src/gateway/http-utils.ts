...
export const SKILLSET_MODEL_ID = "skillset";
export const SKILLSET_DEFAULT_MODEL_ID = "skillset/default";
...
}): Promise<{ cfg: SkillSetConfig; requestAuth: AuthorizedGatewayHttpRequest } | null> {
...
export function resolveTrustedHttpOperatorScopes(
  ...
  const headerValue = getHeader(req, "x-skillset-scopes");
...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-skillset-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-skillset-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === SKILLSET_MODEL_ID || lowered === SKILLSET_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^skillset[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `skillset` or `skillset/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-skillset-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-skillset-model`." };
  }
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-skillset-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-skillset-message-channel")) ??
