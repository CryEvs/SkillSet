...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "skillset-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".skillset", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["SKILLSET_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "skillset");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.SKILLSET_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".skillset");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "skillset-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-wizard-home-"));
      process.env.HOME = tempHome;
      process.env.SKILLSET_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".skillset", "skillset.json");
      const bundledPluginsDir = path.join(tempHome, "skillset-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.SKILLSET_STATE_DIR = path.join(tempHome, ".skillset");
      process.env.SKILLSET_CONFIG_PATH = configPath;
      process.env.SKILLSET_SKIP_CHANNELS = "1";
      ...
      process.env.SKILLSET_SKIP_BROWSER_CONTROL_SERVER = "1";
      process.env.SKILLSET_SKIP_PROVIDERS = "1";
      process.env.SKILLSET_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
      process.env.SKILLSET_TEST_MINIMAL_GATEWAY = "1";
