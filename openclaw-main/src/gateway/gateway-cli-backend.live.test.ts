...
import { clearRuntimeConfigSnapshot, type SkillSetConfig } from "../config/config.js";
...
const CLI_LIVE = isTruthyEnvValue(process.env.SKILLSET_LIVE_CLI_BACKEND);
const CLI_RESUME = isTruthyEnvValue(process.env.SKILLSET_LIVE_CLI_BACKEND_RESUME_PROBE);
const CLI_DEBUG = isTruthyEnvValue(process.env.SKILLSET_LIVE_CLI_BACKEND_DEBUG);
const CLI_CI_SAFE_CODEX_CONFIG = isTruthyEnvValue(
  process.env.SKILLSET_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG,
);
  ...
  it(
    ...
    async () => {
      const preservedEnv = new Set(
        parseJsonStringArray(
          "SKILLSET_LIVE_CLI_BACKEND_PRESERVE_ENV",
          process.env.SKILLSET_LIVE_CLI_BACKEND_PRESERVE_ENV,
        ) ?? [],
      );
      ...
      const rawModel = process.env.SKILLSET_LIVE_CLI_BACKEND_MODEL ?? DEFAULT_MODEL;
      const parsed = parseModelRef(rawModel, "claude-cli");
      if (!parsed) {
        throw new Error(
          `SKILLSET_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${rawModel}`,
        );
      }
      ...
      const cliCommand = process.env.SKILLSET_LIVE_CLI_BACKEND_COMMAND ?? providerDefaults?.command;
      if (!cliCommand) {
        throw new Error(
          `SKILLSET_LIVE_CLI_BACKEND_COMMAND is required for provider "${providerId}".`,
        );
      }
      ...
      const cliClearEnv =
        parseJsonStringArray(
          "SKILLSET_LIVE_CLI_BACKEND_CLEAR_ENV",
          process.env.SKILLSET_LIVE_CLI_BACKEND_CLEAR_ENV,
        ) ??
      ...
      const cliImageArg =
        process.env.SKILLSET_LIVE_CLI_BACKEND_IMAGE_ARG?.trim() || providerDefaults?.imageArg;
      const cliImageMode =
        parseImageMode(process.env.SKILLSET_LIVE_CLI_BACKEND_IMAGE_MODE) ??
        providerDefaults?.imageMode;
      if (cliImageMode && !cliImageArg) {
        throw new Error(
          "SKILLSET_LIVE_CLI_BACKEND_IMAGE_MODE requires SKILLSET_LIVE_CLI_BACKEND_IMAGE_ARG.",
        );
      }

      const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "skillset-live-cli-"));
      ...
      const bootstrapWorkspace =
        backendResolved?.bundleMcpMode === "claude-config-file"
          ...
          : null;
      const disableMcpConfig = process.env.SKILLSET_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG !== "0";
      ...
      const cfgWithCliBackends = cfg as SkillSetConfig & {
        agents?: {
          defaults?: {
