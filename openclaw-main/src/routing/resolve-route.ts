...
function listAgents(cfg: SkillSetConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: SkillSetConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<SkillSetConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: SkillSetConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: SkillSetConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: SkillSetConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<SkillSetConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  SkillSetConfig,
  {
    bindingsRef: SkillSetConfig["bindings"];
    agentsRef: SkillSetConfig["agents"];
    sessionRef: SkillSetConfig["session"];
...
function resolveRouteCacheForConfig(cfg: SkillSetConfig): Map<string, ResolvedAgentRoute> {
