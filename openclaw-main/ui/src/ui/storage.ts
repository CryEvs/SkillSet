const SETTINGS_KEY_PREFIX = "skillset.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "skillset.control.settings.v1";
const LOCAL_USER_IDENTITY_KEY = "skillset.control.user.v1";
const LEGACY_TOKEN_SESSION_KEY = "skillset.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "skillset.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    normalizeOptionalString(window.__SKILLSET_CONTROL_UI_BASE_PATH__);
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("skillset.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
