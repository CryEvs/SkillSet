...
export async function handleSendChat(
  ...
  const hasAudioAttachment = attachmentsToSend.some((att) => att.mimeType.startsWith("audio/"));

  if (!message && hasAudioAttachment) {
    message =
      [
        "Используй ТОЛЬКО инструмент edu_planner.",
        "НЕ используй exec, bash, shell и другие инструменты.",
        "Сначала вызови action=analyze_lecture_audio.",
        "В payload передай путь к аудио из контекста сообщения (MediaPath или первый элемент MediaPaths) в поле audioPath.",
        "После успешного вызова определи, это лекция или формулировка задания (или оба формата).",
        "Верни строго на русском:",
        "1) Кратко о содержании аудио",
        "2) Что было объяснено/сделано",
        "3) Какое задание дано (если есть)",
        "4) Что сделать дальше",
        "Только планирование, без решения заданий.",
      ].join("\n");
  } else if (message && hasAudioAttachment) {
    message = [
      "Системная инструкция для аудио-вложения: используй только edu_planner (action=analyze_lecture_audio) и не используй exec/shell.",
      message,
    ].join("\n\n");
  }
