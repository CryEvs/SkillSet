...
const UPDATE_BANNER_DISMISS_KEY = "skillset:control-ui:update-banner-dismissed:v1";
...
export function renderApp(state: AppViewState) {
  ...
  const isChat = state.tab === "chat" || state.tab === "skillset";
  ...
  return html`
    ...
    <div
      class="shell ${isChat ? "shell--chat" : ""} ${chatFocus
        ...
        : ""} ${state.onboarding ? "shell--onboarding" : ""} ${state.tab === "skillset"
        ? "shell--skillset"
        : ""}"
    >
      <button
                ...
                ${navCollapsed
                  ...
                  : html`
                      ...
                      <span class="sidebar-brand__copy">
                        ...
                        <span class="sidebar-brand__title">SkillSet</span>
                      </span>
                    `}
              </div>
              <button
      ...
      <main class="content ${isChat ? "content--chat" : ""}">
        ...
        ${state.tab === "nodes"
          ...
          : nothing}
        ${state.tab === "chat" || state.tab === "skillset"
          ? renderChat({
              sessionKey: state.sessionKey,
              onSessionKeyChange: (next) => {
              ...
              onCreatePlannerTasks: (tasks, dueDateIso) => {
                ...
                const deadlineLine = dueDateIso
                  ? `Используй dueDate=${dueDateIso} для всех задач.`
                  : "Создай задачи без dueDate (пользователь выставит дедлайн позже).";
                const prompt = [
                  "Используй ТОЛЬКО инструмент edu_planner.",
                  "Для каждого пункта ниже вызови action=create_assignment.",
                  "Минимальный payload: title, description, subject, studentId.",
                  deadlineLine,
                  "studentId: student-1",
                  "subject: lecture",
                  "description: Создано из плана лекции.",
                  "Список задач:",
                  ...normalizedTasks.map((item, idx) => `${idx + 1}. ${item}`),
                  "После выполнения верни короткий отчет с количеством созданных задач.",
                ].join("\n");
