...
export function applySettingsFromUrl(host: SettingsHost) {
  ...
  if (hasTokenParam) {
    if (queryToken != null) {
      warnQueryToken = true;
      console.warn(
        "[skillset] Auth token passed as query parameter (?token=). Use URL fragment instead: #token=<token>. Query parameters may appear in server logs.",
      );
    }
...
export function inferBasePath() {
  ...
  const configured = window.__SKILLSET_CONTROL_UI_BASE_PATH__;
...
function applyTabSelection(
  ...
  if ((prev === "chat" || prev === "skillset") && next !== "chat" && next !== "skillset") {
    resetChatViewState();
  }

  if (next === "chat" || next === "skillset") {
...
export function syncUrlWithTab(host: SettingsHost, tab: Tab, replace: boolean) {
  ...
  if ((tab === "chat" || tab === "skillset") && host.sessionKey) {
