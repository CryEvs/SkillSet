...
export function renderToolCard(
  ...
  opts: {
    ...
    embedSandboxMode?: EmbedSandboxMode;
    allowExternalEmbedUrls?: boolean;
    onCreatePlannerTasks?: (tasks: string[], dueDateIso: string | null) => void;
  },
) {
...
export function renderExpandedToolCardContent(
  ...
  const visiblePreview = card.preview
    ...
    : nothing;
  const plannerTasks = extractLectureTasksFromToolCard(card);
            ...
            <div class="chat-tool-card__actions">
              <button
                ...
                @click=${() => {
                  ...
                  const dueDateIso = resolveDueDateIso(deadlineInput.trim());
                  onCreatePlannerTasks?.(plannerTasks, dueDateIso);
                }}
              >
...
function extractLectureTasksFromToolCard(card: ToolCard): string[] {
  if (card.name !== "edu_planner" || !card.outputText?.trim()) {
    return [];
  }
  let parsed: unknown;
  try {
    parsed = JSON.parse(card.outputText);
  } catch {
    return [];
  }
  ...
  const summary = result.lectureSummary;
  if (!summary || typeof summary !== "object") {
    return [];
  }
  const whatToDo = (summary as Record<string, unknown>).whatToDo;
...
function resolveDueDateIso(input: string): string | null {
  if (!input) {
    return null;
  }
  if (/^\+\d+$/.test(input)) {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(input)) {
    const date = new Date(`${input}T18:00:00`);
