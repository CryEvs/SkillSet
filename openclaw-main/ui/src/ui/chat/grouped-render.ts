...
const SKIP_DELETE_CONFIRM_KEY = "skillset:skipDeleteConfirm";
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__skillset__|media)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__skillset__/assistant-media?${params.toString()}`;
}
    ...
    <div class="chat-tools-inline">
      ${toolCards.map((card, index) =>
        renderToolCard(card, {
          ...
          onCreatePlannerTasks: opts.onCreatePlannerTasks,
        }),
      )}
    </div>
  `;
